...
**Entendendo o cache KV com um exemplo simples.** Suponha que o contexto tenha quatro tokens [A, B, C, D] e que o modelo esteja prestes a gerar o quinto token, E. A operação central da atenção compara o vetor de consulta (Query) de E com os vetores de chave (Key) dos tokens existentes para calcular as pontuações de correspondência — para uma explicação intuitiva do produto escalar, consulte o Experimento 2-2. Em seguida, essas pontuações são usadas para calcular uma soma ponderada dos vetores de valor (Value), produzindo a representação de saída de E.

Sem o cache KV, sempre que um novo token é gerado, os vetores K e V de todos os tokens anteriores precisam ser recalculados do zero: para gerar E, é necessário calcular cinco conjuntos de K e V; para gerar o sexto token, seis conjuntos; e assim por diante. Ao chegar ao enésimo token, é preciso calcular N conjuntos, e o volume total de computação é proporcional a N².

Com o cache KV, os vetores K e V de A, B, C e D são armazenados depois de calculados uma vez. Ao gerar E, basta calcular os vetores K e V do próprio E e realizar o cálculo da atenção usando-os em conjunto com os quatro conjuntos armazenados. Observe que o cache KV evita recalcular as projeções K e V dos tokens anteriores, de modo que cada etapa de decodificação não precise recomputar todo o prefixo. No entanto, o cálculo da atenção de cada novo token ainda precisa percorrer todos os valores K e V armazenados, e seu custo cresce linearmente com o tamanho do contexto. É por isso que a decodificação de contextos longos fica cada vez mais lenta e que a memória e a largura de banda usadas pelo cache KV se tornam gargalos da inferência.
