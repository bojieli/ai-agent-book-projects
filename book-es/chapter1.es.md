...
Estas cinco etapas no se sustituyen unas a otras, sino que cada una contiene a la anterior: la ingeniería de prompts es un subconjunto de la ingeniería de contexto, la ingeniería de contexto es un subconjunto de la Ingeniería de Harness y la Ingeniería de Harness es un subconjunto de la Ingeniería de Loops, que a su vez es un subconjunto de la Ingeniería de Grafos: un único bucle de Agent es un nodo del grafo de ejecución. Cada capa amplía el ámbito de atención y de influencia del ingeniero sobre la base de la anterior. **Cuando las capacidades de los modelos de distintos proveedores convergen y dejan de ser el factor diferenciador decisivo, la ventaja competitiva se desplaza hacia las prácticas de ingeniería externas al modelo**.
