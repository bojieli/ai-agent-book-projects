...
#### Nota de implementación: estándar frente a formato de transporte

El estándar Agent Skills define el orden de divulgación, no un rol literal de mensaje de la API: el catálogo debe estar visible antes de las instrucciones completas, y el `SKILL.md` completo solo se carga después de seleccionar el Skill. Anthropic describe el catálogo como contexto de sistema cargado al inicio, mientras que Claude Code documenta que las instrucciones de un Skill invocado se incorporan como mensajes user en el punto de invocación. Por tanto, un `<system-reminder>` con rol user observado en una traza concreta de Claude Code es una representación de transporte propia de una versión y de su Harness, no un requisito del protocolo Agent Skills.

Codex CLI utiliza otro Harness. La [documentación de Skills de OpenAI](https://developers.openai.com/codex/skills/) indica que su catálogo inicial contiene el nombre, la descripción y la ruta de cada Skill. El código fuente público actual representa ese catálogo como un fragmento de contexto `developer` y el cuerpo del Skill seleccionado como un fragmento user envuelto en `<skill>` ([`fragments.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/fragments.rs), [`extension.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/extension.rs)). Algunas pasarelas pueden mostrar el contenido `developer` en una zona similar a system. En consecuencia, la tabla atribuida a DeepSeek solo sirve como instantánea de versiones concretas: el rol, la envoltura y la reconstrucción por turno son detalles del Harness. El coste de caché también debe evaluarse por runtime, sin trasladar automáticamente el modelo de Claude Code a todos los clientes.
