...
| 7-6 | [정식 학습 보고서](voice-sft-training-report/) · [orpheus](orpheus/) · [sesame](sesame/) | ✅ | 변경 불가능한 실행 완료 업스트림 노트북 2개에 loss 358행과 원본 WAV 4개를 보존하고, 실제 Voxtral 직접 오디오 심사 2회를 원시 응답과 함께 기록합니다. 실행 가능한 증거는 본문의 메커니즘 표기가 반대임을 보여 줍니다. Orpheus는 텍스트 준언어 태그를 사용하고 Sesame은 이전 오디오 대화 문맥으로 화자·스타일을 조건화합니다. Orpheus 음성이 하나뿐이고 Sesame 프롬프트에는 태그가 없어 두 통제 효과는 측정하지 못했다고 명시하며, 체크포인트는 배포하거나 승인 요건으로 삼지 않습니다. |
