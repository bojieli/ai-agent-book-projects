...
| 7-6 | [规范训练报告](voice-sft-training-report/) · [orpheus](orpheus/) · [sesame](sesame/) | ✅ | 两份不可变上游执行 notebook 保留 358 条 loss、4 段原始 WAV 与 2 次真实 Voxtral 直音频评审；可执行证据显示正文机制标签相反：Orpheus 使用文本副语言标签，Sesame 通过历史音频上下文保持说话者/风格。单条 Orpheus 音频无法检验跨句一致性，Sesame 提示未带标签，二者均如实记为未测；checkpoint 不随书分发也不作为验收门槛 |
