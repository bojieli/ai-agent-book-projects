...
| 7-9 | [cot-distillation](cot-distillation/) | ✅ | OpenRouter 経由で Claude などの先端モデルから CoT 軌跡を蒸留し、ルールベース検証器でフィルタして SFT データを生成する（実験 7-9）。 |
...
| 7-14 | [RLVP](RLVP/) | 📖 | Rewarding Outcomes, Penalizing Paths（RLVP）のポストトレーニング研究（実験 7-14）。完全な訓練・評価コードは外部論文リポジトリ `19PINE-AI/rlvp` にあり、別途 clone が必要。 |
