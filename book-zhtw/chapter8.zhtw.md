...
> **2026 年修訂。** 修訂版明確指出「SFT 記憶、RL 泛化」是 GeneralPoints/V-IRL 受控對照中的觀察，而非普遍定律；同時區分模型模擬工具回傳值與模擬完整環境動態，並把模擬器偏差視為訓練的上限。
>
> 本版凸顯兩條提升樣本效率的路徑：On-Policy Distillation 把一次 rollout 的終點獎勵轉成逐 token 指導；RLVP 把原本浪費的路徑回饋轉成可學習訊號。沒有更強教師時，OPSD 使用特權資訊，讓同一模型分飾教師與學生。
>
> 本版實驗順序：7-13 SimpleVLA-RL；7-14 ReTool；7-15 AWorld-train；7-16 RLVP。

本書的核心公式是 Agent = LLM + 上下文 + 工具。本章聚焦於最佳化 LLM 這個「大腦」——透過後訓練讓模型更好利用上下文和工具，從而提升整個 Agent 系統的能力。第七章結尾指出，評估體系與模擬環境是後訓練的兩塊基石：評估環境為訓練提供練習場，評估指標為訓練定義目標。本章就建立在這兩塊基石之上，討論如何真正改動模型權重，把能力沉澱進引數。
...
**後訓練也會塑造模型何時行動。** 以 Coding 模型為例，GPT 系列與 Claude 系列經常呈現不同的預設行動閾值：前者可能先讀更多儲存庫資訊再修改，後者可能用較少檔案完成定位、先實作再藉測試回饋修正。這不是把模型擬人化成「謹慎」或「有直覺」，而是參數中的策略在估計：多讀一個檔案的預期價值，是否仍高於提交目前修補並驗證的預期價值。若 SFT 示範反覆包含廣泛調查後才編輯的軌跡，模型就會模仿較高的行動閾值；若 RL 的過程或結果獎勵持續認可快速定位、儘早進入可驗證迴圈，機率質量就會移向較早行動的軌跡。第七章實驗 7-7 在完全相同的中性 Coding Harness 中換模，確實測到這種差異隨模型變化，說明 Harness 無須強制流程，模型自身也會攜帶穩定的工具使用策略。Harness 可以調節它，但行為的主要來源可以位於後訓練後的模型參數中。由於廠商不公開完整資料與獎勵配方，這個實驗能證明的是模型端的行為差異，不能據此斷言某一種具體的私有演算法造成了它。
...
- **其一，離線的天花板是資料，線上的天花板是任務。** SFT 的最優結果是把示範「完美復現」——所以它的上限就是**示範者的水平**，最多逼近、幾乎不可能超過；一份由 60 分老師標註的資料，訓不出 90 分的學生。RL 不看示範、只看結果獎勵：**任何**能拿到更高獎勵的行為都會被強化，哪怕沒有任何人演示過。於是 RL 能自己**發現示範里根本不存在的更優策略**——本章後面實驗 8-13 的 SimpleVLA 裡，模型自創的「推切」動作從未在人類演示中出現，正是「超越示範」的直接證據。RL 的上限由任務本身（獎勵能認可什麼）決定，而不是由資料裡恰好有什麼決定。
...
> **實驗 8-1 ★ `[設計方案]`：Q-learning 在尋寶遊戲中的表現**
>
...
以 Kimi K3 這類面向工具呼叫和長鏈思考最佳化的模型為例，可以看到 LLM+RL 正規化的典型方向：在大規模語言預訓練基礎上，透過後訓練強化問題分解、工具呼叫和自我糾錯能力。**OpenVLA**（詳見第六章）則展示了 LLM 時代的 VLA（視覺～語言～動作）架構正規化：視覺編碼器處理環境觀察、語言模型理解指令並推理、動作解碼器生成控制訊號，實現語言條件控制與跨任務泛化。需要澄清的是，OpenVLA 本身是在近百萬條機器人**演示軌跡**上透過模仿學習（行為複製）訓練的，屬於 SFT 性質而非 RL；真正把 RL 引入機器人、在這類 VLA 架構之上用獎勵進一步最佳化的代表，是本章後面實驗 8-13 的 SimpleVLA-RL。
...
> **實驗 8-2 ★★ `[設計方案]`：傳統 RL 與 LLM Agent 的對比研究**
>
>
...
在動手做 SFT 之前，有一個繞不開的實操問題：**SFT 資料從哪來？** 工業界的答案基本就三條路：**人工專家示範**——質量天花板最高，但貴且慢，適合用來定義格式與風格的「種子資料」；**教師模型生成**——即合成資料，讓強模型批次產出「輸入—輸出」對，過濾後再蒸餾給學生（實驗 8-8、7-9 都是這條路）；**模型自舉**——模型自己對同一問題取樣多條候選，用驗證器篩出正確樣本再反過來訓練自己，這就是拒絕取樣微調，詳見實驗 8-9。三條路經常組合使用：先用少量人工種子立住格式，再用教師模型放大規模，最後用拒絕取樣把質量拉齊。無論走哪條路，構造流程都大同小異：先定義任務分佈與輸出 schema，再批次生成候選，然後用規則校驗、格式檢查加人工抽檢做質量過濾，最後去重、平衡配比、保證多樣性。量級上不必貪多——數千到數萬條高質量樣本通常就足以固化協定，與其堆十萬條髒資料，不如精修一萬條乾淨資料：資料裡的每一處噪聲，SFT 都會忠實地寫進引數。
...
>
> **實驗 8-7 ★★★ `[設計方案]`：多語言思考——讓模型用任意語言思考 `[擴充套件實驗]`**
>
...
第七章的 bad case 也能轉成訓練資料。對於過早結束的 Coding Agent，截取宣稱完成前的軌跡前綴，把原宣稱作為 rejected，把「先執行測試、逐條核對驗收條件，再下結論」作為 chosen。這類資料更適合 DPO 或決策邊界示範，而不是普通 SFT；失敗原因、適用條件和驗證器都應隨樣本保存。實驗 8-17 的 `build_preference_data.py` 提供決定性模板和教師模型路徑，並分開保存訓練集與評估集。

同一批任務也可成為 RL 練習環境。SFT 使用已驗證的軌跡，RL 則讓目前策略重新執行任務，由外部驗證器判定結果；bad case 因而定義了模型需要改善的決策邊界，而不只是供模型背誦的例子。

## 何時選擇 SFT，何時選擇 RL

7.1 節講清了 SFT 與 RL 的**本質區別**，這一節回答一個更實操的問題：**面對一個具體任務，到底該用哪一個？** 下面的決策框架部分結論會在後續 RL 實驗（實驗 8-10、實驗 8-11）中進一步驗證，讀者可先建立初步判斷，讀完 RL 部分再回來對照。
...
>
> **實驗 8-11 ★★ `[外部倉庫]`：GeneralPoints——單輪 RL 的 「記憶與泛化」 對照**
>
>
...
> **實驗 8-12 ★★★ `[外部倉庫]`：V-IRL-VL——多輪視覺導航**
>
> V-IRL[^ch8-24]讓 Agent 在真實城市街景中連續導航：訓練使用紐約路線，測試遷移到不同城市，並同時改變方向表達和視覺外觀。RL 在規則和視覺 OOD 上都明顯優於 SFT，說明在多輪任務中，策略需要學會根據當前觀測重新規劃，而不是重現訓練軌跡。實驗使用帶價值網路的 PPO，並觀察到逐步回饋能緩解長時序信用分配。

> **實驗 8-13 ★★★ `[外部倉庫]`：SimpleVLA-RL——結果獎勵下的開放探索 `[擴充實驗]`**
>
> SimpleVLA-RL 在 LIBERO 機器人任務中只使用成功/失敗結果獎勵。每個任務僅用一條示範軌跡做 SFT 冷啟動，隨後 RL 將成功率從 17.3% 提升到 91.7%，並發現示範中沒有出現的「推切」動作。它與 V-IRL 形成對照：過程訊號容易定義時能加速學習，最佳路徑未知時稀疏結果獎勵反而保留更大的探索空間。
...
**InstructGPT 的三段式管線。** OpenAI 的 InstructGPT[^ch8-4] 確立了沿用至今的標準流程：
...
**獎勵模型會被「過度最佳化」。** RM 終究只是人類偏好的代理指標（proxy）。Goodhart 定律說：一個指標一旦成為最佳化目標，它就不再是好指標——把代理指標推到極端，它與真實目標的相關性就會失真。OpenAI 的研究[^ch8-5]系統測量了這種**獎勵模型過最佳化（reward model over-optimization）**現象：隨著 RL 訓練推進，代理獎勵（RM 分數）單調上升，而真實質量（人類評估）先升後降。模型逐漸學會的不是「更好回答」，而是「讓 RM 打高分」——冗長、討好、貌似嚴謹的空話。這正是 reward hacking 在 RLHF 語境下的具體形態，KL 懲罰與早停是最常用的緩解手段；本章末尾「常見陷阱」中的獎勵駭客問題與此同源。

**DPO：跳過顯式獎勵模型。** DPO（Direct Preference Optimization，直接偏好最佳化）[^ch8-6]的出發點是：既然「訓練 RM + PPO」的組合最終效果是「提高被偏好回答的機率、壓低被拒絕回答的機率，同時不離參考模型太遠」，那不如跳過顯式 RM，把偏好對直接變成一個帶隱式獎勵的分類損失——數學上可以證明這等價於帶 KL 約束的離線偏好最佳化，獎勵模型被隱式地藏進了策略本身。DPO 訓練像 SFT 一樣簡單：不需要線上取樣、不需要價值網路、不需要單獨維護 RM。代價是它完全離線——無法探索偏好資料之外的新行為，效能天花板由偏好資料的質量與覆蓋面決定。
...
**GRPO（Group Relative Policy Optimization）**由 DeepSeek 提出，是目前 RL 訓練最常用的演算法之一。它的核心做法是比較同一問題的一組 rollout 來估計相對優勢，而不需要另外訓練價值網路。
...
![圖 8-13 GRPO 演算法流程](images/fig8-13.svg)
...
表 8-4 總結了主流方法的核心特點。閱讀時注意區分兩件常被混為一談的事：**獎勵從哪來**（規則驗證器、學習到的獎勵模型，還是人類偏好資料）與**用什麼演算法最佳化**。PPO 和 GRPO 對獎勵來源並不挑剔——既可以接規則驗證器（RLVR），也可以接獎勵模型（RLHF）；它們的真正差異在於優勢估計方式（價值網路 vs 組內相對基線）。

表 8-4 後訓練與推理時最佳化方法對比
...
回到本章的實驗，如實交代各自所用的演算法：GeneralPoints 與 V-IRL（實驗 8-11、7-12）來自同一項研究，用的是帶價值網路的 PPO；AdaptThink（實驗 8-10）用的是自訂的約束最佳化目標加重要性取樣；後文的 ReTool（實驗 8-14）用的是基於 veRL 改造的 PPO（訓練資料取自 DAPO-Math-17k，但最佳化演算法仍是 PPO），SimpleVLA（實驗 8-13）與 RLVP（實驗 8-16）則基於 GRPO。多輪場景下信用分配問題更復雜，不同演算法各有優劣。
...
這是全章我最想讓你記住的一節，也是本章主線二的正面陳述。前面花了不少篇幅講演算法，但工業界一線的經驗恰恰相反：**演算法的重要性，遠不及三個更基礎的要素——模擬環境的保真度、訓練資料的質量、基礎模型的能力。** 現成演算法你會用就行；真正拉開差距的，是環境和資料做得好不好。這也呼應了第七章的結論（評估與模擬環境是後訓練的基石），以及本章 7.2 節提到的 OpenAI 認知反轉——幾十年 RL 研究把優先順序搞反了，真實的排序是**先驗（基礎模型）> 環境 > 演算法**。
...
**第一個層次：模型合成工具呼叫的回傳值。** 以 ZeroSearch（2025）為例[^ch8-13]：訓練「會搜尋的模型」通常離不開真實搜尋引擎，而搜尋 API 有成本、有速率限制，回傳結果還不可控。ZeroSearch 乾脆用一個 LLM 扮演搜尋引擎：學生模型發出搜尋 query，由這個「模擬引擎」生成檢索結果回傳。更妙的是它用了**課程式**設計——訓練初期讓模擬引擎回傳高品質、強相關的文件，隨著訓練推進逐步摻入噪聲、降低回傳品質，逼學生學會在真實搜尋引擎那種不完美的回傳裡提取有用資訊。最終，訓練全程沒見過真實搜尋引擎的模型，直接對接真實搜尋依然表現良好。

**第二個層次：模型模擬整個環境的動態。** 不只是單個工具的回傳值，連「執行動作後世界會變成什麼樣」也可以交給模型。DreamGym（2025）[^ch8-14] 把環境動態蒸餾進一個推理式的「經驗模型」：給定當前狀態與 Agent 的動作，它逐步推理出狀態轉移和回饋訊號，從而在不訪問真實環境的情況下批次合成 rollout 用於線上 RL。客服、銷售類 Agent 的訓練普遍用 LLM 扮演使用者（使用者模擬器），τ-bench 系列評測正是建立在這個思路上——同一個模型模擬器，既能當考場，也能當練習場。
...
資料質量具體指什麼？至少三個維度：**覆蓋面**（有沒有覆蓋到部署時會遇到的各種情況，尤其是長尾和邊界情況）、**多樣性**（示範裡的說話者、風格、解法夠不夠豐富，否則模型會塌縮到單一模式，比如實驗 8-6 裡「所有人一個腔調」）、**標註準確性**（示範答案本身對不對，尤其思維鏈蒸餾裡，錯誤的思考過程會被學生一併模仿——所以實驗 8-9 要用規則驗證器先過濾掉答案錯誤的軌跡）。這三點的投入產出比，通常遠高於換一個更花哨的演算法。
...
拒絕取樣主要在給定題目上篩選答案，更進一步則可以讓 Agent 改變**題目分布本身**。Autodata 的 Agentic Self-Instruct 讓一個主 Agent 協調四個角色：challenger 生成任務，弱求解器與強求解器分別嘗試，verifier 判斷答案品質並將回饋傳回出題環節。系統據此尋找「強模型能夠解決、弱模型仍有困難、評價器又能可靠判斷」的任務，將推理算力轉化為位於目前能力前沿的新訓練資料[^ch8-12]。
...
Agent 在線上服務中會累積大量真實業務資料，例如使用者請求、工單、工具呼叫紀錄和任務結果。其中最值得重複運用的，通常不是某位使用者說過的原話或某張真實訂單，而是這些資料所揭示的**任務結構**：使用者想完成什麼、Agent 當時能看到什麼、可以呼叫哪些工具、必須遵守哪些業務限制、常在哪一步出錯，以及成功後系統狀態應滿足什麼條件。因此，不要把真實工單和紀錄簡單改寫後就拿去訓練；更穩妥的做法是先移除可識別個人身分的資訊並彙整案例，從中提煉出**任務藍圖**，再使用全新的虛構人物、訂單和檔案，在隔離環境中重新建構任務。這樣既保留真實難點，也能降低模型記住隱私資訊、客戶資料或內部憑證的風險。

一條可執行的流程可以概括為：**線上真實資料 → 提煉任務類型 → 建構合成任務 → 多次 rollout → 雙重驗證 → 形成訓練資料**。2026 年的多項工作從不同方向收斂到這套原則：使用技能分類或技能圖系統化地涵蓋少見的能力組合，而不是隨意拼接任務[^ch8-18]；在可執行環境中合成包含診斷和修復的軌跡[^ch8-19]；盡量將測試、參考解法和求解過程分開，並用「修改前測試失敗、正確執行後測試通過」（fail-to-pass）等檢查，過濾掉「看似合理、其實訓練價值不高」的樣本[^ch8-20]；以及分別驗證任務本身和實際執行的軌跡[^ch8-17]。落地時可以分成三步：

1. **先合成任務，再取樣軌跡。** 從線上資料中抽取使用者意圖、初始狀態、可用工具、業務限制、成功條件和常見失敗方式，將相近情況整理成任務類型清單；接著為每一類重新產生姓名、編號、檔案內容和系統狀態，放進可重設的沙盒。例如，線上紀錄反覆出現「已部分出貨的訂單不能整筆退款」，合成器可以建立一批虛構訂單，改變商品數量、付款方式和出貨比例，但保留「只能退還未出貨商品款項」這條業務規則。每個**任務套件**至少要包含 Agent 看得到的任務說明和初始狀態，以及 Agent 看不到的驗證器、參考結果和任務產生說明。接著讓一個或多個較強的教師模型透過目標 Agent Harness 完整執行任務，記錄訊息、工具呼叫、工具回傳結果和最終狀態；同一任務嘗試多次，只保留通過驗證的軌跡。如果線上常見問題主要來自逾時、錯誤參數或中間檔案損壞，也可以在沙盒中安全地製造這些故障，專門合成「發現錯誤 → 診斷 → 修復 → 再驗證」的成功軌跡，而不只是一帆風順的專家示範。
2. **驗證器要分成「任務驗證」和「軌跡驗證」。** 任務驗證回答「這是不是一道好題」：參考解法能否在乾淨環境中完成任務；任務開始時測試是否失敗、正確執行後能否通過；說明是否完整但沒有洩漏答案；難度是否合適；聲稱需要某個工具或 Skill 的任務，在移除它之後是否明顯更難。軌跡驗證回答「這是不是一個好示範」：最終的資料庫狀態、檔案或外部操作結果是否遵守所有必要的業務規則，任務是否正常結束、工具呼叫是否合法；如果訓練目標包含特定流程，還要檢查相關 Skill 是否在決策前被讀取並真正影響了動作，而不是事後才提到。能寫成單元測試、資料庫斷言、資料格式驗證或狀態差異檢查的條件，應優先使用確定性程式碼；「溝通是否得體」這類難以形式化的部分，再由模型評估器補充，並持續用人工抽查校準。測試最好由沒有看過教師軌跡的獨立角色建構，再使用已知正確、已知錯誤，以及刻意漏掉一項限制的樣本做反向測試；否則驗證器很容易只會辨認參考答案的表面形式。
3. **同一套任務和驗證設施可以用於 SFT 和 RL，但用法不同。** 冷啟動時，使用通過驗證的完整成功軌跡進行 SFT：系統提示、使用者任務和工具回傳只作為上下文，不計入損失；主要監督 Agent 自己產生的推理、工具呼叫和最終回覆。這樣學到的是整個執行過程，而不只是最終答案。之後可以把合成任務產生器和驗證器直接變成 RLVR 環境，讓目前模型重新 rollout，並依據通過測試的程度給予獎勵。不要把失敗軌跡直接當成正確示範；它們可以用來建立偏好資料、找出訓練課程尚未涵蓋的任務類型，或在補上正確診斷與修復後成為復原示範。訓練資料還應先去除重複，再依客戶、時間區段或任務範本分組切分；獨立評估集則應來自與訓練集不重疊的真實業務樣本和任務類型。參考解法、隱藏測試和驗證器回饋絕不能出現在模型看得到的訓練內容中。

這套方法的重點不是盡可能多地產生資料，而是**根據線上真實情況決定要合成什麼，再用可執行的檢查決定留下什麼**。在這裡，品質永遠優先於數量：少量貼近真實任務、經過嚴格驗證的軌跡，通常比大量未經驗證的合成軌跡更有訓練價值。合成器負責補足任務涵蓋範圍，驗證器負責守住資料品質，SFT 教會基本動作，RLVR 再利用同一套任務和驗證設施提高成功率。

第六章會再次呼應這條判斷：語音識別裡模型「該不該收話」總在搖擺，根源不在模型結構，而在訓練標籤是用「上帝視角」標的——把標籤改成「只用決策當下能拿到的資訊」，問題就消失了。**很多時候，資料比架構更關鍵。**
...
「過早結束」是一個具體例子。模型宣稱完成時，Harness 在模型看不到的隔離工作區執行驗收測試，通過才給正獎勵，失敗給負獎勵。測試必須讀取真實檔案或環境狀態，不能只檢查「已完成」這句話。評估時分開未完成任務邊界集與確實完成任務保留集，前者觀察過早結束，後者確認模型仍能正常收尾。
...
從單輪到多輪，複雜性發生了質的躍遷。策略不僅要選擇當前最優動作，還要考慮未來的狀態價值；不僅要處理即時回饋，還要在延遲獎勵下進行**信用分配（Credit Assignment）**——判斷多步序列中到底哪一步對最終結果貢獻最大。比如一個客服 Agent 用了 10 輪對話解決了使用者問題，最終獲得好評——但這個好評該歸功於第 2 輪的精準提問，還是第 7 輪的耐心解釋？
...
最可靠的來源是**可驗證獎勵（RLVR）**：用測試案例、資料庫斷言、狀態差異或格式檢查直接判斷結果。數學答案、程式碼測試和結構化工具呼叫都適合從二元結果獎勵開始；規則越確定，獎勵越便宜、可重現，也越不容易被模型鑽漏洞。
...
這裡還要區分兩個容易混淆的概念：**reward hacking** 是鑽規則或實作漏洞拿高分，**reward seeking** 則是模型先在心裡建立一個「評判器會看什麼」的模型，再按這個猜測調整行為。後者不一定竄改測試或偽造結果，卻可能在長程任務中自設一個很淺的檢查，剛好通過就提前結束，交付物因此只滿足代理指標而沒有滿足真實意圖[^ch8-29]。所以「通過了 grader」不能自動等價於「任務完成了」：評判器是意圖的代理，訓練越強，模型越可能把代理當成目標本身。
...
「過早結束」是一個具體例子：模型說任務完成時，Harness 在隔離工作區執行模型看不到的驗收測試；通過才給正獎勵，未通過則給負獎勵。測試必須讀取真實檔案或環境狀態，不能只檢查模型是否說了「已完成」，否則模型可能學會口頭承諾驗證而不真正驗證。評估時還要把任務未完成的邊界集和確實已經完成的保留集分開，前者觀察過早結束率，後者觀察模型是否仍然能夠正常收尾，避免把模型訓練成永遠不敢結束。

**過程獎勵（PRM）**在中間步驟提供回饋，例如檢查身分驗證、工具參數、測試通過數或導航動作。OpenAI 的《Let's Verify Step by Step》[^ch8-7]展示了逐步驗證在數學推理中的價值。過程獎勵能緩解長時序信用分配，卻可能把模型限制在設計者預設的路徑上，而且標註和驗證成本更高。V-IRL-VL（實驗 8-12）採用逐步導航回饋，SimpleVLA-RL（實驗 8-13）則保留終點獎勵，兩者構成「密集回饋換收斂速度、稀疏回饋換探索空間」的對照。
...
結果獎勵解決「事情有沒有辦成」，卻表達不了「是否按規定辦成」。真實 Agent 可能透過改測試檔案、跳過身分驗證或執行破壞性指令獲得表面成功。RLVP（Reinforcement Learning with Verified Penalty）[^ch8-9]的原則是：**獎勵結果，懲罰路徑**。它針對的是可機器判定的、與最終成敗無關的**結果中性約束**；它不能取代對語意意圖、交付完整性和早停行為的獨立檢查。
...
在實作層面，可以先把驗證器輸出拆成兩路，再交給現有的策略最佳化器：
...
RLVP 的關鍵不是「獎勵越密越好」，而是能否補回組內差異。純結果獎勵在全敗組和全勝組都會產生零變異數、沒有梯度；違規動作通常容易偵測，懲罰幾乎總能補回差異；進展獎勵只有在部分進展可達時才有效。設計時應遵循四點：只懲罰具體動作，不懲罰「不夠努力」；結果獎勵始終保留，避免模型學會什麼都不做；每個懲罰最好配一條可達的合規路徑；規則必須確定、難以鑽漏洞。如果基礎策略根本不會取樣合規動作，應先用少量示範把這條路徑「種」出來，待合規行為穩定後再逐步減弱路徑塑形。換句話說，懲罰是通常可達的那一半，進展獎勵是受可達性門控的那一半。

> **實驗 8-16 ★★★ `[外部倉庫]`：RLVP——獎勵結果、懲罰路徑**
>
> 在 GRPO 上加入結果獎勵 $O$ 與路徑訊號 $\Phi$，對比純結果獎勵。TerminalBench 上違規次數由 3.71 降至 0.66，而成功率基本持平；miniF2F 上可達的部分獎勵把達到 0.9 成功率所需迭代從 7.0 降至 4.4。軟體修復中若所有 rollout 都無法通過任何測試，進展訊號不可達，加入它不會帶來收益。這個實驗提醒我們：先測訊號可達性，再決定是否增加獎勵維度。
...
> **實驗 8-14 ★★★ `[外部倉庫]`：ReTool——程式碼直譯器增強數學解題**
>
>
> ![圖 8-17 ReTool 互相影響文字～程式碼思考與沙盒執行回饋迴圈](images/fig8-17.svg)
>
>
...
>
> **實驗 8-15 ★★★ `[外部倉庫]`：AWorld-train——在沙盒中學習使用工具**
>
>
...
On-Policy Distillation（在軌蒸餾）由 Thinking Machines Lab 於 2025 年系統提出並推廣[^ch8-10]，如今已經是後訓練裡非常主流的一種方法，值得單獨講清楚。要理解它解決了什麼，先看 SFT 和 RL 各自的一個致命短板——它恰好把兩者的優點合到了一起。
...
一個巧妙的破題思路是 **On-Policy Self-Distillation（OPSD，在軌自蒸餾）**[^ch8-15]：**讓同一個模型分飾教師和學生兩角，區別只在於上下文。** 教師版能看到「特權資訊」（privileged information）——比如題目的標準答案、一條已驗證的正確解答——它不需要真的「會做」這道題，只需要拿著答案把學生走出的每一步**合理化**，給出逐 token 的目標分佈；學生版只看到問題本身，在自己取樣的軌跡上向教師版對齊。背後的直覺是：「對著答案講題」遠比「獨立解題」容易——這與 RLVR 賴以成立的「驗證—生成不對稱」同構，只不過這裡的不對稱性被用來產生稠密的監督訊號，而不是一個稀疏的成敗純量。
...
當然，這個範式的邊界也很清楚，主要來自教師的能力上限被鎖在學生自己身上：**收益大小取決於「特權資訊能帶來多少額外能力」。** 如果模型拿著答案也講不清解題過程（比如答案來自窮舉搜尋而非可用語言解釋的推理），自蒸餾就沒有訊號來源。已有研究也觀察到樸素 OPSD 的失敗模式，例如自蒸餾過程中模型逐漸丟失原有的思考風格，需要額外的正則來穩定[^ch8-16]。「同一模型、不同上下文、互為師生」這個構想仍在快速演化，但它已經為「沒有更強教師」的普遍困境打開了一條路。

## 從 bad case 到後訓練

這一節回到第七章留下的問題：如何把由生產 bad case 建立的評估資料，轉成後訓練輸入？失敗歸因記錄、端到端回歸任務、軌跡前綴回歸任務和 Rubric 評分，各自對應不同的訓練用法。

表 8-4 第七章評估資料到第八章訓練用法的映射

| 第七章的評估資料 | 第八章的訓練用法 |
|---|---|
| 含驗證器的端到端回歸任務 | RL rollout 任務與可驗證獎勵（RLVR）；拒絕採樣微調（RFT）的取樣池 |
| 軌跡前綴回歸任務 | DPO 偏好對、決策邊界的 SFT 示範、On-Policy Distillation 的教師狀態 |
| 失敗歸因記錄（首個錯誤步驟與錯誤類別） | 過程監督（PRM）的負標籤；RLVP 路徑懲罰規則 |
| Rubric 多維評分與人工金標集 | 向量獎勵的各個維度；生成式獎勵模型（GRM）的訓練與校準資料 |

### 案例 1：Coding Agent 過早結束

**從 bad case 到歸因。** Coding Agent 可能在執行測試前就宣稱「已完成」，多目標任務只完成一部分便收尾，或失敗幾次後就判定任務不可能。首個錯誤是它在缺乏證據時準備下結論的決策邊界；後續測試失敗和重試都是結果。使用者糾正、負面回饋和事後稽核都能暴露這一類問題。

**構造訓練資料。** 端到端回歸任務在 Agent 宣稱完成時執行模型看不到的驗收測試：通過給正獎勵，失敗給負獎勵。軌跡前綴任務把過早宣稱標為 `rejected`，把「先執行測試、逐條核對驗收條件，再下結論」標為 `chosen`。教師模型生成候選，再由確定性驗證器篩選；之後變化任務類型、缺少的驗收條件和完成措辭，以小比例混入通用指令資料做 LoRA。

**評估。** 未完成任務的邊界集必須和確實完成任務的保留集一起評估。前者檢查模型是否先驗證而不是過早停止，後者檢查模型是否仍能正常收尾；否則模型可能變得過度謹慎、永遠不敢結束。

> **實驗 8-17 ★★：從「過早結束」bad case 到 DPO**
>
> **實驗目標**：跑通從失敗歸因、軌跡前綴回歸資料、DPO 偏好對、7B LoRA 訓練，到邊界集/保留集分開評估的完整流程。
>
> **資料構造**：配套專案提供 24 個涵蓋四類失敗的寫實案例，以及互不重疊的 held-out 集（邊界 12 個、保留 8 個）。這是教學實驗；生產資料應覆蓋更多任務族，並使用模型無法修改、也不能只靠口頭聲稱執行過的隱藏測試。

### 案例 2：中文引號

「把中文文章中的直引號統一成彎引號」不是全域字元替換規則。同一個 ASCII 引號可能位於中文正文、英文引文、Markdown 程式碼、程式碼區塊、註解、JSON 或路徑中。中文正文和中文註解可以轉換；可執行程式碼、英文原文、JSON/schema、路徑、識別符和無法判斷的區域必須保留。

**從 bad case 到歸因。** Harness 應先依作用域切分文件，比對模型輸出與允許/受保護片段，並執行 Markdown、JSON 和原始語言的語法檢查。如果渲染或序列化先改變了輸入，問題屬於 Harness；如果模型收到原始 byte，卻改動受保護的引號或漏改允許的中文引號，首個差異才是可交給後訓練的作用域判斷錯誤。

**構造訓練資料。** 用 Skill 定義正面和負面作用域規則。樣本成對保存原文與目標：中文正文、嵌套引號和中文註解是可修改正例；英文文字、字面量、JSON、路徑、行內程式碼和程式碼區塊是必須保護的負例。train、holdout、boundary 按模板、體裁、變數組合和語言隔離，SFT 前執行機器門禁與分層人工抽查。

**評估。** 同時報告目標引號轉換率、受保護區保留率、非目標修改、語法有效性和整段文字 exact match。生產環境還需要一組已經正確的保留集，用來捕捉過度修改。

> **實驗 8-18 ★★：作用域敏感的彎引號 SFT**
>
> **實驗目標**：測試 LoRA SFT 能否只轉換允許的引號，並在未見過的上下文組合中保留受保護語法。
>
> **設定與資料**：Qwen3-8B bf16 LoRA，訓練 2 個 epoch、256 次更新；16 種片段、10 種文章體裁和 9 種程式語言；train 1024、holdout 256、boundary 256。Skill 同時作為標註、品質門禁與回歸規範，並分層人工抽查 48 筆。
>
> **結果**：基座模型留出集 exact 為 0%，適配後 holdout 為 96.9%，boundary 為 97.7%，受保護區保留率 100%。Python、JavaScript、Java、Go、Rust、SQL、Shell、YAML、Markdown 均為 100%；JSON 仍為 68.8%，需要獨立的結構化資料軌道。

### 案例 3：編輯檔案經常失敗

Coding Agent 常用 `edit_file(path, old_string, new_string)`。工具會精確比對 `old_string`，因此只要一個 byte 改變——空格、換行、反斜線、Unicode 組合或低頻 token——就會出現「old_string not found」。重試是症狀，不一定是根因。

**從 bad case 到歸因。** 沿著以下鏈路比較首個差異：
...
模型生成前的變化應歸因於檔案讀取器、序列化器或 Harness；tokenizer 的 encode→decode round-trip 也要獨立稽核。只有模型收到原始 byte，且模型輸出是鏈路上首個分歧點，才把案例分類為模型複製失敗並送入後訓練。

**構造訓練資料。** 使用三種可驗證任務：逐字複製、在相似 hard negative 中選出標記目標，以及把精確目標放入工具 JSON 的 `old_string` 欄位。隨機化長度、token 組合和上下文，特別加入空格、真實換行、字面量 escape、反斜線、Unicode 組合字元、中文和零寬字元；依 seed、長度、token 組成和上下文包裝隔離 split。

**評估。** 將 byte-exact、code-point-exact、token-exact、首次差異位置和 tokenizer round-trip，與端到端工具成功率分開報告。如果直接複製正確但 `edit_file` 仍失敗，應修復序列化或工具協議，而不是繼續訓練模型。

> **實驗 8-19 ★★：特殊字串的 exact-copy SFT**
>
> **實驗目標**：確認模型輸出是首個分歧層後，測試 LoRA SFT 對未見隨機字串的精確複製能力，並用獨立 tokenizer 稽核排除詞元化假象。
>
> **設定與資料**：Qwen3-8B bf16 LoRA 訓練 2 個 epoch；在 `verbatim`、`decoy_copy` 和 `tool_json` 三類任務上使用 train 1024、holdout 256、boundary 256。生成器使用可重現的隨機字串、hard negative、10 種語言上下文、8 種文章體裁，以及特殊空白、escape、Unicode、中文和零寬字元。
>
> **結果**：holdout byte-exact 由基座 37.5% 升至 78.9%，boundary 為 80.1%，平均首次 byte 差異位置為 54.0 和 54.2。512 筆 probe 中，Qwen3/Qwen2.5 tokenizer round-trip 為 80.1%，Mistral 為 100%；tokenizer 與 Harness 問題必須和模型複製結果分開回報。
...
本章回答了如何透過更新參數來實現 Agent 持續進化的問題。下一章我們會看到，參數只是 Agent 自我進化的四種載體之一，另外三種是知識、指令與程式。
...
[^ch8-9]: 本節的路徑懲罰設計、四條原則與實驗資料見 Li, Bojie and Noah Shi, “RLVP: Penalize the Path, Reward the Outcome” , 2026. arXiv:2607.07435.
[^ch8-10]: On-Policy Distillation 的方法與實驗見 Thinking Machines Lab, “On-Policy Distillation” , 2025.
[^ch8-11]: 這組 Agent 時間感的後訓練對照——DPO 與四種 RL 各自的失敗模式、以及 On-Policy Distillation 的突破——見 Li, Bojie and Noah Shi, “Agents That Sense Physical Time: Urgency, Persistence, and Vigilance as Missing Controls for LLM Agents” , 2026. https://01.me/research/physical-time-agent
