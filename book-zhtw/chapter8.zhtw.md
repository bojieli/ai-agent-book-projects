...
> **實驗 8-6 ★★★：讓 Hermes 閱讀本書並完成審查驅動的自我更新**
>
> **實驗目標**：將本章閉環施加到真實 Agent 程式庫。固定並複製 Hermes，讓它閱讀十章英文正文，審計讀者提出的四項缺口：產品級消融基礎設施、模型可見的 Agent Status Bar、持久記憶的遺忘與整合，以及具獨立執行證據的通用 Proposer–Reviewer 閉環。四項主張由 Prompt 提供，因此測量的是審計、候選產生與修正，而非自主發現問題。
>
> **真實執行與閉環**：2026 年 8 月 2 日，commit `85c8956e` 的 Hermes 透過 OpenRouter 使用 `openai/gpt-5.6-luna`，提出可選的 `<agent_status>`。沒有 proposer 會話上下文的全新 Hermes reviewer 審查每版候選；每次 `VERDICT: REJECT` 都送回原 Hermes 會話，使其修改自己的 checkout。五次終局拒絕發現快取、tool loop、輸出界限、重啟持久化、重試冪等性與既有記憶/plugin sidecar 組合等缺陷；第六次 fresh review 回傳 `VERDICT: ACCEPT`。
>
> **結果邊界**：接受版通過 **8 項新增行為測試與 36 項既有迴歸測試**，也通過編譯、空白、憑證掃描及乾淨 clone 套用補丁檢查。這完整展示「閱讀 → 自審 → 自改 → 被拒 → 依回饋修正 → 獨立接受」，不是半成品。補丁未合併至上游，也未執行下游消融，因此不能宣稱狀態列提升任務成功率。Runner、九輪 proposer 互動、六次 acceptance review、報告、補丁與去憑證 manifest 位於 [`hermes-self-evolution`](../chapter8/hermes-self-evolution/)。
