...
>
> **挑戰**：這次不是給 Hermes 一道特製題目，而是把整本書和它自己的程式碼一起交給它：「讀完，再回頭看看自己能怎麼變好，而且真的動手改。」讀者提供消融實驗、狀態列、記憶遺忘與 Reviewer 四條線索；Hermes 仍得自己讀相關章節，判斷哪些適合它。
>
> **它不只寫建議書**：讀完十章後，Hermes 把書中想法與自己的架構逐一對照，選擇先讓模型看見目前工作狀態。接著它直接修改自己的原始碼並執行驗證。
>
> **被退回，就繼續學**：新的 Reviewer 檢查修改；每次未通過，意見就送回原 Hermes。它重新看程式碼、找出問題、修改再試，直到新的 Reviewer 接受。流程很簡單：**閱讀 → 對照 → 修改 → 審查 → 學習 → 再修改**。
>
> **為什麼有意思**：一本書也能成為 Agent 的升級手冊。Hermes 證明自己能理解原則、映射到自身實作，並透過回饋完成真正的修改。這裡證明自我更新閉環跑通；是否改善所有下游任務，仍需另一個消融實驗。完整執行見 [`hermes-self-evolution`](../chapter8/hermes-self-evolution/)。
