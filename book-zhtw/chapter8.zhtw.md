...
RL 策略不直接模仿參考回答的 token，而是用獎勵評估模型**自己生成**的回答；獎勵計算仍可以使用參考答案或偏好資料。要從這種訊號裡學習，至少要滿足兩個前提：輸出能被驗證，並且目前策略偶爾能探索到有價值的行為。

第一個前提是**格式支援**。如果任務要求輸出 JSON 或工具呼叫，而模型吐出的是無法解析的文字，獎勵函式連「成功還是失敗」都判斷不了。此時 SFT 可以先扮演「把話說利索」的角色：用少量示範穩定格式與基本流程，使獎勵可計算，再由 RL 最佳化策略。這就是常見的「先 SFT、後 RL」。

第二個前提更根本，是**能力支援**。先在留出任務上以與訓練相近的溫度取樣，測量 `pass@1` 和 `pass@k`。若單次成功機率為 $p$，近似獨立取樣 $k$ 次至少成功一次的機率為

$$
\operatorname{pass@}k = 1-(1-p)^k.
$$

如果 `pass@1` 低、但 `pass@k` 隨 $k$ 明顯上升，說明正確策略已經處在模型分布裡，只是機率質量太小；RL、拒絕取樣或蒸餾都有東西可放大。反之，如果在合理的 $k$、取樣溫度和任務覆蓋下，實測 `pass@k` 仍接近 0，那麼基礎模型幾乎生成不出成功軌跡。只給終點 0/1 獎勵時，GRPO 的一組 rollout 很可能全為 0，組內優勢直接消失；PPO 也看不到「應該往哪裡移動」的正向樣本。把取樣量繼續放大只能以約 $1/p$ 的速度等待偶然成功，效率會迅速失去實際意義。

這時應先問缺的是什麼：缺領域語言、事實、程式碼模式或長上下文基礎能力，優先用 Mid-training 補底座；已有能力但不會按介面表達，先用 SFT；能夠局部前進但到不了終點，可以加入可驗證的部分獎勵或課程學習。RL 擅長把**已有但機率較低**的成功行為推高，不擅長從全零獎勵中憑空創造模型從未學過的知識和能力。
