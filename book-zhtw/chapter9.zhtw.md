...
觀察介面透過更早傳遞動態變化並將其留在記憶中，回答「兩張截圖之間發生了什麼」。但它本身不會消除規劃成本：Computer Use Agent 仍可能重複「截圖—思考—點選」的序列迴圈，每個動作後都重新考慮下一步。OSWorld-Human 顯示，人類等級的準確率可以伴隨遠多於人類的步驟和等待。

人會以預測方式操作桌面：先預測動作後果，觀察到的狀態符合預測時就沿用原計畫，不必從頭規劃；只有偏差才回到觀察和規劃。這就是推測執行，而世界模型讓 Agent 也能採用它。**世界模型解決的是另一半問題**：預測下一個狀態，符合就繼續，不符合就重新規劃或停止。
...
> **實驗 9-10 ★★：以 Gemini Robotics-ER 1.5 自主控制真實 XLeRobot**
>
> **目的:** 讓 Agent 取代人類，觀察桌面並呼叫受限的 pick、place、verify 技能，同時維持 9-8 的真機、任務和成功條件。
>
