...
以上所有討論都是工程視角：如何讓一組 Agent 協作完成任務。接下來一節的**體例與全書其餘部分不同**：前面九章半回答的都是「該怎麼建」，而它回答的是「正在發生什麼」——它是一份對當前多 Agent 社會實驗的綜述，給出的是觀察和開放問題，而不是可直接照做的工程方案。之所以仍然放在本章，是因為這些實驗恰恰把前面討論的上下文共享、協作拓撲和失敗模式推到了極端規模，而且其中兩個（史丹佛 AI 小鎮與語音狼人殺）本書提供了可重現與可執行的配套專案。讀這一節時，可以把它當作把前面的工程結論放進更長時間尺度後的壓力測試。
...
本章也是全書正文的最後一章。從第一章的 Agent = LLM + 上下文 + 工具，到建構、評估與進化，再到這一章的協作，工程側的路線已經走完。但有兩個問題是工程本身回答不了的：模型持續變強之後，今天這些 Harness 還會剩下多少；以及屬於具體使用者、具體公司的知識，能不能靠「再訓練一次」補上。後記接著討論這兩個問題。
