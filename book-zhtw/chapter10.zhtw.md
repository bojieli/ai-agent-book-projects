...
**不共享上下文**意味著每個 Agent 維護完全獨立的上下文和對話歷史，彼此無法直接存取對方的可見訊息、工具記錄及供應商返回狀態。這就像不同部門之間的協作：每個人在自己的工位上獨立工作，透過共享文件和會議紀要交換資訊，而不是時刻盯著別人的螢幕。這種模式的模組化和隔離性更好，每個 Agent 只需關注與自身職責相關的資訊；系統也更易擴充套件和維護——增加新 Agent 不需要改動現有 Agent 的內部邏輯，只需定義好介面和資料格式。
...
需要澄清的是，兩種架構都是真正的多 Agent 系統（因為每個階段的系統提示詞和工具集不同，就是不同的 Agent），區別在於協調方式。**共享上下文**依賴隱式協調——後續 Agent 繼承前序 Agent 的可見訊息、工具記錄，以及協定要求續傳的供應商返回狀態，資訊透過上下文字身傳遞；這不表示它能看到另一個模型未被 API 暴露的原始思維鏈。**不共享上下文**依賴顯式協調——Agent 之間透過檔案、訊息或結構化資料介面交換資訊，每個 Agent 只看到與自己相關的內容。
...
| 資訊隔離 | 不需要（所有角色共享可用軌跡） | 需要（如安全審查不應繼承前序自由文字分析或敏感中間輸出） |
...
**用共享檔案系統獲取狀態**。最徹底的形態是**軌跡持久化**（trajectory persistence）：子 Agent 在執行過程中，把自己的軌跡（第一章定義的 trajectory——使用者訊息、模型可見輸出、工具呼叫與結果，以及協定要求儲存的供應商返回狀態）即時序列化為 JSON，追加寫入檔案系統中的日誌檔案（通常每個會話一個檔案、每行一個事件，即 JSONL 格式）。主 Agent 無需任何狀態上報協定，直接讀取這個檔案就能看到子 Agent 的可觀察執行記錄：它最近發出了哪個工具呼叫、工具返回了什麼、是否卡在反覆失敗的重試上；它看不到 API 未暴露的原始思維鏈。軌跡記錄粒度細，卻不等於另一個程序的完整記憶體，而且事無鉅細也是負擔：軌跡動輒數萬 token，主 Agent 讀完還得自行提煉，既費時又費 token。因此多數場景下更合理的是**約定進度檔案**：主 Agent 啟動子 Agent 時約定「把進度寫到 progress.md」，子 Agent 每完成一項就更新這份任務清單，主 Agent 隨時讀這個輕量檔案即可掌握進展。這相當於兩個程序在共享儲存裡劃出一小塊約定格式的狀態區，暴露的是提煉後的進度，而非全部執行時狀態。進度檔案還附帶提供了**卡住偵測**：progress.md（或軌跡檔案）的最後修改時間超過 N 分鐘沒有變化，即可判定子 Agent 無活動、觸發超時兜底（呼應第四章的 Heartbeat 與 monitor_shell），避免系統被阻塞的子 Agent 拖累。

軌跡持久化的價值遠不止監控。回顧第一章的結論「Agent 的上下文 = 靜態字首 + 軌跡」：重新載入軌跡並拼上靜態字首，可以重建 Agent 的**對話與模型狀態**，這是會話恢復的重要基礎；若協定要求續傳供應商返回的思考狀態，也必須一併持久化。但軌跡不是完整的執行時檢查點。檔案和資料庫內容、瀏覽器與工具會話、鎖與租約、憑證及授權狀態、正在執行的請求，以及郵件、付款、訂單等已經發生的外部副作用，都位於軌跡之外；外部服務的狀態還可能具有非確定性。

因此，可靠恢復需要把軌跡放進一套更大的恢復協定：為有副作用的操作記錄持久化狀態並分配冪等鍵，對外部資源建立檢查點，在重啟時核對每項操作究竟未開始、進行中、已完成還是結果未知，再決定繼續、重試或執行補償。盲目重放整段軌跡可能重複傳送郵件、重複付款或重複下單。它與預寫日誌（write-ahead log）的類比仍然有用，但軌跡只是恢復協定中的一份日誌，必須與外部狀態檢查點和操作記錄配合，不能單靠重放保證安全恢復。這樣構建的多 Agent 系統才具備可恢復、可稽核、可移交的工程基礎：Manager 可以從最後一個已核對的狀態繼續，讓另一個 Agent 接手，或按記錄定位失敗原因。
...
多 Agent 系統有兩個正交的核心設計維度：上下文是否共享，以及協作拓撲如何組織。共享上下文是一種「繼承式」的多 Agent 協作——後續 Agent 繼承前序 Agent 的完整上下文，資訊零損耗但上下文膨脹快；不共享上下文則是完全獨立的多 Agent 協作，透過提煉後的移交包、檔案系統或訊息傳遞來交換資訊。在協作拓撲上，對等模式適合少量 Agent 的迭代改進，管理者模式適合需要動態排程的複雜任務，去中心化模式適合職責對等、控制權需要在 Agent 之間自主流轉的場景。這一切都架在兩套與拓撲無關的基礎設施之上，其設計藍本來自作業系統——Agent 之於執行時，恰如程序之於核心：靜態字首是程式，軌跡承載對話與模型狀態，LLM 是分時複用的 CPU。作為資料平面的**共享檔案系統**，本質是一棵掛載了 Agent 專屬工作區、多 Agent 共享空間、外部資源與系統內建資源四類區域的虛擬目錄樹，Agent 間透過傳遞檔案路徑交換產物；作為控制平面的**通訊與控制機制**，則支援訊息傳遞、狀態查詢、執行終止與資源調度。狀態查詢同樣落在兩大通訊正規化之內：或經訊息非同步問答，或經共享檔案系統旁路觀測——讀子 Agent 即時持久化的軌跡檔案，或讀雙方事先約定的輕量進度檔案。軌跡可以重建對話與模型狀態，卻不是完整的執行時檢查點；崩潰恢復還需要持久化操作記錄、冪等鍵、外部狀態檢查點與重啟後的狀態核對，避免重放造成重複副作用。訊息匯流排是控制平面的常見實現，適用於即時、非同步、多方的訊息協調；跨越組織邊界時，則需要 A2A 這樣的標準化互操作協定。
