...
**第二，必須建立評估（Evaluation）機制。** 這也是本書反覆強調的一點：沒有評估，就沒有進步。評估讓你能分辨一次改動究竟是真的變好了，還是隻是運氣，從而讓 Agent 的迭代方向不再依賴直覺。說到底，我們主張的是用科學的方法去做工程、去做 Agent，而評估正是這套方法的地基。第七章會專門展開這套方法。
...
書裡出現的實驗數字分兩類，讀者可以按不同的力度採信。**一類有可重現的出處**：給出了樣本量、執行次數或信賴區間，或附有配套倉庫中的執行記錄與原始輸出，第七至九章的實驗大多屬於這一類。**另一類是示意性觀察**：來自一次小規模的對照執行，用來說明方向和機制，不足以支撐「某方案比另一方案高多少」的量級結論；這類數字會在實驗框裡明確標註。第七章會專門討論為什麼區分這兩者很重要——一個單次跑出來的百分點差異，往往落在統計雜訊之內。
