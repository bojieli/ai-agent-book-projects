...
2025 年 8 月至 10 月，我在圖靈《AI Agent 實戰營》上進行了一系列技術講座。講座的初衷很簡單：把 AI Agent 的設計從 「感覺驅動」 變成 「原則驅動」：不只是教大家跑通一個 Demo，而是深入理解 Agent 為什麼要這樣設計，每一個架構決策背後的取捨是什麼。這本書正是從那些講座的講稿和實驗中整理、擴展而來的。
...
**第二，必須建立評估（Evaluation）機制。** 這也是本書反覆強調的一點：沒有評估，就沒有進步。評估讓你能分辨一次改動究竟是真的變好了，還是只是運氣，從而讓 Agent 的迭代方向不再依賴直覺。說到底，我們主張的是用科學的方法去做工程、去做 Agent，而評估正是這套方法的地基。第七章會專門展開這套方法。
...
本書共十章，沿四個層次展開（圖0-2）。第一章建立 Agent 的基礎框架；第二至六章討論如何建構 Agent，依次展開上下文、知識、工具、程式碼生成與互動；第七至九章討論如何評估並持續改進 Agent，從度量系統、模型後訓練一直推進到由執行經驗驅動的持續進化；第十章則把視野擴展到多 Agent 協作。
...
- **第四章（工具）**探討 Agent 與外部世界互動的橋樑：工具就像是 Agent 的「手腳」，讓它能夠搜尋網頁、呼叫 API、操作資料庫等。介紹 MCP 工具互操作標準和五類工具的設計原則（感知、執行、協作、事件觸發、使用者溝通），展開多模態感知的三條技術路線（原生多模態處理、提取為文字、工具化多模態分析），並重點闡述執行工具的安全機制；事件觸發、使用者溝通與非同步執行環境則在第六章展開。
...
- **第六章（互動：觀察與動作空間的擴展）**撤掉前五章預設的「輪流發言」前提，沿**模態**與**時機**兩根軸展開 Agent 與世界的互動：非同步與事件驅動讓世界主動喚醒 Agent（事件觸發工具、使用者溝通工具、虛擬身分與隔離執行環境），語音把互動推到毫秒尺度（從級聯流水線到端到端全模態與全雙工），Computer Use 讓 Agent 像人一樣操作圖形介面，機器人操作則把動作延伸到物理世界（VLA 控制與 Sim2Real 遷移）。四個場景共享同一組原語——喚醒、安全點、取消、搶佔與快慢分離。
- **第七章（Agent 的評估）**建構一套科學的評估方法論。涵蓋評估環境（工具呼叫型和人機互動型兩種核心範式，以及章末單獨討論的模擬環境）、資料集的設計原則、LLM-as-a-Judge 自動化評判方法、評估驅動的模型選型，以及將評估結果轉化為系統改進的完整閉環。
...
- **如果你是 Agent 開發者**，建議按順序閱讀第一至九章：前六章給出建構方法，第七章建立評估基礎，第八章解釋怎樣訓練模型，第九章再把參數與其他更新載體納入完整的持續進化閉環。第十章可根據多 Agent 協作的需要選讀。
- **如果你時間有限**，優先閱讀第一章（建立全域認知）和第二章（掌握最關鍵的上下文工程）。第二章中 KV Cache 的底層原理較為技術化，初次閱讀可先跳過原理部分、只記住開頭給出的三條核心結論，不影響後續理解。
