...
實驗標題裡的星級只表達**難度**，不表達**投入**。預設情況下，實驗都配有本倉庫自帶的可執行程式碼，配好憑證即可跑；三種例外會在標題裡顯式標出：`[外部倉庫]` 表示需要自行 clone 訓練框架或評測基準等外部倉庫，`[設計方案]` 表示正文只給出架構與驗收標準、可執行程式碼需要讀者自己完成，`[需要真機]` 表示必須有真實機械臂等硬體才能執行。每章配套 README 裡還有逐項的專案型別表和外部相依清單。
