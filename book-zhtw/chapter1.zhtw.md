...
前面討論了模型如何透過強化學習將工具呼叫策略內化為原生能力。但 Agent 的行為改變不只發生在訓練階段。按照更新發生的位置和持續時間，可以把它理解為三條互補路徑（圖 1-1）：任務內的上下文適應、跨任務的外部產物（artifact）更新，以及訓練週期中的參數更新。
...
- **模型回覆**（Assistant Messages）：模型之前返回的可見輸出通常包括文字內容（`content`，即對使用者的回覆）和工具呼叫請求（`tool_calls`，即 Agent 採取行動的方式），有些 API 還會返回供應商特定的思考狀態。開源或本地模型可能透過 `<think>`、`reasoning_content` 等形式暴露思考 token；另一些 API 只返回需要原樣回傳的不透明、帶簽名或加密狀態，或提供可選的思考摘要。OpenAI 託管的思考模型不提供原始內部思維鏈；Responses API 可能返回不透明或加密的 reasoning item 以及可選摘要。框架應按對應模型的協定儲存和回傳這些狀態，但不能把原始思維鏈假定為一個通用、可讀的 `reasoning` 欄位，也不能把它當作忠實的解釋或稽核證據。
...
要驗證各個元件在特定系統中的作用，最直接的方法是**消融實驗**（Ablation Study）：就像醫生診斷時逐一排除病因——先去掉 A 元件看系統是否還正常，再去掉 B 元件，以此類推，從而判斷每個元件的貢獻。實驗 1-1 正是按這個思路對上述五個元件做了系統性測試，結果表明：去掉工具定義，Agent 完全喪失行動能力；缺少工具執行結果時，由於看不到上一步的回饋，Agent 會反覆呼叫同一個工具，陷入無限迴圈；在本實驗所用模型和 API 中，丟棄介面返回且協定要求回傳的思考狀態後，前後決策開始互相矛盾；至於歷史訊息，沒有它 Agent 等於失憶，於是從頭開始整個任務流程，重複執行已完成的步驟。這裡的消融結論描述的是這個模型與介面的行為，不能外推為「所有 Agent 都必須暴露或回傳可讀思維鏈」。
...
實驗結果揭示了各個上下文元件在這套實驗配置中的作用。**工具定義**（Tool Definitions，靜態字首的一部分）是 Agent 行動能力的基礎，沒有它，Agent 就無法識別和呼叫任何工具。**工具執行結果**（Tool Results）是閉環控制的關鍵，缺失它會導致 Agent「盲目」執行，陷入無限迴圈。**供應商返回的思考狀態**在本實驗使用的模型協定中需要隨 assistant 訊息保留；移除它會破壞多步決策的連貫性，但這不意味著該狀態必然可讀，更不意味著它等於模型做出決策的完整原因。**歷史訊息**（之前輪次的使用者訊息、模型回覆和工具執行結果）則防止了冗餘操作，保持任務執行的連貫性，避免重複犯同樣的錯誤。
...
讓我們透過一個多幣種收入彙總的具體例子來理解 Agent 的**軌跡**（trajectory）。軌跡是 Agent 在執行任務過程中不斷積累的訊息歷史——使用者訊息、模型回覆（可見文字、工具呼叫，以及協定要求儲存的供應商返回狀態）、工具執行結果。每一次呼叫 LLM 時，它接收的完整上下文由**靜態字首**（系統提示詞 + 工具定義）和**軌跡**（動態訊息歷史）兩部分組成（圖 1-3）。這揭示了一個關鍵事實：**Agent 的上下文 = 靜態字首 + 軌跡**。具體地說，靜態字首對應前文五個元件中的前兩項（系統提示詞 + 工具定義），軌跡對應後三項（使用者訊息 + 模型回覆 + 工具執行結果，隨互動不斷增長）。基於這個完整上下文，LLM 生成下一步的響應，然後這個響應又追加到軌跡中，供下一次呼叫使用。

![圖 1-3 Agent 軌跡——多幣種彙總任務的 ReAct 迴圈](images/fig1-3.svg)

讓我們透過虛擬碼來理解 Agent 軌跡的結構。下面的 `reasoning_state` 只是「按供應商協定儲存並回傳的狀態」這一抽象佔位符：它可能是可讀的 `reasoning_content`，也可能是不透明區塊，甚至根本不存在；註解中的任務分析是作者為了幫助讀者理解流程而新增的摘要，不是 API 暴露的原始思維鏈。
...
軌跡 = [
  ...
  {role: “assistant” , 
   reasoning_state: “<按供應商協定保留；內容未展示>” ,
   # 作者摘要：需要將所有貨幣轉換為 USD
   content: “” ,  # 沒有直接回複使用者
   tool_calls: [
...
這種設計的精妙之處在於**上下文的累積性**。每次 LLM 呼叫都能看到完整的軌跡，這讓它能夠理解當前處於任務的哪個階段、之前嘗試了什麼、得到了什麼結果。就像人類解決問題時會不斷回顧和總結，Agent 透過軌跡保持著對整個任務的全域性認知。同時，軌跡的結構化特性也讓系統具有高度的可除錯性：使用者訊息、可見回覆、工具呼叫和工具執行結果都被清晰地區分開來，可以據此複盤可觀察到的行為。這樣的軌跡並不等於模型內部推理的忠實解釋；若介面另行提供思考摘要，也應把它視為摘要而非原始思維鏈或稽核證據。
