...
這三個元件與 RL（強化學習，詳見第七章）的三個核心概念相關，但不是嚴格的一對一等同：上下文是 Agent 內部對觀察與歷史的表示；工具定義觀察／行動介面，而工具背後的物件仍屬於 Environment。
...
**閉源模型。** 目前 Agent 開發中最常用的兩大閉源模型廠商是 OpenAI（GPT/o 系列）和 Anthropic（Claude 系列）。閉源模型通常在能力上領先，但成本較高且受限於廠商的 API 策略。選模型時不要只看排行榜，**要在你自己的任務上做評估**（見第六章）。
