...
| 系統級驗證 | 第七章（評估） | 評估環境、資料集、自動化評估、可觀測性 | — |
| 模型層面的糾正 | 第八章（後訓練） | SFT（監督微調）、強化學習——將 Harness 中積累的回饋訊號寫入模型參數，可看作 Harness 工程的延伸 | 目標偏離、對齊與穩健性 |
| 經驗驅動的持續糾正 | 第九章（持續進化） | 軌跡學習訊號、知識/指令/程式/參數更新、自我修改、驗證與回滾 | 記憶污染、不安全的自我修改、能力漂移 |
| 多模態上下文與工具 | 第六章（多模態與即時互動） | 語音 Agent、Computer Use、機器人操作 | 多模態輸入的安全過濾、即時互動中的權限控制 |
