...
- **變更頻率**：頻繁變化的能力用 Skill 來編寫和分發，通常比修改專用工具更輕量；而穩定的底層操作更適合做成專用工具。但「文字更容易改」只說明交付形式輕量，不代表變更風險更低。
- **模型能力**：SOTA 模型可以用 Skill + 通用執行器的方式表達更多能力、減少工具數量；較弱的模型則需要結構化的工具 schema 來引導正確呼叫。第八章將討論 Agent 在持續進化中沉澱新能力時如何做出同樣的選擇。

**Skill 變更應當像程式碼變更一樣治理。** Skill 雖然是自然語言文件，卻可能指揮 Agent 執行 shell 命令、發布服務、修改資料庫或呼叫高權限工具，其實際影響完全可能與程式變更相同。因此，Skill 應進入版本控制，標明來源、所有者和適用權限，經過同行評審與自動化 lint/schema 檢查；合併前在沙盒中執行代表性任務和回歸測試，發布時採用分階段或金絲雀放量，並保留可立即回滾的版本。凡涉及生產寫入、資金、憑證或其他高後果動作，還應增加專門審批。把 Skill 當作程式碼治理，並不抵消它易於編寫和部署的優勢，而是讓這種速度具備可控的安全邊界。
...
在工具設計方面，粒度權衡、通用性設計、描述規範等 ACI 原則適用於所有工具；MCP 協定統一了工具互操作的標準，而層次化組織、動態工具發現和 Skills 回應了工具過多時的選擇挑戰——同時，接入第三方 MCP 伺服器意味著引入新的信任邊界，工具描述投毒、工具遮蔽和憑證管理風險需要在接入前審查、在執行時防禦；能夠引導真實操作的 Skill 變更也必須像程式碼變更一樣接受版本控制、評審、測試、分階段發布和回滾。貫穿所有工具設計的一條底線是引數傳遞的保真性：模型感知到的世界與工具操作的世界之間不能存在系統性的偏差。
