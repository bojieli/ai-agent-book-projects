...
> **實驗 7-17 ★★：從「過早結束」bad case 到 DPO 與 GRPO**
>
> 這個實驗把第六章的失敗接到兩條訓練路徑。`build_preference_data.py` 將 24 個過早結束案例轉成軌跡前綴偏好對：原完成宣稱是 rejected，先驗證再下結論是 chosen；`train_dpo.py` 提供 Qwen2.5-7B-Instruct 的 LoRA-DPO，選用的 `train_grpo_optional.py` 則用隱藏驗收測試比較線上可驗證獎勵。
>
