...
級聯系統從串行轉為串流時，最重要的不是把每個函式改成 `async`，而是允許**增量結果失效並被取消**。
...
這裡真正該被重新設計的，不是「動作介面」，而是「**觀察介面**」[^ch6-9]。核心思想是建構 Agent–電腦觀察介面（AOI），把連續的環境觀察轉換成模型便於處理的離散事件。其中有幾個關鍵技術：第一，**幀間關鍵幀捕獲**——先跳過幾乎沒變的畫面，再用一個小模型判斷畫面是否發生了有意義的變化，只在變化時才截一幀；第二，**音量門控的語音轉寫**，有聲音時才呼叫語音辨識，讓 Agent 能夠聽到聲音；第三，**把畫面敘述成文字**，讓模型把捕獲到的幀描述成一句話，這樣**即使原圖之後被清理出上下文，這句文字仍留在記憶裡**，實現了多模態互動歷史壓縮的效果。
...
> **閱讀提示**：本節始終使用同一個任務——「把紅色杯子放進托盤，把黃色廢紙放進垃圾盒，最後重新觀察並確認桌面狀態」。實驗 6-9 和 6-11 是 XLeRobot 真機實驗，需要機械手臂、標定、緊急停止裝置和現場觀察員；實驗 6-10、6-12 和 6-13 是對應的本機 GPU 實驗。真機與模擬會明確分開報告，但任務目標、動作語意和成功條件保持一致。
...
>
> 驗收標準與實驗 6-9 完全相同：杯子位於托盤內、廢紙位於垃圾盒內、機械手臂回到安全姿態且沒有碰撞或越界。區別在於，自主實驗的任務語意必須來自模型觀察，真實動作必須來自工具呼叫，最終狀態必須由新觀察確認；人只能負責啟動、緊急停止和安全監護，不能在中途代替 Agent 完成動作。這樣，實驗 6-9 與 6-11 才能直接比較「同一硬體、同一任務，人類閉環與模型閉環之間還差什麼」。
