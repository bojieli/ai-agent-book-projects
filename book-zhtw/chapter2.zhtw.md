...
>
> 上面三個維度的數字來自一次小規模的對照執行，是**示意性觀察**：它們說明的是方向——組織結構和工具描述的影響遠大於語氣風格——而不是可重現的量級。按第七章的標準，這樣的單次差異需要給出樣本量和重複次數才能支撐「高多少」的結論。讀者自己跑這個實驗時，重點應放在三個維度的相對排序是否穩定，而不是能否重現出同樣的百分點。
>
...
實驗 2-8 是個小規模的定性演示，給的是直覺。這套「提前算好、直接查一眼」的做法到底有多大用、邊界又在哪，筆者和合作者用一個專門的基準量化了一遍[^ch2-7]（這套做法有個統一的名字，叫**上下文蒸餾，Context Distillation**——Agent 狀態列是它最日常的形態）。結論：
...
>
> **詳細錯誤資訊**：包含四層內容——錯誤型別和描述、完整引數的 JSON、呼叫棧資訊，以及針對性的修復建議（如遇到 FileNotFoundError 時建議驗證路徑、檢查工作目錄、使用絕對路徑）。啟用後，Agent 在錯誤場景中找到替代方案的成功率從 60% 提升到了 95%，從盲目重試轉變為分析性的問題解決（同樣是一次小規模執行的**示意性觀察**，說明的是「把錯誤訊息寫詳細」這個方向有效，而不是這個幅度可重現）。
>
