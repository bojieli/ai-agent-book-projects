...
本節以 **OpenAI-compatible Chat Completions** 協定為例，詳細拆解 Agent 每次呼叫大模型時的完整請求構成。它不僅用於 OpenAI，也被 DeepSeek、Kimi、Qwen，以及 vLLM、Ollama 等開源模型服務廣泛實現，是目前跨廠商開發最常用的共同介面。Anthropic、Google 等廠商的原生 API 有各自的訊息與工具結構，不在本節逐一展開。

OpenAI 還提供了更新的 Responses API：它以 `instructions` 和輸入/輸出 item 組織請求，可使用 `developer` 指令，並可透過 `previous_response_id` 或 Conversations 管理會話狀態。不過，為了讓範例直接適用於上述開源模型與相容服務，本章仍以可移植性更強的 Chat Completions 為主；下面的 system、user、assistant、tool 是跨實現最常見的四種角色。
...
這個請求只包含兩條訊息：一條 system（開發者寫的規則）和一條 user（使用者的輸入）。模型返回一條 assistant 訊息作為回覆。在本章採用的基礎 Chat Completions 模式中，**每次請求都按無狀態方式構造，模型需要的資訊必須由客戶端在訊息列表中提供**；這不是所有 API 的普遍限制，例如 Responses API 還可以引用前一個響應或持久化的 Conversation。
...
1. **第二次請求包含了第一次的全部對話歷史**——system 訊息、user 訊息、第一次的 assistant 回覆（包含工具呼叫），以及新增的 tool 結果。這就是前面所說的基礎 Chat Completions 無狀態請求模式：Agent 框架必須在下一次請求中送回所需歷史。
...
> 4. **流式響應的優勢**：在這個本地實驗中，所選 Qwen 模型及其模板會輸出 `<think>` token，因此開發者能即時觀察它暴露的思考文字、工具呼叫決策和結果處理。這是本地模型的實驗可觀測性，不是託管 API 的通用屬性；生產系統通常也不應把這類原始文字直接展示給終端使用者。
...
> **ReAct 迴圈的實際案例。**
>
> 專案中的多輪工具呼叫遵循第一章介紹的 ReAct 思考～行動～觀察迴圈，此處不再重複其原理。上一節已經用 OpenAI-compatible Chat Completions 的 JSON 格式展示了這個過程的完整訊息結構。在本地部署的實驗中，這些 API 訊息會被服務端（如 vLLM、Ollama）自動轉換為模型內部的 token 格式。本實驗的 `local_llm_serving` 專案允許你直接觀察模型的原始輸入輸出 token 流，包括以下在通常 API 層面不可見的細節：
>
> **模型模板暴露的思考 token**：本實驗中的 Qwen3 在生成工具呼叫之前，會先在 `<think>` 標籤內輸出一段分析——例如評估哪些工具適用、規劃呼叫順序。這些 token 有助於研究特定本地模型的生成行為，但不能假定為完整、忠實的內部決策解釋，也不應作為安全稽核證據。
>
...
**第一，解釋了為什麼必須使用標準 API 格式**。如果開發者繞過 API、自行拼接訊息（比如把工具結果作為普通 user 訊息而非 tool 型別傳遞），Chat Template 會誤將工具響應識別為新的使用者查詢，破壞模型預期的多輪格式。以 Qwen3 的 Chat Template 為例：模型可能在一輪工具呼叫中保留 `<think>` 標籤內的內容；當模板偵測到新的使用者查詢時，又可能按模板規則清理這部分內容。如果工具結果被錯誤地標記為 user 訊息，就會誤觸發清理，影響多步任務的連貫性。

思考狀態的回傳規則不是跨模型統一的。DeepSeek R1 的官方協定要求在後續輪次剝離 `reasoning_content`；另一些 DeepSeek、Kimi、GLM 等模型或版本則要求把介面返回的 `reasoning_content` 原樣放回對應 assistant 訊息。Claude 的原生 API 使用帶簽名的 thinking block，並規定其回傳範圍。OpenAI 的 Responses API 不暴露原始思維鏈，但可能返回需要保留的不透明或加密 reasoning item 和可選摘要；在無狀態續接時，應把相關輸出 item 一併帶回。由此得到的通用結論不是「可見思考必須保留」，而是：**思考相關狀態屬於供應商協定的一部分，是否回傳、以何種形式回傳，都必須遵循具體模型和 API 的最新契約**。不要自行偽造、解析或向使用者暴露不透明狀態，也不要把可讀的思考文字當作忠實解釋。
