...
這三類資訊——程式碼、流程、環境——構成了 Agent 有效工作的最低資訊需求。模型本身的智力只是基礎，**上下文的品質才是 Agent 能力的真正關鍵**。一個中等能力的模型配上精心組織的上下文，往往能勝過一個頂級模型在資訊匱乏下的盲目摸索。
...
從上下文管理的角度看，Skills 機制對 KV Cache 極為友好。如果把所有專用程式碼工具的定義都放在系統提示詞中，數量膨脹會消耗大量的 token，而且會干擾模型的注意力；而在 Skill + 通用執行器的模式下，工具數量始終很少（如第五章所示僅需七個核心工具），Skill 的內容透過前述的漸進式披露機制按需載入，不會影響已快取的前綴。兩種形態的詳細對比和選擇框架見第四章，第八章則探討 Agent 在持續進化中如何判斷一項經驗應寫成知識、指令、程式還是模型參數。
...
實驗 2-7 是個小規模的定性演示，給的是直覺。這套「提前算好、直接查一眼」的做法到底有多大用、邊界又在哪，筆者和合作者用一個專門的基準量化了一遍[^ch2-7]（這套做法有個統一的名字，叫**上下文蒸餾，Context Distillation**——Agent 狀態列是它最日常的形態）。結論：
