...
>
> **模型的內部思考過程**：支援思維鏈的模型（如 Qwen3）在生成工具呼叫之前，會先在 `<think>` 標籤內進行思考——分析使用者意圖、評估哪些工具適用、規劃呼叫順序。這個思考過程對除錯 Agent 行為非常有價值。需要注意的是，`<think>` 是模型原始輸出流中的標籤，不一定會原樣出現在 API 的回傳結果裡：本實驗向 Ollama 傳入 `think=True`，Ollama 會在回傳前把這段思考解析出來，放進獨立的 `thinking` 欄位（串流下是逐塊的 `thinking` 增量），此時 `content` 裡就看不到 `<think>` 標籤了。兩者只是同一份思考內容的不同呈現方式，實驗程式碼對兩種情況都做了處理。
>
