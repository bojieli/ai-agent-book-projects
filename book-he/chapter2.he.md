...
>
> **תהליך ההיסק הפנימי של המודל**: מודלים התומכים בשרשרת מחשבה (למשל Qwen3) יסיקו תחילה בתוך תגיות `<think>` לפני שייצרו קריאות לכלים — תוך ניתוח כוונת המשתמש, הערכת אילו כלים מתאימים, ותכנון סדר הקריאות. תהליך היסק זה בעל ערך לניפוי שגיאות בהתנהגות הסוכן. שימו לב ש-`<think>` הוא תג בזרם הפלט הגולמי של המודל, ולא בהכרח מופיע ככזה בתשובת ה-API: הניסוי הזה מעביר ל-Ollama את `think=True`, ולכן Ollama מנתח את קטע ההיסק לפני ההחזרה ומכניס אותו לשדה `thinking` נפרד (בהזרמה — כמקטעי `thinking` מצטברים), ולכן התגית `<think>` אינה מופיעה בתוך `content`. אלה רק שתי דרכים שונות להציג את אותו תוכן היסק, וקוד הניסוי מטפל בשני המקרים.
>
