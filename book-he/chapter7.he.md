...
רשימה זו מכסה רק כשלים שמכריזים על עצמם. יש להוסיף גם את הקטגוריות שאינן משאירות שום שגיאה: הבנת דרישות וטיפול בעמימות, כאשר הסוכן בונה את מה שהוא עצמו ניסח מחדש ולא את מה שהתבקש, או בוחר בשקט קריאה אחת של בקשה עמומה; פריצת סביבת האימות, כאשר הוא עורך assertion, מוסיף `skip`, מחליף במוק את הלוגיקה הנבדקת, או מצהיר שעבר מבחן שמעולם לא הורץ; שינוי חלקי, כאשר שלוש נקודות קריאה מתעדכנות והרביעית — קריאה דינמית, binding בשפה אחרת, schema — נשמטת ובכל זאת הקוד מתקמפל; דיווח מידע שגוי למשתמש, כאשר כל קריאות הכלים והמצב הסופי נכונים אך הסכום, הסטטוס או התאריך בתשובה אינם; ורגרסיות לא-פונקציונליות, כאשר API ציבורי או schema משתנים בלי מיגרציה, או שוולידציה נמחקת כדי שבדיקה תעבור. בכל אלה השגיאה הראשונה אינה ערך שכלי החזיר אלא **assistant message** — שיפוט, הנחה, או שאלה שהיה צריך לשאול ולא נשאלה.
...
>
> שלב 1: דגימה מרובדת. בחרו עשרה מסלולי כישלון מתוך `t3a_failed.md`, ולפחות שלושה מהם חייבים להיות כשלים שקטים ללא שום שגיאת כלי — המבחן הוא שאף קריאת כלי לא החזירה שגיאה, שהסוכן הכריז על סיום או מיצה את הצעדים, ושרק פסיקת המאמת בסוף מסמנת כישלון.
>
