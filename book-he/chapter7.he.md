...
מדמה המשתמש עוקב אחר תסריט קבוע (מידע ידוע + כללי חשיפה), ובכך מבטיח יכולת שחזור של ההערכה תוך דימוי סגנון ההבעה ההדרגתי של משתמש אמיתי. למשתמש המדומה יש לרוב גם **סבלנות מוגבלת**: אם הסוכן מתקשר ביעילות נמוכה, המשתמש המדומה רשאי לסיים את השיחה, וכך המשימה נכשלת.
...
פרק זה נסב סביב שאלה אחת מרכזית: כיצד לקבוע אם סוכן השתפר או הידרדר? מהגדרת קריטריוני ההצלחה והבחנה בין Pass@k, ‏Best@k ו-Pass consecutive@k, דרך בניית סביבות בדיקה בנות-שחזור, תכנון מערכי נתונים שעומדים במבחן הדליפה, הצבת LLM כשופט וייחוס בר-ביקורת של מסלולי כישלון, ועד לשימוש בתוצאות ההערכה כדי להניע בחירת מודל ואיטרציה — כל חוליה בשרשרת הזו משפיעה על מידת האמינות של המסקנה.
