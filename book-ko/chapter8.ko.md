...
첫 번째 경계는 **증거와 지시를 분리하는 것**입니다. 원시 웹페이지와 도구 출력은 신뢰할 수 없는 증거이므로 스킬이나 비슷한 능력에 직접 기록해서는 안 되며 먼저 LLM이 요약해야 합니다. 쓰기는 버전 관리하고 풀 리퀘스트로 제출해야 하며, 서로 다른 출처의 검토자 LLM이 검토한 뒤에만 병합합니다.
