...
> **실험 8-6 ★★★: Hermes가 이 책을 읽고 리뷰 기반 자기 업데이트를 완료하게 하기**
>
> **목표:** 이 장의 루프를 실제 Agent 저장소에 적용합니다. 고정된 Hermes를 clone하여 영어판 10개 장을 읽고, 독자가 제시한 네 가지 격차—제품 수준 ablation 인프라, 모델이 볼 수 있는 Agent Status Bar, 영속 메모리의 망각·통합, 독립 실행 증거를 갖춘 범용 Proposer–Reviewer 루프—를 감사하게 합니다. 네 주장은 Prompt로 주어졌으므로 측정 대상은 감사·후보 생성·수정이며 자율 발견은 아닙니다.
>
> **실제 실행과 루프 완료:** 2026년 8월 2일 commit `85c8956e`의 Hermes가 OpenRouter를 통해 `openai/gpt-5.6-luna`를 사용해 선택형 `<agent_status>`를 제안했습니다. proposer 세션 문맥이 없는 새 Hermes reviewer가 각 후보를 검사했고, 모든 `VERDICT: REJECT`를 원래 Hermes 세션에 돌려보냈습니다. Hermes는 자기 checkout을 수정한 뒤 새 reviewer에게 다시 검사를 받았습니다. 다섯 번의 최종 거절이 cache, tool loop, 출력 한계, 재시작 영속성, retry 멱등성, 기존 memory/plugin sidecar 조합 결함을 찾아냈고 여섯 번째 fresh review가 `VERDICT: ACCEPT`를 반환했습니다.
>
> **결과 경계:** 수락된 버전은 **새 행동 테스트 8개와 기존 회귀 테스트 36개**, 컴파일, 공백 검사, 자격 증명 스캔, clean clone 패치 적용을 통과했습니다. 따라서 “읽기 → 자기 감사 → 자기 수정 → 거절 → 피드백 수정 → 독립 수락” 루프가 완결되며 중간 결과로 남지 않습니다. 패치는 upstream에 병합되지 않았고 downstream ablation도 실행하지 않았으므로 status bar가 업무 성공률을 높인다고 주장하지 않습니다. runner, proposer 상호작용 9회, acceptance review 6회, 보고서, 패치, credential-free manifest는 [`hermes-self-evolution`](../chapter8/hermes-self-evolution/)에 있습니다.
