...
>
> **도전:** 일부러 만든 문제 대신 책 전체와 Hermes 자신의 코드를 함께 건넸습니다. “다 읽고 나서 자신을 돌아보고, 실제 개선 하나를 완성해 봐.” 독자는 ablation, 상태 표시줄, 메모리 망각, Reviewer라는 네 가지 힌트를 주었지만 무엇이 자신에게 맞는지는 Hermes가 책을 읽고 판단해야 했습니다.
>
> **제안서로 끝나지 않았습니다:** 10개 장을 읽은 Hermes는 책의 아이디어를 자기 구조와 비교하고, 현재 작업 상태를 모델이 볼 수 있게 하는 개선을 먼저 골랐습니다. 그리고 자기 소스 코드를 직접 수정하고 검증을 실행했습니다.
>
> **거절도 다음 학습이 됩니다:** 새로운 Reviewer가 변경을 검사하고, 통과하지 못하면 의견을 원래 Hermes에 돌려보냈습니다. Hermes는 코드를 다시 읽고 문제를 고쳐 재도전했으며, 수락될 때까지 반복했습니다. 흐름은 간단합니다. **읽기 → 비교 → 수정 → 리뷰 → 학습 → 다시 수정**.
>
> **왜 흥미로운가:** 책은 사람뿐 아니라 Agent의 업그레이드 안내서가 될 수 있습니다. Hermes는 원칙을 이해하고 자기 구현에 연결해 피드백 속에서 실제 변경을 완성했습니다. 다만 모든 업무 성능이 좋아지는지는 별도의 ablation 실험이 답해야 합니다. 전체 기록은 [`hermes-self-evolution`](../chapter8/hermes-self-evolution/)에 있습니다.
