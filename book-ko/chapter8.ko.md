...
RL 정책은 참조 응답의 토큰을 그대로 모방하지 않고, 모델이 **스스로 생성한** 응답을 보상으로 평가합니다. 보상 계산에는 여전히 참조 정답이나 선호 데이터를 쓸 수 있습니다. 이런 신호로부터 학습하려면 최소한 두 가지 전제가 충족되어야 합니다. 출력이 검증 가능해야 하고, 현재 정책이 가끔은 가치 있는 행동을 탐색할 수 있어야 합니다.

첫 번째 전제는 **형식 지원**입니다. 과제가 JSON이나 도구 호출을 요구하는데 모델이 파싱할 수 없는 텍스트를 내놓는다면, 보상 함수는 “성공인지 실패인지”조차 판정하지 못합니다. 이때 SFT가 먼저 “말을 매끄럽게 만드는” 역할을 할 수 있습니다. 소수의 시연으로 형식과 기본 절차를 안정시켜 보상을 계산할 수 있게 만든 다음, RL로 정책을 최적화하는 것입니다. 이것이 흔히 말하는 “먼저 SFT, 다음 RL”입니다.

두 번째 전제는 더 근본적인 **능력 지원**입니다. 먼저 홀드아웃 과제에서 학습과 비슷한 온도로 샘플링해 `pass@1`과 `pass@k`를 측정합니다. 한 번의 성공 확률이 $p$라면, 근사적으로 독립인 $k$회 샘플링에서 적어도 한 번 성공할 확률은

$$
\operatorname{pass@}k = 1-(1-p)^k.
$$

`pass@1`이 낮아도 `pass@k`가 $k$에 따라 뚜렷이 올라간다면, 올바른 정책은 이미 모델의 분포 안에 있고 확률 질량만 너무 작다는 뜻입니다. RL이든 거부 샘플링이든 증류든 증폭할 대상이 있습니다. 반대로 합리적인 $k$와 샘플링 온도, 과제 커버리지에서도 실측 `pass@k`가 여전히 0에 가깝다면, 기반 모델은 성공 궤적을 거의 만들어 내지 못하는 것입니다. 종점에만 0/1 보상을 줄 경우 GRPO의 rollout 한 묶음이 전부 0이 되기 쉬워 그룹 내 어드밴티지가 그대로 사라지고, PPO 역시 “어느 쪽으로 움직여야 하는지”를 알려 줄 양성 표본을 보지 못합니다. 샘플 수를 계속 늘려 봐야 대략 $1/p$의 속도로 우연한 성공을 기다릴 뿐이어서, 효율은 금세 실용성을 잃습니다.

이때는 먼저 무엇이 부족한지를 물어야 합니다. 도메인 언어, 사실, 코드 패턴, 긴 컨텍스트 기초 능력이 부족하다면 Mid-training으로 기반부터 채웁니다. 능력은 있는데 인터페이스에 맞게 표현하지 못한다면 먼저 SFT를 씁니다. 부분적으로는 전진하지만 종점에 닿지 못한다면 검증 가능한 부분 보상이나 커리큘럼 학습을 더할 수 있습니다. RL이 잘하는 일은 **이미 존재하지만 확률이 낮은** 성공 행동을 끌어올리는 것이지, 전부 0인 보상에서 모델이 배운 적 없는 지식과 능력을 무에서 만들어 내는 것이 아닙니다.

중요한 경계가 하나 있습니다. “반드시 SFT를 먼저”라는 원칙은 출력 형식이나 기본 행동이 아직 자리 잡지 않았을 때에만 성립합니다. 실험 8-11에서 보듯이 Llama-3.2-Vision-11B는 엄격한 구조화 출력 설정에서 SFT 없이 곧바로 RL을 하면 실패합니다. 그러나 충분히 강하고 이미 0이 아닌 성공률을 가진 기반 모델은 SFT를 건너뛸 수 있으며, DeepSeek-R1-Zero가 바로 그런 경우입니다. 이 모델이 나중에 콜드 스타트 SFT를 추가한 것은 주로 가독성과 언어 일관성을 개선하기 위해서였지, RL을 위해 과제 지식을 주입하려던 것이 아닙니다. 더 완전한 Mid-training/SFT/RL 선택 절차는 뒤의 독립된 의사 결정 절에서 다룹니다.
...
Mid-training이 주로 메우는 것은 두 종류의 공백입니다.

- **지식 공백**: 범용 사전 학습이 대상 언어, 금융·의료·법률 도메인, 사내 문서, 특정 코드베이스를 충분히 다루지 않아 모델이 개념과 용어조차 이해하지 못하는 경우.
- **기초 역량 공백**: 목표 과제가 기반 모델이 아직 형성하지 못한 긴 컨텍스트, 코드 패턴, 수학적 유도, 멀티모달 표현을 요구하는 경우. 이때 문제는 답변 형식이 아니라, 충분히 많이 샘플링해도 모델이 정답에 거의 도달하지 못한다는 데 있습니다.

이는 SFT를 지식 주입의 주된 도구로 삼아서는 안 되는 이유이기도 합니다. SFT도 물론 소량의 사실은 기억할 수 있고, Mid-training 뒤에 놓여 도메인 질문에 답하는 법을 가르치는 데 자주 쓰입니다. 그러나 적은 수의 QA 쌍은 한정된 질문 방식만 덮으며, SFT가 잘하는 일은 “어떻게 꺼내고 어떻게 표현하는가”를 훈련하는 것이지 규모가 크고 서로 얽힌 원천 지식을 담아내는 것이 아닙니다. 반대로 Mid-training이 도메인 텍스트의 언어 모델 손실을 낮췄다고 해서 모델이 사용자의 질문에 맞춰 지식을 알아서 꺼내 준다는 보장도 없습니다. 계속 사전 학습과 지시 학습의 순서 및 데이터 구성이 지식을 QA 형태로 접근할 수 있는지에 크게 영향을 준다는 사실이 기존 연구에서 확인되었습니다[^ch8-31]. 견고한 레시피는 대개 Mid-training으로 지식과 역량을 흡수하고 → 소규모 SFT로 출력 형식을 세운 뒤 → 성공률이 0이 아니게 되면 RL로 성공률과 일반화를 끌어올리는 순서입니다.
...
‘사전 학습에서 RL까지: 네 단계의 전체상’ 절에서 세 가지 학습의 메커니즘을 정리했습니다. 이 절이 제시하는 것은 실무 진단입니다. **먼저 부족한 것이 기반인지, 프로토콜인지, 정책인지를 판단하고, “모델이 잘 못한다”를 모두 RL이 필요하다는 결론으로 뭉뚱그리지 마십시오.**
...
실제 의사 결정은 다음 순서로 진행할 수 있습니다.

1. **먼저 가중치를 바꾸지 않아도 되는 방안을 배제합니다.** 프롬프트, 도구, 코드 제약, 컨텍스트 관리로 행동 문제가 풀린다면 학습할 필요가 없습니다. 사실을 자주 갱신·인용·삭제해야 한다면 RAG를 우선합니다.
2. **목표 홀드아웃 세트에서 역량 지지를 측정합니다.** 탐욕적 `pass@1`만 보지 말고, 샘플링 설정을 고정한 상태에서 `pass@k`, 부분 진척률, 형식 파싱률까지 보고 실패 원인을 사람이 감사합니다. `pass@k`가 여전히 0에 가깝고 실패가 지식·기초 역량에 몰려 있다면 먼저 Mid-training을 하고, 다시 평가한 뒤 다음 단계를 정합니다.
3. **SFT로는 프로토콜을 세우되, 지식 베이스를 억지로 밀어 넣지 마십시오.** 모델이 “할 줄은 알지만 요구대로 못 하는” 상태라면, 고품질 시연으로 JSON 스키마, 도구 호출, 용어 사용, 절차, 문체를 굳힙니다. 소량의 사실은 시연과 함께 파라미터로 들어와도 되지만, 대량의 사실 지식을 소수의 QA 쌍이 짊어지게 해서는 안 됩니다.
4. **탐색 여지가 있을 때만 RL을 올립니다.** 현재 정책이 이미 채점 가능하고 가끔 성공하는 rollout을 만들어 내며 보상이 배포 목표를 충실히 반영한다면, 확률이 낮은 성공 전략을 끌어올리고 시연 밖의 경로를 탐색하는 데 RL이 적합합니다. `pass@k`가 0에 가깝다면 먼저 Mid-training/SFT로 보충하거나 도달 가능한 커리큘럼과 부분 보상을 설계하십시오. 전부 0인 rollout 위에 PPO/GRPO를 바로 얹으면 대개 샘플링 예산만 소모합니다.

이 흐름은 모든 프로젝트가 세 가지 학습을 순서대로 다 거쳐야 한다는 요구가 아닙니다. 강한 기반 모델은 곧바로 RL로 들어갈 수 있고, 형식 위주의 과제는 SFT만으로 충분할 수 있으며, 안정적인 도메인 지식이라면 Mid-training만 한 뒤 기존 정렬 능력을 그대로 재사용할 수도 있습니다. 핵심은 각 단계마다 측정 가능한 진입 조건이 있다는 것이지, “Mid-training → SFT → RL”을 의례적인 파이프라인으로 취급하는 것이 아닙니다.
