...
> **2026년 개정.** 개정판은 “SFT는 암기하고 RL은 일반화한다”는 표현이 GeneralPoints/V-IRL의 통제된 비교에서 관찰된 경향일 뿐 보편 법칙은 아니라는 점을 분명히 합니다. 도구 반환값을 모델로 시뮬레이션하는 것과 전체 환경 동역학을 시뮬레이션하는 것도 구분하며, 시뮬레이터의 편향을 학습의 상한으로 다룹니다.
>
> 샘플 효율을 높이는 두 경로도 강조합니다. On-Policy Distillation은 rollout의 최종 보상을 토큰 단위 지침으로 바꾸고, RLVP는 버려지던 경로 피드백을 학습 가능한 신호로 바꿉니다. 더 강한 교사가 없을 때 OPSD는 특권 정보를 사용하고, 같은 모델이 교사와 학생 역할을 모두 맡습니다.
>
> 이 판의 실험 순서: 7-13 SimpleVLA-RL, 7-14 ReTool, 7-15 AWorld-train, 7-16 RLVP.
...
**post-training은 모델이 언제 행동하는지도 형성한다.** Coding 모델이 구체적인 예다. GPT 계열과 Claude 계열은 서로 다른 기본 행동 임계값을 보이는 경우가 많다. 전자는 편집 전에 저장소를 더 넓게 읽고, 후자는 더 적은 파일로 위치를 특정해 먼저 구현한 뒤 테스트 피드백으로 방향을 바로잡을 수 있다. 이는 한 모델을 ‘신중하다’, 다른 모델을 ‘직관적이다’라고 의인화하는 이야기가 아니다. 파라미터 안의 정책이 파일 하나를 더 읽는 기대가치가 현재 패치를 제출하고 검증하는 기대가치보다 여전히 큰지를 추정하는 것이다. SFT 시범이 편집 전의 폭넓은 조사를 반복해서 포함하면 모델은 더 높은 행동 임계값을 모방한다. 과정 보상이나 결과 보상이 빠른 위치 특정과 조기 검증 루프를 반복해서 인정하면 확률 질량은 더 이른 행동으로 이동한다. 실험 7-7은 완전히 동일한 중립 Coding Harness 안에서 모델을 교체해 이 행동이 모델과 함께 변함을 측정했다. Harness가 절차를 강제하지 않아도 모델 자체가 안정적인 도구 사용 정책을 가질 수 있다. Harness는 이를 조절할 수 있지만 주된 원천은 post-training된 모델 파라미터에 있을 수 있다. 공급자는 전체 데이터와 보상 레시피를 공개하지 않으므로, 이 실험이 확립하는 것은 모델 쪽의 행동 차이지 그것을 만든 특정 독점 알고리즘이 아니다.
...
- **첫째, 고정된 시연 밖의 후보를 평가할 수 있습니다.** SFT의 직접적인 지도 신호는 데이터에 기록된 응답에서 오지만, RL은 보상 함수가 채점할 수 있는 새 행동도 강화할 수 있습니다. 실험 8-13(SimpleVLA-RL)의 ‘pushcut’ 동작처럼 시연에 없던 전략을 발견할 가능성이 생깁니다. 다만 보상이 알아보지 못하는 품질은 학습할 수 없고, 탐색하지 못한 전략도 발견할 수 없습니다.
...
> **실험 8-1 ★ `[설계안]`: 보물찾기 게임에서 Q-learning의 성능**
>
...
> **실험 8-2 ★★ `[설계안]`: 전통적인 RL과 LLM 에이전트 비교 연구**
>
>
...
SFT를 직접 시작하기 전에 피할 수 없는 실용적인 질문이 있습니다. **SFT 데이터는 어디에서 오는가?** 업계의 답은 세 가지 경로로 요약됩니다. **인간 전문가 시연**은 품질 상한이 가장 높지만 비싸고 느려서 형식과 스타일을 정의하는 ‘시드 데이터’에 가장 적합합니다. **교사 모델 생성**, 즉 합성 데이터는 강력한 모델이 ‘입력-출력’ 쌍을 대량 생성하게 한 뒤 필터링하고 학생 모델에 증류하는 방법으로, 실험 8-8과 7-9가 이 경로를 사용합니다. **모델 자기 부트스트래핑**은 같은 문제에서 여러 후보를 샘플링한 뒤 검증기가 정답을 고르고, 선택한 샘플로 다시 모델 자체를 학습하는 방법입니다. 실험 8-9에서 자세히 다룰 거부 샘플링 미세 조정입니다. 세 경로는 흔히 결합합니다. 소량의 인간 시드 데이터로 먼저 형식을 고정하고 교사 모델로 규모를 늘린 뒤, 거부 샘플링으로 품질을 기준에 맞춥니다. 어느 경로든 구축 파이프라인은 대체로 같습니다. 작업 분포와 출력 스키마를 정의하고, 후보를 대량 생성하고, 규칙 기반 검증·형식 검사·수동 표본 검사로 품질을 걸러낸 뒤, 중복을 제거하고 혼합 비율의 균형과 다양성을 확보합니다. 규모를 탐낼 필요는 없습니다. 일반적으로 고품질 예시 수천~수만 건이면 프로토콜을 고정하기에 충분합니다. 지저분한 예시 10만 건을 쌓기보다 깨끗한 1만 건을 다듬으십시오. SFT는 데이터의 잡음까지 빠짐없이 파라미터에 충실히 기록합니다.
...
>
> **실험 8-7 ★★★ `[설계안]`: 다국어 사고—어떤 언어로든 사고하게 만들기 `[확장 실험]`**
>
...
7장에서 수집한 bad case는 학습 데이터로도 바꿀 수 있습니다. 코딩 에이전트가 너무 일찍 끝내는 경우 완료 선언 직전의 궤적 접두사를 잘라, 당시 선언을 rejected로, ‘테스트를 실행하고 인수 조건을 하나씩 확인한 뒤 결론을 내리기’를 chosen으로 둡니다. 이는 일반 SFT보다 DPO나 의사결정 경계 시연에 적합합니다. 실패 원인·적용 조건·검증기를 샘플과 함께 보존해야 추적과 재검토가 가능합니다. 실험 8-17의 `build_preference_data.py`는 결정적 템플릿과 교사 모델 경로를 제공하고 학습·평가 데이터를 분리합니다.

같은 작업을 RL 연습 환경으로 만들 수도 있습니다. SFT는 검증된 궤적을 사용하지만, RL은 현재 정책으로 작업을 다시 수행하게 하고 외부 검증기가 결과를 판정합니다. bad case를 단순히 외우게 하는 대신, 개선해야 할 의사결정 경계로 사용하는 방식입니다.

## SFT를 선택할 때와 RL을 선택할 때

7.1절에서는 SFT와 RL의 **본질적인 차이**를 설명했습니다. 이 절은 더 실용적인 질문에 답합니다. **구체적인 작업이 주어졌을 때 어느 쪽을 사용해야 하는가?** 아래 의사결정 프레임워크의 일부 결론은 이후 RL 실험인 실험 8-10과 7-11에서 추가로 검증합니다. 먼저 잠정적인 판단을 세운 뒤 RL 절을 읽고 돌아와 교차 확인해도 좋습니다.
...
>
> **실험 8-11 ★★ `[외부 저장소]`: GeneralPoints—단일 턴 RL에서 ‘암기와 일반화’ 비교**
>
...
> **실험 8-12 ★★★ `[외부 저장소]`: V-IRL-VL — 다중 턴 시각 내비게이션**
>
> V-IRL[^ch8-24]은 에이전트가 실제 도시 거리 장면에서 연속적으로 내비게이션하게 합니다. 훈련에는 뉴욕 경로를 쓰고, 테스트는 다른 도시로 옮기면서 방향 표현과 시각적 외형을 동시에 바꿉니다. RL은 규칙 OOD와 시각 OOD 모두에서 SFT를 뚜렷하게 앞서며, 다중 턴 과제에서는 정책이 훈련 궤적을 재현하는 대신 현재 관측에 따라 다시 계획하는 법을 배워야 함을 보여줍니다. 이 실험은 가치 네트워크를 갖춘 PPO를 사용하며, 단계별 피드백이 장기 기여도 할당의 어려움을 완화한다는 점이 관찰되었습니다.

> **실험 8-13 ★★★ `[외부 저장소]`: SimpleVLA-RL — 결과 보상 아래의 열린 탐색 `[확장 실험]`**
>
> SimpleVLA-RL은 LIBERO 로봇 과제에서 성공/실패 결과 보상만 사용합니다. 과제마다 시연 궤적 하나로만 SFT 콜드 스타트를 하고, 이어지는 RL이 성공률을 17.3%에서 91.7%로 끌어올리며 시연에는 없던 "밀어 자르기" 동작을 발견합니다. 이는 V-IRL과 대조를 이룹니다. 과정 신호를 정의하기 쉬울 때는 학습을 가속하지만, 최적 경로를 모를 때는 희소한 결과 보상이 오히려 훨씬 큰 탐색 여지를 남깁니다.
...
**InstructGPT의 3단계 파이프라인.** OpenAI의 InstructGPT[^ch8-4]는 오늘날에도 사용하는 표준 절차를 확립했습니다.
...
**보상 모델은 ‘과도하게 최적화’될 수 있습니다.** 결국 RM은 인간 선호도의 대리 지표일 뿐입니다. Goodhart의 법칙에 따르면 지표가 최적화 목표가 되면 더는 좋은 지표가 아닙니다. 대리 지표를 극단까지 밀어붙이면 실제 목표와의 상관관계가 왜곡됩니다. OpenAI의 연구[^ch8-5]는 이러한 **보상 모델 과최적화** 현상을 체계적으로 측정했습니다. RL 학습이 진행될수록 대리 보상인 RM 점수는 단조롭게 오르지만, 실제 품질인 인간 평가는 처음에는 오르다가 나중에는 떨어집니다. 모델이 점차 배우는 것은 더 잘 답하는 법이 아니라 RM에서 높은 점수를 받는 법입니다. 장황하고 비위를 맞추며 엄밀하게 들리지만 내용 없는 말을 생성합니다. 이는 RLHF에서 나타나는 보상 해킹의 구체적인 형태이며, KL 페널티와 조기 중단이 가장 일반적인 완화책입니다. 이 장 말미의 ‘흔한 함정’에서 다룰 보상 해킹 문제도 근원이 같습니다.

**DPO: 명시적인 보상 모델 건너뛰기.** DPO(Direct Preference Optimization)[^ch8-6]는 다음 전제에서 출발합니다. ‘RM 학습 + PPO’의 조합이 결국 ‘선호 응답의 확률은 높이고 거부 응답의 확률은 낮추되 참조 모델에서 너무 멀리 벗어나지 않는’ 결과를 낸다면, 명시적인 RM을 건너뛰고 선호도 쌍을 암묵적 보상을 포함한 분류 손실로 직접 바꾸면 되지 않을까요? 수학적으로 이는 KL 제약이 있는 오프라인 선호도 최적화와 같으며, 보상 모델이 정책 자체에 암묵적으로 내장된다고 증명할 수 있습니다. DPO 학습은 SFT만큼 단순합니다. 온라인 샘플링이나 가치 네트워크가 없고 별도의 RM도 유지할 필요가 없습니다. 대가는 완전히 오프라인이라는 점입니다. 선호도 데이터 너머의 새 행동을 탐색할 수 없으며, 성능 상한은 선호도 데이터의 품질과 범위로 결정됩니다.
...
**GRPO(Group Relative Policy Optimization)**는 DeepSeek이 제안했으며 오늘날 RL 학습에서 가장 널리 쓰이는 알고리즘 중 하나입니다. 별도의 가치 네트워크를 학습하지 않고 같은 문제에 대한 rollout 그룹을 비교해 상대 어드밴티지를 추정하는 것이 핵심입니다.
...
![그림 8-13: GRPO 알고리즘 흐름](images/fig8-13.svg)
...
표 8-4는 주류 방식의 핵심 특징을 요약합니다. 읽을 때 흔히 혼동하는 두 가지를 구분하십시오. **보상이 어디에서 오는가**(규칙 검증기, 학습한 보상 모델, 인간 선호도 데이터)와 **어떤 알고리즘으로 최적화하는가**는 별개의 문제입니다. PPO와 GRPO는 보상 출처를 가리지 않으며 규칙 검증기(RLVR)와 보상 모델(RLHF) 어느 쪽에도 연결할 수 있습니다. 실제 차이는 어드밴티지 추정 방식, 즉 가치 네트워크와 그룹 상대 기준선 중 무엇을 사용하는가에 있습니다.

표 8-4 사후 학습 및 추론 시점 최적화 방식 비교
...
이 장의 실험에서 사용한 알고리즘을 명확히 밝히겠습니다. GeneralPoints와 V-IRL(실험 8-11, 7-12)은 같은 연구에서 나왔으며 가치 네트워크를 사용하는 PPO를 씁니다. AdaptThink(실험 8-10)는 중요도 샘플링을 포함한 맞춤형 제약 최적화 목표를 사용합니다. 뒤에서 다룰 ReTool(실험 8-14)은 veRL 기반의 수정된 PPO 구현을 사용합니다. 학습 데이터는 DAPO-Math-17k에서 가져오지만 최적화 알고리즘은 여전히 PPO입니다. SimpleVLA-RL(실험 8-13)과 RLVP(실험 8-16)는 GRPO를 기반으로 합니다. 다중 턴 환경에서는 기여도 할당 문제가 더 복잡하고 알고리즘마다 장단점이 있습니다.
...
이 장에서 가장 기억해 주었으면 하는 절로, 두 번째 주제를 정면으로 다룹니다. 알고리즘을 설명하는 데 상당한 분량을 썼지만 산업 현장의 경험은 반대 방향을 가리킵니다. **알고리즘은 더 기본적인 세 요소인 시뮬레이션 환경의 충실도, 학습 데이터의 품질, 기반 모델의 역량보다 훨씬 덜 중요합니다.** 기존 알고리즘의 사용법을 알면 충분하며, 팀의 실력을 가르는 것은 환경을 얼마나 잘 구축하고 데이터를 얼마나 세심하게 선별하는가입니다. 이는 평가 및 시뮬레이션 환경이 사후 학습의 초석이라는 7장의 결론, 그리고 7.2절에서 살펴본 OpenAI의 우선순위 역전과도 일치합니다. 수십 년 동안 RL 연구는 우선순위를 거꾸로 두었으며 실제 순서는 **사전 지식(기반 모델) > 환경 > 알고리즘**입니다.
...
**첫 번째 수준: 모델이 도구 호출의 반환값을 합성합니다.** ZeroSearch(2025)를 예로 들어 보겠습니다[^ch8-13]. ‘검색할 줄 아는 모델’을 학습하려면 보통 실제 검색 엔진이 필요하지만 검색 API는 비용이 들고 속도 제한이 있으며 반환 결과도 제어할 수 없습니다. ZeroSearch는 아예 LLM에 검색 엔진 역할을 맡깁니다. 학생 모델이 검색 쿼리를 내면 이 ‘시뮬레이션 엔진’이 검색 결과를 생성해 돌려줍니다. 더 나아가 **커리큘럼식** 설계를 사용합니다. 학습 초기에는 시뮬레이션 엔진이 관련성 높은 고품질 문서를 반환하고, 학습이 진행될수록 잡음을 점차 섞어 반환 품질을 낮춥니다. 학생이 실제 검색 엔진의 불완전한 결과에서 유용한 정보를 추출하도록 강제하는 것입니다. 그 결과 학습 중 실제 검색 엔진을 한 번도 보지 못한 모델도 실제 검색에 바로 연결하면 좋은 성능을 냅니다.

**두 번째 수준: 모델이 전체 환경의 동역학을 시뮬레이션합니다.** 도구 하나의 반환값만이 아니라 ‘행동을 한 뒤 세계가 어떤 모습이 되는가’조차 모델에 맡길 수 있습니다. DreamGym(2025)[^ch8-14]은 환경 동역학을 사고형 ‘경험 모델’로 증류합니다. 현재 상태와 에이전트의 행동을 입력하면 단계별로 사고해 상태 전이와 피드백 신호를 만들고, 실제 환경에 접근하지 않은 채 온라인 RL을 위한 롤아웃을 대량 합성합니다. 고객 서비스나 영업 에이전트 학습에서는 LLM에 사용자 역할을 맡기는 사용자 시뮬레이터를 흔히 사용합니다. τ-bench 평가 계열도 바로 이 발상을 토대로 하며, 같은 모델 시뮬레이터가 시험장과 훈련장 역할을 모두 할 수 있습니다.
...
데이터 품질은 구체적으로 무엇을 뜻할까요? 적어도 세 차원이 있습니다. **범위**는 배포 중 마주칠 다양한 상황, 특히 롱테일과 엣지 케이스를 포괄하는지 묻습니다. **다양성**은 시연의 화자, 스타일, 해법이 충분히 풍부한지 묻습니다. 그렇지 않으면 실험 8-6의 ‘모두 같은 말투’처럼 모델이 단일 모드로 붕괴합니다. **주석 정확성**은 시연 답안 자체가 올바른지 묻습니다. 특히 사고 연쇄 증류에서는 잘못된 사고 과정도 학생이 모방하므로 실험 8-9에서 먼저 규칙 검증기로 오답 궤적을 걸러냈습니다. 이 세 가지에 투자한 비용의 수익률은 대개 더 화려한 알고리즘으로 전환하는 것보다 훨씬 높습니다.
...
거부 샘플링은 주어진 질문 세트의 답을 주로 필터링합니다. 한 단계 더 나아가 에이전트가 **질문 분포 자체**를 바꾸게 할 수 있습니다. Autodata의 Agentic Self-Instruct에서는 메인 에이전트가 네 가지 역할을 조율합니다. 도전자가 작업을 생성하고, 약한 해결사와 강한 해결사가 이를 풀며, 검증기가 답의 품질을 판단해 발견한 내용을 작업 생성 과정에 피드백합니다. 시스템은 강한 모델은 풀 수 있지만 약한 모델은 아직 어려워하고 평가자가 확실히 판단할 수 있는 작업을 찾습니다. 이렇게 추론 계산량을 현재 역량의 경계에 있는 새로운 학습 데이터로 전환합니다[^ch8-12].
...
실행 가능한 흐름은 **실제 서비스 데이터 → 작업 유형 추출 → 합성 작업 구성 → 여러 번 rollout → 두 단계 검증 → 학습 데이터 구성**으로 요약할 수 있습니다. 2026년의 여러 연구는 서로 다른 방향에서 같은 원칙에 도달합니다. 작업을 임의로 이어 붙이는 대신 스킬 분류 체계나 스킬 그래프로 드문 역량 조합을 체계적으로 다루고[^ch8-18], 실행 가능한 환경에서 진단과 복구를 포함한 궤적을 합성하며[^ch8-19], 테스트·참조 해법·풀이 과정을 가능한 한 분리하고 변경 전에는 테스트가 실패하지만 올바른 실행 후에는 통과하는 fail-to-pass 검사로 그럴듯해 보여도 학습 가치가 낮은 샘플을 제거합니다[^ch8-20]. 또한 작업 자체와 실제로 실행된 궤적을 별도로 검증합니다[^ch8-17]. 실무에서는 다음 세 단계로 진행할 수 있습니다.
...
6장에서 이 점을 다시 살펴봅니다. 음성 인식에서 모델은 사용자가 말을 마쳤는지 계속 판단하지 못하고 흔들립니다. 근본 원인은 모델 아키텍처가 아니라 ‘전지적 관점’에서 붙인 학습 라벨입니다. 결정 시점에 사용할 수 있는 정보만으로 데이터를 다시 라벨링하면 문제가 사라집니다. **아키텍처보다 데이터가 더 중요한 경우가 많습니다.**
...
‘조기 완료’는 구체적인 예입니다. 모델이 완료했다고 말하면 Harness가 모델에 보이지 않는 격리 작업공간에서 인수 테스트를 실행합니다. 통과할 때만 양의 보상을 주고 실패하면 음의 보상을 줍니다. 테스트는 ‘완료’라는 문장이 아니라 실제 파일이나 환경 상태를 읽어야 합니다. 미완료 작업 경계 세트와 실제 완료 작업 보류 세트를 분리해, 전자는 조기 종료를 줄이는지, 후자는 정상적인 마무리가 유지되는지 측정합니다.

### 다중 턴 작업의 핵심 과제

![그림 8-14: 단일 턴 RL과 다중 턴 RL 비교](images/fig8-14.svg)

![그림 8-15: 다중 턴 상호작용의 기여도 할당](images/fig8-15.svg)
...
> **실험 8-16 ★★★ `[외부 저장소]`: RLVP — 결과에 보상하고 경로를 벌한다**
>
...
> **실험 8-14 ★★★ `[외부 저장소]`: ReTool—코드 인터프리터로 강화한 수학 문제 해결**
>
>
...
>
> **실험 8-15 ★★★ `[외부 저장소]`: AWorld-train—샌드박스에서 도구 사용 학습하기**
>
>
...
Thinking Machines Lab이 2025년에 체계화하고 널리 알린 온폴리시 증류(On-Policy Distillation)[^ch8-10]는 이제 주류 사후 학습 방식으로, 제대로 설명할 가치가 있습니다. 해결하는 문제를 이해하려면 먼저 SFT와 RL 각각의 치명적인 약점부터 살펴보십시오. 온폴리시 증류는 둘의 장점을 결합합니다.
...
물론 이 패러다임의 경계도 명확하며, 주로 교사의 역량 상한이 학생 자신에게 묶여 있다는 사실에서 비롯됩니다. **향상 폭은 ‘특권 정보가 얼마나 많은 추가 역량을 가져오는가’에 달려 있습니다.** 모델이 답을 들고 있어도 풀이 과정을 명확히 설명하지 못한다면, 예컨대 답이 언어로 표현 가능한 사고가 아니라 무차별 탐색에서 나왔다면 자기 증류에는 신호의 원천이 없습니다. 기존 연구에서는 순진한 OPSD의 실패 유형도 관측했습니다. 예를 들어 자기 증류 중 모델이 기존 사고 스타일을 점차 잃을 수 있으므로, 안정성을 유지하려면 추가 정규화가 필요합니다[^ch8-16]. 같은 모델이 서로 다른 컨텍스트에서 교사와 학생을 맡는 구상은 여전히 빠르게 진화하고 있지만, ‘더 강한 교사가 없다’는 흔한 곤경을 위한 길은 이미 열었습니다.

## Bad Case에서 Post-Training으로

이 절에서는 7장에서 남긴 질문으로 돌아갑니다. 운영 환경의 bad case에서 만든 평가 데이터를 어떻게 post-training 입력으로 바꿀 수 있을까요? 실패 원인 기록, end-to-end 회귀 작업, trajectory-prefix 회귀 작업, 다차원 rubric 점수는 각각 다른 학습 용도로 연결됩니다.

표 8-4. 7장 평가 데이터와 8장 학습 용도의 대응

| 7장 평가 데이터 | 8장 학습 용도 |
|---|---|
| 검증기가 있는 end-to-end 회귀 작업 | RL rollout 작업과 검증 가능한 보상(RLVR); rejection-sampling fine-tuning(RFT)의 샘플링 풀 |
| Trajectory-prefix 회귀 작업 | DPO 선호 쌍, 의사결정 경계 SFT 시연, On-Policy Distillation의 교사 상태 |
| 실패 원인 기록(첫 오류 단계와 오류 유형) | 과정 감독(PRM)의 음성 라벨; RLVP 경로 패널티 규칙 |
| 다차원 rubric 점수와 human gold set | 벡터 보상의 차원; 생성형 보상 모델(GRM)의 학습·보정 데이터 |

### 사례 1: 코딩 에이전트의 조기 완료

**Bad case에서 원인 귀속으로.** 코딩 에이전트는 테스트를 실행하기 전에 “완료”라고 말하거나, 여러 목표 중 일부만 끝내고 작업을 닫거나, 몇 번 실패한 뒤 작업이 불가능하다고 선언할 수 있습니다. 근거 없이 결론을 내리려는 순간이 첫 오류이며, 이후의 실패한 테스트와 재시도는 그 결과입니다. 사용자의 수정, 부정적 피드백, 사후 감사가 모두 이 범주를 드러낼 수 있습니다.

**학습 데이터.** 에이전트가 완료를 주장하면 end-to-end 회귀 작업이 숨은 인수 테스트를 실행합니다. 통과에는 양의 보상, 실패에는 음의 보상을 줍니다. Trajectory-prefix 작업에서는 조기 선언을 `rejected`로, ‘테스트를 실행하고 모든 인수 조건을 하나씩 확인한 뒤 결론 내리기’를 `chosen`으로 둡니다. 교사가 생성한 후보는 결정론적 검증기로 필터링하고, 작업 유형·누락 조건·완료 표현을 바꾼 뒤 일부를 일반 지시 데이터와 섞어 LoRA를 학습합니다.

**평가.** 미완료 작업의 경계 집합은 실제로 완료된 작업의 retention 집합과 함께 평가해야 합니다. 전자는 모델이 일찍 멈추지 않고 검증하는지, 후자는 정상적으로 결론을 낼 수 있는지를 측정합니다. 후자가 없으면 모델이 지나치게 조심스러워져 전혀 멈추지 않을 수 있습니다.

> **실험 8-17 ★★: ‘조기 완료’ bad case에서 DPO**
>
> **실험 목표**: 실패 원인 귀속에서 trajectory-prefix 회귀 데이터, DPO 선호 쌍, 7B LoRA 학습, 분리된 boundary/retention 평가까지 전체 파이프라인을 실행합니다.
>
> **데이터 구성**: 함께 제공되는 프로젝트는 네 가지 실패 유형을 다루는 현실적인 사례 24개와 서로 겹치지 않는 holdout(경계 사례 12개, retention 사례 8개)을 제공합니다. 교육용 실험이므로 실제 운영 데이터에는 더 많은 작업군과 모델이 수정하거나 실행했다고 주장할 수 없는 숨은 테스트가 필요합니다.

### 사례 2: 중국어 따옴표

“중국어 문서의 직선 따옴표를 곡선 따옴표로 바꿔 달라”는 요구는 전역 치환 규칙이 아닙니다. 같은 ASCII 따옴표라도 중국어 본문, 영어 인용, Markdown 코드, 코드 블록, 주석, JSON, 경로에서 역할이 다릅니다. 중국어 본문과 중국어 주석은 바꿀 수 있지만 실행 가능한 코드, 영어 원문, JSON/schema, 경로, 식별자, 모호한 영역은 보존해야 합니다.

**Bad case에서 원인 귀속으로.** Harness는 문서를 범위별로 분할하고, 모델 출력을 변경 허용 영역과 보호 영역에 대조하며, Markdown·JSON·소스 언어 문법 검사를 실행해야 합니다. 렌더링이나 직렬화가 먼저 입력을 바꿨다면 문제는 Harness에 있습니다. 모델이 원본 byte를 받았는데 보호된 따옴표를 바꾸거나 허용된 중국어 따옴표를 놓쳤다면 첫 차이는 범위 선택 오류이며 post-training 데이터로 만들 수 있습니다.

**학습 데이터.** Skill은 양성·음성 범위 규칙을 정의합니다. 샘플은 입력과 출력을 짝지어 중국어 본문, 중첩 따옴표, 중국어 주석을 긍정적 편집으로, 영어, 리터럴, JSON, 경로, 인라인 코드, 코드 블록을 보호되는 부정적 예로 포함합니다. train·holdout·boundary는 템플릿, 장르, 변수 조합, 언어로 분리하고 SFT 전에 기계 게이트와 층화 수동 감사를 수행합니다.

**평가.** 대상 따옴표 변환, 보호 영역 보존, 비대상 편집, 문법 유효성, 전체 텍스트 exact match를 보고합니다. 이미 올바른 문서로 구성한 retention 집합을 운영 환경에 두어 과도한 편집을 잡아야 합니다.

> **실험 8-18 ★★: 범위 민감 곡선 따옴표 SFT**
>
> **실험 목표**: 보지 못한 문맥 조합에서도 허용된 따옴표만 바꾸고 보호된 문법을 보존하도록 LoRA SFT가 가르칠 수 있는지 검증합니다.
>
> **설정 및 데이터**: Qwen3-8B bf16 LoRA, 2 epoch, 256 업데이트; 16개 fragment 유형, 10개 문서 장르, 9개 프로그래밍 언어; train 1,024개, holdout 256개, boundary 256개. Skill을 라벨링·품질 게이트·회귀 사양으로 사용하고 48개 층화 수동 표본 검사를 실시합니다.
>
> **결과**: Holdout exact가 기반 모델의 0%에서 96.9%로, boundary exact는 97.7%로 올랐고 보호 영역 보존은 100%였습니다. Python, JavaScript, Java, Go, Rust, SQL, Shell, YAML, Markdown은 100%지만 JSON은 68.8%에 머물러 별도의 구조화 데이터 경로가 필요합니다.

### 사례 3: 파일 편집이 자주 실패하는 문제

코딩 에이전트는 흔히 `edit_file(path, old_string, new_string)`을 사용합니다. 이 도구는 `old_string`을 정확히 일치시켜야 하므로 공백, 줄바꿈, 백슬래시, Unicode 조합, 드문 토큰 중 단 한 바이트만 달라도 “old_string not found”가 됩니다. 반복 재시도는 증상일 뿐 근본 원인이라고 단정할 수 없습니다.

**Bad case에서 원인 귀속으로.** 첫 번째 차이가 어디에서 나타났는지 다음 연쇄를 따라 비교합니다.
...
모델 생성 이전의 변화는 파일 리더, 직렬화기 또는 Harness의 문제입니다. Tokenizer encode→decode도 별도로 감사해야 합니다. 모델이 원본 byte를 받고 모델 출력이 첫 번째 발산 지점임을 확인한 경우에만 모델 복사 실패로 분류해 post-training에 보냅니다.

**학습 데이터.** 검증 가능한 세 가지 작업을 사용합니다. 원문 그대로 복사하기, 비슷한 hard negative 중 표시된 target 고르기, 정확한 target을 `old_string` tool JSON 필드에 넣기입니다. 길이, 토큰 조합, 문맥을 무작위화하고 공백, 실제 줄바꿈, 리터럴 escape, 백슬래시, Unicode 결합 문자, 중국어, zero-width 문자를 포함합니다. seed, 길이, 토큰 구성, wrapper 문맥으로 분할합니다.

**평가.** 모델 byte-exact, code-point-exact, token-exact, 첫 차이 위치, tokenizer round-trip 지표를 end-to-end 도구 성공과 분리해 보고합니다. 직접 복사는 맞는데 `edit_file`가 실패하면 모델을 재학습하는 대신 직렬화나 도구 프로토콜을 고쳐야 합니다.

> **실험 8-19 ★★: 특수 문자열 Exact-Copy SFT**
>
> **실험 목표**: 모델 출력이 첫 번째 발산 계층임을 확인한 뒤 보지 못한 랜덤 문자열에서 LoRA SFT를 시험하고, 별도 tokenizer 감사로 토큰화 artifact를 배제합니다.
>
> **설정 및 데이터**: 2 epoch Qwen3-8B bf16 LoRA; `verbatim`, `decoy_copy`, `tool_json` 세 작업에서 train 1,024개, holdout 256개, boundary 256개. 생성기는 재현 가능한 랜덤 문자열, hard negative, 10개 언어 문맥, 8개 문서 장르, 특수 공백, escape, Unicode, 중국어, zero-width 문자를 사용합니다.
>
> **결과**: Holdout byte-exact 정확도가 37.5%에서 78.9%로, boundary는 80.1%로 올랐고 평균 첫 byte 차이는 54.0과 54.2였습니다. 512 probe에서 Qwen3/Qwen2.5 tokenizer round-trip은 80.1%, Mistral은 100%였으므로 tokenizer와 Harness 오류는 모델 복사 결과와 분리해 보고해야 합니다.
...
이 장은 파라미터 업데이트를 통해 에이전트의 지속적인 진화를 구현하는 방법에 답했습니다. 다음 장에서는 파라미터가 에이전트 자기 진화의 네 가지 매개체(지식, 지시, 프로그램, 파라미터) 중 하나일 뿐임을 살펴봅니다.
...
이 장에서는 ‘어떻게 학습할 것인가’라는 파라미터 업데이트 질문에 답했습니다. 다음 장에서는 모델 파라미터를 완전한 에이전트 시스템 안에 다시 배치합니다. 파라미터는 지식, 지시, 프로그램과 함께 네 가지 업데이트 운반체 중 하나일 뿐입니다. 고유한 과제는 배포 궤적에서 신뢰할 만한 학습 신호를 도출하고, 올바른 업데이트 위치를 선택하고, 각 후보 버전의 검증·출시·롤백을 관리하는 것입니다. 구체적인 학습 알고리즘은 9장에서 반복하지 않고 이 장을 직접 참조합니다.
