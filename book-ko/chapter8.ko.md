...
실험 8-8과 8-9도 실제 모델/API 호출로 실행했습니다. 8-8에서는 모델이 만든 안전하지 않은 후보를 모델 외부의 안전 검사가 올바르게 거부했습니다. 8-9의 첫 실행은 0/8을 탐지하고 정상 텍스트 7/8을 잘못 표시했지만, 모델 외부 필터와 결정적 fallback을 추가한 최종 실행에서는 8/8 탐지, 정상 텍스트 오탐 0/8, 후보 21개를 규칙 8개로 통합하는 결과를 얻었습니다.
