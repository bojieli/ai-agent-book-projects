...
실험 제목의 별은 **난이도**를 나타낼 뿐, **드는 품**을 나타내지 않습니다. 기본적으로 실험에는 이 저장소에 포함된 실행 가능한 코드가 딸려 있어 자격 증명만 설정하면 돌아갑니다. 세 가지 예외는 제목에 명시합니다. `[외부 저장소]`는 훈련 프레임워크나 벤치마크 같은 외부 저장소를 직접 clone 해야 함을, `[설계안]`은 본문이 구조와 합격 기준만 제시하고 실행 코드는 독자가 작성해야 함을, `[실제 하드웨어]`는 실물 로봇 팔 같은 장비가 있어야 실행할 수 있음을 뜻합니다. 각 장의 부속 README에는 실험별 프로젝트 유형 표와 외부 의존성 목록도 있습니다.
