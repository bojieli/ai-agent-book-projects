...
> ‘뇌/눈/손발’ 공식에 따라 먼저 약한 고리를 찾습니다. 일반적으로는 컨텍스트를 보강하여 관찰 공간을 넓히는 일을 우선합니다. 업무가 모델의 사고 능력을 넘어서면 더 강력한 모델로 바꿉니다. 행동 공간이 부족하다면(예: 사내 시스템에 접근할 수 없음) 도구를 추가합니다. 실패 궤적을 분석하여 병목이 지각, 의사 결정, 행동 가운데 어디에 있는지 찾으면 판단할 수 있습니다.

**2. (★★★) ReAct 루프에서는 에이전트가 LLM을 호출할 때마다 전체 과거 궤적을 전달하므로 궤적이 길어질수록 설계 비용이 이차 함수 형태로 증가합니다. 중요한 정보를 잃지 않으면서 이차 증가를 끊을 수 있을까요?**
