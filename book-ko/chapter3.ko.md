...
메모리 갱신은 두 단계로 나뉩니다[^uac]. **메모리 단계**에서는 각 세션이 끝날 때 LLM이 대화에서 사실을 문자열로 하나씩 추출하여 추가 전용 사실 로그에 덧붙입니다. **구조화 단계**에서는 주기적으로 LLM이 전체 사실 로그에서 타입이 지정된 Python 표현 전체를 다시 생성합니다. 사실을 dataclass로 정리하고, 날짜에는 `date()`, 컬렉션에는 타입이 지정된 목록, 유형화하기 어려운 기타 항목에는 `notes: list[str]`을 사용합니다. 데이터베이스의 고전적인 “선행 기록 로그(write-ahead log) + 주기적 체크포인트” 설계를 LLM 메모리에 적용한 것입니다. 추가 전용 로그는 사실을 하나도 잃지 않게 하고, 주기적 체크포인트는 사실을 깔끔하고 질의 가능한 구조로 압축합니다. 이 주기적 재구성 과정은 이 장 뒤에서 다룰 “메모리 압축과 정리 메커니즘”과 일치하지만 출력이 텍스트가 아니라 코드라는 차이가 있습니다.
...
밀집 임베딩이 의미 유사도를 포착하는 것과 달리, 희소 임베딩은 전통적인 정보 검색에 뿌리를 두며 핵심은 키워드의 완전 일치입니다. 희소 임베딩은 문서를 차원이 매우 높은 벡터로 나타냅니다. 문서에 등장한 단어에 해당하는 차원만 0이 아니고 나머지 대부분은 0입니다. 이론적 토대는 텍스트를 “단어 주머니”로 취급하는 고전적인 Bag of Words(BoW) 모델입니다. 어떤 단어가 몇 번 등장하는지만 따지고 어순은 완전히 무시하므로 “cat chases dog”와 “dog chases cat”을 동일하게 봅니다. 이 토대에서 더 정교한 확률적 순위 알고리즘이 발전했습니다.
...
구체적인 예로 직관을 익혀 보겠습니다. 기술 문서 100개가 있는 지식 베이스에서 사용자가 “model distillation”을 검색한다고 가정합니다. “model”은 문서 60개에 등장하여 너무 흔하고 구별하는 힘이 약하지만, “distillation”은 문서 3개에만 등장하여 매우 드물고 구별하는 힘이 큽니다. 좋은 검색 알고리즘이라면 “distillation”에 더 큰 가중치를 주어야 합니다. 이 단어가 들어간 문서가 사용자가 실제로 찾는 내용일 가능성이 더 크기 때문입니다. 이것이 TF-IDF와 BM25의 핵심 아이디어입니다.

TF-IDF는 단순한 직관에 기반합니다. 한 단어가 어떤 문서에 자주 나타날수록(TF, Term Frequency), 그리고 전체 문서 중 그 단어를 포함한 문서가 적을수록(Document Frequency, DF가 낮고 따라서 Inverse Document Frequency, IDF가 높을수록) 그 단어는 중요합니다. 앞의 예에서 “model”은 `df/N = 60%`이므로 IDF 값이 낮고, “distillation”은 `df/N = 3%`이므로 IDF 값이 높습니다. 따라서 순위 계산에 “distillation”이 훨씬 크게 기여합니다. 그러나 TF-IDF는 문서 길이를 고려하지 않으며 긴 문서일수록 자연히 단어 빈도가 높습니다. 또한 단어 빈도가 선형으로 늘어납니다. 어떤 단어가 10번 등장하면 5번 등장할 때보다 정말 두 배 중요할까요? BM25는 이 문제를 보정하는 두 가지 핵심 매개변수를 도입합니다. `k1`은 단어 빈도의 “포화”를 제어합니다. “distillation”을 20번 언급한 문서가 10번 언급한 문서보다 실제로 두 배 관련 있는 것은 아닙니다. `k1`은 단어 빈도가 증가할수록 기여도가 점차 평탄해지게 하여 긴 문서가 단어 빈도의 누적만으로 부당하게 우위를 차지하지 않도록 합니다. `b`는 문서 길이 정규화를 제어해 길이가 다른 문서를 더 공정하게 비교하게 합니다. 그 결과 BM25는 더 견고하고 효과적인 순위 함수가 되었으며, 오늘날에도 주요 검색 엔진의 핵심 요소로 빠지지 않습니다.
...
전문 데이터베이스가 아니라 Markdown 일반 텍스트를 지식의 기반 표현으로 선택한 것은 언뜻 직관에 어긋나 보이지만 신중히 내린 엔지니어링 결정입니다. 5장에서는 오픈 소스 에이전트 프레임워크 OpenClaw의 비슷한 선택을 다룹니다. 일반 텍스트를 사용하면 사용자가 에이전트의 지식을 직접 읽고 편집하며 바로잡을 수 있고, Git으로 변경 사항의 버전을 관리하고 되돌릴 수 있습니다. 더 중요한 점은 에이전트가 `write_file` 기능을 얻으면 지식을 스스로 기록하고 정리할 수 있다는 것입니다. 세션이 끝날 때 시스템은 사용자 선호의 갱신 내용을 `user/memories/`에, 작업 기록을 `agent/memories/`에 쓸 수 있습니다. 전자는 이 장에서 다루는 사용자 지식 관리에 속합니다. 후자는 결과 평가와 여러 궤적에 걸친 일반화, 이후의 검증을 거쳐야만 8장에서 말하는 경험 학습이 됩니다. 임의의 단일 작업을 곧바로 믿을 만한 경험으로 취급해서는 안 됩니다.
