...
여기까지의 논의는 모두 공학의 관점이었습니다. 여러 에이전트가 어떻게 협업해 과제를 끝낼 것인가입니다. 다음 절은 **책의 나머지 부분과 체재가 다릅니다**. 앞의 아홉 장 반이 답한 것은 "어떻게 만들 것인가"였지만, 이 절이 답하는 것은 "지금 무엇이 시도되고 있는가"입니다. 현재의 멀티 에이전트 사회 실험에 대한 개관으로, 그대로 따라 할 수 있는 공학적 방안이 아니라 관찰과 열린 물음을 제시합니다. 그럼에도 이 장에 두는 까닭은, 이 실험들이 앞서 논한 컨텍스트 공유·협업 토폴로지·실패 양상을 극단적 규모까지 밀어붙이기 때문이고, 그중 둘(스탠퍼드 AI 소도시와 음성 마피아)에 대해 이 책이 재현 가능하고 실행 가능한 부속 프로젝트를 제공하기 때문입니다. 이 절은 앞의 공학적 결론을 훨씬 긴 시간 척도에 놓았을 때의 스트레스 테스트로 읽으십시오.
...
이 장은 본문의 마지막 장이기도 합니다. 1장의 Agent = LLM + 컨텍스트 + 도구에서 출발해 구축, 평가와 진화를 거쳐 이 장의 협업까지, 공학 쪽 노선은 다 지나왔습니다. 그러나 공학 자체로는 답할 수 없는 물음이 둘 남습니다. 모델이 계속 강해진 뒤에 오늘의 하네스는 얼마나 남을 것인가, 그리고 특정 사용자와 특정 회사에 속한 지식을 "한 번 더 훈련"으로 메울 수 있는가입니다. 후기가 이 둘을 이어받습니다.
