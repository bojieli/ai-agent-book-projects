...
**공유 컨텍스트 없는 핸드오프에서 무엇을 전달할까요?** 그림 10-10은 두 핸드오프 유형을 대조합니다. 실험 10-1의 `transfer_to_agent`는 공유 컨텍스트를 사용하므로 새 역할이 전체 기록을 자동으로 물려받습니다. 핸드오프 체인 패턴은 컨텍스트를 공유하지 않으므로 보내는 에이전트가 받는 에이전트에 필요한 정보를 명시적으로 조립해야 합니다.
