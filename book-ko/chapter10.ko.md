...
궤적 영속화의 가치는 모니터링을 훨씬 넘어섭니다. 1장의 결론인 “에이전트의 컨텍스트 = 정적 접두부 + 궤적”을 떠올려 보십시오. 정적 접두부(시스템 프롬프트, 도구 정의)는 코드로 정해지고 에이전트는 궤적 외에 런타임 상태를 갖지 않습니다(작업 산출물은 이미 파일 시스템에 있습니다). **궤적은 에이전트의 전체 상태**입니다. 궤적을 실시간으로 파일에 영속화하는 것은 항상 완전한 체크포인트를 보유하는 것과 같습니다. 에이전트 프로세스가 중단되거나, 전원이 꺼지거나, 사용자가 세션을 닫아도 궤적 파일을 다시 로드하고 정적 접두부를 앞에 붙이면 멈춘 곳에서 실행을 재개할 수 있습니다. Claude Code와 Codex CLI 같은 코딩 에이전트의 세션 재개 기능도 바로 이렇게 구현됩니다. 데이터베이스의 미리 쓰기 로그(WAL)와 같은 발상입니다. 모든 이벤트를 먼저 추가 전용 로그에 기록하면 언제나 로그를 재생해 상태를 복원할 수 있습니다(3장의 ‘사실 로그 + 주기적 체크포인트’ 메모리 설계는 같은 발상을 메모리 시스템에 적용한 것입니다). 멀티 에이전트 시스템에서는 하위 에이전트가 자연스럽게 **복구 가능하고 감사 가능하며 핸드오프하기 쉬워집니다.** 관리자는 중단 뒤 마지막 유효 상태에서 하위 에이전트를 다시 시작하고, 궤적을 이벤트별로 재생해 실패 원인을 찾으며, 궤적과 업무를 다른 에이전트에 넘겨 계속하게 할 수도 있습니다.
...
제어 평면은 메시징, 상태 조회, 종료, 리소스 스케줄링을 담당합니다. 에이전트는 메시지로 상태를 비동기 보고하거나 부모 에이전트가 하위 에이전트가 실시간 갱신하는 파일을 관찰할 수 있습니다. 파일은 전체 궤적일 수도 있고 합의한 진행 상황 파일일 수도 있습니다. 궤적은 에이전트의 전체 상태를 포착하므로 장애 뒤에 다시 불러오면 세션을 재개할 수 있습니다. 실시간·비동기·다자간 조율을 위한 제어 평면은 흔히 메시지 버스로 구현합니다. 조직 간 협업에는 A2A 같은 표준 상호 운용 프로토콜도 필요합니다.
