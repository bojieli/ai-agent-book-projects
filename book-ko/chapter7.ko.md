...
이 장은 강화 학습이나 모델 학습에 관한 배경지식이 없는 독자를 전제로 합니다. 경사도나 정책 최적화를 알고 있다고 기대하지 않습니다. 대신 모델은 애초에 어떻게 학습되는가라는 질문에서 출발해 각 단계의 목적과 작동 원리, 해결하는 문제를 분명히 설명합니다. 이 장을 마치면 다음 질문에 답할 수 있을 것입니다. 모델의 역량은 몇 단계에 걸쳐 만들어지는가? 각 단계는 무엇을 하는가? 왜 반드시 이 순서여야 하는가? 여러분의 프로젝트에서는 어디에 노력을 집중해야 하는가?

**먼저 가장 중요한 지도를 그려 보겠습니다. 현대 모델의 역량은 세 단계를 거쳐 만들어집니다.** 이 세 단계는 맞물려 있으며 어느 하나도 빠질 수 없습니다.
...
3.  **강화 학습(RL, Reinforcement Learning)**: 강아지를 훈련하듯 모델이 반복해서 시도하고 보상과 벌을 바탕으로 개선하게 합니다. 잘하면 간식을 주고 못하면 주지 않는 식입니다. 모범 답안을 보여 주는 대신 스스로 시도하게 하여 좋은 행동의 확률은 높이고 나쁜 행동의 확률은 낮춥니다. 이 단계는 모델이 **처음 보는 상황**에서도 합리적인 결정을 내리도록 가르칩니다. 또한 이 장에서 가장 많은 분량을 차지하고 가장 많은 엔지니어링 노력이 필요한 단계입니다.

직관적으로 비유하면 사전 학습은 ‘만 권의 책 읽기’(지식 축적), SFT는 ‘교사가 모범 풀이를 차근차근 보여 주기’(시연 모방), RL은 ‘직접 문제를 풀고 정오답을 통해 다듬기’(시행착오 학습)입니다. 셋은 서로 대체하는 선택지가 아니라, 먼저 책을 읽고 다음으로 시연을 본 뒤 마지막으로 실전 연습을 하는 하나의 파이프라인입니다.

**이 장 전체를 관통하는 두 가지 주제가 있습니다. 이후의 모든 내용이 이 두 주제를 뒷받침하므로 기억해 두십시오.**

*   **주제 1: SFT는 암기하고, RL은 일반화합니다.** 같은 작업과 예산이라면 SFT는 학습 데이터의 답을 **암기**하는 경향이 있어 배포 환경이 학습 환경과 달라지면 실패합니다. 반면 RL은 전이 가능한 전략을 **학습**하는 경향이 있어 처음 보는 상황에서도 더 안정적입니다. 이는 단순한 구호가 아니라 측정할 수 있는 현상이며, 이 장에서는 통제 실험을 통해 거듭 검증합니다. 7.1절에서는 이 차이가 생기는 **근본적인 이유**를 한 절에 걸쳐 설명합니다.
...
| **RL** | 작업 + 보상 함수(모범 답안 없음) | 기대 보상 최대화 | 전이 가능한 의사결정 전략, 새롭게 발견한 해법 | 높음(흔히 SFT의 수십~수백 배) |
...
이를 이해하면 ‘SFT는 암기한다’는 결론도 자연스럽게 따라옵니다. SFT의 최적화 목표는 **라벨이 붙은 응답의 모든 토큰 확률을 최대화하는 것**, 쉽게 말해 ‘이 모범 답안을 외우는 것’입니다. 같은 질문이 주어지면 시연을 최대한 가깝게 재현하도록 학습됩니다. 목표가 명확하고 형식이 고정된 작업에는 매우 효율적이어서 수천 건의 예시만으로도 효과를 내지만, 그 역량은 시연 데이터에 엄격히 한정됩니다. 시연에 없는 상황은 배우지 않았고, 환경이 바뀌어 시연 답안이 더는 맞지 않아도 그대로 재현합니다.
...
### SFT를 반드시 RL보다 먼저 해야 하는 이유

세 단계의 순서는 임의로 정한 것이 아닙니다. 사전 학습이 먼저라는 데에는 이견이 없습니다. 언어와 지식의 토대 없이는 다른 무엇도 가능하지 않기 때문입니다. 설명이 필요한 부분은 **왜 SFT가 반드시 RL보다 먼저여야 하는가?**입니다.

그 답은 RL의 작동 방식에 있습니다. RL은 모범 답안을 보지 않습니다. 모델이 스스로 응답을 **생성**하게 한 다음 응답 품질에 따라 보상이나 벌을 줍니다. 하지만 품질을 판단하려면 먼저 모델 출력을 **파싱**할 수 있어야 합니다. 작업에서 JSON 객체나 도구 호출을 출력해야 하는데 모델이 형식도 갖추지 못한 텍스트를 쏟아 낸다면, 보상 함수는 성공과 실패조차 구분하지 못해 계산할 근거가 없고 RL도 학습할 수 없습니다.

따라서 SFT는 **먼저 모델이 형식에 맞는 출력을 내도록 만드는** 역할을 합니다. 소수의 시연으로 출력 형식을 안정화해 확실히 파싱할 수 있게 하고, RL이 채점 가능한 출발점을 마련합니다. 이것이 업계에서 가장 견고하게 쓰이는 **‘SFT 후 RL’** 2단계 패러다임입니다. RL을 먼저 하고 SFT를 나중에 하는 방식은 작동하지 않습니다. 출력이 안정적이지 않으면 보상 신호가 그저 잡음이기 때문입니다. 중국 회화의 개념을 빌리면 SFT가 먼저 **‘형(形)’**(형식, 구조)을 세우고, RL이 이어서 **‘신(神)’**(전략, 일반화)을 추구합니다. 즉 **선형후신(先形後神)**입니다.
...
지금까지 ‘SFT는 암기하고, RL은 일반화한다’고 거듭 말했습니다. 이제 그 근본적인 이유를 자세히 설명하겠습니다. 둘 사이의 모든 차이는 **최적화 목표가 다르다**는 점에서 비롯됩니다.

*   **SFT는 ‘모범 답안과 얼마나 비슷한가’를 최적화합니다.** 목표는 라벨이 붙은 답안의 확률을 최대화하는 최대 우도입니다. 질문 하나에는 시연 답안이라는 단 하나의 ‘정답’ 출력만 있습니다. 모델은 이 단일 경로로 끌려가며 ‘이 입력을 보면 저 출력을 내놓는다’는 고정 매핑을 학습합니다. 따라서 **암기**합니다. 학습할 때 J/Q/K를 모두 10으로 취급하면 ‘J/Q/K는 10이다’라는 규칙을 기억하고, 테스트에서 J가 11로 바뀌어도 여전히 10을 사용해 오답을 냅니다.
*   **RL은 ‘결과가 얼마나 좋은가’를 최적화합니다.** 목표는 기대 보상을 최대화하는 것입니다. 질문 하나에 대해 높은 보상을 얻는 출력이라면 단 하나가 아니라 **어떤** 출력이든 좋습니다. 모델은 여러 경로를 스스로 탐색하고 좋은 결과를 낸 경로를 강화합니다. ‘어떤 과정을 거쳐야 올바른 결과에 이르는가’라는 더 일반적인 **전략**을 학습하므로 J가 11로 바뀌면 암기한 답을 적용하는 대신 같은 전략으로 다시 계산합니다. 이것이 **일반화**입니다.
...
| 학습 신호 | 단일 모범 답안(모든 토큰에 지도 신호) | 스스로 생성한 여러 응답 + 보상(응답마다 성공/실패 신호 하나) |
| 데이터 형식 | ‘입력-출력’ 시연 쌍 | 작업 + 보상 함수(모범 답안 불필요) |
| 학습하는 내용 | 고정된 ‘입력→출력’ 매핑(암기) | 전이 가능한 의사결정 전략(일반화) |
| 분포 이동 상황 | 환경이 바뀌어도 예전 답을 적용해 성능 하락 | 같은 전략으로 다시 풀어 더 안정적 |
...
알아 둘 만한 더 깊은 메커니즘은 RL이 소수의 좋은 전략으로 수렴하는 이유를 설명하는 **모드 추구(mode-seeking)**입니다. 모델이 한 질문에 내놓을 수 있는 모든 답의 확률 분포에는 여러 ‘모드’가 있을 수 있으며, 각 모드는 합리적인 답변 방식의 한 계열을 나타냅니다. SFT가 사용하는 최대 우도는 **질량 포괄(mass-covering)** 방식입니다. 시연에 나타난 모든 모드를 포괄하려 하고, 품질이 낮은 모드에도 확률을 배분합니다. 이른바 ‘부를 나누는’ 방식입니다. 반면 RL, 특히 수학적으로 **역방향 KL 발산**에 해당하는 KL 제약 정책 최적화는 **모드 추구** 방식입니다. 보상이 가장 높은 소수의 모드를 찾아 확률을 집중하고 나머지는 과감히 버리는 ‘승자 독식’ 성향을 보입니다. RL로 학습한 모델이 더 ‘단호한’ 답을 내고 고품질 전략에 더 집중하는 동시에 다양성을 희생하는 경향이 있는 이유입니다. 질량 포괄과 모드 추구라는 개념을 기억하십시오. RLHF 절에서 KL 발산을 다룰 때 겉보기에는 무미건조하지만 매우 중요한 설계 선택을 이 개념으로 설명합니다.

**RL의 상한이 SFT보다 높은 이유는 ‘온라인’이기 때문입니다.** 이는 SFT와 RL 사이의 더 깊은 차이이자, 추가 비용을 감수하면서 RL을 사용할 가치가 있는 근본적인 이유입니다. SFT는 **오프라인** 방식입니다. 고정된 시연 데이터 세트에서만 학습하며 그 밖의 세계는 볼 수 없습니다. RL은 **온라인** 방식입니다. 모델이 스스로 응답을 생성하고 피드백에 따라 개선하며 시행착오로 학습합니다. ‘온라인/오프라인’과 더 엄밀한 ‘온폴리시/오프폴리시’라는 용어는 7.8절에서 정식으로 구분하므로 여기서는 직관만 익히겠습니다. 온라인 방식에는 SFT가 가질 수 없는 세 가지 장점이 있으며, 이들이 함께 성능 상한을 높입니다.

- **첫째, 오프라인의 상한은 데이터이고 온라인의 상한은 작업입니다.** SFT의 최적 결과는 시연을 ‘완벽하게 복제’하는 것이므로 상한은 **시연자의 수준**입니다. 기껏해야 그 수준에 가까워질 수 있을 뿐 거의 뛰어넘지 못합니다. 100점 만점에 60점을 받는 교사가 라벨링한 데이터 세트로 90점을 받는 학생을 만들 수는 없습니다. RL은 시연이 아니라 결과 보상만 봅니다. 더 높은 보상을 얻는 행동이라면 누가 시연한 적이 없어도 **모두** 강화됩니다. 따라서 RL은 시연에 존재하지 않는 더 우수한 전략을 스스로 **발견**할 수 있습니다. 이 장 뒤의 실험 7-13(SimpleVLA-RL)에서 모델이 스스로 고안한 ‘pushcut’ 동작은 인간 시연에 한 번도 없었으며, ‘시연을 넘어선다’는 직접적인 증거입니다. RL의 상한은 데이터에 우연히 무엇이 들어 있는지가 아니라 작업 자체, 즉 보상이 무엇을 알아볼 수 있는지에 따라 결정됩니다.
- **둘째, 생성보다 검증이 쉽다는 사실이 RL이 더 높은 곳까지 오를 수 있는 근본적인 이유입니다.** SFT에는 누군가 정답을 먼저 **작성**해 시연해야 하지만, RL에는 답이 좋은지 **판단**해 보상을 줄 방법만 있으면 됩니다. 많은 작업에서는 정답을 만드는 것보다 정오를 판단하는 편이 훨씬 쉽습니다. 수학 답은 정답표와 비교할 수 있고, 코드는 테스트를 실행할 수 있으며, 정리 증명기는 증명을 검증할 수 있습니다. 좋은 결과를 알아보는 일이 만드는 일보다 쉽기만 하면 RL은 이용할 수 있는 어떤 시연자보다도 강한 모델을 학습할 수 있습니다. 모델이 무작위로 시도하고 환경이 좋은 시도를 골라 강화하기 때문입니다. 이처럼 검증과 생성이 비대칭이라는 점이 RLVR 같은 검증 가능 보상 방식의 힘을 만들어 냅니다.
- **셋째, 온라인 학습을 이용하면 모델이 실제로 따를 궤적에서 학습하고 스스로 저지른 실수에서 회복하는 법을 익힐 수 있습니다.** 오프라인 모방에는 **공변량 이동(covariate shift)**이라는 고전적인 문제가 있습니다. 학생이 스스로 행동하다가 시연에서 벗어나 데이터에 없던 상태에 들어가면, 그 상태에서 회복하는 법을 배우지 못했으므로 궤적을 따라 오류가 누적됩니다. 이론적으로 순수 모방의 오류는 궤적 길이 T에 따라 대략 $T^2$만큼 증가하지만, 온라인 데이터로 학습하면 약 $T$로 줄일 수 있습니다. 온라인 방식은 그 반대입니다. 모델은 배포 중 마주칠 분포와 같은 분포에서 학습하고, 매 회차의 피드백은 현재의 약점을 직접 겨냥합니다. 데이터는 항상 ‘신선’합니다. 다른 존재인 교사의 행동을 설명하며 모델이 개선될수록 관련성이 떨어지는 오프라인 데이터와 다릅니다. 이 장 뒤의 온폴리시 증류(On-Policy Distillation, 7.12절)가 강력한 이유는 본질적으로 이 ‘온라인’ 장점과 SFT의 ‘조밀한 지도 신호’ 장점을 결합하기 때문입니다.

비유하자면 **SFT는 다른 사람이 그린 지도를 베껴 그리므로 기껏해야 그 지도와 같아질 수 있지만, RL은 보상이라는 나침반을 들고 탐험하므로 지도 밖의 길을 발견할 수도 있습니다.** 그래서 ‘SFT로 기반을 다지고 RL로 더 높이 오른다’는 방식이 주류가 되었습니다.
...
| **고객 서비스 에이전트** | 대화 기록, 지식 베이스 | 개방형(사고, 대화, API 호출) | 문제 해결(+), 처리 시간(-) |
| **코드 어시스턴트 에이전트** | 요구사항 문서, 코드베이스 | 개방형(사고, 검색, 편집, 실행) | 테스트 통과(+), 버그 발생(-) |

이 표에서 중요한 통찰을 얻을 수 있습니다. 체스와 로봇 공학 같은 영역의 전통적인 RL 에이전트는 닫힌 행동 공간을 갖지만, 고객 서비스나 코딩을 담당하는 현대 LLM 기반 에이전트는 거의 무한한 개방형 행동 공간을 갖습니다. 이들은 ‘내부 사고’라는 특수한 행동을 활용해 역량을 높일 수도 있습니다.

### 두 가지 에이전트 패러다임: MDP에서 LLM+RL로

두 패러다임의 가장 근본적인 차이는 행동 공간에 있습니다. MDP는 행동 공간이 유한하고 닫혀 있다고 가정하지만(위/아래/집기/놓기), LLM의 행동 공간은 조합적으로 폭발하는 자연어 시퀀스로 이루어져 개방되어 있습니다. 이 차이로 인해 알고리즘 설계, 샘플 효율, 일반화 측면에서 두 패러다임 사이에 근본적인 간극이 생깁니다. 이제 각각을 살펴보겠습니다.

**전통적 패러다임: MDP와 Q-learning.**

MDP(Markov Decision Process, 마르코프 결정 과정)는 상태, 행동, 보상 같은 핵심 요소를 정의하는 강화 학습의 수학적 프레임워크입니다. 핵심 가정은 미래가 앞선 이력이 아니라 현재 상태에만 좌우된다는 **마르코프 속성**입니다. 예를 들어 체스에서는 지금의 보드만 봐도 최적의 수를 결정할 수 있으며 이전의 모든 수를 검토할 필요가 없습니다. 이 가정은 문제를 단순하게 만들지만 과거 의존성을 모델링하는 능력도 제한합니다.

![그림 7-2: 마르코프 결정 과정(MDP) 도식](images/fig7-2.svg)

전통적인 RL 에이전트의 핵심 특징은 **닫힌 행동 공간**입니다. 에이전트가 취할 수 있는 모든 행동이 미리 정의된 유한 집합을 이룹니다. 가장 대표적인 예는 **고전적인 보드게임 에이전트**입니다. 바둑에는 가능한 착점이 361곳이나 되지만 모두 완전히 정해진 유한 집합입니다. 체스도 기물마다 이동 규칙이 다르지만 가능한 행동을 열거할 수 있고, Atari 게임에는 몇 개에서 십여 개 정도의 이산 행동만 있습니다. **로봇 에이전트**는 연속적이지만 경계가 있는 행동 공간을 사용합니다. 관절 각도, 속도, 파지력은 연속 값이지만 최대 회전각, 최대 토크, 속도 제한 같은 물리적 경계가 명확하고, 차원도 로봇의 자유도에 따라 정해집니다.

이처럼 닫혀 있다는 특성은 계산상의 이점을 줍니다. 모든 행동을 열거해 하나씩 평가할 수 있어 동적 계획법과 몬테카를로 트리 탐색을 적용하기 쉽고, 행동 가치 함수도 테이블이나 간단한 함수로 근사할 수 있습니다. 그러나 표현력과 일반화는 제한됩니다. 전통적인 RL 에이전트는 아무것도 모르는 상태에서 시작해 오직 시행착오로 학습합니다. 무작위 정책에서 출발해 경험을 수집하고 가치 함수나 정책을 업데이트하는 과정을 수렴할 때까지 반복합니다.
...
Q-learning은 **오프폴리시(off-policy)** 방식의 한 종류로, 무작위 탐색을 비롯해 어떤 정책이 생성한 데이터로도 최적 정책을 학습할 수 있습니다. 온폴리시와 오프폴리시 방식의 엄밀한 정의와 LLM 사후 학습에 어떻게 대응하는지는 뒤의 ‘강화 학습 알고리즘 비교’ 절에서 설명합니다.
...
>
> 전체 학습은 10초도 걸리지 않아 시뮬레이션이 매우 효율적이지만, 완전한 시도를 거의 10,000번 해야 합니다. 이는 Q-learning의 핵심 특징을 보여 줍니다. 무작위 탐색을 대량으로 거쳐 우연히 전체 경로를 완주해야 하고, 가치 신호가 매우 느리게 전파되므로 반복해서 강화해야 합니다. 사전 지식 없는 순수 기호 학습은 상태 공간을 무차별 대입으로 탐색할 수밖에 없습니다.
>
> 게임 시뮬레이터에서는 10,000번 시도하는 데 10초밖에 걸리지 않으므로 비용이 무시할 만합니다. 그러나 현실의 에이전트 환경에서는 전화 한 통마다 비용이 들고, 브라우저 조작마다 지연이 생기며, 잘못된 결정 하나가 되돌릴 수 없는 결과를 낳을 수 있습니다. 따라서 10,000번의 시도는 전혀 받아들일 수 없습니다. 현대 에이전트가 LLM 기반 방식으로 전환한 이유가 바로 여기에 있습니다. 사전 학습에서 쌓은 지식을 활용해 최소한의 상호작용만으로 효과적인 결정을 내리는 것입니다.
>
> MDP의 근본적인 한계는 세 가지입니다. 간단한 작업을 배우는 데도 방대한 상호작용이 필요해 샘플 효율이 낮고, 한 환경에서 배운 지식을 다른 환경으로 옮기기 어려워 일반화가 부족하며, 각 새 작업을 처음부터 학습해야 하므로 사전 지식을 활용할 수 없습니다. 자연어나 고차원 시각처럼 복잡한 상태 공간을 다룰 때 이러한 한계가 특히 두드러집니다.
>

**현대적 패러다임: LLM+RL 기반 에이전트.**

대규모 언어 모델은 에이전트를 구축하는 방식, 특히 행동 공간 설계를 근본적으로 바꾸는 새로운 패러다임을 가져왔습니다.

전통적인 RL에서 에이전트는 체스 말을 움직이거나 미로에서 한 칸 이동하는 식으로 환경을 바꿔야만 피드백을 받을 수 있습니다. 그러나 LLM은 내부 사고라는 완전히 새로운 행동을 도입합니다. 사고는 외부 세계를 바꾸지 않지만 최종 행동의 품질을 크게 높일 수 있습니다. 이 변화로 모든 것이 달라집니다. 이제 에이전트의 행동 공간에는 ‘무엇을 할 것인가’뿐 아니라 ‘얼마나 오래, 무엇을 생각할 것인가’도 포함됩니다.

가장 중요한 혁신은 **사고를 특수한 행동으로 행동 공간에 포함한 것**입니다. 전통적인 RL 에이전트는 이동, 공격, 집기처럼 환경 상태를 바꾸는 외부 행동만 할 수 있습니다. LLM 에이전트에서는 **내부 사고가 행동 공간의 핵심 구성 요소**가 됩니다. 외부 환경을 직접 바꾸지 않고 즉각적인 보상도 없지만, 거의 제한 없이 수행할 수 있으며 비용도 비교적 낮습니다.

전통적인 RL은 이런 행동을 다루기 어렵습니다. 근본적으로 탐색 공간이 지나치게 크고 구조가 없기 때문입니다. 처음부터 학습하는 에이전트는 눈을 가린 채 사막에서 보물을 찾으며 무작정 헤매는 것과 같습니다. LLM은 다릅니다. 방대한 텍스트 사전 학습을 통해 인간의 사고 규칙을 내면화했습니다. 수학 문제는 ‘조건 파악 → 공식 떠올리기 → 단계별 계산’, 코드 작성은 ‘요구사항 이해 → 구조 설계 → 세부 구현’의 순서로 해결한다는 식입니다. 덕분에 LLM의 사고는 구조화된 경로를 따라 진행하며 탐색 공간을 크게 압축합니다. 따라서 추가 RL 학습 없이도 사전 학습된 LLM은 기본적인 논리적 사고 연쇄(Chain of Thought, CoT)를 생성할 수 있습니다. 이러한 기본 논리는 사전 학습 말뭉치에 담긴 방대한 인간의 사고 과정, 예컨대 수학 풀이, 코드 주석, 토론 답변 등에서 나옵니다. 모델은 다음 토큰 예측을 통해 ‘다음 사고 단계는 어떤 모습이어야 하는가’를 암묵적으로 학습합니다.

RL 사후 학습은 이어서 외부 보상을 사용해 LLM이 이러한 규칙을 특정 작업에서 더 효율적으로 활용하도록 가르칩니다. 언어의 구조 자체도 암묵적인 내부 보상을 제공합니다. ‘외화를 USD로 환산해야 하므로 먼저 환율을 찾아보자’처럼 논리적으로 일관된 사고 연쇄는 생성 확률이 높지만, ‘통화를 환산해야 하므로 먼저 날씨를 확인하자’처럼 논리가 뒤죽박죽인 사고 연쇄는 확률이 극히 낮습니다. 따라서 모델은 자연스럽게 합리적인 경로로 유도됩니다.

![그림 7-5: 고전 RL과 현대 LLM 에이전트 비교](images/fig7-5.svg)

언어에 내재한 규칙을 토대로 한 이 사고 능력 덕분에 LLM 에이전트는 처음 보는 지시를 이해하는 제로샷 일반화와 극소수의 예시로 새 작업을 익히는 퓨샷 적응을 수행합니다. 광범위한 시행착오가 필요한 전통적인 MDP 에이전트 패러다임과 극명하게 대비됩니다. 새 패러다임은 알려진 개념을 재조합해 새로운 상황을 다루는 조합적 일반화, 프롬프트와 예시를 통해 빠르게 적응하는 인컨텍스트 학습, 시각·언어·행동 같은 모달리티를 자연스럽게 통합하는 멀티모달 이해도 지원합니다. 단, 제로샷 일반화나 퓨샷 적응 같은 인컨텍스트 학습의 **효과**와 그 **내부 메커니즘**은 별개의 문제입니다. 2장에서 분석했듯 어텐션 메커니즘은 사고보다 검색에 가깝게 작동하지만, 그렇다고 작업 적응에서 발휘하는 강력한 실용적 효과가 줄어들지는 않습니다.

닫힌 행동 공간에서 열린 행동 공간으로의 진화는 AI 에이전트 패러다임의 근본적인 전환을 나타냅니다. 내부 사고뿐 아니라 자연어 쿼리, 프로그램 코드, 복잡한 JSON, 멀티모달 콘텐츠처럼 도구 파라미터가 다양해 실제 행동 공간은 거의 무한합니다. 코드 인터프리터는 이론적으로 계산 가능한 모든 작업을 실행할 수 있고, 검색 도구는 인터넷 전체의 정보 공간을 탐색할 수 있습니다. 이는 새로운 기회와 과제를 동시에 가져옵니다. 에이전트는 전에 없던 작업을 처리하고 기본 도구를 조합해 복잡한 문제를 해결할 수 있지만, 열린 환경에서 보상 함수를 정의하고 최적화하는 방법과 무한한 행동 공간을 효율적으로 탐색하는 방법을 찾아야 합니다.
...
이러한 효율에는 학습 분포에 강하게 의존한다는 대가가 따릅니다. SFT는 일반화보다 암기하는 경향이 있습니다. 테스트 중 학습에서 보지 못한 상황을 만나면 성능이 눈에 띄게 저하되는 경우가 많습니다. 다음 실험들은 ‘프로토콜 고정’ 과정을 여러 각도에서 보여 줍니다.
...
네 실험에는 ‘안정적인 매핑과 프로토콜을 파라미터에 기록한다’는 공통점이 있습니다. 음성 SFT는 스타일 제어 프로토콜을, 다국어 SFT는 사고 구성 템플릿을, 증류 SFT는 입력에서 출력으로 가는 직접 매핑을 고정합니다. 목표가 명확하고 형식이 깨끗하며 평가 기준이 안정적이어서 SFT는 매우 높은 샘플 효율로 성능을 높입니다. 하지만 분포가 이동하면 암기 성향이 드러나며 성능이 저하됩니다. 이는 7.1절 ‘SFT와 RL의 본질적인 차이’에서 설명한 암기와 일반화의 간극이 실험으로 나타난 것입니다.
...
**SFT는** JSON 출력이나 일관된 대화 스타일처럼 형식을 안정화해야 하고, 고품질 전문가 시연을 이용할 수 있으며, 배포 환경이 학습 환경과 매우 유사한 작업에 적합합니다. **RL이 필요해지는** 상황은 다릅니다. 학습과 배포가 체계적으로 다르거나(학습에서는 J/Q/K 카드가 모두 10이지만 배포에서는 11/12/13으로 바뀌는 규칙 변화, 학습에서는 검은색 무늬를 사용하지만 배포에서는 붉은색 무늬를 사용하는 외형 변화), 최적 전략을 발견해야 하거나(전문가 시연이 반드시 최적이라고 할 수 없음), 모든 경로를 시연하기에는 주석 비용이 너무 클 때입니다.

가장 견고한 전략은 **‘SFT 후 RL’** 2단계 파이프라인입니다. SFT의 일차 목표는 작업 성능 최대화가 아니라 출력의 **형식 안정성** 확보입니다. 모델이 파싱 가능한 JSON과 올바른 도구 인터페이스 호출을 생성하도록 하는 것입니다. 출력 형식이 안정되어야 RL 보상 신호를 확실하게 계산할 수 있습니다. SFT 없이 기반 모델에 직접 RL을 수행하면 출력 형식이 엉망이고 보상을 계산할 수 없어 학습에 실패하는 경우가 많습니다. 다만 이 결론에는 경계 조건이 있습니다. 뒤의 실험 7-11처럼 ‘비교적 작은 기반 모델 + 엄격한 구조화 출력 요구사항’이라는 설정에서 나온 결과입니다. DeepSeek-R1-Zero는 충분히 강력한 기반 모델이라면 SFT를 생략하고 직접 RL을 수행해 성공하며 성찰과 긴 연쇄 사고 역량을 출현시킬 수 있음을 보여 주었습니다. 다만 출력 가독성이 낮고 여러 언어가 섞이는 대가가 따랐고, 바로 이 때문에 DeepSeek는 최종 R1에 ‘콜드 스타트 SFT’를 다시 추가했습니다. R1이 Zero에서 콜드 스타트로 왕복한 과정은 ‘선형후신’을 가장 잘 보여 줍니다. RL은 스스로 ‘신’인 전략과 사고 역량을 키울 수 있지만, ‘형’인 형식과 가독성은 여전히 SFT로 빠르고 안정적으로 확립합니다.

각 방식에는 비용이 따릅니다. SFT는 샘플 효율이 높고 빠르게 수렴하지만 일반화가 부족합니다. RL은 전이 가능한 전략을 학습하지만 많은 샘플이 필요하고 학습이 불안정합니다. 실용적인 판단 기준은 다음과 같습니다. 시연을 더 추가해도 새로운 상황에서 성능이 더 이상 개선되지 않는다면 RL로 전환할 때입니다. 문제의 근원은 시연 수가 아니라 SFT의 최적화 목표 자체입니다.
...
3. **SFT가 충분하지 않다면 RL을 추가하십시오.** 새로운 상황에 대한 일반화나 최적 전략 탐색이 필요하거나 주석 비용이 지나치게 높은 경우에 적합합니다. 먼저 SFT로 출력 형식을 안정화한 뒤 그 위에 RL을 적용해야 합니다.
...
>
> 이 실험의 핵심 기여는 ‘SFT는 암기하고 RL은 일반화한다’는 현상을 체계적으로 정량화하고, 텍스트 전용과 비전-언어 모달리티 모두에서 이 패턴이 성립함을 보여 준 것입니다. SFT와 RL의 상호 보완 관계도 드러냅니다. SFT는 형식을 안정화하고, RL은 그 토대 위에서 암기의 한계를 넘어섭니다. 둘 모두 빠질 수 없습니다. 중국 회화의 표현을 빌린 ‘선형후신’ 학습 패러다임은 먼저 외형인 형식과 구조를 정확히 그린 뒤 내면의 정신인 일반화와 전략을 추구하며, 이후 다중 턴·멀티모달 작업을 위한 방법론적 토대를 마련합니다.
...
**(1) KL 발산이란 무엇이며 페널티는 어디에 적용되는가?** KL 발산(Kullback-Leibler Divergence)은 두 확률 분포의 차이를 측정합니다. 분포가 비슷할수록 KL이 작고 동일하면 0이며, 다를수록 커집니다. 비교하는 두 분포는 같은 앞선 컨텍스트가 주어졌을 때 **현재 정책** $\pi_\theta$(학습 중인 모델)과 **참조 정책** $\pi_{\text{ref}}$(학습 출발점, 보통 SFT 모델)이 만드는 다음 토큰 확률 분포입니다. $\beta$는 페널티 강도를 제어하며 학습 스크립트에서 흔히 보는 `kl_coef` 하이퍼파라미터입니다. 엔지니어링 관점에서 이 페널티는 **토큰마다 계산해 보상에 더합니다**(토큰별 KL). 모델이 토큰 하나를 생성할 때마다 그 토큰의 확률을 같은 위치에서 참조 모델이 부여한 확률과 비교하고, 차이가 클수록 해당 단계의 보상에 더 큰 페널티를 적용합니다. 다시 말해 KL은 별도의 손실 항이 아니라 **보상 신호에 섞이고**, 이어 PPO/GRPO의 어드밴티지 계산을 거칩니다. 정확히 이 지점에서 작용합니다.

**(2) 왜 ‘현재 정책을 먼저, 참조 정책을 나중에’ 두는가?** KL 발산은 $\mathrm{KL}(P\|Q)\neq\mathrm{KL}(Q\|P)$인 비대칭 값이므로 방향이 임의적이지 않습니다. 여기서는 현재 정책을 먼저 두어 $\mathrm{KL}(\pi_\theta\|\pi_{\text{ref}})$라고 쓰며, 수학적으로 **역방향 KL**이라고 합니다. ‘$\pi_\theta$가 어떤 지점에 높은 확률을 주지만 $\pi_{\text{ref}}$는 거의 0에 가까운 확률을 주는’ 상황, 즉 **참조 모델이 가지 말아야 한다고 보는 곳으로 모델이 가는 것**에 페널티를 줍니다. 이것이 바로 원하는 효과입니다. 참조 모델인 SFT 모델은 모델의 말이 자연스럽고 정상적인 형식을 따르는 안전 영역을 나타냅니다. 역방향 KL은 현재 정책이 마구 표류하지 않고 이 영역 가까이에 머물게 합니다. 반대로 **정방향 KL** $\mathrm{KL}(\pi_{\text{ref}}\|\pi_\theta)$을 사용하면 ‘참조 모델에는 있지만 현재 모델에는 없는’ 패턴에 페널티를 줍니다. 그러면 참조 모델의 모든 표현 스타일을 포괄하도록 강제하는데, 이는 RLHF의 목표가 아닙니다.

**(3) 왜 이렇게 설계하는가?—모드 추구의 기원.** 역방향 KL에는 **모드 추구**라는 핵심 특성이 있습니다. 7.1절에서 토대를 마련했듯, 역방향 KL을 사용하면 SFT의 최대 우도인 질량 포괄 방식처럼 모든 것을 고르게 포괄할 필요 없이 **보상이 높은 소수의 ‘모드’만 유지하고 나머지를 과감히 버릴 수 있습니다.** RLHF에서는 바로 이 효과를 원합니다. 가능한 모든 응답을 학습하는 대신 RM이 높은 점수를 주는 한두 가지 응답 스타일을 선택해 일관되게 생성합니다. RL 후의 모델이 더 ‘단호’하고 다양성은 낮아 보이는 이유이기도 합니다. 역방향 KL의 모드 추구 행동과 모델을 참조 분포 가까이에 유지하는 능력이 함께 RLHF의 안정성을 뒷받침합니다.

**(4) 이것이 없으면 어떻게 되는가?** 직관은 간단합니다. **출발점에서 너무 멀리 벗어나면 보상 모델의 점수를 신뢰할 수 없습니다.** RM은 참조 정책 주변의 출력 분포에서 학습합니다. 모델이 RM에서 한 번도 보지 못한 분포로 최적화되면 RM의 점수는 근거 없는 외삽값이 되고, 높은 점수가 더는 높은 품질을 의미하지 않습니다. 따라서 KL 페널티는 두 가지 문제를 동시에 막습니다. 모델이 실제 작업을 잘하지 않고 보상의 허점을 악용해 높은 점수만 얻는 **보상 해킹**과, 출력이 반복이나 무의미한 문자열 같은 극단적인 형태로 퇴화하는 **분포 붕괴**입니다. 검증 가능한 보상을 사용하는 RLVR 학습에서도 안정화를 위해 KL 정규화를 유지하는 경우가 많습니다. DAPO와 Open-Reasoner-Zero 같은 일부 연구는 의도적으로 제거했지만, DeepSeek-R1-Zero의 GRPO 자체에는 여전히 KL 항이 명시적으로 포함되어 있습니다.
...
**PPO**는 ‘클리핑’을 사용해 단계별 업데이트 크기를 제한하고 정책이 한 번에 너무 멀리 벗어나지 않도록 합니다.
...
여기서 $\rho$는 새 정책과 이전 정책의 확률 비율이며, $\epsilon$(예: 0.2)은 단계별 조정 범위를 제한합니다. 뒤에서 언급할 ‘Clip-Higher’는 특히 상한 $1+\epsilon$을 완화합니다.
...
즉 ‘그룹 평균보다 좋으면 양수, 나쁘면 음수’로 정하며 가치 네트워크가 필요 없습니다. 바로 이 때문에 비용이 더 낮습니다. 위 공식에서는 KL 정규화 항을 생략했습니다. 실제 학습에서는 보통 직전 절에서 소개한 토큰별 KL 페널티를 더해 정책이 참조 모델 근처에 머물도록 제약합니다.
...
설득력 있는 산업 사례로 Anthropic을 들 수 있습니다. 2025년 이전의 사후 학습 방법은 주로 두 부분으로 이루어졌습니다. **방대한 고품질 데이터의 SFT**와 **RLAIF**(Reinforcement Learning from AI Feedback. Bai et al. 2022의 Constitutional AI는 ‘헌법’을 기준으로 모델이 자신의 응답을 채점하고 정렬하게 함)입니다. 반면 현재 코드 및 사고 작업의 표준인 **RLVR(Reinforcement Learning with Verifiable Rewards)에는 거의 의존하지 않았습니다.** 그런데도 당시의 코딩 모델은 이미 뛰어났습니다. 그 이유는 대체로 알고리즘이 아니라 SFT와 RLAIF 양쪽에서 Anthropic이 데이터 품질을 얼마나 높은 수준까지 끌어올렸는지에 있습니다. 이는 앞의 판단, 즉 **SFT 데이터가 충분히 좋다면 단순한 방법만으로도 정교한 검증 가능 보상 RL이 필요하지 않을 수 있다**는 점을 확인해 줍니다. 그렇다고 RL이 쓸모없다는 뜻은 아닙니다. Anthropic은 2025년 이후 RL에 대규모로 투자했으며, 좋은 데이터라는 토대 위에서 RL은 역량 상한을 더 높입니다. **데이터는 얼마나 멀리 갈 수 있는지 결정하고, RL은 거기서 얼마나 더 높이 오를지 결정합니다.**
...
**작동하는 이유—그룹 내 분산이라는 하나의 설명.** 7.8절을 떠올려 보십시오. GRPO는 가치 네트워크를 학습하지 않습니다. 같은 프롬프트에서 G개의 롤아웃을 한 그룹으로 샘플링한 뒤 그룹 평균에 대한 각 롤아웃의 **상대적 위치**를 어드밴티지로 사용합니다. 여기에 중요한 수학적 사실이 있습니다. **GRPO의 어드밴티지는 본질적으로 그룹 내 분산입니다.** 그룹의 모든 롤아웃이 **정확히 같은 보상**을 받으면 분산은 0이고 모든 어드밴티지도 0이 되어 그 그룹은 경사에 전혀 기여하지 않습니다. 샘플 그룹 전체를 헛되이 생성한 셈입니다.

결과 보상만 사용하면 이러한 ‘분산 0 교착’이 두 가지 상황에서 필연적으로 발생하며, 공교롭게도 **학습 초반과 후반**에 가장 흔합니다.

- **전부 실패하는 그룹(학습 초반)**: 작업이 너무 어려워 그룹의 모든 롤아웃이 실패하고 O가 모두 0 → 그룹 내 분산 0 → 경사 없음. 학습 초반에는 거의 모든 그룹이 이 상태라 값비싼 샘플을 대량 낭비합니다.
- **전부 통과하는 그룹(학습 후반)**: 작업을 거의 학습해 모든 롤아웃이 성공하고 O가 모두 1 → 역시 분산 0 → 경사 없음.

다시 말해 **순수 결과 보상은 성공률 양극단에서 아무것도 보지 못합니다.** 커뮤니티의 기존 해법은 이러한 분산 0 그룹을 아예 **버리는 것**이었습니다. DAPO의 동적 샘플링은 모든 롤아웃이 맞거나 모두 틀린 프롬프트를 제외합니다. RLVP는 질문을 뒤집습니다. **버리는 대신 어떤 조밀한 신호로 사라진 분산을 복구할 수 있을까요?** 이렇게 물으면 답은 곧바로 드러납니다.

- **검증 가능한 페널티는 언제나 분산을 복구할 수 있습니다.** 그룹의 모든 롤아웃이 실패해도 실패한 *방식*은 대개 다릅니다. 일부는 파괴적인 명령을 실행하고 일부는 그렇지 않습니다. 페널티를 더하면 전부 실패한 그룹에도 곧바로 내부 차이인 분산이 생기고 경사가 다시 살아납니다. 나쁜 행동은 저렴하게 확인할 수 있으므로 **페널티는 ‘언제나 도달 가능한’ 해법의 절반**입니다.
...
요약하면 **조밀한 신호는 결과 보상에서 사라진 그룹 내 분산을 복구할 수 있을 때만 유용합니다.** 페널티는 나쁜 행동을 확인할 수 있으므로 언제나 이 조건을 만족하지만, 진전 보상은 부분 성공에 도달할 수 있을 때만 만족합니다. 그래서 논문은 페널티를 ‘보편적으로 사용 가능한 절반’, 진전 보상을 ‘조건부 절반’이라고 부릅니다.
...
>
> - **Clip-Higher(탐색 상한 완화)**: 표준 PPO는 학습 단계마다 정책 변화량을 제한합니다. 변화가 너무 크면 학습이 불안정해질 수 있기 때문입니다. 그러나 지나치게 엄격한 제한은 모델이 ‘새 경로를 시도하기 두려워하게’ 만듭니다. Clip-Higher는 이 제한을 적당히 완화합니다. 모델이 우연히 명백히 더 좋은 경로를 발견하면 그쪽으로 더 과감히 조정하도록 허용해 탐색을 장려합니다.
...
물론 이 패러다임의 경계도 명확하며, 주로 교사의 역량 상한이 학생 자신에게 묶여 있다는 사실에서 비롯됩니다. **향상 폭은 ‘특권 정보가 얼마나 많은 추가 역량을 가져오는가’에 달려 있습니다.** 모델이 답을 들고 있어도 풀이 과정을 명확히 설명하지 못한다면, 예컨대 답이 언어로 표현 가능한 사고가 아니라 무차별 탐색에서 나왔다면 자기 증류에는 신호의 원천이 없습니다. 기존 연구에서는 순진한 OPSD의 실패 유형도 관측했습니다. 예를 들어 자기 증류 중 모델이 기존 사고 스타일을 점차 잃어 안정성을 유지하려면 추가 정규화가 필요합니다[^ch7-16]. ‘같은 모델, 다른 컨텍스트, 서로의 교사와 학생’이라는 구상은 여전히 빠르게 진화하고 있지만, ‘더 강한 교사가 없다’는 흔한 곤경을 위한 길은 이미 열었습니다.

## 완전한 사후 학습 지형과 실전 요점

이 장은 사전 학습의 ‘다음 토큰 예측’ 목표에서 출발해 긴 여정을 따라왔습니다. SFT는 형식을 고정하고, RL은 일반화를 발전시키며, 다중 턴 작업은 기여도 할당 문제를 일으킵니다. 보상 설계는 결과 보상에서 결과에는 보상하고 과정을 제약하는 경로 신호로 확장되고, 도구 사용은 조합 폭발을 가져옵니다. 모든 실험을 관통하는 한 가지 주제는 모델이 무엇을 배우는지가 학습 신호가 무엇을 가르치는지에 달려 있고, 그 신호의 품질은 알고리즘이 아니라 주로 데이터와 환경이 결정한다는 것입니다.

**상호 보완적 패러다임**: 앞의 GeneralPoints 실험 요약에서는 중국 회화의 ‘선형후신’ 원칙으로 이 패러다임을 요약했습니다. ‘형식이 안정되고 기본 역량을 갖출’ 때까지 SFT를 사용한 뒤 그 토대 위에서 RL로 전략을 형성합니다. 두 방식은 서로 다른 수준에서 작동합니다. SFT는 프로토콜과 구조(JSON 형식, 대화 템플릿, 도구 인터페이스)를 고정하고, RL은 전략과 일반화(산술 규칙, 공간 사고, 행동 시퀀스)를 최적화합니다. 균형의 핵심은 SFT를 지나치게 학습하면 모델이 학습 분포에 붕괴해 RL의 최적화 공간을 제한할 수 있다는 것입니다.
...
2.  **형식이 안정되기 전에 RL 도입**—모델이 기본 JSON을 안정적으로 생성하지 못한다면(파싱 실패율 > 20%) RL 학습은 완전히 실패합니다. 반드시 SFT를 먼저 해야 합니다.
...
SFT와 RL은 경쟁하는 선택지가 아니라 순차적인 단계입니다. 먼저 SFT로 출력 형식을 안정화합니다. 그렇지 않으면 RL 보상 신호 자체를 계산할 수 없습니다. 이어 RL이 그 토대 위에서 일반화하는 법을 배웁니다. ‘SFT는 암기하고 RL은 일반화한다’는 말은 구호가 아니라 측정할 수 있는 현상입니다.
