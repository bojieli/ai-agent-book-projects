...
이 목록은 스스로 드러나는 실패만 담고 있습니다. 아무 오류도 남기지 않는 범주를 더해야 합니다. 요구 이해와 모호성 처리는 사용자가 요청한 것이 아니라 Agent가 바꿔 말한 것을 만들거나, 모호한 요청의 한 해석을 말없이 고르는 경우입니다. 검증 환경 해킹은 단언을 고치고 `skip`을 넣고 검사 대상 로직을 mock으로 덮거나, 실행하지도 않은 테스트를 통과했다고 말하는 경우입니다. 불완전한 수정은 호출 지점 세 곳을 고치고 네 번째—동적 호출, 다른 언어 바인딩, schema—를 빠뜨렸는데도 컴파일이 되는 경우입니다. 사용자에게 잘못된 정보를 보고하는 경우는 도구 호출과 최종 상태가 모두 맞는데 답변의 금액·상태·날짜가 틀린 것입니다. 비기능적 회귀는 마이그레이션 없이 공개 API나 schema를 바꾸거나 검사를 통과시키려고 검증을 지우는 것입니다. 이 모두에서 첫 오류는 도구 반환이 아니라 **assistant message**입니다. 판단이거나, 가정이거나, 했어야 했는데 하지 않은 질문입니다.
...
>
> 1단계: 층화 표본 추출. `t3a_failed.md`에서 실패 궤적 10개를 뽑되, 그중 최소 3개는 도구 오류가 전혀 없는 조용한 실패여야 한다. 판별 기준은 실패한 도구 반환이 없고, Agent가 스스로 완료를 선언했거나 단계 상한에 도달했으며, 마지막 검증기 판정만이 실패를 가리키는 경우다.
>
