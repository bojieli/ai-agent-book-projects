...
> **실험 7-17 ★★: ‘조기 완료’ bad case에서 DPO와 GRPO로**
>
> 6장의 실패를 두 학습 경로에 연결합니다. `build_preference_data.py`는 조기 완료 bad case 24개를 궤적 접두사 선호쌍으로 바꿉니다. 원래 완료 선언은 rejected, 먼저 검증하고 나중에 결론을 내리는 행동은 chosen입니다. `train_dpo.py`는 Qwen2.5-7B-Instruct LoRA-DPO 경로를 제공하고, 선택적 `train_grpo_optional.py`는 숨은 인수 테스트로 온라인 검증 보상을 비교합니다.
>
