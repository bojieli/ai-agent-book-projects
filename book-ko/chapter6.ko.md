...
부속 실험은 같은 60개 질문을 세 가지 메모리 구성에 적용해 실제 API 실행 궤적 180개를 남겼습니다. 표 6-3의 전체 성공률에는 성공한 문제 수도 함께 적어 표본 크기를 숨기지 않았습니다.

표 6-3 세 사용자 메모리 구성의 난이도별 성공률
...
> **실험 6-5 ★★: 여러 표현을 이용한 trajectory-prefix 경계 평가**
>
...
> **실험 6-6 ★★: 완전히 자동화된 TTS 품질 평가 파이프라인 구축하기**
>
...
최소한의 평가 프로브는 직접 복창, 긴 컨텍스트에서의 추출, 도구 인자에 넣기, 유사 문자열 선택, 그리고 공백·줄바꿈·역슬래시·유니코드 결합 문자·저빈도 token을 포괄합니다. 지표로는 byte-exact match, code-point-exact match, token-exact match, 첫 분기 위치, 실제 도구 성공률을 사용합니다. 직접 프로브에서는 맞는데 도구 호출이 실패한다면 tokenizer·직렬화·Harness·도구 프로토콜을 고쳐야 하며, 첫 차이가 모델 자신의 출력에서 나타날 때에만 그 사례를 7장의 복사 학습 데이터로 전환합니다.
...
> **실험 6-7 ★★: 쌍대 비교 데이터로 모델 리더보드 구축하기**
>
...
> **실험 6-8 ★★: 고정된 Coding Harness에서 모델의 행동 임계값 측정**
>
...
비용이 실제로 어디서 생기는지 보기 위해 부속 실험은 8라운드 환불 흐름을 고정했습니다. 주문, 배송, 환불 정책, 지식 베이스를 조회한 뒤 위험 확인, 환불, 알림, 종료까지 수행합니다. 실제 gpt-4o-mini 호출에서 ‘안정적인 접두사’와 ‘기록 압축’을 각각 켜고 꺼 2×2로 비교했습니다. 네 구성은 같은 업무를 완료했고 표 6-4의 비용은 저장된 token 사용량과 당시 가격으로 계산했습니다.

표 6-4 8라운드 에이전트 작업의 실제 비용
...
>
> **실험 6-11 ★★: 사용자 메모리 시스템의 엔드투엔드 선택 평가**
>
...
세 라운드의 실측 결과는 표 6-5과 같습니다. 각 그룹에 작업이 네 개뿐이므로 이 수치로 판단할 수 있는 것은 실험을 확대할 가치가 있는지까지입니다. AndroidWorld 전체 성공률을 추정할 수는 없습니다.

표 6-5 AndroidWorld Wi-Fi 하위 집합의 세 라운드
...
> **실험 6-12 ★★★: AndroidWorld에서 평가하고 개선하기**
>
...
> **실험 6-13 ★★: OpenVLA와 RoboTwin2의 체화 지능 환경 구성하기**
>
