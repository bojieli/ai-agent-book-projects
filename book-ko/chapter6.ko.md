...
캐스케이드는 오디오를 텍스트로 바꾸는 경계에서 감정·억양·환경음을 잃을 수 있다. Omni는 하나의 모델로 듣고 답하고 말해 이를 보존할 가능성이 있지만 학습·디버깅·교체 비용이 높다. 엔드투엔드의 이점은 지연과 비텍스트 정보이지 반드시 정확도는 아니다. self-cascade는 텍스트가 충분한 과제에서는 오류를 고치지만 말속도·감정이 필요한 과제에서는 텍스트 병목이 증거를 지운다. Omni도 턴을 가정해 숫자 중간의 침묵을 끝으로 오인할 수 있다.
