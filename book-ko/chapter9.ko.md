...
> **실험 9-9 ★: 시뮬레이터에서 같은 과제의 이상적 제어 상한 측정**
>
...
> **실험 9-10 ★★: Gemini Robotics-ER 1.5로 실제 XLeRobot 자율 제어**
>
> **목적:** 인간을 책상을 관찰하고 제한된 pick·place·verify 기술을 호출하는 Agent로 바꾸되, 실험 9-8과 같은 로봇·과제·성공 조건을 유지합니다.
>
...
### Computer Use를 위한 월드 모델

관찰 인터페이스는 동적 변화를 더 빨리 전달하고 기억에 남겨 “두 스크린샷 사이에 무슨 일이 일어났는가”에 답합니다. 하지만 계획 비용을 없애지는 못하므로 Agent는 여전히 “스크린샷—생각—클릭”의 직렬 루프를 반복하고 매 행동 뒤 다음 단계를 다시 생각할 수 있습니다. OSWorld-Human은 인간 수준의 정확도가 사람보다 훨씬 많은 단계와 대기 시간과 함께 나타날 수 있음을 보여 줍니다.

사람은 데스크톱을 예측적으로 조작합니다. 행동의 결과를 예상하고 관찰된 상태가 예측과 맞으면 재계획 없이 기존 계획을 계속하며, 어긋날 때만 관찰과 계획으로 돌아갑니다. 이것이 speculative execution이고 월드 모델이 Agent에게 제공하는 방식입니다. **월드 모델은 문제의 나머지 절반을 해결합니다**. 다음 상태를 예측하고 일치하면 계속하며 불일치하면 재계획하거나 멈춥니다.
