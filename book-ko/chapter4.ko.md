...
이 장에서는 먼저 다섯 가지 도구 범주를 개괄하고, 모든 도구에 공통으로 적용되는 설계 원칙과 MCP 프로토콜이 도구 생태계를 통합하는 방식을 설명합니다. 이를 바탕으로 계층형 구성, 동적 발견, 스킬을 사용해 도구 선택 문제를 해결합니다. 이어서 에이전트가 능동적으로 호출하는 인식·실행·협업 도구를 자세히 살펴보고, 이벤트 기반 비동기 아키텍처와 이벤트 트리거·사용자 커뮤니케이션 도구를 다룹니다. 마지막으로 도구가 수백, 수천 개에 이를 때의 능동적 발견 문제를 설명합니다.
...
개정된 보안 설계는 저위험 에이전트에 프로세스 격리를, 신뢰할 수 없는 입력에는 컨테이너나 microVM을 적용하고 모든 계층에 자원 할당량을 둡니다. 경량 Sidecar는 구조화된 도구 호출을 실행 게이트로 검사하며, 거부가 반복되면 회로 차단기를 작동시켜 사용자의 판단을 요청합니다. 멱등성이 없는 작업은 “사전 점검 후 확인”의 2단계 흐름을 사용합니다.
...
개정판은 Hooks가 OpenClaw 내부 수명주기에서 발생하고 Cron과 Heartbeat는 시간 기반이라는 점을 명확히 합니다. 외부 이메일과 API 콜백에는 PineClaw의 Channel 같은 즉시 입력 경로가 필요합니다.
...
> **실험 4-5 ★★★: 이벤트 기반 이메일 처리 에이전트**
>
>
> ![그림 4-4 실험 4-5 이벤트 기반 에이전트 아키텍처](images/fig4-4.svg)
>
>
...
> **실험 4-6 ★★★: 병렬 실행과 중단 기능을 갖춘 비동기 에이전트**
>
>
> ![그림 4-6 실험 4-6 비동기 에이전트 중단과 복구](images/fig4-6.svg)
>
>
...
일곱 가지 실험은 기초에서 아키텍처로 차근차근 나아갑니다. 실험 4-1부터 4-4까지는 인식·실행·협업이라는 세 가지 기본 도구 집합을 만듭니다. 실험 4-5는 이메일 처리 에이전트로 이벤트 기반 처리를 도입합니다. 실험 4-6은 병렬 실행, 중단 복구, 상태 관리를 구현합니다. 실험 4-7은 대규모 도구 라이브러리에서 능동적 도구 발견의 가치를 검증합니다. 이 장의 범위는 **기존 도구**를 설명하고 발견하며 안전하게 사용하는 것까지입니다. 8장에서는 에이전트가 실패와 반복 작업을 근거로 언제 도구를 만들고 수정하며 재검증하거나 폐기할지 판단하는 방법을 다룹니다.
