...
>
> 위 세 차원의 수치는 한 번의 소규모 대조 실행에서 나온 **예시적 관찰**입니다. 이것이 말해 주는 것은 방향—조직 구조와 도구 설명의 영향이 어조와 문체보다 훨씬 크다는 것—이지 재현 가능한 크기가 아닙니다. 7장의 기준으로 보면 이런 단회 차이가 "얼마나 높은가"를 뒷받침하려면 표본 수와 반복 횟수가 필요합니다. 직접 이 실험을 돌릴 때는 같은 퍼센트포인트가 재현되는지가 아니라 세 차원의 상대 순위가 안정적인지를 보십시오.
>
>
...
실험 2-7은 직관을 보여 주는 소규모 정성적 데모입니다. “미리 계산해 두고 곧바로 확인하는” 방식이 얼마나 유용하고 한계가 어디인지 저자와 공동 연구자들은 전용 벤치마크로 정량화했습니다[^ch2-7](이 방식의 통칭은 **컨텍스트 증류, Context Distillation**이며 에이전트 상태 표시줄은 가장 일상적인 형태입니다). 결론은 다음과 같습니다.
...
>
> **상세한 오류 정보**: 네 계층으로 구성합니다. 오류 유형과 설명, 전체 파라미터 JSON, 호출 스택 정보, 상황별 수정 제안입니다. 예를 들어 FileNotFoundError가 발생하면 경로 확인, 작업 디렉터리 점검, 절대 경로 사용을 제안합니다. 이 정보를 제공하면 에이전트의 오류 복구 성공률이 60%에서 95%로 높아집니다. 에이전트는 무작정 다시 시도하지 않고 실패를 진단해 대안을 고를 수 있습니다（이 역시 한 번의 소규모 실행에서 나온 **예시적 관찰**로, "오류 정보를 상세히 쓴다"는 방향이 유효함을 보여 줄 뿐 이 폭이 재현된다는 뜻은 아닙니다）。
>
