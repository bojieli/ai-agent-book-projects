...
>
> **모델의 내부 사고 과정**: Qwen3처럼 사고 사슬을 지원하는 모델은 도구 호출을 생성하기 전에 `<think>` 태그 안에서 먼저 사고합니다. 사용자의 의도를 분석하고, 알맞은 도구를 평가하며, 호출 순서를 계획합니다. 이 사고 과정은 에이전트 행동을 디버깅하는 데 유용합니다. 참고로 `<think>`는 모델의 원본 출력 스트림에 나타나는 태그일 뿐, API 응답에 그대로 나타난다는 보장은 없습니다. 이 실험은 Ollama에 `think=True`를 전달하므로, Ollama는 응답을 반환하기 전에 이 사고 부분을 파싱해 별도의 `thinking` 필드(스트리밍에서는 `thinking` 증분 청크)에 담습니다. 따라서 `content`에서는 `<think>` 태그를 볼 수 없습니다. 둘은 같은 사고 내용을 다르게 표현한 것일 뿐이며, 실험 코드는 두 경우를 모두 처리합니다.
>
