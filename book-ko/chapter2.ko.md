...
코드, 프로세스, 환경이라는 세 범주는 에이전트가 효과적으로 일하는 데 필요한 최소 컨텍스트를 이룹니다. 모델에 내재된 능력은 토대일 뿐이며, **컨텍스트 품질이 에이전트 능력을 좌우하는 진정한 핵심입니다.** 잘 정리된 컨텍스트를 갖춘 보통 수준의 모델이 부족한 컨텍스트로 작동하는 더 강한 모델보다 나은 성과를 내는 경우도 많습니다.
