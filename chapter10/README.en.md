...
| 10-2 | [multi-role-transfer](multi-role-transfer/) | ✅ | Compares `transfer_to_agent` (dynamic system prompt/tool isolation) with fixed-prefix `load_skill` on shared trajectories. The Luna-ready runner evaluates cost, cache, actual effect and boundary instruction following, with eight complex rule/task specifications; the historical Kimi/Tavily transfer acceptance remains evidence for that arm only. |
