...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--phone-transport", choices=["webrtc", "local", "twilio"], default="webrtc",
    ...
    p.add_argument("--acceptance-report", default="artifacts/acceptance_report.json",
                   help="写入机器可读验收门禁")
    p.add_argument("--webrtc-headless", action="store_true",
                   help="无界面运行 WebRTC 参与者（仅用于安全自动验收）")
    p.add_argument("--webrtc-port", type=int, default=0,
                   help="WebRTC 本地通话页端口；0 表示自动选择空闲端口")
    p.add_argument(
        "--webrtc-answers-json",
        default=None,
        help=("安全自动验收：字段名映射到一个回答或回答数组；回答会先合成语音，"
    ...
    try:
        ...
        if args.scripted_json:
            ...
            answer_plan = (
                ...
                if args.webrtc_answers_json else None
            )
        ...
        webrtc_receipt = (
            ...
            if transport == "webrtc" and hasattr(channel, "acceptance_receipt") else None
        )
        webrtc_pass = bool(
            webrtc_receipt
        ...
        repeated_questions = [
            message for message in bus.history
        ...
        persisted_trace = Path(args.trace).read_text(encoding="utf-8") if Path(args.trace).exists() else ""
        ...
        overall_pass = bool(
            fill_pass and autonomy_pass and concurrency_pass
            and webrtc_pass and live_audio_pass and reask_pass and privacy_pass
        ...
        report = {
            ...
            "gates": {
                ...
                "privacy_redaction_and_ephemeral_audio": {"status": "pass" if privacy_pass else "fail"},
                ...
                "real_webrtc_session": {
                    "status": "pass" if webrtc_pass else "not_run" if transport != "webrtc" else "fail",
                    "reason": None if webrtc_pass else "requires a connected offer/answer and bidirectional RTP audio",
                },
                "bidirectional_webrtc_audio_and_real_asr_tts": {
                    "status": "pass" if live_audio_pass else "not_run" if transport == "scripted" else "fail",
                    "reason": None if live_audio_pass else "audio media or provider operations did not complete",
                },
            },
