...
Phần này dùng giao thức **OpenAI-compatible Chat Completions** làm ví dụ cụ thể. Không chỉ OpenAI mà DeepSeek, Kimi, Qwen cùng các hệ thống phục vụ mô hình nguồn mở như vLLM và Ollama đều triển khai giao thức này, khiến nó trở thành giao diện phổ biến nhất giữa nhiều nhà cung cấp. Anthropic, Google và các nhà cung cấp khác có cấu trúc tin nhắn và công cụ riêng; phần này không trình bày từng cấu trúc đó.

OpenAI còn cung cấp Responses API mới hơn. API này tổ chức yêu cầu quanh `instructions` và các input/output item, hỗ trợ chỉ thị `developer`, đồng thời có thể quản lý trạng thái hội thoại qua `previous_response_id` hoặc Conversations. Tuy nhiên, để các ví dụ có thể dùng trực tiếp với mô hình nguồn mở và hệ thống phục vụ tương thích, chương này tiếp tục dùng Chat Completions; system, user, assistant và tool là bốn vai trò được hỗ trợ phổ biến nhất giữa các cách triển khai.
...
Yêu cầu này chỉ chứa hai tin nhắn: system (quy tắc do nhà phát triển viết) và user (đầu vào của người dùng). Mô hình trả về một tin nhắn assistant. Trong mẫu Chat Completions cơ bản dùng ở chương này, **mỗi yêu cầu được dựng theo cách không trạng thái, nên client phải cung cấp thông tin mô hình cần trong danh sách tin nhắn**. Đây không phải giới hạn phổ quát của mọi API: Responses có thể tham chiếu response trước đó hoặc một Conversation bền vững.
...
1. **Yêu cầu thứ hai chứa toàn bộ lịch sử hội thoại của yêu cầu đầu tiên** - tin nhắn hệ thống, tin nhắn người dùng, câu trả lời assistant đầu tiên (bao gồm lệnh gọi công cụ) và kết quả công cụ mới. Điều này minh họa mẫu yêu cầu Chat Completions không trạng thái cơ bản: framework Agent phải đưa lịch sử cần thiết vào yêu cầu tiếp theo.
...
> 4. **Ưu điểm của phản hồi phát trực tuyến**: Trong thí nghiệm cục bộ này, mô hình Qwen và template được chọn phát ra token `<think>`, nên nhà phát triển có thể quan sát theo thời gian thực phần văn bản suy luận được để lộ, quyết định gọi công cụ và cách xử lý kết quả. Đây là khả năng quan sát phục vụ thí nghiệm của một mô hình cục bộ, không phải thuộc tính phổ quát của API được lưu trữ; hệ thống production nói chung không nên để lộ văn bản thô này cho người dùng cuối.
...
>
> Nhiều vòng gọi công cụ trong dự án tuân theo chu trình suy nghĩ-hành động-quan sát ReAct được giới thiệu trong Chương 1 nên nguyên tắc không được lặp lại ở đây. Phần trước đã dùng JSON OpenAI-compatible Chat Completions để hiển thị cấu trúc tin nhắn hoàn chỉnh của quá trình này. Trong triển khai cục bộ, máy chủ như vLLM hoặc Ollama tự động chuyển đổi các tin nhắn API sang định dạng token nội bộ của mô hình. Dự án `local_llm_serving` cho phép quan sát trực tiếp luồng token đầu vào và đầu ra thô của mô hình, gồm những chi tiết thường không thấy ở cấp API:
>
> **Token suy luận do template mô hình để lộ**: Trong thí nghiệm này, Qwen3 phát ra phần phân tích trong thẻ `<think>` trước khi tạo lệnh gọi công cụ—chẳng hạn đánh giá công cụ phù hợp và lập kế hoạch thứ tự gọi. Các token này có thể giúp nghiên cứu hành vi sinh của mô hình cục bộ cụ thể này, nhưng không nên được giả định là lời giải thích đầy đủ, trung thực cho quyết định của nó hoặc dùng làm bằng chứng kiểm toán bảo mật.
>
...
**Thứ nhất, điều này giải thích vì sao phải dùng định dạng API chuẩn.** Nếu nhà phát triển bỏ qua API và tự ghép tin nhắn—chẳng hạn gửi kết quả công cụ dưới dạng tin nhắn user thông thường thay vì tin nhắn tool—Chat Template có thể biểu diễn sai cuộc hội thoại. Với Chat Template của Qwen3, một vòng gọi công cụ có thể giữ nội dung trong thẻ `<think>`, còn khi phát hiện lượt user mới, template có thể xóa nội dung đó theo quy tắc của nó. Vì vậy, đánh dấu nhầm kết quả công cụ thành tin nhắn user có thể kích hoạt việc đặt lại sai thời điểm và làm suy yếu tính liên tục nhiều bước.

Quy tắc gửi lại trạng thái suy luận không đồng nhất giữa các mô hình. Giao thức chính thức của DeepSeek R1 yêu cầu client loại bỏ `reasoning_content` ở các lượt sau; một số mô hình hoặc phiên bản DeepSeek, Kimi và GLM khác lại yêu cầu đặt nguyên vẹn `reasoning_content` được trả về vào đúng tin nhắn assistant. API riêng của Claude dùng thinking block có chữ ký và quy định thời điểm phải gửi lại. Responses API của OpenAI không để lộ chuỗi suy nghĩ thô, nhưng có thể trả về reasoning item mờ đục hoặc được mã hóa cần được lưu giữ, cùng bản tóm tắt tùy chọn; khi tiếp nối không trạng thái, các output item liên quan phải được gửi lại cùng nhau. Bài học chung không phải là "luôn phải giữ phần suy nghĩ nhìn thấy được", mà là **trạng thái liên quan đến suy luận thuộc hợp đồng của nhà cung cấp; có gửi lại hay không và gửi bằng cách nào phải tuân theo hợp đồng hiện hành của mô hình và API đó**. Không được tự tạo, phân tích cú pháp hoặc để lộ trạng thái mờ đục, và không được coi văn bản suy luận đọc được là lời giải thích trung thực.
