...
Cần phải làm rõ rằng cả hai kiến trúc đều là hệ thống multi-Agent thực sự (vì system prompt và bộ công cụ của từng giai đoạn khác nhau nên đó là các Agent khác nhau); sự khác biệt nằm ở phương thức phối hợp. **Ngữ cảnh được chia sẻ** dựa vào sự phối hợp ngầm—Agent tiếp theo kế thừa tin nhắn hiển thị, bản ghi công cụ và trạng thái do nhà cung cấp trả về mà giao thức yêu cầu tiếp tục truyền; thông tin được truyền qua chính ngữ cảnh đó. Điều này không có nghĩa Agent có thể thấy chuỗi suy nghĩ thô của mô hình khác nếu API không để lộ nó. **Không có ngữ cảnh chung** dựa vào sự phối hợp rõ ràng—Agent trao đổi thông tin qua tệp, tin nhắn hoặc giao diện dữ liệu có cấu trúc và mỗi Agent chỉ thấy nội dung liên quan đến mình.
...
**Lấy trạng thái bằng hệ thống tệp dùng chung**. Hình thức đầy đủ nhất là **lưu bền trajectory** (trajectory persistence): trong khi thực thi, Agent con tuần tự hóa từng sự kiện trajectory thành JSON và ghi nối vào tệp nhật ký trong hệ thống tệp—thường mỗi phiên một tệp, mỗi dòng một sự kiện, tức JSONL. Trajectory được định nghĩa ở Chương 1 chứa tin nhắn người dùng, đầu ra mô hình hiển thị, lệnh gọi và kết quả công cụ, cùng mọi trạng thái do nhà cung cấp trả về mà giao thức yêu cầu client lưu giữ. Agent chính không cần giao thức báo cáo trạng thái riêng; bằng cách đọc trực tiếp tệp này, nó có thể xem bản ghi thực thi quan sát được của Agent con: lệnh gọi công cụ gần nhất, kết quả trả về và việc Agent có mắc kẹt trong vòng lặp thử lại thất bại hay không. Nó không thể thấy chuỗi suy nghĩ thô mà API không để lộ. Trajectory cung cấp độ chi tiết quan sát cao, nhưng không phải toàn bộ bộ nhớ của một tiến trình khác.

Mức chi tiết này cũng là gánh nặng: trajectory dễ dài đến hàng vạn token, Agent chính đọc xong còn phải tự chắt lọc, vừa tốn thời gian vừa tốn token. Vì vậy trong đa số tình huống, hợp lý hơn là **quy ước một tệp tiến độ**: khi khởi động Agent con, Agent chính yêu cầu "hãy ghi tiến độ vào `progress.md`"; Agent con cập nhật danh sách sau mỗi hạng mục hoàn thành, còn Agent chính có thể đọc tệp nhẹ này bất cứ lúc nào để nắm tiến độ. Cách này giống như dành một vùng nhỏ có định dạng thống nhất trong bộ nhớ chia sẻ, chỉ để lộ tiến độ đã chắt lọc thay vì toàn bộ trạng thái runtime.

Tệp tiến độ còn hỗ trợ **phát hiện bị kẹt**: nếu thời gian sửa đổi cuối cùng của `progress.md` hoặc tệp trajectory không thay đổi quá N phút, hệ thống có thể coi Agent con không còn hoạt động và kích hoạt cơ chế dự phòng timeout (tương ứng với Heartbeat và `monitor_shell` trong Chương 4), tránh để Agent con bị nghẽn kéo sập toàn hệ thống.

Giá trị của việc lưu bền trajectory không chỉ dừng ở giám sát. Nhìn lại kết luận của Chương 1 "ngữ cảnh của Agent = tiền tố tĩnh + trajectory": nạp lại trajectory rồi ghép tiền tố tĩnh có thể dựng lại **trạng thái hội thoại và mô hình** của Agent, đây là nền tảng quan trọng để tiếp tục phiên. Nếu hợp đồng nhà cung cấp yêu cầu gửi lại trạng thái suy luận, trạng thái đó cũng phải được lưu bền. Tuy nhiên, trajectory không phải checkpoint runtime hoàn chỉnh. Tệp và nội dung cơ sở dữ liệu, phiên trình duyệt và công cụ, khóa và lease, thông tin xác thực và trạng thái ủy quyền, yêu cầu đang chạy, cùng tác dụng phụ bên ngoài như email, thanh toán và đơn hàng đều nằm ngoài trajectory; dịch vụ bên ngoài cũng có thể không xác định.

Do đó, khôi phục đáng tin cậy đặt trajectory trong một giao thức lớn hơn: lưu bền bản ghi thao tác và idempotency key cho hành động có tác dụng phụ, checkpoint tài nguyên bên ngoài, rồi sau khi khởi động lại phải đối soát xem từng thao tác chưa bắt đầu, đang chạy, đã hoàn tất hay có kết quả chưa biết trước khi chọn tiếp tục, thử lại hoặc bù trừ. Phát lại transcript một cách mù quáng có thể gửi email, thanh toán hoặc đặt hàng hai lần. Phép loại suy write-ahead log (WAL) vẫn hữu ích, nhưng trajectory chỉ là một nhật ký trong giao thức khôi phục; nó phải kết hợp với checkpoint trạng thái bên ngoài và bản ghi thao tác bền vững, thay vì được phát lại như thể một mình nó bảo đảm khôi phục an toàn. Đây là nền tảng kỹ thuật cho việc khôi phục, kiểm toán và bàn giao trong hệ thống đa Agent.
...
Hệ thống Multi-Agent có hai chiều thiết kế cốt lõi trực giao: ngữ cảnh có được chia sẻ hay không và cấu trúc liên kết cộng tác được tổ chức như thế nào. Ngữ cảnh được chia sẻ là sự cộng tác đa Agent được "kế thừa"—Agent tiếp theo kế thừa toàn bộ ngữ cảnh khả dụng của Agent trước đó, không mất thông tin nhưng ngữ cảnh mở rộng nhanh chóng; ngữ cảnh không chia sẻ là sự cộng tác đa Agent hoàn toàn độc lập, trao đổi thông tin qua các gói chuyển giao đã chắt lọc, hệ thống tệp hoặc truyền tin nhắn. Về cấu trúc liên kết cộng tác, chế độ ngang hàng phù hợp cho cải tiến lặp của một số ít Agent, chế độ người quản lý phù hợp với tác vụ phức tạp cần lập lịch động và chế độ phi tập trung phù hợp khi trách nhiệm ngang nhau và quyền kiểm soát cần tự động chuyển giữa các Agent.

Tất cả điều này được xây dựng trên hai bộ cơ sở hạ tầng không phụ thuộc cấu trúc liên kết, với bản thiết kế đến từ hệ điều hành. Agent đối với runtime cũng như tiến trình đối với kernel: tiền tố tĩnh là chương trình, trajectory mang trạng thái hội thoại và mô hình, còn LLM là CPU được chia sẻ theo thời gian. Là mặt phẳng dữ liệu, **hệ thống tệp dùng chung** về cơ bản là một cây thư mục ảo gắn bốn loại khu vực: không gian làm việc riêng của Agent, không gian dùng chung giữa nhiều Agent, tài nguyên bên ngoài và tài nguyên tích hợp của hệ thống; các Agent trao đổi sản phẩm bằng cách truyền đường dẫn tệp.

Là mặt phẳng điều khiển, **cơ chế điều khiển và giao tiếp** hỗ trợ truyền tin nhắn, truy vấn trạng thái, chấm dứt thực thi và điều phối tài nguyên. Agent có thể báo cáo trạng thái không đồng bộ qua tin nhắn, hoặc Agent cha có thể quan sát tệp mà Agent con cập nhật theo thời gian thực—trajectory hoặc tệp tiến độ đã quy ước. Trajectory có thể dựng lại trạng thái hội thoại và mô hình, nhưng không phải checkpoint runtime hoàn chỉnh; khôi phục sau sự cố còn cần bản ghi thao tác bền vững, idempotency key, checkpoint trạng thái bên ngoài và đối soát sau khi khởi động lại để việc phát lại không nhân đôi tác dụng phụ. Bus tin nhắn là cách triển khai phổ biến của mặt phẳng điều khiển, phù hợp để phối hợp nhiều bên theo thời gian thực và không đồng bộ; khi vượt qua ranh giới tổ chức, cần giao thức tương tác chuẩn hóa như A2A.
