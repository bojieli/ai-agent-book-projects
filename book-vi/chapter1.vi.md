...
- **Assistant Message**(Trả lời của mô hình): Đầu ra hiển thị trước đó của mô hình thường gồm nội dung văn bản (`content`, tức câu trả lời cho người dùng) và yêu cầu gọi công cụ (`tool_calls`, tức cách Agent hành động). Một số API còn trả về trạng thái suy luận riêng của nhà cung cấp. Mô hình nguồn mở hoặc chạy cục bộ có thể để lộ token suy luận qua `<think>` hoặc `reasoning_content`; các API khác chỉ trả về trạng thái mờ đục, có chữ ký hoặc được mã hóa mà client phải gửi lại nguyên vẹn, hoặc cung cấp bản tóm tắt suy luận tùy chọn. Các mô hình suy luận do OpenAI lưu trữ không để lộ chuỗi suy nghĩ nội bộ thô; Responses API có thể trả về reasoning item mờ đục hoặc được mã hóa cùng bản tóm tắt tùy chọn. Framework phải lưu giữ và gửi lại trạng thái này theo hợp đồng của mô hình, nhưng không được giả định chuỗi suy nghĩ thô là một trường `reasoning` phổ quát, có thể đọc được, cũng không được coi nó là lời giải thích trung thực hay bằng chứng kiểm toán.
...
Để đo vai trò của từng thành phần trong một hệ thống cụ thể, phương pháp trực tiếp nhất là Ablation Study: giống như bác sĩ lần lượt loại trừ từng nguyên nhân khi chẩn đoán—bỏ thành phần A để xem hệ thống còn hoạt động không, rồi đến thành phần B, và cứ thế—nhằm xác định đóng góp của từng thành phần. Thí nghiệm 1.1 áp dụng đúng cách này cho năm thành phần nói trên. Kết quả cho thấy: nếu không có định nghĩa công cụ, Agent hoàn toàn mất khả năng hành động; khi thiếu kết quả công cụ, Agent không thấy phản hồi từ bước trước nên gọi đi gọi lại cùng một công cụ và rơi vào vòng lặp vô hạn; với mô hình và API dùng trong thí nghiệm này, việc loại bỏ trạng thái suy luận mà giao diện đã trả về và yêu cầu client gửi lại khiến các quyết định liên tiếp bắt đầu mâu thuẫn; nếu không có lịch sử tin nhắn, Agent mất tính liên tục của nhiệm vụ, khởi động lại toàn bộ quy trình từ đầu và lặp lại những bước đã hoàn thành. Kết quả cắt bỏ này mô tả hành vi của mô hình và giao diện được thử nghiệm; nó không chứng minh rằng mọi Agent đều phải để lộ hoặc gửi lại chuỗi suy nghĩ có thể đọc được.
...
Kết quả thực nghiệm cho thấy vai trò của từng thành phần ngữ cảnh trong cấu hình cụ thể này. **Định nghĩa công cụ** (một phần của tiền tố tĩnh) là cơ sở cho khả năng hành động của Agent. Nếu không có nó, Agent không thể nhận dạng và gọi bất kỳ công cụ nào. **Kết quả công cụ** là chìa khóa để điều khiển vòng kín; thiếu nó khiến Agent hành động "mù quáng" và rơi vào vòng lặp vô hạn. **Trạng thái suy luận do nhà cung cấp trả về** phải đi kèm tin nhắn assistant theo hợp đồng của mô hình dùng trong thí nghiệm này; loại bỏ nó làm gián đoạn tính liên tục nhiều bước, nhưng điều đó không có nghĩa trạng thái ấy nhất thiết phải đọc được hoặc chứa đầy đủ lý do của quyết định. **Lịch sử tin nhắn** (tin nhắn người dùng, phản hồi mô hình và kết quả công cụ từ các vòng trước) ngăn thao tác dư thừa, duy trì tính liên tục khi thực thi nhiệm vụ và tránh lặp lại cùng một lỗi.
...
# Lần lặp đầu tiên - LLM nhìn thấy trajectory trên và tạo ra phản hồi
  {role: “assistant” ,
reasoning_state: "<được lưu theo hợp đồng nhà cung cấp; không hiển thị nội dung>" ,
# Tóm tắt của tác giả: cần chuyển đổi tất cả các loại tiền tệ sang USD
...
# Lần lặp thứ hai - LLM nhìn thấy toàn bộ trajectory, bao gồm cả kết quả công cụ
  {role: “assistant” ,
reasoning_state: "<được lưu theo hợp đồng nhà cung cấp; không hiển thị nội dung>" ,
# Tóm tắt của tác giả: đã có kết quả quy đổi; cần tổng hợp và tính toán
   content: “” ,
   tool_calls: [
...
# Lần lặp thứ ba - LLM nhìn thấy trajectory hoàn chỉnh và đưa ra câu trả lời cuối cùng
  {role: “assistant” ,
reasoning_state: "<được lưu theo hợp đồng nhà cung cấp; không hiển thị nội dung>" ,
# Tóm tắt của tác giả: mọi phép tính đã hoàn tất; cần tổng hợp kết quả
...
Vẻ đẹp của thiết kế này là tính chất tích lũy của ngữ cảnh. Mỗi cuộc gọi LLM có thể thấy trajectory hoàn chỉnh, cho phép nó hiểu nhiệm vụ đang ở giai đoạn nào, trước đó đã thử gì và nhận được kết quả gì. Cũng như con người liên tục xem xét và tóm tắt khi giải quyết vấn đề, Agent duy trì cái nhìn toàn cục về nhiệm vụ thông qua trajectory. Cấu trúc này còn giúp hệ thống dễ gỡ lỗi: tin nhắn người dùng, phản hồi hiển thị, lệnh gọi công cụ và kết quả công cụ được phân tách rõ ràng, cho phép dựng lại hành vi có thể quan sát. Trajectory không phải lời giải thích trung thực về suy luận nội bộ của mô hình; nếu API cung cấp riêng bản tóm tắt suy luận, vẫn phải coi đó là bản tóm tắt chứ không phải chuỗi suy nghĩ thô hay bằng chứng kiểm toán.
