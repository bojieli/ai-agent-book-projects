...
- **Assistant Messages**: The model's prior visible output usually includes text content (`content`, the response to the user) and tool-call requests (`tool_calls`, how the Agent takes action). Some APIs also return provider-specific reasoning state. Open-source or local models may expose reasoning tokens through `<think>` or `reasoning_content`; other APIs return only opaque, signed, or encrypted state that must be passed back unchanged, or an optional reasoning summary. OpenAI's hosted reasoning models do not expose raw internal chain-of-thought; the Responses API may return opaque or encrypted reasoning items and optional summaries. A framework should preserve and replay this state according to the model's contract, but it must not assume that raw chain-of-thought is a universal, readable `reasoning` field or treat it as faithful explanation or audit evidence.
...
The most direct way to measure each component's role in a particular system is an **ablation study**—the diagnostic method of ruling out causes one at a time: remove component A and see whether the system still works, then component B, and so on, until each component's contribution is clear. Experiment 1-1 applies exactly this method to the five components above. The results are direct: without tool definitions, the Agent is completely incapable of action; without tool results, it does not receive feedback from the previous step, so it calls the same tool repeatedly, becoming stuck in an infinite loop; with the model and API used in this experiment, discarding reasoning state that the interface returned and required the client to replay made consecutive decisions contradict each other; without message history, the Agent loses task continuity and restarts the whole task from the beginning, repeating steps already done. This ablation result describes the tested model and interface; it does not show that every Agent must expose or replay readable chain-of-thought.
...
The experimental results reveal the role of each context component in this specific setup. **Tool Definitions** (part of the static prefix) are the foundation of the Agent's action capability; without them, the Agent cannot recognize or call any tools. **Tool Results** are key to closed-loop control; their absence deprives the Agent of execution feedback and causes it to fall into an infinite loop. **Provider-returned reasoning state** must accompany assistant messages under the model contract used in this experiment; removing it disrupts multi-step continuity, but this does not mean the state is necessarily readable or that it contains the complete reasons for a decision. **Message history** (user messages, assistant messages, and tool results from previous rounds) prevents redundant operations, maintains task execution coherence, and avoids repeating the same mistakes.
...
Consider a concrete example—aggregating revenue across multiple currencies—to understand an Agent's **trajectory**: the message history that accumulates as the Agent works, comprising user messages, assistant output (visible text, tool calls, and any provider-returned state that the protocol requires the client to preserve), and tool results. On every LLM call, the complete context the model receives is the **static prefix** (system prompt + tool definitions) plus the **trajectory** (dynamic message history) (Figure 1-3). This shows a key fact: **Agent context = static prefix + trajectory**. Concretely, the static prefix is the first two of the five components above (system prompt + tool definitions); the trajectory is the last three (user messages + assistant messages + tool results, growing with each interaction). From this complete context the LLM generates its next response, which is then appended to the trajectory for the subsequent call.

![Figure 1-3: Agent trajectory—ReAct loop for a multi-currency aggregation task](images/fig1-3.svg)

Here is the structure of a trajectory, in pseudocode. `reasoning_state` below is only an abstract placeholder for state preserved and replayed under a provider's contract: it may be readable `reasoning_content`, an opaque block, or absent entirely. The task-analysis comments are author-written summaries for the reader, not raw chain-of-thought exposed by an API.
...
trajectory = [
  ...
  {role: "assistant",
   reasoning_state: "<preserved per provider contract; content not shown>",
   # Author summary: convert all currencies to USD
  ...
  {role: "assistant",
   reasoning_state: "<preserved per provider contract; content not shown>",
   # Author summary: conversion results obtained; aggregate and calculate
   content: "",
   tool_calls: [
  ...
  {role: "assistant",
   reasoning_state: "<preserved per provider contract; content not shown>",
   # Author summary: calculations complete; summarize the result
...
The elegance of this design lies in the **cumulative nature of the context**. Every LLM call receives the complete trajectory, so the model knows which stage of the task it is in, what was tried before, and what the outcome was. Just as people keep reviewing and summarizing while solving a problem, the Agent maintains a global view of the task through its trajectory. Its structure also makes the system highly debuggable: user messages, visible replies, tool calls, and tool results are cleanly separated, allowing observable behavior to be reconstructed. The trajectory is not a faithful account of the model's internal reasoning; if an API separately provides a reasoning summary, it should still be treated as a summary rather than raw chain-of-thought or audit evidence.
