...
2. ★★★ In a ReAct loop, if each round adds a similar amount of history, every LLM call reprocesses the full trajectory, and no prefix cache (KV Cache) is reused across calls, cumulative input-token processing grows approximately quadratically with the number of rounds. Actual cost depends on the caching and billing model. Can this growth be reduced without losing critical information?
