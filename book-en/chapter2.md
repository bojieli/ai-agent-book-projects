...
>
> **Model's Internal Reasoning Process**: Models that support chain-of-thought (e.g., Qwen3) will first reason inside `<think>` tags before generating tool calls—analyzing user intent, evaluating which tools are suitable, and planning the call order. This reasoning process is valuable for debugging Agent behavior. Note that `<think>` is a tag in the model's raw output stream, and it will not necessarily appear verbatim in the API response: this experiment passes `think=True` to Ollama, so Ollama parses the reasoning out before returning it and places it in a separate `thinking` field (delivered as incremental `thinking` chunks when streaming), which means no `<think>` tag shows up in `content`. The two are simply different presentations of the same reasoning content, and the experiment code handles both cases.
>
