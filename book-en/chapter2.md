...
#### Implementation note: standard versus wire format

The Agent Skills standard defines the disclosure order, not a literal API message role: the catalog must be visible before the full instructions, and the full `SKILL.md` is loaded only after selection. Anthropic documents the catalog as startup system-level context, while Claude Code documents invoked Skill instructions as user messages at the invocation point. A user-role `<system-reminder>` seen in a particular Claude Code trace is therefore a Harness- and version-specific wire representation, not an Agent Skills protocol requirement.

Codex CLI is a different Harness. OpenAI's [Skills documentation](https://developers.openai.com/codex/skills/) says its initial catalog contains each Skill's name, description, and path. The current public source renders that catalog as a `developer` context fragment and the selected Skill body as a user fragment wrapped in `<skill>` ([`fragments.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/fragments.rs), [`extension.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/extension.rs)). Gateways may display `developer` content in a system-like area. Consequently, the comparison table attributed to DeepSeek is useful only as a snapshot of particular client versions; role, wrapper, and per-turn reconstruction are Harness details. Cache costs must likewise be evaluated per runtime rather than copied from Claude Code to every client.
