...
This section uses the **OpenAI-compatible Chat Completions** protocol as its concrete example. It is implemented not only by OpenAI but also by DeepSeek, Kimi, Qwen, and open-source serving systems such as vLLM and Ollama, making it the most common cross-provider interface. Anthropic, Google, and other providers have distinct native message and tool structures that are not covered individually here.

OpenAI also offers the newer Responses API. It organizes requests around `instructions` and input/output items, supports `developer` instructions, and can manage conversation state through `previous_response_id` or Conversations. To keep the examples directly portable to open-source models and compatible serving systems, however, this chapter continues to use Chat Completions; system, user, assistant, and tool are the four roles most commonly supported across implementations.
...
This request contains only two messages: one system message containing rules written by the developer and one user message containing the user's input. The model returns an assistant message as the reply. In the basic Chat Completions pattern used in this chapter, **each request is constructed statelessly, so the client must supply the information the model needs in the message list**. This is not a universal limitation of every API: Responses can instead refer to a previous response or a persistent Conversation.
...
> 4. **Advantages of Streaming Responses**: In this local experiment, the selected Qwen model and template emit `<think>` tokens, so developers can observe the exposed reasoning text, tool-call decisions, and result processing in real time. This is experimental observability for a local model, not a universal property of hosted APIs; production systems generally should not expose such raw text to end users.
...
>
> The multi-round tool calling in this project follows the ReAct (Think-Act-Observe) loop introduced in Chapter 1, so its principles will not be repeated here. The previous section already showed the complete message structure of this process using OpenAI-compatible Chat Completions JSON. In a local deployment, the server (e.g., vLLM or Ollama) converts these API messages into the model's internal token format. The `local_llm_serving` project lets readers inspect the model's raw input and output token stream, including the following details that are normally hidden at the API level:
>
> **Reasoning Tokens Exposed by the Model Template**: In this experiment, Qwen3 emits analysis inside `<think>` tags before generating tool calls—for example, evaluating suitable tools and planning their order. These tokens can help study the generation behavior of this particular local model, but they should not be assumed to be a complete, faithful explanation of its decisions or used as security-audit evidence.
>
...
**First, it explains why standard API formats must be used.** If a developer bypasses the API and manually concatenates messages—for example, passing tool results as ordinary user messages rather than tool messages—the Chat Template may represent the conversation incorrectly. With Qwen3's Chat Template, a tool-call round may retain content inside `<think>` tags, while a newly detected user turn may clear it according to the template's rules. Incorrectly marking a tool result as a user message can therefore trigger a reset at the wrong time and weaken multi-step continuity.

Rules for replaying reasoning state are not uniform across models. DeepSeek R1's official protocol requires clients to strip `reasoning_content` in later turns; other DeepSeek, Kimi, and GLM models or versions require the returned `reasoning_content` to be placed back in the corresponding assistant message verbatim. Claude's native API uses signed thinking blocks and defines when they must be replayed. OpenAI's Responses API does not expose raw chain-of-thought, but it may return opaque or encrypted reasoning items that must be preserved, plus optional summaries; in stateless continuation, relevant output items should be sent back together. The general lesson is not that "visible thinking must be retained," but that **reasoning-related state is part of the provider contract, and whether and how to replay it must follow the current contract for that model and API**. Do not fabricate, parse, or expose opaque state, and do not treat readable reasoning text as a faithful explanation.
