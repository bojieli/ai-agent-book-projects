...
Evaluating a trajectory means answering three questions in order: **Did it get done? Was it done in a permitted way? Was it done well?** The evidence behind each question is softer than the one before, and that is what determines who should answer it — Figure 9-2 organizes them into a three-layer verification structure.

![Figure 9-2 Three-layer trajectory verification from environmental outcomes to an LLM Rubric](images/fig9-2.svg)

**The outcome verifier answers “Did it get done?”** It reads test results, database state, and tool returns: a Coding Agent can run tests, type checks, and performance benchmarks; an Agent processing a refund for a user can query the order status and actual refund amount. Such signals come from real environmental states and are generally more reliable than the model’s descriptions of its own behavior, which makes this the layer worth fighting hardest for.

**The process verifier answers “Was it done in a permitted way?”** A correct outcome does not imply a correct process: deleting failing test cases can also make tests pass, and telling a user, “We will issue your refund within seven days; please be patient,” may buy temporary satisfaction. This layer checks business rules, permissions, and action sequences, separating “it looks done” from “it was done by the rules.” Policy stores, permission tables, and action traces are all determinate, so this layer can likewise be judged by code.

**The quality verifier answers “Was it done well?”** Whether customer service was patient, whether it offered compliant alternatives, whether a research report identified the key evidence, whether generated text is natural and concise — none of these has a single correct answer, and all require contextual judgment. This layer can use the LLM-as-a-Judge introduced in Chapter 7, but the judge should not merely assign a vague overall score: define a Rubric in advance and require item-by-item scoring that cites evidence from the trajectory.

The three layers are not a choice of one. A single trajectory passes through all three in turn, and failing any one of them is a diagnosable failure. The point of layering is to shrink the language model’s share of the judgment: the lower the layer, the more it should rely on code and environmental ground truth, and only what resists formalization goes to the language model.

Take a customer-service Agent. Table 9-1 expands the three layers into seven dimensions that can be scored one by one: task outcome belongs to the outcome layer; rule compliance, privacy boundaries, and promise-action consistency belong to the process layer; expression quality and compliant workarounds belong to the quality layer; factual reliability straddles the two, in that whatever can be matched against tool returns is checked by code and the rest goes to the Rubric. This decomposition is more diagnostically useful than “was the user satisfied”: a user may be satisfied because the Agent issued a non-compliant refund, or dissatisfied because of a compliance limit, and a single satisfaction score cannot tell the two apart.
...
**The shape of the verifier’s output is what makes it a learning signal.** An overall score can say how well this run went, but not what to change. An evaluation that later stages can learn from needs at least four things: whether the task succeeded, partially succeeded, or failed; a separate verdict for each dimension; the evidence location behind each verdict (which conversation turn, which tool call); and a failure-type label. The verifier must also be allowed to decline to score when evidence is insufficient — a low-confidence case is better left out of the learning set than hardened into a fact. With those four in hand, the four update methods in the next section can tell whether to update knowledge, the Prompt, a program, or model parameters.
