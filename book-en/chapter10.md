...
To be clear, both architectures are genuine multi-agent systems because the system prompt and tool set differ at each stage, making them different Agents. The difference lies in the coordination method. **Shared context** relies on implicit coordination: subsequent Agents inherit preceding Agents' visible messages and tool records, along with any provider-returned state that the protocol requires them to continue, and receive information through the context itself. This does not mean they can see raw chain-of-thought that the API never exposed. **Non-shared context** relies on explicit coordination: Agents exchange information through files, messages, or structured data interfaces, and each Agent sees only the content relevant to its own work.
...
| Information isolation | Not needed (all roles share the available trajectory) | Needed (e.g., security review should not inherit prior free-form analysis or sensitive intermediate output) |
...
**Getting status via the shared file system.** The most thorough form is **trajectory persistence**: as it executes, the sub-agent serializes each trajectory event to JSON and appends it to a filesystem log file—usually one file per session, one event per line, i.e., JSONL. The trajectory, defined in Chapter 1, contains user messages, visible model output, tool calls and results, and any provider-returned state that the protocol requires the client to preserve. The main Agent needs no status-reporting protocol; by reading this file directly, it can inspect the sub-agent's observable execution record: its latest tool call, the result returned, and whether it is stuck in a loop of repeated failed retries. It cannot see raw chain-of-thought that the API did not expose. A trajectory offers fine observation granularity, but it is not another process's complete memory.

Such exhaustive detail is also a burden. A trajectory can easily run to tens of thousands of tokens, and the main Agent must distill it after reading, consuming both time and tokens. In most scenarios, an **agreed-upon progress file** is more practical: when starting the sub-agent, the main Agent instructs it to update `progress.md` as it completes each item. The main Agent can read this lightweight file at any time to gauge progress. This resembles two processes reserving a small agreed-format area in shared storage, exposing distilled progress rather than complete runtime state.
...
The value of trajectory persistence goes well beyond monitoring. Recall Chapter 1's conclusion that "an Agent's context = static prefix + trajectory." Reloading a trajectory and prepending the static prefix can reconstruct the Agent's **conversation and model state**, which is an important basis for session resume; if the provider contract requires reasoning state to be replayed, that state must be persisted as well. But a trajectory is not a complete runtime checkpoint. Files and database contents, browser and tool sessions, locks and leases, credentials and authorization state, in-flight requests, and external side effects such as emails, payments, and orders all live outside the trajectory; external services may also be nondeterministic.

Reliable recovery therefore places the trajectory inside a larger protocol: persist operation records and idempotency keys for side-effecting actions, checkpoint external resources, and on restart reconcile whether each operation never started, is in flight, completed, or has an unknown outcome before choosing to resume, retry, or compensate. Blindly replaying a transcript can send an email, make a payment, or place an order twice. The write-ahead log (WAL) analogy remains useful, but the trajectory is only one log in the recovery protocol; it must be combined with external-state checkpoints and durable operation records rather than replayed as if it alone guaranteed safe recovery. This is what gives a multi-agent system an engineering foundation for recovery, audit, and handoff.
...
These patterns rely on two topology-independent components inspired by operating systems. An Agent relates to its runtime as a process relates to a kernel: the static prefix is the program, the trajectory carries conversation and model state, and the LLM is a time-shared CPU. The data plane is a shared file system, represented as a virtual directory tree with four types of mounted areas: Agent-specific workspaces, multi-agent shared workspaces, external resources, and built-in system resources. Agents exchange artifacts by passing file paths.

The control plane handles messaging, status queries, termination, and resource scheduling. Agents can report status asynchronously through messages, or a parent Agent can observe files that a sub-agent updates in real time—either a trajectory or an agreed-upon progress file. A trajectory can reconstruct conversation and model state, but it is not a complete runtime checkpoint; crash recovery also requires durable operation records, idempotency keys, external-state checkpoints, and post-restart reconciliation so that replay does not duplicate side effects. A message bus commonly implements the control plane for real-time, asynchronous, multi-party coordination. Collaboration across organizations also requires a standardized interoperability protocol such as A2A.
