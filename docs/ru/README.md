...
Перед запуском эксперимента, который обращается к модели, скопируйте `.env.example` в `.env` и укажите хотя бы один ключ провайдера. Пользователи локального Ollama могут запустить Ollama и добавить к команде `--provider ollama`.
