...
| 10-4 | External concurrent-agent reproduction | **Blocked by external authentication; incomplete.** The pinned TalkAct source and six runtime file hashes match and Python 3.12 is available, but both fast/slow tiers construct official Anthropic clients and the configured credential returned HTTP 401 `authentication_error: API key is invalid.` The required 16-episode duplex/strawman campaign has zero attempted episodes. See the [credential-free preflight](../chapter10/talkact-reproduction/validation/exp10-4-anthropic-auth-20260803-v1/preflight.json). |
