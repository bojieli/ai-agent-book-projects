...
| 9-6 | External reference implementation | **Blocked by external authentication; incomplete.** The official quickstart source and Dockerfile hash match the pin and Docker is available, but a 2026-08-03 request to Anthropic's official Messages endpoint using the Demo's default model returned HTTP 401 `authentication_error: API key is invalid.` No container or native tool execution is claimed. See the [credential-free preflight](../chapter9/claude-computer-use-native/validation/exp9-6-anthropic-auth-20260803-v1/preflight.json). The completed open-model 9-7 arm does not close this provider-specific arm. |
