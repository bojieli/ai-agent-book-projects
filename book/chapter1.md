...
**事件触发工具**与前三类在调用方式上有本质的区别——它们不是 Agent 主动调用的，而是作为外部输入来驱动 Agent 开始执行任务。比如收到一封新邮件、到了某个预定时间点、或另一个系统发出了 Webhook 回调，这些事件会激活 Agent，让它开始后续的思考和行动。虽然事件触发不是 Agent 主动调用的，但它是 Environment 向 Agent 提供观察的通道之一，因此本书将事件适配器归入广义的工具体系；邮件、时间变化和 Webhook 的来源仍属于 Environment。
...
| 上下文扩展（知识持久化） | 第三章（用户记忆和知识库） | 用户记忆、RAG、结构化索引、智能体化 RAG | 敏感信息暴露、隐私保护 |
| 工具设计与安全约束 | 第四章（工具设计） | 工具分类、权限控制、MCP 标准、主动工具发现 | 误操作、未授权访问、不可逆操作 |
| 工具的验证与纠正 | 第五章（Coding Agent 与通用 Agent） | Coding Agent 的 Harness、测试驱动、代码化规则 | 身份冒用、责任归属 |
| 观察与动作空间的扩展 | 第六章（交互） | 事件驱动异步架构、语音 Agent、Computer Use、机器人操作 | 多模态输入的安全过滤、异步打断与实时交互中的权限控制 |
| 系统级验证 | 第七章（评估） | 评估环境、数据集、自动化评估、可观测性 | — |
| 模型层面的纠正 | 第八章（后训练） | SFT（监督微调）、强化学习——将 Harness 中积累的反馈信号写入模型参数，可看作 Harness 工程的延伸 | 目标偏离、对齐与鲁棒性 |
| 经验驱动的持续纠正 | 第九章（持续进化） | 轨迹学习信号、知识/指令/程序/参数更新、自我修改、验证与回滚 | 记忆污染、不安全的自我修改、能力漂移 |
