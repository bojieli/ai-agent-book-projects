...
**护栏**（Guardrails）构成了保障 Agent 行为安全可控的分层防线，可用于管理数据隐私风险（例如防止系统提示泄露）或声誉风险（例如确保模型行为与品牌形象一致）。实践中可以先针对已识别的风险设置护栏，再在发现新漏洞时逐步添加。本节只做高层次的概览；具体的实现细节将在第二章（提示注入防护）、第四章（工具权限控制）和第五章（代码执行安全）分别展开，初次阅读无需深究。
