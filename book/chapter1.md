...
- **模型回复**（Assistant Messages）：模型之前返回的可见输出通常包括文本内容（`content`，即对用户的回复）和工具调用请求（`tool_calls`，即 Agent 采取行动的方式），有些 API 还会返回供应商特定的思考状态。开源或本地模型可能通过 `<think>`、`reasoning_content` 等形式暴露思考 token；另一些 API 只返回需要原样回传的不透明、带签名或加密状态，或提供可选的思考摘要。OpenAI 托管的思考模型不提供原始内部思维链；Responses API 可能返回不透明或加密的 reasoning item 以及可选摘要。框架应按对应模型的协议保存和回传这些状态，但不能把原始思维链假定为一个通用、可读的 `reasoning` 字段，也不能把它当作忠实的解释或审计证据。
...
要验证各个组件在特定系统中的作用，最直接的方法是**消融实验**（Ablation Study）：就像医生诊断时逐一排除病因——先去掉 A 组件看系统是否还正常，再去掉 B 组件，以此类推，从而判断每个组件的贡献。实验 1-1 正是按这个思路对上述五个组件做了系统性测试，结果表明：去掉工具定义，Agent 完全丧失行动能力；缺少工具执行结果时，由于看不到上一步的反馈，Agent 会反复调用同一个工具，陷入无限循环；在本实验所用模型和 API 中，丢弃接口返回且协议要求回传的思考状态后，前后决策开始互相矛盾；至于历史消息，没有它 Agent 等于失忆，于是从头开始整个任务流程，重复执行已完成的步骤。这里的消融结论描述的是这个模型与接口的行为，不能外推为“所有 Agent 都必须暴露或回传可读思维链”。
...
实验结果揭示了各个上下文组件在这套实验配置中的作用。**工具定义**（Tool Definitions，静态前缀的一部分）是 Agent 行动能力的基础，没有它，Agent 就无法识别和调用任何工具。**工具执行结果**（Tool Results）是闭环控制的关键，缺失它会导致 Agent“盲目”执行，陷入无限循环。**供应商返回的思考状态**在本实验使用的模型协议中需要随 assistant 消息保留；移除它会破坏多步决策的连贯性，但这不意味着该状态必然可读，更不意味着它等于模型做出决策的完整原因。**历史消息**（之前轮次的用户消息、模型回复和工具执行结果）则防止了冗余操作，保持任务执行的连贯性，避免重复犯同样的错误。
...
轨迹 = [
  ...
  {role: “assistant” , 
   reasoning_state: “<按供应商协议保留；内容未展示>” ,
   # 作者摘要：需要将所有货币转换为 USD
   content: “” ,  # 没有直接回复用户
   tool_calls: [
...
这种设计的精妙之处在于**上下文的累积性**。每次 LLM 调用都能看到完整的轨迹，这让它能够理解当前处于任务的哪个阶段、之前尝试了什么、得到了什么结果。就像人类解决问题时会不断回顾和总结，Agent 通过轨迹保持着对整个任务的全局认知。同时，轨迹的结构化特性也让系统具有高度的可调试性：用户消息、可见回复、工具调用和工具执行结果都被清晰地区分开来，可以据此复盘可观察到的行为。这样的轨迹并不等于模型内部推理的忠实解释；若接口另行提供思考摘要，也应把它视为摘要而非原始思维链或审计证据。
