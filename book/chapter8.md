...
Skill 学习遵循同样原则，但作用范围更局部。可以把 Skill 理解为一份按需打开的岗位操作手册：若多条经验共同形成一套完整的保险理赔流程，系统可以生成或修订相应 Skill。候选 Skill 不应只是一次对话摘要，而应至少说明何时加载、前置条件、操作步骤、已知陷阱和验证方法，并保存来源轨迹。系统先在已有 Skill 库中搜索近似能力：存在相同流程时优先做局部 `patch`，只有确实出现新的独立能力时才创建新目录，避免库中堆满名称不同、内容近似的手册。Anthropic 的 Skill Creator[^anthropic-skill-creator] 展示了“起草—测试—评价—修订”的生成循环；真正困难的是哪些运行证据足以触发生成、如何处理冲突，以及修改后能否通过领域任务和旧任务回归。

“AI 味”反馈说明了模糊意见如何变成 Skill 规则。把用户修改前后的文本放在一起比较，可以把“写得自然一点”拆成若干可检查的模式，例如破折号密度过高、模板化的“首先/其次/最后”、口号式开头和空洞比喻。能用正则或统计量判断的规则交给确定性检查；较难判断的规则先在人工标注的金标集上校准，再放入 Skill。规则还要合并重复项、检查阈值冲突，并用未完成任务集和正常文本保留集同时检查检出率与误伤率。实验 8-9 的 `ai-style-skill` 实现了这条增量更新路径；当规则越来越多、上下文成本开始影响遵循率时，同一批 before/after 对还可以继续用于第七章的偏好训练。

> **实验 8-9 ★★：把反馈整理成写作 Skill**
>
...
弯引号案例说明，Skill 不只适用于主观的“AI 味”反馈，也适用于边界复杂、但可以写出作用域和验证条件的文档规范。用户说“中文文章中的直引号应改成弯引号”后，系统不能生成一个全局替换，而应先规定 ASCII 直引号、中文弯引号和 Markdown 反引号的区别，再写清中文自然语言、英文原文、行内代码、代码块、代码注释和 JSON 的优先级。程序负责解析和保护代码边界，Skill 表达需要理解语境的判断，独立验证器检查 Markdown、JSON 和源代码仍然有效。

这条 Skill 还可以成为第七章后训练的数据规范：从带作用域标签的文档蓝图合成正例、保护区反例和歧义样本，经过验证器过滤后做 SFT。训练后不应删除 Skill；它仍然是可读规范、合成数据生成器、模型回归集和规则变更时的重训依据。模型参数内化的是跨任务识别作用域和采取正确动作的习惯，而不是取代规范本身。

特殊字符串的 Bad Case 则走另一条路径。若 `old_string` 在模型直接复述探针中已经发生 byte-level 漂移，可以用第七章的 copy-focused SFT 和 copy replay 恢复逐 token 保真度；若模型输出正确而 tokenizer、序列化或工具接口改变了字符串，应更新 Harness 或工具，而不是创建一个 Skill 掩盖系统错误。这两个案例应分别报告 Skill 遵循率、保护区误伤率和 byte-exact 复制率，不能合成一个“文本质量”总分。

因此，持续进化的完整生命周期是：**反馈 → 首个错误归因 → Skill 或接口规范 → 合成数据 → 参数候选 → 边界集与保留集验证 → 灰度发布或回滚**。Skill 和参数更新不是相互替代的两个终点，而是同一条可追溯链路中的规范层与执行层。
...
>
> **验收标准**：所有检查通过后只生成 `release_to_canary`；任一静态检查、失败重放或旧任务回归失败，都返回 `reject_candidate`。`release_manifest.json` 必须记录同一类失败、来源轨迹、问题原因、目标组件与文件、代码 diff、预期修复、潜在回退、检查结果、候选版本和回滚版本；拒绝的候选也要保留其失败原因，供下一轮生成时查阅。生成补丁的 Agent 不能修改稳定代码、验证器、审计日志或批准自身发布的门槛。
>
> 配套实现见 [`self-modifying-agent`](../chapter8/self-modifying-agent/)，可选择确定性候选生成器或真实 LLM Coding Agent，两条路径共用同一发布门槛。

实验 8-8 是同一协议在验证层的应用。多条用户纠正、点踩和事后审计都指向“高风险操作未经确认”时，系统才创建修改请求，并把候选写入隔离目录。风险分类器按工具名和参数识别删除工作区外路径、`git push --force`、无 `WHERE` 的数据库删除等调用；一次性确认令牌绑定具体操作和参数，用过即失效。候选必须通过 AST/静态检查、边界集回放（包括伪造令牌和令牌复用）以及保留集回放，才允许灰度发布；“门禁放行一切”的候选应被拒绝。配套实现见 [`harness-safety-gate`](../chapter8/harness-safety-gate/)。
...
参数学习通常与外部方法协同。医疗影像模型用参数学习视觉表征，用知识库提供最新指南，用代码测量病灶和计算风险；自然客服语气可通过偏好训练塑造整体分布，再用 Prompt 规定当前品牌身份，用用户记忆适配个人沟通偏好。对于特殊字符串复制，模型参数可以学习逐 token 保真度，但工具接口仍应提供字节级校验和可回滚的失败诊断。持续进化不是在四种方式中选出唯一答案，而是把每种能力放到最适合表达和治理它的位置。
