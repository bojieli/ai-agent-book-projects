...
Andrej Karpathy 将这种做法称为**系统提示学习**（System Prompt Learning）[^karpathy-system-prompt-learning]：模型遇到问题后，用一句清楚的话提醒未来的自己。DSPy[^dspy-2023] 在开发集上搜索指令和示例；OPRO[^opro-2023] 根据历史提示词及其得分提出候选；GEPA[^gepa-2025] 从失败轨迹的自然语言反思中生成并筛选候选提示。这些方法适合离线批量优化；生产环境更适合使用可审计的最小补丁，并保留快速回滚路径。

系统提示学习与第二章的提示工程不是一回事。第二章讨论怎样组织一份好 Prompt；本节讨论什么反馈足以触发修改，以及候选修改怎样安全发布。修改应是带来源的最小 diff，而不是让模型每次都重写整份 Prompt。候选版本必须同时在**触发失败的边界集**和**正常工作的保留集**上测试，前者要改善，后者不能退化。
...
>
> **实验说明**：从失败轨迹中提取规则遵从、任务解决和合规变通三个维度，生成一条带来源的最小 Prompt 补丁，再与初始版本、人工调优版本做同条件对照。候选只有在边界案例改善、旧任务不退化并通过发布门槛后，才进入灰度阶段。
>
> **实验说明了什么**：Prompt 自动优化的重点不是让模型自由改写一大段文字，而是把可归因的失败转成作用域明确、可回滚、可验证的局部规则。

#### 例子二：Skill 学习——以“去 AI 味”写作为例

Skill 是按需加载的 `SKILL.md`，可以把它理解成**一份员工手册**，而不是一次对话的摘要（见[第二章的 Skills 机制](chapter2.md#动态提示词与-agent-skills)）。

“去 AI 味” 说明了为什么需要 Skill。用户说“写得自然一点”并不能直接执行。也不能像 “九十九条军规” 一样堆砌大量规则。要把规则组织成**少数有优先级、有例外、有验证方式的判断路径**。

Skill 稳定后还可以继续做**内化训练**。沿用[第七章的 SFT 数据合成流程](chapter7.md#sft-数据合成从示范到可训练轨迹)，把 Skill 的判断路径渲染成正例、保护区反例和歧义样本，经验证器过滤后用于 SFT、偏好训练或蒸馏。训练让模型在新任务中更习惯先判断作用域再行动；Skill 仍应保留，作为可读规范、数据生成依据和回归标准。

> **实验 8-4 ★★：把“去 AI 味”反馈整理成写作 Skill**
>
> **实验目的**：检验系统能否把“写得自然一点”这类模糊反馈，转化为具体、可执行且不会无限膨胀的写作 Skill。
>
> **实验说明**：从用户修改前后的文本对中提炼候选规则，合并重复项，并同时用问题文本和正常文本检查检出与误伤。规则不写成大量禁用词，而组织为带作用域、例外和验证方式的判断路径。
>
> **实验说明了什么**：Skill 学习的核心不是积累“九十九条军规”，而是形成一份可操作的员工手册。稳定的 Skill 还可以成为合成训练数据和后训练的规范，使模型逐步内化这些判断习惯。
...
>
> **实验说明**：把一次成功轨迹编译成带参数和状态检查的候选工作流，与“每次都重新探索”的基线比较；再改变页面或制造假成功，观察候选是否失效并回退到完整 Agent。
>
...
>
> **实验说明**：比较“只在 Prompt 中提醒不要重试”和“修改程序中的重试策略”两种修复。候选修改必须经过失败轨迹重放、正常故障回归和安全发布门槛，且不能修改验证器或稳定版本。
>
> **实验说明了什么**：能确定执行的约束应该进入程序，而不是继续堆在 Prompt 里。Agent 可以提出代码候选，但“是否能发布”必须由模型外的测试、审计和回滚机制决定。
>

验证层也可以采用同一协议：当多条用户纠正、点踩或事后审计都指向“高风险操作未经确认”时，系统生成一个确认门禁候选。候选必须在边界任务和正常任务上都通过验证，且安全门本身不能被候选修改。

> **实验 8-7 ★★：由用户反馈触发高风险操作确认门禁**
>
> **实验目的**：检验系统能否从用户纠正和事后审计中发现安全流程缺口，并为高风险工具调用生成确认门禁。
>
> **实验说明**：用危险操作边界集和正常操作保留集共同评价候选门禁。候选既要拦住未经确认的高风险调用，也不能阻断正常任务；生成候选的 Agent 无权修改安全测试和批准规则。
>
> **实验说明了什么**：安全能力的进化不能由修改者自证成功。真实模型生成的候选可能被安全门拒绝，这正说明独立验证器和不可修改的可信根比“候选看起来合理”更重要。
...
>
> **实验说明了什么**：自我修改不是“模型读完资料后直接改代码”，而是“理解原则 → 提出候选 → 接受外部检验 → 根据反馈修正”的闭环。一次候选被接受，只能证明更新流程成立，不能自动证明下游能力已经提升。
