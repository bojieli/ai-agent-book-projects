...
>
> **实验目标**：不再使用专门为实验编写的参考 Agent，而是把本章闭环施加到真实 Agent 代码库。克隆固定版本的 Hermes，让它检查本书十章英文正文，并审计一位读者提出的四项缺口：产品级消融基础设施、第二章所述的模型可见 Agent Status Bar、持久记忆的遗忘与整合，以及带独立执行证据的通用 Proposer–Reviewer 产物审查闭环。四项主张由任务 Prompt 提供，因此实验测量的是审计、候选生成和纠错能力，而不是“Hermes 自主发现了这四个问题”。
>
> **真实运行**：2026 年 8 月 2 日，commit `85c8956e` 的 Hermes 通过 OpenRouter 请求 `openai/gpt-5.6-luna`。它把产品级消融基础设施判为缺失，把另外三项判为部分存在，并有意推迟范围过大的机制，只生成一个边界明确的候选：可选的、模型可见的 `<agent_status>`，包含 API 调用预算和当前 TODO。
>
> **完成 Reviewer 闭环**：候选由无 proposer 会话上下文的 fresh Hermes reviewer 审查。每次 `VERDICT: REJECT` 都作为新事件返回原 Hermes 会话，让运行中的 Hermes 修改自己的 checkout，再由新的 reviewer 重审。五次终局拒绝进一步发现 tool-loop 放置、未界定 TODO、`content=None`、数据库重启丢失、重试重复追加，以及与既有 memory/plugin sidecar 冲突等缺陷；第六次 fresh review 最终返回 `VERDICT: ACCEPT`。最终版本在真实字符串持久化边界上 fail closed，状态位于最新工具证据之后，经数据库关闭/重开仍保持字节稳定，并能幂等地处理同一请求重试。它通过 **8 个新增行为测试与 36 个既有回归测试**，同时通过编译、空白检查、凭据扫描和干净 clone 上的补丁可应用性检查。
>
> **结果边界**：这个运行完整证明了“阅读书本 → 审计自身 → 修改自身 → 独立拒绝 → 根据反馈继续修改 → 独立接受”的闭环，而不是留下一个审查失败的半成品。最终补丁仍未合入 Hermes 上游，报告所设计的下游消融 campaign 也没有执行，因此不能进一步宣称状态栏提高了任务成功率。自我更新是否完成与更新是否带来因果收益是两个不同命题。可复现 runner、九轮 proposer 交互、六次 acceptance review、报告、最终补丁与去凭据 manifest 均保存在 [`hermes-self-evolution`](../chapter8/hermes-self-evolution/)。
