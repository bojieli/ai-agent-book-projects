...
持续进化需要来自可追溯的运行经验、能够改变后续行为，并经过验证没有造成明显退化。本章首先讨论如何判断一次运行究竟好在哪里、错在哪里；然后比较四种更新方法及其适用边界；接下来讨论这些更新如何在长期运行中被验证、发布、修订与淘汰；最后以两个从用户反馈出发的端到端案例（实验 8-8、8-9）把整条链路走通。
...
Skill 学习遵循同样原则，但作用范围更局部。可以把 Skill 理解为一份按需打开的岗位操作手册：若多条经验共同形成一套完整的保险理赔流程，系统可以生成或修订相应 Skill。候选 Skill 不应只是一次对话摘要，而应至少说明何时加载、前置条件、操作步骤、已知陷阱和验证方法，并保存来源轨迹。系统先在已有 Skill 库中搜索近似能力：存在相同流程时优先做局部 `patch`，只有确实出现新的独立能力时才创建新目录，避免库中堆满名称不同、内容近似的手册。Anthropic 的 Skill Creator[^anthropic-skill-creator] 展示了“起草—测试—评价—修订”的生成循环；它解决了怎样制作和改进 Skill，真正困难的仍是哪些运行证据足以触发生成、如何处理冲突，以及修改后能否通过领域任务和旧任务回归。本章末尾的实验 8-9 用一个写作 Skill 的完整案例回答这几个问题。
...
可修改的对象远不止新工具。操作层可以把浏览器轨迹编译为参数化工作流，或为变化的 API 生成适配器；控制层可以修改工具路由、重试、熔断和上下文压缩策略；验证层可以根据生产失败新增参数检查、状态验证器和回归测试；架构层则可以增加 Reviewer Agent，改变规划与执行之间的信息流。本章末尾的实验 8-8 给出验证层更新的一个完整案例。
...
前面四种更新方式各配了实验，但大多从管道的中段讲起——失败轨迹已经摆在那里，只欠候选修改。生产系统的真实起点更靠前：一条用户纠正、一个点踩、一次事后审计发现的异常。本节用两个端到端案例把第六章的评估资产、瓶颈判断和本章的更新协议串起来：第一个案例把用户反馈变成 Harness 代码的自动更新，第二个案例把模糊的主观反馈逐步提炼成写作 Skill 的具体规则。两个案例也对应第六章分流判断的一侧——瓶颈在 Harness 时更新代码与指令；瓶颈在模型参数时，同样的反馈数据走向第七章的训练链路（实验 7-17）。

### 案例一：高风险操作的确认门禁（自动更新 Harness）

**信号与归因。** 一个 Coding Agent 在一周内积累了多条指向同一模式的负面反馈：用户纠正（“谁让你直接删的？那份报告我还没定稿”）、两条针对未确认删除操作的点踩、事后审计发现的一次未经确认的 `git push --force`（覆盖了同事的提交）。第六章所说的三类 bad case 信号在这里齐备，按错误分类属于“流程与规范缺失”。聚合模块要等同一模式在多条轨迹中达到支持门槛才创建修改请求——单条偶发反馈只配继续观察。

**瓶颈判断与更新位置。** 先做模型替换实验确认瓶颈位置：换更强的模型，这类错误的发生率下降但依然出现——提示词里写了“删除前请确认”，模型在长任务中照样跳过。这与第六章实验 6-5 的发现一致：三种上下文表示都未能通过“高风险删除前确认”的边界用例。获得一条信息和正确将它用于当前决策是两种能力，而“高风险操作前必须确认”恰好是可以确定性执行的硬约束——它不应依赖模型每次都想起来，应由 Harness 保证。更新位置因此定为验证层：在工具调度器前增加确认门禁。

**候选与验证。** Coding Agent 读取失败归因、必须保留的既有行为和稳定源码，生成候选：一个风险分类器（按工具名与参数模式识别删除工作区外路径、`force` 推送、无 `WHERE` 的 `DROP`/`DELETE` 等高风险调用），外加一次性确认令牌——令牌绑定具体操作与参数、用过即作废，防止一次确认被挪用到其他操作。候选只能写入隔离目录，发布决定由模型外的门槛做出：AST 静态检查、边界集回放（高风险调用必须被挂起、确认后才执行，含伪造令牌与令牌复用的对抗用例）、保留集回放（读写文件等正常操作不受影响、已确认操作正常放行）。全部通过才得到 `release_to_canary`，任何一项失败都返回 `reject_candidate`，并把失败原因留给下一轮候选生成。

> **实验 8-8 ★★：由用户反馈触发的高风险操作确认门禁**
>
> **实验目标**：从三类用户反馈信号出发，检验系统能否把“高风险操作未经确认”这一反复出现的故障模式，转化为对 Harness 验证层的最小、可审计、可回滚的自动更新。
>
> **流程**：`failure_trajectories.json` 提供 11 条轨迹（含用户纠正、点踩、事后审计三类信号，以及低于支持门槛的单条故障和已确认操作的对照轨迹）。诊断模块按故障模式跨轨迹聚合，达到支持门槛后归因到 `stable/tool_dispatcher.py`；确定性候选生成器（或真实 LLM Coding Agent）产出确认门禁模块与最小接入 diff，只写入隔离目录。模型外门槛依次执行静态编译、AST 安全扫描、门禁契约检查、边界集回放（8 条，含第六章实验 6-5 的“高风险删除前确认”场景及伪造令牌、令牌复用等对抗用例）、保留集回放（7 条，正常操作与已确认操作不受影响）。
>
> **对照与指标**：一个“门禁放行一切”的退化候选作为负对照，必须在边界集回放中被拒绝；记录三个失败簇的支持度、各项检查结果、补丁大小与发布决定。离线路径下确定性候选通过全部门槛并获得 `release_to_canary`，负对照按预期被 `reject_candidate`；stable 目录的哈希在生成前后不变。
>
> 配套实现见 [`harness-safety-gate`](../chapter8/harness-safety-gate/)。与实验 8-5 的分工：8-5 改控制层（重试与熔断）、信号来自系统内部错误；本实验改验证层（安全确认）、信号来自用户反馈。

### 案例二：把“AI 味”内化为写作 Skill

**信号与归因。** 写作类 Agent 收到的最高频负反馈之一是“这段文字 AI 味太重”。这类反馈既真实又棘手：它是模糊的整体感受，不像“删错了文件”那样能定位到某一步动作。直接在 Skill 里写一句“写得自然一点，不要有 AI 味”没有用——这与第六章批评的“回应展示了深刻理解”式抽象 Rubric 是同一个错误：不可检查的标准产生不了可检查的行为。

**规则的具体化。** 转机藏在用户纠正的 before/after 对里。用户把“它不是简单的记录软件——而是你的第二大脑”改成平直的陈述，删掉“首先……其次……最后……总而言之”的连接词，把三个 emoji 减到零。逐条对比这些修改，“AI 味”就从整体感受分解成可枚举的模式：破折号过密、“不是……而是……”句式滥用、三段排比堆砌、“让我们”口号式开头、“首先/其次/最后”模板序列、空洞比喻与假拟人、emoji 泛滥。每条模式被提炼为一条自包含规则：定义、检测方法、坏的例子、好的改写范例、适用场景和来源反馈。检测方法分两层：能确定性判定的用正则或密度统计（如破折号每千字超过阈值）；空洞比喻这类微妙模式交给 LLM judge，但必须先在人工标注的金标集上校准，一致率不达标就不允许上线——这是第六章评判者校准在 Skill 场景的应用。

**防膨胀与防误伤。** 持续追加规则很快会撞上本章前文警告过的“99 条军规”问题：Skill 越来越长，规则相互重叠，模型在长清单前的遵循率反而下降。配套实验把反馈分批进入，演示三个机制：相同检测模式的候选合并去重（21 条候选规则合并为 8 条）、阈值矛盾的冲突检测、长期无新证据或被推翻的规则归档。另一半是防误伤：破折号不是原罪，真正构成对比的“不是……而是……”是好句子。评估因此必须包含保留集——合法使用这些模式的文本不应被检出。配套实验中，排比检测最初用“分句长度相近”作判据，把普通散文误判为排比；收紧为“同开头或同结尾字”之后误伤才清零。这类误伤只能靠保留集发现——这是“改动不能破坏既有行为”在 Skill 层面的版本。

**升级路径。** 表8-2 把“生成风格”划给模型参数，为什么这个案例先用 Skill？因为反馈来得快、规则可审计、修改没有训练成本——Skill 是这类问题最便宜的第一站。但当规则积累到几十条、遵循成功率开始下降、上下文成本超过预算时，同样的 before/after 对就是现成的偏好数据，可以把风格内化进参数——第七章实验 7-17 的 DPO 路径原样适用。持续进化不是在四种载体中做一次选择，而是让每种能力停在当下最适合它的位置。

> **实验 8-9 ★★：把“AI 味”反馈内化为写作 Skill**
>
> **实验目标**：演示“模糊反馈 → 可检查规则 → Skill → 防膨胀与防误伤”的完整循环，并验证增量维护机制能否在规则持续增长时保持检出率与低误伤。
>
> **数据与流程**：`data/feedback_pairs.json` 提供 20 条用户纠正的 before/after 文本对（产品发布稿、公众号文章、邮件等场景，分三批进入）。提炼模块对比每对文本，归纳候选规则（确定性模式库离线可用，真实 LLM 路径带证据回执）；Skill 管理器按检测模式合并去重、检测阈值冲突、归档失效规则，生成 house 风格的 `SKILL.md`（每条规则含定义、检测方法、坏例、好例、作用域与来源）。judge 模块对确定性规则直接执行检测，对 LLM 类规则先在 10 条金标集上校准。
>
> **指标与验收**：`evaluate.py` 报告逐条规则的精确率与召回率、boundary 集检出率（带明显 AI 味的文本应被命中）、retention 集误伤率（合法使用这些模式的好文本不应被命中），以及逐批反馈进入时的规则数增长曲线。离线路径下：boundary 检出 8/8，retention 误伤 0/8，21 条候选合并为 8 条规则。规则数增长明显慢于反馈条数、误伤率为零，才算增量维护机制有效；只看“当前反馈是否被解决”不构成验收。
>
> 配套实现见 [`ai-style-skill`](../chapter8/ai-style-skill/)。真实 LLM 路径（规则提炼、judge 校准、按 Skill 改写）通过 `run_experiment_8_9.py --provider ...` 运行，证据回执存入 `validation/`。

## 本章小结

持续学习正在成为 Agent 最重要的能力之一，但今天的模型还无法自行完成可靠的持续学习。推理时的上下文适应不会自动持久化，未经验证的在线参数更新又会放大噪声、攻击和能力漂移。因此，现阶段更可行的路径，是在模型外围建立可验证的学习系统。

Agent 从与环境的交互和评价中获得学习信号，再根据能力的表示性质更新知识、Prompt、Skill、程序或模型参数。系统也可以进一步优化管理和生成这些产物的方法，但应优先采用可归因、可验证、可回滚的局部修改。章末的两个端到端案例把这条链路完整走了两遍：高风险操作的反复失败经聚合、归因与瓶颈判断，变成 Harness 确认门禁的最小补丁；模糊的“AI 味”反馈经 before/after 对比，被提炼成可检查、可合并、可归档的 Skill 规则。两者的验收都不只看“当前失败是否消失”，而是同时依赖边界集与保留集——修复不能以误伤既有行为为代价。
