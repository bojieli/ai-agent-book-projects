...
>
> **挑战**：这一次，我们没有让 Hermes 解一道专门设计的题，而是把整本书和它自己的代码一起交给它：“先读完，再回头看看自己哪里可以变得更好，而且要真的动手修改。”一位读者提出了四条线索，包括消融实验、状态栏、记忆遗忘和 Reviewer 机制；Hermes 需要自己阅读相关章节，再判断哪些建议适合自己。
>
> **它真的改了自己**：Hermes 读完十章后，对照自己的设计逐项检查，最后选择先补上一个对模型可见的运行状态，让自己在工作时更清楚剩余预算和当前任务。它没有只写一份“未来可以改进”的报告，而是直接修改自己的源码并运行验证。
>
> **被拒绝，就继续学**：新的 Reviewer 检查修改；只要没有通过，意见就会送回原来的 Hermes。Hermes 重新读代码、找到问题、修改并再试。这个过程重复了多次，直到新的 Reviewer 接受为止。整个故事可以缩成一行：**读书 → 对照自己 → 动手修改 → 接受审查 → 根据反馈再改**。
>
> **这个实验说明了什么**：一本书不只可以教人，也可以成为 Agent 的“升级手册”。Hermes 展示了它能理解书中的原则、映射到自己的实现，并在反馈下把修改做完。这里证明的是自我更新闭环确实跑通；新能力是否让所有下游任务都更好，仍要通过单独的消融实验回答。完整运行记录与复现方法见 [`hermes-self-evolution`](../chapter8/hermes-self-evolution/)。
