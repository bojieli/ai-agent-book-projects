...
第一章提出过一个论断：在底层模型固定时，提升 Agent 任务表现最主要的系统工程手段，往往是重新定义或扩展**观察空间**与**动作空间**。第二至五章一直在兑现这句话——上下文工程决定观察里装什么，记忆与知识库把观察延伸到跨会话，工具定义 Agent 能做什么，代码生成则让它自己创造新的动作。

但这些扩展都发生在同一个前提之下：**Agent 与世界轮流发言**。用户说完一句，Agent 想一段、调用几个工具，再回一句；在它思考的这段时间里，世界被默认为静止的。这个前提如此自然，以至于很少被当成一个假设写出来。
...
- **时机**决定观察和动作的**节奏**：观察是 Agent 主动去取，还是世界主动推来；动作是必须在一个回合内做完，还是可以跨越回合、中途被打断、被更紧急的事抢占。
...
这一章的核心命题可以压缩成一句话：**回合制是训练留下的假设，不是环境的性质。**

模型的训练语料几乎全是回合制的——提问后面跟着回答，工具调用后面跟着工具结果，一句话说完了对方才开口。于是模型学到的策略默认世界会等它。但真实环境不等：邮件在它思考时到达，用户在它说到一半时插话，页面在两次截图之间已经变了样，杯子在机械臂伸手的途中被碰倒。**本章的四节，就是这个假设在四个不同时间尺度上被逐一放松的过程。**

先看它们的位置：

| 尺度 | 场景 | 观察侧的变化 | 动作侧的变化 |
|---|---|---|---|
| 秒 — 天 | 异步与事件驱动 | 世界主动唤醒 Agent（邮件、定时器、回调） | 动作跨回合：先发起，后续靠事件收尾 |
| 10 毫秒 — 1 秒 | 语音 | 边说边听，不等一句说完 | 边想边说，可被打断、可中途改口 |
...
**阅读顺序上有一个刻意的安排：本章给语音的篇幅明显多于后两个场景。** 在实时交互这条演进线上，语音是走得最完整、最值得当作参考系的一个：从“串行流水线延迟太高”这个问题出发，经过端到端、全双工、边想边说一系列方案，一直走到今天相对成型的终局，问题→方案→终局的全程都已经跑通。因此我们把它讲透，后面的 Computer Use 和机器人都可以对照语音这条脉络来看——它们各自走到了这条演进线的哪一段、卡在了哪里。

而本章从**异步与事件驱动**开篇，则是因为它离读者最近：模态还是纯文本，变的只有时机。它是从前五章的回合制世界迈出的第一步，也是“回合制假设”这个命题第一次落地的地方。
...
10. ★★★ 本章覆盖了异步事件、语音、Computer Use 和机器人四种交互形态。交互架构可以通过端到端统一、模块化级联或前台交互与后台推理解耦来改进，而不必和智能上限沿同一条路线增长。未来五年的 Agent 应该优先追求更强的统一模型，还是保留可替换的快慢分工？请结合延迟、可观测性、模型迭代速度和任务风险讨论。
