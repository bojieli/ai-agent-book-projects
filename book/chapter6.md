...
在传统的同步接口实现中，**事件在每轮循环的边界被消费**。当 LLM 正在推理、工具正在执行时，新事件先在队列中等待，待本轮到达一个**安全点**（一段推理结束、一次工具返回）再处理。原生 steering 则把接收新输入的时机推进到 response 仍在生成的时候，由服务端在安全的输出边界接续。两者都有边界，只是边界由不同层管理。工具取消仍需执行器响应取消信号，类似 Go 中对 `ctx.Done()` 的检查；接收一条“停止”消息本身不会撤销已经发生的动作。
...
这套方案的作用是维持同步接口要求的工具配对关系。只有需要打断时才引入明确表示“未完成”的占位符；后台真实结果到达后，再通过有来源和任务 ID 的事件送入轨迹。对于已经支持原生异步的接口，应优先保留悬空调用及其原始 `call_id`，无需为了配对而提前返回占位符。
...
### 原生异步：GPT-6 Astra 的两条接口

截至 2026 年 9 月 5 日，OpenAI 已为 GPT-6 Astra（`gpt-6-astra`）公开提供 **Async tool calling** 和 **Mid-turn steering**，分别处理工具结果延迟返回与用户中途更新。开发者现在可以直接使用这两项能力，同时为不支持它们的模型保留前述兼容方案。图6-4 展示两条路线的区别。[^ch6-22][^ch6-23]

![图6-4 同步接口兼容与 Astra 原生异步](images/fig6-4.svg)

**异步工具调用：先发起，模型继续工作，结果随后归还。**

在 Responses API 的 function 或 custom 工具定义上设置 `async: true`，模型发出该调用后，可以继续推理、调用其他工具或回答不依赖结果的部分。应用仍负责实际执行工具；流式客户端可在完整工具调用项到达时启动后台任务。任务完成后，通过后续 Responses 请求归还真实结果，function 工具使用 `function_call_output`，custom 工具使用 `custom_tool_call_output`，两者都必须带上原始 `call_id`。如果期间已有其他续接，`previous_response_id` 应指向最新 response，而非最初发起工具的 response。[^ch6-22]

例如，查询场地报价需要几秒钟，Agent 可以先给出与报价无关的会议准备清单，等工具结果到达后再选择场地。这里有两个独立的并发者：应用执行后台任务，模型继续工作。仅在 Python 中使用 `async def`，或把整次模型生成放进 `background` 模式，都不能代替工具定义上的原生异步语义。应用也可以提供同步的等待工具，让模型在确实依赖结果时等待；等待工具是应用自己定义的，不能替代原始调用对应的结果。

**回合中途 steering：在生成尚未结束时提交用户更新。**

通过同一条 Responses WebSocket 连接，收到 `response.created` 后便可针对其 ID 发送 `response.steer`。例如，模型已经开始制定会议计划时，用户追加“预算改为 1000”和“人数改为 10”。请求只包含三个字段：[^ch6-23]
...
```json
{
  ...
  "input": [
    {"role": "user", "content": "预算改为 1000。"},
    {"role": "user", "content": "人数改为 10，同时保留前一条预算更新。"}
  ]
}
```

`response.steer.accepted` 只表示服务端已接收并排队。通常，服务端在安全的输出边界结束当前 response，再自动创建包含更新的 continuation；被 steering 截断的 response 以 `response.incomplete` 结束，原因为 `steered`。原 response 也可能先正常完成，再产生 continuation。因此要区分**用户的一次任务回合**与 **API 的一个 response**：更新发生在用户任务进行中，但 API 可以用多个 response ID 完成接续。这不是在客户端任意改写同一个 response 的 token 流，也不能据此声称隐藏推理的每个 token 都被原样续写。[^ch6-23]

如果服务端还需要客户端工具结果，更新会保持排队，并可通过 `response.steer.pending` 列出 `required_input`。应用应在同一连接上，用 `response.create` 返回这些结果，不能重复发送已经 accepted 的 steering，也不应重新执行已经完成的工具。没有待补输入时，则继续读取自动 continuation，避免另发一次 `response.create` 与之竞争。[^ch6-23][^ch6-24]

**事件可以统一调度，协议身份仍须保留。** `response.steer` 接受用户消息；工具结果必须走带原始 `call_id` 的结果通路。邮件、定时器和第三方回调由运行时接入，保留来源和信任级别，再按语义路由，不能把所有外部事件都伪装成用户指令。Steering 也不负责取消已启动的工具、撤销已经执行的动作，或保证新指令被立即执行。

这两项原生能力并非所有模型都支持。官方文档将异步工具支持范围列为 GPT-6 Astra 及后续模型，并明确 GPT-5.6 及更早模型不支持 steering；后者还要求受支持的单 Agent Responses WebSocket 请求，绑定 conversation 或启用自动 compaction 等组合不支持。模型选择、接口选择与执行模式需要一起核对。[^ch6-22][^ch6-23]

### 从“能接收异步消息”到“可靠处理异步任务”

原生接口解决了消息时序的表达和接续问题，复杂任务中的可靠性仍需评估。至少要检查三件事：

1. **结果归属与未完成状态**：能否把延迟、乱序到达的结果匹配到正确调用，在结果缺席时避免编造数据？
2. **任务恢复与动作控制**：能否在处理新要求后继续原任务，并区分“修改计划”“取消生成”和“停止后台工具”？
3. **多条更新的综合使用**：能否同时遵守预算与人数等约束，而不是只记住最后一条消息？

针对这些问题，可以用异步环境中的训练和强化学习改进策略，也可以通过任务状态机、来源标记、幂等执行和专项评估提高系统可靠性。训练是进一步改进的途径；公开接口的存在并不披露其具体训练方法，不能反推 Astra 一定采用了某种异步 RL 配方。对开发者而言，应先使用并验证已经具备的模型能力，再判断剩余问题需要改进模型还是运行时。
...
>
>
> 在实验 6-1 的简单事件队列基础上，本实验通过兼容同步接口的运行时实现**并行工具执行、执行取消和状态管理**。Agent 不再只是逐个处理事件，而是需要同时管理多个并发任务，处理打断和恢复，并根据实时状态做出动态决策。原生 Astra 接口的对照见实验 6-14。
>
...
>
> [实验代码与完整记录](../chapter6/astra-async-steering/README.md)通过真实 Responses WebSocket API，对照“运行时兼容异步”之外的原生能力。为保持既有实验链接稳定，新增实验使用编号 6-14。
>
> **任务与对照。** 为一次模拟会议选择场地。初始预算 2000、人数 20；本地查询工具延迟 8 秒返回三个场地及随机回执。场地 A 的价格/容量为 1800/30，B 为 900/12，C 为 600/8，因此初始条件应选 A。Steering 同时把预算改为 1000、人数改为 10，正确选择随之变为 B。随机回执只在工具完成时产生，最终答案必须原样引用，以检查模型确实使用了到达的结果。
>
> 五组各重复三次：Astra 同步工具；同一工具设置 `async: true`；Astra 已发出未完成的 reasoning 项时发送 steering；异步工具仍在运行时发送 steering；以及用 `gpt-5.6-sol` 检查不支持的接口是否明确拒绝。前两组保持模型、提示词和任务相同，仅改变工具的异步标记。用户更新的内容和注入时机由脚本控制，模型生成与 WebSocket 通信均为真实调用。
>
> **2026-09-05 实测结果：** 同步组 0/3 在工具完成前输出准备清单，异步组 3/3 做到了；推理中 steering 的 3/3 都产生 `incomplete(reason=steered)`，随后自动续接并采用两条新约束；工具挂起时 steering 的 3/3 都保留原始调用，只执行一次查询，在自动续接后收到真实结果并选中 B。`gpt-5.6-sol` 的 3/3 均明确返回 `steering_not_supported`。正式运行共 15/15 达到各自判据，其中 3 次属于预期拒绝的负对照。
>
> **如何解释。** 本实验验证了工具与模型工作重叠、运行中的用户更新，以及结果的正确归属。同步/异步两组的端到端耗时中位数分别为 12.58/12.68 秒，小样本未显示最终完成更快；异步带来的直接改善是等待期间已有独立工作产出。工具挂起组的原 response 在三次运行中都正常完成后才自动续接，说明 steering 不必总以 `incomplete` 结束。工具真实结果在本实验中通过后续 `response.create` 交付，不能把这些记录说成向同一个 response 的隐藏推理 token 流任意插入数据。完整事件、实际 model ID、验收结果和源码哈希均已保存，可离线复核。

异步与事件驱动让世界能够在任务进行中唤醒 Agent，原生 steering 又让用户不必等完整回答结束才能提交更新。接下来三节进一步压缩时间尺度：当环境变化的速度达到甚至超过模型的生成速度时，能接收更新还不够，系统还必须及时作出反应。
...
[^ch6-22]: OpenAI, “[Async tool calling](https://developers.openai.com/api/docs/guides/async-tool-calling)”；“[Using GPT-6 Astra](https://developers.openai.com/api/docs/guides/latest-model)”，核对日期：2026-09-05。异步适用于应用执行的 function/custom 工具，不适用于托管内置工具；应用仍负责后台作业和原始 `call_id` 的对应关系。
[^ch6-23]: OpenAI, “[Mid-turn steering](https://developers.openai.com/api/docs/guides/steering)”；“[Responses WebSocket events — response.steer](https://developers.openai.com/api/reference/resources/responses/websocket-events#response.steer)”，核对日期：2026-09-05。`accepted` 表示排队，不能单独证明更新已作用于后续答案；支持范围与请求组合以官方文档为准。
[^ch6-24]: OpenAI, “[WebSocket mode](https://developers.openai.com/api/docs/guides/websocket-mode)”，核对日期：2026-09-05。使用同一连接和 `previous_response_id` 接续，并按协议处理工具结果及断线恢复。
