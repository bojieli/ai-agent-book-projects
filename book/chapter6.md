...
>
> 对比同步工具、原生异步工具和回合中途引导三种交互方式，观察等待是否阻塞其他工作、新要求是否进入后续计划，以及结果到达后能否继续原任务。再选择一个不支持原生能力的模型作为对照，理解模型能力与运行时编排各自解决什么问题。[配套实验](../chapter6/astra-async-steering/README.md)提供实现与复现方法。
