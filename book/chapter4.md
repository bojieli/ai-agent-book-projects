...
本章首先给出五类工具的分类总览；然后讨论适用于所有工具的通用设计原则，以及 MCP 协议如何统一工具生态；再回答一个横跨所有工具的共同问题：当工具多到成百上千，一次该让模型看见多少；最后逐类深入 Agent 主动调用的三类工具——感知、执行、协作。其中“一次看见多少”与开头的“能力做成什么形态”是两个彼此独立的决策——形态定每条能力的常驻成本与传参方式，披露定同时有多少条摆在模型面前。两者在书中只隔一节 MCP 协议，因为披露一节要拿协议作铺垫。至于由外部事件驱动的另外两类工具（事件触发与用户沟通），它们的设计与事件驱动的异步运行时密不可分，留到第六章与实时交互一并讨论。
...
**本节问的是形态，不是数量。** 一项能力做成专用工具还是 Skill，与“一次让模型看见多少条能力”是两个独立的决策，四种组合都真实存在：一个挂着几百个专用工具的 MCP 后端可以只暴露一份索引、按需加载，也可以把全部 schema 一次性注入；二十来个 skill 的目录可以全量常驻上下文，成百上千个 skill 同样需要分层检索。形态决定的是**每条能力常驻多少 token、参数怎么传、谁能改**；披露策略决定的是**有多少条同时摆在模型面前**。两者容易被混为一谈，是因为 skill 的目录条目比工具 schema 便宜一个数量级，把“全量常驻”的可行边界推远了不少——但那只是让披露这一侧更宽松，并没有替你做出披露策略的选择。本节只回答形态问题，规模问题留到 MCP 协议之后的“工具太多怎么办”一节。
...
统一了接口之后，还有两件事需要交代：MCP 不负责什么，以及接入第三方服务器带来了什么风险。至于工具数量增长本身带来的挑战——上下文开销、层次化组织、按需披露——正是下一节的主题。
