...
输入验证和权限控制挡住明显的越界；对不可逆操作，还要引入不共享同一盲区的 Reviewer。提议者—审核者有两种时机：执行前批准，或执行后用另一种模态验证（例如渲染截图、沙盒运行结果）。两个模型应共享任务约束，但最好来自不同模型家族且能力相近；Proposer 偏向完成任务，Reviewer 偏向风险和规则。拒绝理由要作为结构化工具结果回到轨迹，而不是静默重试；高风险或 Reviewer 不确定时升级人工。

**Sidecar 机制：与主思考并行的安全校验。**

提议者—审核者关注一次操作的合理性或结果；**Sidecar** 则在主模型流式生成时旁路判断工具调用，并在放行前门控执行。它适合风险分类、记忆筛选和长输出摘要等较窄任务，所以可以使用轻量模型；但安全判断不能读取带有自由指令的完整轨迹。

提示注入的防线是把工具名、解析后的参数、权限元数据和不可信字段隔离传给 Sidecar。连续拒绝应触发熔断并升级人工，不能让 Agent 无限改写同一调用。界面可以并行展示“正在读取……”等进度，但真正执行必须等待门控完成。
...
Sidecar 也可在主模型思考时预取相关记忆、摘要工具输出或读取最新状态，结果准备好后再注入下一轮上下文。
...
这里的输入隔离是关键边界：Reviewer 看到工具名、解析后的参数和权限元数据，而不是可能携带提示注入的完整轨迹；必须传递的自由文本要标成数据而不是指令，并经过长度、编码和内容策略检查。完整的路径校验、linter、沙盒和幂等实现见 [实验 4-3](../chapter4/execution-tools/README.md)。
...
取消点必须是工具或推理能够安全收尾的位置；未完成的工具结果用显式占位符表示，不能伪造成功。`chapter4/async-agent/runtime.py::AgentRuntime` 的 `_dispatcher` 与 `_handle_interrupt` 展示了这一骨架的可运行版本。
...
首次加载后的 schema 固定在轨迹原位置，静态前缀仍可复用；实际的路由、阈值和降级策略见 [实验 4-7](../chapter4/active-tool-discovery/README.md)。
