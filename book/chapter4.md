...
- **专用代码工具**：结构化的函数调用，确定性高、可测试、参数受 schema 约束，代价是每个工具的定义要占去数百个 token。
- **Skill + 通用执行器**：用自然语言编写的 Skill 文档来描述操作流程，Agent 通过终端或代码解释器来执行，只需少量的通用工具就能覆盖大量场景（如第五章将论证的七个核心工具）；一份 skill 在目录里只占几十个 token，正文用到时才读。
...
- **模型能力**：SOTA 模型可以用 Skill + 通用执行器的方式表达更多能力、减少工具数量；较弱的模型则需要结构化的工具 schema 来引导正确调用。
...
“能力的表达形式”一节把能力分成了两种形态。形态定下来之后还有一步：能力得能被分发出去、被别人装上。两种形态各自长出了一条分发渠道——专用工具那一侧是 MCP 协议，Skill 那一侧是注册表。先看前者。
...
统一了接口之后，还有三件事需要交代：MCP 不负责什么、Skill 那一侧的能力靠什么分发，以及从第三方引入能力扩大了多大的信任边界。

**MCP 的局限性**。MCP 的重点是标准化 Agent 与外部能力之间的交互，而不是提供一个完整的事件运行时。协议已经能够支持多轮交互、变化订阅和长任务等复杂流程，但这些机制解决的是“一次工作流如何继续”，并不负责让 Agent 始终在线。跨会话、多事件源、离线唤醒的事件驱动架构——例如新邮件到达时启动 Agent、外部系统回调时恢复任务——仍需要在协议之上另行构建[^ch4-mcp-current]。构建方式是分层的：MCP 负责能力调用的标准化，Agent 框架负责事件接入、调度、并发和唤醒。第六章讨论的正是后一层问题。

[^ch4-mcp-current]: Model Context Protocol, “2026-07-28 Specification”. https://modelcontextprotocol.io/specification/2026-07-28

**能力分发的另一半：Skill Hub**。MCP 统一的是**专用工具**那一侧的接入方式。Skill 那一侧不需要协议——一个 skill 就是一个装着 `SKILL.md` 的文件夹，分发它只需要一个包管理器，于是这一侧长出来的是**注册表**（registry）而不是协议。Vercel 于 2026 年 1 月上线的 skills.sh 是其中影响较大的一个：一条 `npx skills add <owner>/<repo>` 命令即可安装，工具会探测本机装了哪些 Agent，再把 skill 分别写进各自的目录，公布的兼容客户端包括 Claude Code、Codex、Cursor、Windsurf、GitHub Copilot、Gemini CLI、OpenCode、Goose 等十余种[^ch4-skills-sh]。OpenClaw 生态则有自己的 ClawHub，提供 `clawhub skill publish` 这样的发布命令和一份可浏览的目录[^ch4-clawhub]。第二章介绍的 Anthropic 官方 skills 仓库也是经由这类渠道被安装的——它的 `frontend-design` 长期位居 skills.sh 排行榜前列。
...
[^ch4-clawhub]: ClawHub，OpenClaw 生态的 skill 与插件注册表（原域名 clawdhub.com 现跳转至此）。https://clawhub.ai/

**两条渠道的成本落在不同的地方**。接入一个 MCP 服务器，是在运行时建立一条连接，它暴露的全部工具定义会进入**每一次会话**的上下文，成本按会话计；安装一个 skill，只是往磁盘上拷一个文件夹，常驻上下文的只有目录里的 `name` 和 `description`，成本按条目计，便宜一到两个数量级。这解释了为什么“能力太多”在两侧撞上的墙并不相同：MCP 侧先撞上的是上下文预算，Skill 侧先撞上的是目录膨胀与选择过载。但两侧的走向是同一个——**引入一条新能力的成本已经降到一条命令**，头部 skill 的安装量早已是百万量级；当“接进来”不再是瓶颈，可用能力的数量就必然涨到超出模型一次能看清的范围。下一节要回答的，正是“接进来之后怎么办”。

**第三方能力的信任模型与安全风险**。无论走 MCP 还是走 Skill Hub，引入第三方能力都意味着同一件事：把一段不受自己控制的文本注入了 Agent 的上下文，往往还把一份凭证交到了别人手里。以 MCP 服务器为例，主要风险有四类。
...
**Skill 那一侧的风险同构，但多了一条**。上述四类在 Skill Hub 上一一对应：`SKILL.md` 的正文同样原样进入上下文，因而同样可以投毒；注册表上的包同样会在升级时被替换或劫持；同名 skill 同样可以遮蔽正规 skill；skill 调用的接口同样可能拿到用户的凭证。区别在于**执行边界**：MCP 服务器至少还隔着一个独立进程和一层协议，而 skill 附带的脚本是 Agent 直接在本地终端里跑起来的，中间没有任何边界。这也是多家 skill 注册表把“自动安全扫描”当作卖点的原因。缓解手段与上面一致——把 `SKILL.md` 及其引用的脚本当作不可信输入来审计、锁定版本、拒绝静默升级——并把执行放进本章“执行工具”一节讨论的沙盒里。
...
**就像查工具书或维基百科。** 这更接近人使用参考资料的方式：没有人会把一本工具书或整个维基百科从第一页读到最后一页，而是顺着索引和目录，根据当下需要逐个查阅词条。工具的详细定义不必全部常驻上下文，用到哪条查哪条。相比上一节，Agent 靠通用的文件阅读能力（`grep`、读取文件）翻阅 skill 目录即可，不必把 “发现工具” 单独建模成一次特殊的语义检索。Skills 是一种更现代、也更省心的工具发现思路。

**Skills 省心的真正原因是层级内建。** 容易被误认为是“自然语言比 JSON 灵活”，其实不是。一个 skill 天然就分成三层——目录条目（`name` 与 `description`）、SKILL.md 正文、正文里引用的脚本和子文档——模型顺着这三层往下读，本身就是一次渐进式披露，不需要任何额外机制。专用工具的 schema 则是**原子的**：要么整块在上下文里，要么不在，没有“先看一眼摘要”这个中间态。所以专用工具要达到同样的按需暴露，必须在工具之外另建一层——嵌入索引、检索元工具、`tool_search` 与 `tool_reference` 这类 API 原语，也就是上一节那套基础设施存在的理由。**这仍然与“能力做成什么形态”是两个决策**：前面 `pi-mcp-adapter` 的例子表明，一个纯 MCP 的专用工具后端照样可以只暴露一个代理工具、按需展开；只是它得自己把那一层层级搭出来。

**两条分发渠道正在互相靠拢。** 前面把 MCP 与 Skill Hub 讲成两条并行的渠道，但它们并非互不相干：MCP 官方已经在推动 skill 经由 MCP 被发现和传递——Skills 负责组织和披露能力，MCP 负责跨客户端把它们送到[^ch4-skills-over-mcp]。也就是说，同一个 skill 既可以躺在注册表里等 `npx` 来装，也可以由一台 MCP 服务器供给。
