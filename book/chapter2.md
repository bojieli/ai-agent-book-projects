...
> 
>**模型的内部思考过程**：支持思维链的模型（如 Qwen3）在生成工具调用之前，会先在 `<think>` 标签内进行思考——分析用户意图、评估哪些工具适用、规划调用顺序。这个思考过程对调试 Agent 行为非常有价值。需要注意的是，`<think>` 是模型原始输出流中的标签，不一定会原样出现在 API 的返回结果里：本实验向 Ollama 传入 `think=True`，Ollama 会在返回前把这段思考解析出来，放进独立的 `thinking` 字段（流式下是逐块的 `thinking` 增量），此时 `content` 里就看不到 `<think>` 标签了。两者只是同一份思考内容的不同呈现方式，实验代码对两种情况都做了处理。
> 
