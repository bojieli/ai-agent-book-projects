...
>
> 上面三个维度的数字来自一次小规模的对照运行，是**示意性观察**：它们说明的是方向——组织结构和工具描述的影响远大于语气风格——而不是可复现的量级。按第七章的标准，这样的单次差异需要给出样本量和重复次数才能支撑“高多少”的结论。读者自己跑这个实验时，重点应放在三个维度的相对排序是否稳定，而不是能否复现出同样的百分点。
...
>
> **详细错误信息**：包含四层内容——错误类型和描述、完整参数的 JSON、调用栈信息，以及针对性的修复建议（如遇到 FileNotFoundError 时建议验证路径、检查工作目录、使用绝对路径）。启用后，Agent 在错误场景中找到替代方案的成功率从 60% 提升到了 95%，从盲目重试转变为分析性的问题解决（同样是一次小规模运行的**示意性观察**，说明的是“把错误信息写详细”这个方向有效，而不是这个幅度可复现）。
>
