...
本节以 **OpenAI-compatible Chat Completions** 协议为例，详细拆解 Agent 每次调用大模型时的完整请求构成。它不仅用于 OpenAI，也被 DeepSeek、Kimi、Qwen，以及 vLLM、Ollama 等开源模型服务广泛实现，是目前跨厂商开发最常用的共同接口。Anthropic、Google 等厂商的原生 API 有各自的消息与工具结构，不在本节逐一展开。

OpenAI 还提供了更新的 Responses API：它以 `instructions` 和输入/输出 item 组织请求，可使用 `developer` 指令，并可通过 `previous_response_id` 或 Conversations 管理会话状态。不过，为了让示例直接适用于上述开源模型与兼容服务，本章仍以可移植性更强的 Chat Completions 为主；下面的 system、user、assistant、tool 是跨实现最常见的四种角色。
...
这个请求只包含两条消息：一条 system（开发者写的规则）和一条 user（用户的输入）。模型返回一条 assistant 消息作为回复。在本章采用的基础 Chat Completions 模式中，**每次请求都按无状态方式构造，模型需要的信息必须由客户端在消息列表中提供**；这不是所有 API 的普遍限制，例如 Responses API 还可以引用前一个响应或持久化的 Conversation。
...
1. **第二次请求包含了第一次的全部对话历史**——system 消息、user 消息、第一次的 assistant 回复（包含工具调用），以及新增的 tool 结果。这就是前面所说的基础 Chat Completions 无状态请求模式：Agent 框架必须在下一次请求中送回所需历史。
...
> 4. **流式响应的优势**：在这个本地实验中，所选 Qwen 模型及其模板会输出 `<think>` token，因此开发者能实时观察它暴露的思考文本、工具调用决策和结果处理。这是本地模型的实验可观测性，不是托管 API 的通用属性；生产系统通常也不应把这类原始文本直接展示给终端用户。
...
> **ReAct 循环的实际案例。**
>
> 项目中的多轮工具调用遵循第一章介绍的 ReAct 思考-行动-观察循环，此处不再重复其原理。上一节已经用 OpenAI-compatible Chat Completions 的 JSON 格式展示了这个过程的完整消息结构。在本地部署的实验中，这些 API 消息会被服务端（如 vLLM、Ollama）自动转换为模型内部的 token 格式。本实验的 `local_llm_serving` 项目允许你直接观察模型的原始输入输出 token 流，包括以下在通常 API 层面不可见的细节：
>
> **模型模板暴露的思考 token**：本实验中的 Qwen3 在生成工具调用之前，会先在 `<think>` 标签内输出一段分析——例如评估哪些工具适用、规划调用顺序。这些 token 有助于研究特定本地模型的生成行为，但不能假定为完整、忠实的内部决策解释，也不应作为安全审计证据。
>
...
**第一，解释了为什么必须使用标准 API 格式**。如果开发者绕过 API、自行拼接消息（比如把工具结果作为普通 user 消息而非 tool 类型传递），Chat Template 会误将工具响应识别为新的用户查询，破坏模型预期的多轮格式。以 Qwen3 的 Chat Template 为例：模型可能在一轮工具调用中保留 `<think>` 标签内的内容；当模板检测到新的用户查询时，又可能按模板规则清理这部分内容。如果工具结果被错误地标记为 user 消息，就会误触发清理，影响多步任务的连贯性。

思考状态的回传规则不是跨模型统一的。DeepSeek R1 的官方协议要求在后续轮次剥离 `reasoning_content`；另一些 DeepSeek、Kimi、GLM 等模型或版本则要求把接口返回的 `reasoning_content` 原样放回对应 assistant 消息。Claude 的原生 API 使用带签名的 thinking block，并规定其回传范围。OpenAI 的 Responses API 不暴露原始思维链，但可能返回需要保留的不透明或加密 reasoning item 和可选摘要；在无状态续接时，应把相关输出 item 一并带回。由此得到的通用结论不是“可见思考必须保留”，而是：**思考相关状态属于供应商协议的一部分，是否回传、以何种形式回传，都必须遵循具体模型和 API 的最新契约**。不要自行伪造、解析或向用户暴露不透明状态，也不要把可读的思考文本当作忠实解释。
