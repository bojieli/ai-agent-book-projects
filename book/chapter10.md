...
以上所有讨论都是工程视角：如何让一组 Agent 协作完成任务。接下来一节的**体例与全书其余部分不同**：前面九章半回答的都是“该怎么建”，而它回答的是“正在发生什么”——它是一份对当前多 Agent 社会实验的综述，给出的是观察和开放问题，而不是可直接照做的工程方案。之所以仍然放在本章，是因为这些实验恰恰把前面讨论的上下文共享、协作拓扑和失败模式推到了极端规模，而且其中两个（斯坦福 AI 小镇与语音狼人杀）本书提供了可复现与可运行的配套项目。读这一节时，可以把它当作把前面的工程结论放进更长时间尺度后的压力测试。
...
本章也是全书正文的最后一章。从第一章的 Agent = LLM + 上下文 + 工具，到构建、评估与进化，再到这一章的协作，工程侧的路线已经走完。但有两个问题是工程本身回答不了的：模型持续变强之后，今天这些 Harness 还会剩下多少；以及属于具体用户、具体公司的知识，能不能靠“再训练一次”补上。后记接着讨论这两个问题。
