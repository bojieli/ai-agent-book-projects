...
> **实验 10-2 ★★：共享上下文中的多角色转换——system prompt 与 Skill 的对比**
>
> **前置要求**：建议先了解第二章 Agent Skills 机制和第六章“系统评估”。本实验比较的是两条可部署的架构路径，而不是只替换一行提示词的纯消融。
>
> **共同控制变量**：两条路径使用同一模型、同一用户任务、同一工具实现、同一份角色规程和完整共享轨迹。五份 `skills/*/SKILL.md` 是角色规程的唯一真源：路径一把当前角色的同一份正文放入动态 system prompt，路径二通过 `load_skill(name)` 把正文追加为 tool result。除转换机制所需的最小说明外，不重复编写两套角色提示词。
>
> 角色目录仍包含五个角色：`triage`（拆解与收尾）、`research`（真实联网检索）、`coding`（真实执行 Python）、`data_analysis`（定量计算）和 `writing`（成稿与篇幅检查）。
>
> **路径一：`transfer_to_agent`**。每个角色只暴露自己的专属工具和 `transfer_to_agent(target_role, reason)`。移交时 Harness 保存历史，替换 system prompt 与工具 schema，再让目标角色继续同一轨迹。工具 schema 的硬隔离是这条路径的主要安全收益；代价是每次边界都可能使前缀缓存从差异点起失效，还要实现动态提示词、角色状态机、回退、防循环和边界观测。
>
