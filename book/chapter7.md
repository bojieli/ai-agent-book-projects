...
本书的核心公式是 Agent = LLM + 上下文 + 工具。本章聚焦于优化 LLM 这个“大脑”——通过后训练让模型更好地利用上下文和工具，从而提升整个 Agent 系统的能力。第六章结尾指出，评估体系与仿真环境是后训练的两块基石：评估环境为训练提供练习场，评估指标为训练定义目标。本章就建立在这两块基石之上，讨论如何真正改动模型权重，把能力沉淀进参数。本章末尾还会回到第六章的 bad case 分析，用一个完整案例展示端到端回归任务与轨迹前缀回归任务如何转化为偏好对与奖励信号（实验 7-17）。
...
## 从 bad case 到训练数据：评估资产如何进入后训练

前面各节回答了“怎样训练”，这一节回到第六章留下的问题：从生产 bad case 构建的评估数据集，如何真正变成后训练的输入。第六章结尾把评估环境和验证器比作后训练的基石，但基石与训练之间还隔着一层翻译——失败归因记录、端到端回归任务、轨迹前缀回归任务、Rubric 评分各自对应不同的训练用法，放错位置，再好的评估资产也产生不了学习信号。表7-4 给出这张映射。

表7-4 第六章评估资产到第七章训练用法的映射

| 第六章的评估资产 | 第七章的训练用法 |
|---|---|
| 端到端回归任务（含验证器） | RL rollout 任务与可验证奖励（RLVR）；拒绝采样（RFT）的采样池 |
| 轨迹前缀回归任务 + 可接受动作集合 | DPO 偏好对、决策边界的 SFT 示范、On-Policy Distillation 的教师状态 |
| 失败归因记录（首个错误步骤与错误类别） | 过程监督的负标签（PRM）、RLVP 路径惩罚的规则来源 |
| Rubric 多维评分与人工金标集 | 向量奖励的各维度、生成式奖励模型（GRM）的训练与校准数据 |

映射表两端的红线仍然有效：评估集本身的具体题目必须与训练数据严格隔离，可以复用的是构造机制——任务模板、验证器、归因分类体系。下面用一个完整案例把这张表走一遍。

### 案例：Coding Agent 的“过早结束”

**从 bad case 到归因。** Coding Agent 最常见、也最难根治的失败之一是**过早结束**：测试还没跑就宣称“已完成”；用户要求改三个端点，改完两个就收尾；遇到两次失败就宣布“这个任务不可能完成”。按第六章的错误分类，这属于“任务完成度与逻辑判断问题”，生产侧的三类信号都能捕获它：用户纠正（“你根本没跑测试”）、点踩、事后审计（宣称完成的轨迹里没有任何测试工具调用）。归因记录把首个错误定位在“准备宣称完成”的那个决策边界上——在此之前，读代码、改代码可能都没错，错的是“在缺乏证据时下结论”这一步。前文奖励设计一节讨论的 reward seeking（自设一个很浅的检查、刚好通过就提前结束），描述的正是这类行为的极端形态。

**为什么这个问题属于参数更新。** 先做第六章的瓶颈判断：把同样的轨迹前缀喂给更强的模型（模型替换实验），过早结束的发生率有所下降但并不消失——Harness 的提示词里明明写着“完成后运行测试”，模型仍然跳过。这说明问题不完全是 Harness 缺信息，而是模型的**行动阈值**（见第六章实验 6-8）本身偏低：参数层面已经形成了“尽快给出完成结论”的策略倾向。7.1 节说过，行动阈值的重要来源正是后训练；要可靠地抬高它，同样需要后训练。这与 Harness 侧的兜底并不矛盾：凡是能写成确定性规则的验收约束都应由代码提供（第八章将展开），后训练负责的是规则写不尽的灰区判断。

**构造训练数据。** 修复从两类回归任务开始，对应映射表的两行。端到端一轨：把“宣称完成前必须跑通验收测试”写成可验证奖励——测试对模型不可见，模型宣称完成时才运行，通过 +1、不通过 −1；这正是“判定交给模型写不了的隐藏测试”（见前文奖励设计）的直接应用，也是本案例可选的 RL 分支。前缀一轨更便宜：截取“准备宣称完成”的决策边界构造**偏好对**——rejected 是当时的过早宣称，chosen 是“先运行测试、逐条核对验收条件，再下结论”的下一步动作。chosen 由教师模型生成、规则验证器过滤（拒绝采样：chosen 中出现完成宣称的一律丢弃），得到一批 DPO 训练对。少量 bad case 再经参数化扩充（换任务类型、换缺失的验证项、换宣称措辞）形成数百条偏好对，但在训练数据中只占很小的比例——按小配比混入通用指令数据做 LoRA 微调，避免把“逢收尾必验证”学成新的过拟合，也降低灾难性遗忘的风险。

**评估：边界集与保留集缺一不可。** 训练后的验证回到第六章的评估资产，而不是训练数据的同类题：held-out 轨迹前缀边界集检查“任务未完成时，模型是否选择继续验证而非宣称完成”；同样重要的是**保留集**——任务确实已完成时，模型应正常宣称完成。只盯前一个指标，会把模型训练成永远不敢收尾的**过度矫正**状态：每个任务都无限验证下去，延迟与成本崩溃。这与第六章反复强调的“改动不能破坏既有行为”是同一原则在参数层面的版本；评估还应抽查通用能力，确认 LoRA 补丁没有波及其他行为。

> **实验 7-17 ★★：从“过早结束” bad case 到 DPO 修复**
>
> **实验目标**：跑通从生产 bad case 到参数更新的完整链路——失败归因 → 轨迹前缀回归任务 → DPO 偏好对 → 7B 模型 LoRA 训练 → 边界集与保留集双集验证。
>
> **数据构造**：配套仓库提供 24 条写实的过早结束 bad case，覆盖四类失败（未跑测试就宣称完成、多目标只完成一部分、验收条件未满足、遇错放弃宣称不可能，含删除失败测试这类更恶劣的 reward hacking 变体），以及与训练数据严格隔离的 held-out 评估集（boundary 12 条 + retention 8 条）。`build_preference_data.py` 把每条 bad case 转成 DPO 偏好对：rejected 为当时的过早宣称，chosen 可由教师模型生成并经规则过滤（拒绝采样，保留证据回执），也可由确定性模板离线合成。
>
> **训练**：`train_dpo.py` 用 DPOTrainer 对 Qwen2.5-7B-Instruct 做 LoRA 微调，单卡即可复现。可选 RL 分支 `train_grpo_optional.py` 使用 GRPO + 隐藏验收测试奖励，对照“离线偏好对”与“在线可验证奖励”两条路径。
>
> **验收**：`evaluate.py` 对比基线与微调后模型——boundary 集的过早宣称率应下降，retention 集的正常收尾率应保持，过度矫正率应接近零；训练回执与评估证据存入 `validation/`。训练前可先运行 `python demo.py` 离线走通数据构造与评估口径。
>
> 配套实现见 [`premature-completion-dpo`](../chapter7/premature-completion-dpo/)。

**这个案例的边界。** 24 条 bad case、数百条偏好对、一个 7B 学生——这个规模演示的是链路与口径，而不是能力提升的幅度。真正投产时，偏好对要覆盖更多任务族，保留集要覆盖更多“正常收尾”的场景，还要警惕奖励作弊的新形态：模型可能学会“口头声称去验证”而不真的验证——这正是端到端一轨的奖励必须依赖模型写不了的隐藏测试、而不是模型自己的声明的原因。过早结束也不只有参数一种治法：能写成确定性规则的验收约束应交给 Harness（第八章），后训练负责规则写不尽的灰区判断。
...
其三，**评估资产只有被消费才有价值**：第六章从 bad case 构建的端到端回归任务、轨迹前缀回归任务和失败归因记录，分别对应 RL 任务与奖励、DPO 偏好对与决策边界示范、过程监督信号（实验 7-17 的过早结束案例走通了全链路）。训练后的验证同样要回到评估体系——边界集看修复，保留集看过度矫正与遗忘，两者缺一不可。
