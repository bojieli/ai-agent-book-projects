...
>
> **复核结论（负结果）**：以上两条是原实验设想，并非本次可执行证据所支持的机制映射。[规范训练报告](../chapter7/voice-sft-training-report/)保留了两份不可变的上游执行 notebook、358 条逐步 loss、4 段原始音频和 2 次真实直音频评审。实际源码中，Orpheus 序列化 SNAC token，并用 `<giggle>`、`<giggles>`、`<laugh>` 等文本标签控制副语言现象；Sesame 则通过 CSM 对话历史传入前序音频，进行说话者/风格条件化，未定义 `<laugh>`/`<sigh>` 控制协议。由于只保留了一段 Orpheus 音频，跨句音色一致性无法检验；三条 Sesame 提示均未带控制标签，标签可控性也无法检验。报告没有强行复现原映射，也不把 post-training 音频冒充为有匹配基线的因果增益；历史 checkpoint 按本书政策不随书分发，也不作为报告验收物。
>
