...
> **实验 7-17 ★★：从“过早结束” bad case 到 DPO 与 GRPO**
>
...
> **实验 7-18 ★★：从中文弯引号 bad case 到作用域敏感 SFT**
>
> 用户反馈“中文文章中的直引号应统一为弯引号”并不足以构成训练标签。实验先把 ASCII 直引号、中文弯引号和 Markdown 反引号分开，再为中文自然语言、英文原文、行内代码、代码块、代码注释和 JSON 标注作用域、保护区域和验证条件。Skill 明确规定：中文自然语言中的引用可转换为 `“”`，英文原文、代码语法、路径、JSON 和不确定区域不得猜测性修改。
>
...
>
> Coding Agent 的 `old_string` 失败首先要经过逐层定位：比较文件原始字节、工具返回、Harness 序列化、模型输出和工具匹配结果。只有首个差异确实出现在模型输出时，才把案例转成训练数据。实验覆盖直接复述、相似字符串选择和工具 JSON 参数三种任务，并使用新的随机字符串、长度和 token 组合作为留出集。
>
...
其三，第六章的评估任务不只用来测分数，也可以转成训练样本、奖励信号和回归用例。端到端任务适合做 RL 环境，轨迹前缀适合构造偏好对，失败原因可以帮助设计过程信号；训练后仍要回到独立的边界集和保留集，既检查修复，也检查过度矫正与遗忘。
