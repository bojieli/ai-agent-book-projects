...
TF-IDF（Term Frequency–Inverse Document Frequency，词频–逆文档频率）是一种衡量词项对文档重要程度的打分方法，也是更早确立的经典基准；BM25（Okapi BM25）则是对它的改进，成为目前实际使用最广的稀疏检索算法。
...
TF-IDF 用一个公式来表达这个直觉。它把词 t 在文档 d 中的重要性——记作 TF-IDF(t, d)——定义为两个因子的乘积：

$$\text{TF-IDF}(t, d) = \text{TF}(t, d) \times \text{IDF}(t), \qquad \text{IDF}(t) = \ln\frac{N}{\text{DF}(t)}$$

其中 `t` 是某个词（词项，term），`d` 是一篇文档；`TF` 是词 t 在文档 d 中出现的次数（词频，Term Frequency），`DF` 是包含该词的文档数（文档频率，Document Frequency），`N` 是文档总数。孤立的 `DF` 说明不了稀有程度——同样出现 60 次，在 100 篇语料里很常见，在 10 万篇里却极稀有——所以要把 `DF` 和总数 `N` 比，`DF/N` 就是含该词的文档比例。`IDF`（逆文档频率，Inverse Document Frequency）取决于 `DF` 占 `N` 的比例，`DF/N` 的比例越小（词越稀有），`IDF` 越大，公式里写成这个比例倒数的对数 `ln(N/DF)`[^ch3-log]。两者相乘的用意在于：一篇文章要因为某个查询词拿到高分，需要这个词既在这篇文章里频繁出现（`TF` 高），又在整个语料里足够稀有（`IDF` 高），缺一不可。回到例子，“模型”的 `DF/N=60%`，比例偏大、`IDF` 很低；“蒸馏”的 `DF/N=3%`，比例很小、`IDF` 很高——于是“蒸馏”对排序的贡献远大于“模型”。然而 TF-IDF 有两个被实践反复证实的缺陷。第一，**词频是线性增长的**：假设两篇长度相近的文章，“蒸馏”一篇出现 2 次、一篇出现 10 次，TF-IDF 会给后者 5 倍的权重，可真正决定相关性的是“这篇文档谈没谈这个主题”，前两次出现就已经确立了主题相关性，再堆 8 次只是冗余，不该让得分翻 5 倍。纯粹的线性计分会鼓励“堆砌关键词”的行为。第二，**它完全不感知文档长度**：同样是“蒸馏”这个词，在一篇 500 字的短文里可能只出现 2 次，换到一篇 5000 字的长文里却能轻松出现 20 次——长文的 TF 更高，但往往只是因为篇幅长、词数多，并不代表它更相关。这两个偏差共同导致 TF-IDF 在长短文档混排的真实语料上排序不够公平。

[^ch3-log]: 取对数而非直接用倒数，是为了压缩取值范围——否则一个只出现在百万分之一文档里的词，其 IDF 会高达百万，让极稀有词在排序中一家独大；对数把这种爆炸式增长压成平缓的曲线。对数的底数并不影响排序结果（换底只会把所有得分按同一比例缩放），所以自然对数 `ln` 和以 10 为底的对数在实践中都很常见。BM25 采用自然对数 `ln` 则是一种惯例。

BM25 针对上述两个缺陷，分别引入词频饱和与长度归一化两项机制，并沿用 IDF，将这三部分统一进同一个打分公式。对查询 Q 和文档 D，BM25 的得分为：

$$\text{Score}(Q, D) = \sum_{i} \text{IDF}(q_i) \cdot \frac{\text{TF}(q_i, D)\,(k_1+1)}{\text{TF}(q_i, D) + k_1\left(1 - b + b \cdot \frac{|D|}{\text{avgdl}}\right)}$$

公式先算出查询中每个词对文档 D 得分的贡献（即求和符号下的每一项），再对查询中的所有词求和（最前的 $Σ$），得到文档 D 对查询 Q 的总分。$q_i$ 是查询中的第 `i` 个词，`D` 是待打分的文档；`|D|` 为文档 D 的长度，`avgdl` 为所有文档的平均长度，`N` 为文档总数，`DF` 为包含该词的文档数。

单个查询词的得分由三部分协同构成，如图 3-8 所示。


![图3-8 BM25 评分机制](images/fig3-8.svg)


**逆文档频率（IDF）** 衡量词的稀有度，决定词的全局权重。词越稀有（出现在越少文档中），IDF 越大；越常见则越小，常见词对排序几乎无影响。BM25 将其写成概率型平滑形式 `IDF = ln((N-DF+0.5)/(DF+0.5))`，其中 0.5 是平滑常数，防止极端词产生过大或过小的值[^ch3-smooth]，这部分相对 TF-IDF 仅是形式调整，逻辑不变。

**词频饱和**让得分随词频升高而增长，但随着词频升高，增长得越来越慢、最终趋于上限不再增加，从而避免长文档靠堆砌关键词占优。对应中间的分式 `TF·(k1+1)/(TF+…)`，其上限为 `k1+1`；参数 `k1` 控制饱和快慢，`k1` 越大，词频影响持续越久。

**长度归一化**按文档长度调整得分，使长文档被压低、短文档获得补偿，从而抵消长文档因篇幅带来的词频虚高。对应分母中的因子 `1 - b + b·|D|/avgdl`（嵌在词频饱和的分母内、不独立相乘），`|D|` 越大该因子越大、撑大分母压低得分；参数 `b` 控制归一化强度。

> **补充说明（可选的手算示例，跳过不影响后续阅读）**：以查询“模型蒸馏”为例，计算“蒸馏”在一篇文档（`TF=3`，长度 200 词）上的贡献。设 `N=10`、`avgdl=250`，`k1=1.5`、`b=0.75`；“蒸馏”`df=2`，故 `IDF=ln((10-2+0.5)/(2+0.5))≈1.22`。代入分式：分子 `3×(1.5+1)=7.5`，分母 `3+1.5×(1-0.75+0.75×200/250)=3+1.5×0.85=4.275`；与 IDF 相乘得最终贡献 `≈2.15`，与后文实验 3-5 中该词对 doc_1 的贡献一致。

综合三部分，IDF 决定词的全局权重，词频饱和限定词在文档内的贡献上限，长度归一化保证不同长度文档可比。正是由于针对性地解决了 TF-IDF 的这两个缺陷，BM25 成为更健壮有效的稀疏检索排序函数，至今仍是各大搜索引擎中不可或缺的核心组件。

[^ch3-smooth]: 这里的 0.5 是一个“伪计数”形式的平滑常数——相当于在统计“含该词的文档数 DF”和“不含该词的文档数 N-DF”时各预先加上半个虚拟文档。其作用是防止 IDF 算出极端值：一个词若出现在全部文档中，不加平滑时 IDF 会变成 -∞；加 0.5 后仅为有限负值。当词出现在超过半数文档中时该形式仍为负值，真实引擎（如 Lucene、Elasticsearch）通常在外面再套一层 `ln(1+…)` 以保证非负。该常数源自 BM25 所基于的概率检索模型。
