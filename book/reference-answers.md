...
> 更值得标准化的是协议之上的事件编排层。MRTR 已能把补充输入拆成携带不透明 `requestState` 与 `inputResponses` 的多轮请求，`subscriptions/listen` 能传递变化，Tasks 能提供可轮询、可恢复的持久句柄；继续增加单次工作流原语的边际收益有限。仍缺的是跨邮件、webhook、定时器等来源的统一事件信封、幂等键、优先级、重试/死信语义、持久化和离线唤醒约定。MCP 可以承载被调度的能力与任务句柄，但常驻运行、权限边界和“何时唤醒哪个 Agent”应由独立的编排层负责，避免把部署生命周期重新耦合进工具协议。
