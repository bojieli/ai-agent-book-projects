...
语音交互、Computer Use 和机器人操作看上去跨越三个完全不同的领域，但做起来会发现卡住的地方高度相似：都要同时处理多种模态的信息，都对延迟极度敏感。
...
> **阅读提示**：本节的核心实验都可以在本地 GPU 上完成：9-8 和 9-9 使用小型、确定性的桌面模拟器，9-10 使用本地 RGB 跨环境测试。

机器人操作比“看图回答问题”难得多。模型不但要看懂画面，还要在真实世界里连续做出动作，而且每个动作都会改变下一刻的情况。一个能用的机器人系统，至少要回答四个问题：
...
本节把这四个问题放在同一个控制闭环中，并分别说明四种技术各自负责什么：长程规划安排任务顺序，VLA 完成当前技能，世界模型估计动作后果，从仿真环境迁移到现实环境则处理训练环境和真实硬件之间的差异。

### 硬件与算法的分工

如果让人通过遥操作来控制，许多低成本机器人已经能完成抓取、放置和擦拭等任务。这说明，在本节讨论的这些任务里，现有硬件通常已经足以支撑基本操作。可是，这并不表示硬件没有问题：触觉传感、灵巧手、精细的力量控制，以及执行器长期工作的可靠性，仍然可能成为瓶颈。为了把两件事分开，我们先测量“一个足够聪明的操作者能做到什么”，再看自主系统离这个水平还有多远。

> **实验 9-8 ★：用理想控制者测量任务上限**
>
> [实验目录](../chapter9/xlerobot-teleoperation/)中的本地 GPU 版本用一个二维桌面模拟器来做这件事。模拟器随机摆放物体，理想控制器再把每个物体移到对应目标。正式验收使用 5 个随机种子、4 种物体数量，每个条件运行 512 个回合，共 10,240 个回合；我们逐格记录成功率、完成步数、95 分位步数和路径长度，还会用同一个随机种子重跑一次，确认结果确实可以复现。这里测的是非致动模拟，不是 XLeRobot 真机结果；真机遥操作仍需要单独的硬件和安全条件。[^ch9-1]
...
| 底层控制与安全层 | 如何稳定、及时地执行 | 关节或末端控制量 | 约 50—1000 Hz |

这是一种常见的工程分工，不是唯一的模型架构。VLA 可以承担一部分高层判断，规划器也可以是规则程序、VLM 或优化器。无论采用哪种实现，都应该把“任务顺序”和“眼前动作”分开，否则高层模型的推理延迟会拖慢底层控制，底层的高频控制也会让高层模型处理大量无关细节。
...
> **实验 9-9 ★★：桌面操作规划与逐步检查**
>
> [实验目录](../chapter9/gemini-xlerobot-navigation/)保留 RoboCrew 风格的智能体循环，并预留 XLeRobot 执行器接入层，但把任务改为“抓起红色杯子放入托盘，再把黄色纸张放入垃圾盒”。这里的“工具”就是智能体可以调用的动作清单和参数格式，实验只开放五个职责明确、权限固定的工具：observe_scene、pick、place、verify_state 和 stop。正式验收使用 3 个随机种子、0/0.25/0.5 三档人为注入的瞬时抓取失败概率，每格 128 个回合，共 3,456 个回合；比较一次性执行、每步重新观察，以及使用短期预测模型选择下一项技能三种方式。实验记录任务成功率、工具调用次数、失败后的恢复次数、动作条件世界模型的训练/测试误差和全部回合的事件日志。真实运行时，pick 和 place 必须由经过标定、限速并带超时的 XLeRobot 动作原语实现，不能让模型直接输出任意关节角。
...
大模型每秒通常只能推理 1—10 次，而传统控制器每秒可能要更新几十到上千次。工程上常用“动作分块”：模型一次生成一小段未来动作，控制线程按较高频率执行这一小段，模型则在后台准备下一段。这样可以把一部分推理等待藏在动作执行时间里。代价是，动作段越长，运动越平滑，模型在这段时间里看到的新画面却越少；如果有人突然伸手或物体被挪走，机器人就可能反应不够快。因此，动作分块是在平滑性和反应速度之间做取舍，而不是没有代价的加速。
...
实验 9-9 的“预测式”模式是一个小型、可验证的模型预测控制演示，也就是先预测几个候选动作的结果，再选择其中一个执行。它不声称等同于 V-JEPA 2 或真实机器人 WAM，只说明一个基本关系：预测模型可以帮助给候选技能排序，但最终是否真的完成，仍要靠真实观察和状态检查来确认。

### 从仿真环境到真实机器人

从仿真环境走到真实机器人，并不是再换一种控制器，而是要处理两个环境之间的差异。训练时可以使用遥操作数据、视频数据或仿真交互数据；真正部署时，机器人会遇到不同的背景、光照、相机位置、物体材质、摩擦系数、传感器噪声和执行器延迟。只要这些差异足够大，训练时学会的动作就可能在现实中失效。

> **实验 9-10 ★★★：RGB 视觉策略的跨环境测试**
>
> [实验目录](../chapter9/rgb-sim2real-grasping/)在本地 GPU 上训练四种结构相同的 RGB 策略：一组只看固定画面，一组只改变背景，一组只改变物体外观，最后一组同时改变背景、外观、光照和噪声。每种策略使用 3 个随机种子；每个种子、每种训练条件使用 4,096 个样本，训练 10 轮，并在两个变化后的测试环境各测试 1,024 个样本，总训练样本为 49,152。验收同时检查训练环境准确率、两个测试环境的平均准确率、不同种子之间的波动，以及固定种子下数据是否完全一致；完整随机化还必须在两个测试环境中都优于固定画面训练。这个实验验证的是“训练时见过更多变化，模型更可能适应新的画面”，不是 SO100 真机成功率；LeRobot、ManiSkill 和 SO100 真机部署仍需单独的硬件和安全条件。[^ch9-6]
...
| 9-10 跨环境视觉 | 3 个种子 × 4 种训练条件 × 4,096 样本 = 49,152 样本 | 固定画面训练环境准确率 93.0%；完整随机化在两个测试环境分别为 76.1% 和 80.2%，均高于固定画面方案的 54.8% 和 56.5% |

画面变化也不是越多越好。变化范围太小，覆盖不了真实环境；变化范围太大，模型又可能学不到稳定的目标。部署前应该先用真实数据校准相机、延迟和关键物理参数，再逐步扩大训练时的变化范围，并把“和训练画面相似的测试”“陌生测试场景”和“真机结果”分开报告。
...
长程规划负责“先做什么”，VLA 负责“当前这一步怎么做”，世界模型帮助判断“做了之后可能怎样”，从仿真环境到真实机器人的过程则要解决训练环境和现实环境之间的差异。它们可以组合在同一个系统里，各自解决不同的问题，并不是互相替代的四种模型。

本节的三个本地 GPU 实验分别回答三个问题：理想控制者能做到什么，逐步检查能否从失败中恢复，训练画面的变化是否有助于适应新环境。真实 XLeRobot、RoboCrew 和 SO100 运行则必须在有硬件、授权和安全证据时单独开启。
