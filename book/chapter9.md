...
评价一条轨迹，就是依次回答三个问题：**事情办成了吗？是以允许的方式办成的吗？办得合适吗？** 三个问题的证据一个比一个软，这决定了各自该由谁来回答——图9-2 把它们组织成三层验证结构。

![图9-2 从环境结果到 LLM Rubric 的三层轨迹验证](images/fig9-2.svg)

**结果验证器回答“事情办成了吗”。** 它读取测试结果、数据库状态和工具返回：Coding Agent 可以运行测试、类型检查和性能基准，替用户办理退款的 Agent 可以查询订单状态和实际退款金额。这类信号来自环境中的真实状态，通常比模型对自己行为的描述可靠，是三层里最该争取的一层。

**过程验证器回答“是以允许的方式办成的吗”。** 结果正确并不代表过程正确：删除失败的测试用例也能让测试通过，口头承诺用户 “我们会在 7 天内退款，请耐心等候” 也可能换来暂时的满意反馈。这一层检查业务规则、权限和动作序列，把“看起来办成了”与“真的按规矩办成了”区分开。政策库、权限表和动作轨迹都是确定的，因此这一层同样可以由代码判定。

**质量验证器回答“办得合适吗”。** 客服是否耐心、是否提供了合规范围内的变通方案，研究报告是否抓住了关键证据，生成文本是否自然简洁，都没有单一的正确答案，只能结合语境判断。这一层可以使用第七章介绍的 LLM-as-a-Judge，但不能只让评委给出一个模糊总分，而要预先定义评价量表（Rubric），要求逐项给分并引用轨迹证据。

三层不是三选一：同一条轨迹要依次过完三层，任何一层不通过都是一次可诊断的失败。分层的意义在于把语言模型的判断压到最小——越靠下的层次越应依赖代码和环境真值，只有难以形式化的部分才交给语言模型。

以客服 Agent 为例，表9-1 把这三层展开成七个可以逐条打分的维度：任务结果属于结果层，规则遵从、隐私边界、承诺—行动一致性属于过程层，表达质量与合规变通属于质量层，事实可靠性则横跨两者——能对上工具返回的部分由代码核对，其余交给 Rubric。这样的拆分比“用户是否满意”更有诊断价值：用户可能因为 Agent 违规退款而满意，也可能因为合规限制而不满，单一满意度无法区分两者。
...
**验证器输出成什么形态，决定了它是不是“学习信号”。** 一个总分只能说明这次做得好不好，说不出该改什么。可供后续学习的评价至少要包含四样东西：任务是成功、部分成功还是失败；每个维度各自的结论；每条结论对应的证据位置（哪一轮对话、哪一次工具调用）；以及失败的类型标签。还要允许验证器在证据不足时拒绝评分——低置信度的案例宁可不进学习集，也好过被当成事实固化下来。有了这四样东西，下一节的四种更新方法才知道该更新知识、提示词、程序还是模型参数。
