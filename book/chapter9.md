...
讨论延迟时还必须先统一起止点。若把对话响应延迟定义为“用户实际说完”到“Agent 播放第一个音节”，在不做抢跑生成的流式级联中，其关键路径近似为（前两项都从用户实际说完的时刻起算）：

**响应延迟 ≈ max(结束点判断，ASR 最终文本就绪) + LLM 产出首个可播报文本块 + TTS 产出首个音频块 + 网络与排队。**

流式 ASR 让转录与结束点等待并行，并缩短最终文本的收尾时间；如果必须等 VAD 结束后才把整段音频送去 ASR，上式的 `max` 就会退化为两段时间相加。LLM 流式输出把关键路径上的“生成完整回复”缩短为“生成首个可播报文本块”，流式 TTS 则把“合成完整波形”缩短为“产出首个音频块”；但 LLM 的 TTFT 和内部 reasoning 仍在关键路径上。因此，不能仅凭“全链路流式化”推导出一个固定的 600-800ms 端到端数字。任何延迟结果都应同时说明起止点、VAD/结束点配置、模型与服务、网络环境以及输入和回复长度；若从“VAD 已确认轮次结束”才开始计时，就不能再与包含 500-800ms 静默等待的数字直接比较。
