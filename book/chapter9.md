...
#### 例子一：基于失败轨迹优化提示词中的转接边界

[第七章](https://github.com/bojieli/ai-agent-book/blob/main/book/chapter7.md)解剖过 τ²-bench 的 telecom 领域。它的 Agent 政策里，关于何时转人工只有两句话：请求超出你的动作范围时才转接，转接前先尽力解决。这两句是原则而不是可执行的规则——它没有说清什么算“超出范围”。

用一个能力较弱的模型跑这套环境，失败模式高度一致：工具报错后反复重试，最后以转人工收场。把这些失败轨迹交给模型，让它自己归纳出若干条可操作的规则追加到政策末尾，再在一批从未参与提炼的任务上重跑，通过率从 12.3% 升到 19.3%，而且没有一条原本通过的任务被改坏。

比这个数字更值得记住的是三点。

**第一，提炼器能学到什么，取决于你给它看什么。** 只把失败摘要和报错文本交给它，它归纳出的是“同一个工具报错就别再调了”；额外告诉它 Agent 与用户各自能调用哪些工具之后，它才归纳出“设备侧的检查属于用户，应当引导用户在自己手机上执行，而不是自己去调”。同样的轨迹、同样的提炼模型，只因为多给了一份工具清单，学到的东西就从“别重试”变成了理解职责边界。

**第二，模型从失败中总结的第一版规则，可能把错误固化下来。** 第一版规则里有两条是“连续失败三次就转人工”“用户两次不给号码就转人工”——模型在轨迹里反复看到转人工，就把它当作了合理的兜底。可在这套评估里，转人工必然判失败。提炼器会把观察到的行为当成应该的行为，因此更新提案必须经过独立验证，而不是提炼出来就直接上线。

**第三，被修好的往往是很朴素的东西。** 基线里最典型的一条轨迹是这样的：Agent 需要用户的电话号码，于是连续五次调用查询工具，把“请您提供您的电话号码”当作参数填了进去——它知道该问用户，却把话说给了工具。规则生效后，它先在对话里问用户要号码，拿到后再查询；随后想检查 SIM 卡状态被拒，就改为引导用户自己重新插拔 SIM 卡，任务通过。

> **实验 9-2 ★★：从 τ²-bench 失败轨迹提炼转接与工具使用规则**
>
> 复用第七章的 τ²-bench telecom 环境。提炼集与迁移集在上游仓库中互不相交，规则提炼过程从未见过迁移集上的任务。
>
> 先用弱模型跑提炼集、收集失败轨迹，再由模型（而不是人工）从这些轨迹归纳规则并追加到原始政策末尾，最后在迁移集上对照原始策略与两个进化版本。三臂只替换策略文件，用户模拟器保持不变。
>
> 除通过率外，还要记录三项与规则直接对应的行为指标：转人工率、误调用户侧工具的次数、空参数调用次数。后两项在进化版本中都下降了七成以上——提升来自规则修好了具体行为，而不是碰巧。

换一个领域，做法是一样的。航空客服 Agent 的典型问题案例是：用户对退票、改签费或行李政策不满，Agent 没有查政策、解释规则或寻找允许的替代方案，就调用 `transfer_to_human`。普通政策争议不需要转接，**用户明确要求人工或出现安全风险时才必须转接**。诊断同样落在“转接边界没写清”上，修法同样是把它变成一条带来源的最小规则。
