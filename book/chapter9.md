...
学习信号说明 Agent 应当改变，但没有说明改变应发生在哪里。事实和经验适合写成知识文档；可以清楚语言化的策略适合写入提示词或 Skill；可以精确执行的流程与约束适合写成程序；感知、语言风格和隐式策略等高维能力则必须进入模型参数。图9-3展示了这四种方式及其关系。
...
#### 案例：DeepSeek Harness 一切皆插件的自我进化

第一章的框架对比表把 DeepSeek Harness（`dsh`）归为 “Agent 自进化框架”[^dsh-2026]。它的底座 Cordis 有一篇同期论文，把这件事的难点讲得比 “模型能不能写出新代码” 更清楚：传统意义上的组合是**静态**的，函数调用、模块导入和类继承都在编译期确定，运行期不再变化；而插件系统和自进化 Harness 需要的是**动态组合**，组件要在运行中被装入、卸下和重新配置[^cordis-2026]。Agent 的每一次自我修改，本质上都是一次动态组合。

论文把动态组合拆成两个正交的维度，这个拆法对本章很有用。**时间可组合性**（temporal composability）问的是：一个组件被移除时，它对共享环境做过的修改能否被完整、安全地撤销——这要求运行时追踪它的每一次资源分配、事件注册和状态变更。**空间可组合性**（spatial composability）问的是：组件之间能否以结构化、可验证的方式声明、发现和解析彼此的依赖，并在依赖变化时协调各自的生命周期。前者关心**改了什么**，后者关心**依赖什么**。

在静态设置下这两件事几乎是免费的：时间维度退化为词法作用域，空间维度退化为模块导入解析。困难全部来自动态——**要撤销的副作用是长期存活、带状态的，其作用范围并不由词法结构划定；要解析的依赖会在运行中出现、消失或改变身份**。这正是 Agent 自我修改的处境：它改的不是一份即将重新编译的源码，而是一个正在服务请求的运行时。

论文也指出了为什么自进化 Harness 是这个问题最尖锐的场景——这类修改持续发生，而且很少有人工监督。缺少时间可组合性，每次自我修改都要整体重启，丢掉进程内积累的全部状态，进行中的任务被反复打断，**更糟的是，一次错误的自我修改可能让本该用于恢复的那个进程本身失效**。缺少空间可组合性，每个模块只能用临时手段自己察觉依赖的出现、消失和改变，而**一次朴素的代码替换可能静默地破坏依赖方，或者引入只在重新加载时才暴露的循环依赖**。

“改错了就重启” 并不是免费的答案。操作系统和容器编排确实提供了粗粒度的替代品：前者给出进程粒度的时间可组合性，后者给出服务粒度的空间可组合性。但重启会丢弃全部进程内状态，重建需要数秒到数分钟，维持可用性还得靠冗余副本；容器级编排则无法表达共享地址空间内组件之间的依赖，并把本可以是本地函数调用的交互变成网络开销。**问题在于粒度错配**：兜底机制作用在进程和容器的边界上，而系统实际组合的层次要细得多。Agent 想在一次任务中途给自己换掉一个重试策略，这个粒度根本不在进程边界上。

Cordis 的思路是把两个原本属于编译期的概念提升到运行时。效果系统本来用于推理 “计算如何修改环境”，被提升为**可撤销效果**：每一次对上下文的变换都携带一个显式的逆操作，由运行时追踪，组件移除时上下文随之恢复。协效果（coeffect）系统本来用于推理 “计算对环境有什么要求”，被提升为**反应式协效果**：组件把自己需要的依赖声明成一份规格，上下文每次变化都按这份规格通知它激活、失活还是无关。论文进一步用一套动态组合演算，把这个性质从单个组件推广到相互交错的组件系统——**可组合性必须是可传递的，单个插件干净并不等于整个系统干净**。

这条线索接回本节前面的讨论，就得到一个更准确的说法：**自我进化的上限，不取决于模型能写出多好的代码，而取决于承载它的系统有多可组合**。论文把 “模型合成可复用工具” 称作组件级自我修改的一个较窄的前身，也就是上一段 Alita 所做的事——能新增，但还谈不上安全地卸下和重新接线。反过来，一个把主循环、工具注册表和上下文压缩策略都焊死在核心里的 Harness，无论模型多强，可修改面都只剩下 Prompt；本节开头列出的操作层、控制层、验证层和架构层，只有在它们本来就是可替换组件时才谈得上被修改。这也是 `dsh` 让模型适配器、工具注册表、会话日志乃至 Agent 主循环本身都成为插件的原因：**不存在一个只能由人类维护的特权内核**。

可组合性解决了 “能不能安全地装卸”，没有解决 “该不该装”。`dsh` 在这上面加了两道人为落差：模型写出插件与插件真正生效是两个动作，前者只做校验和记录，后者才载入运行时；而模型写出的插件只活在进程内存里，重启即消失，**不能被自动提升为正式插件**——要留下来必须另走前面说的 worktree 加 Pull Request 那条更慢的路。它的沙箱也不自称安全边界，文档要求像对待 bash 权限一样对待整套工具集，默认不出现在任何发行组合中。这与本章反复出现的结论一致：**自我修改可以做得又快又可回滚，但真正的保证只能来自被修改者触及不到的那一层**。

最后，进化本身也有代价。运行中的插件会改变模型可见的工具集和提示片段，请求前缀一变，第二章讨论的 KV Cache 就从变化处开始失效。一次自我修改的收益必须扣掉这部分上下文成本才算得准：为一次性任务临时装上一个工具，可能比直接写几行代码更贵。
...
[^dsh-2026]: DeepSeek AI, *DeepSeek Harness: Everything is a Plugin*, 2026. https://github.com/deepseek-ai/deepseek-harness

[^cordis-2026]: Shi, Yifan, Wei Zhang, and Tianyi Cui. *A Programming Paradigm for Spatiotemporal Composability.* 预印本，2026 年 8 月 13 日草稿. https://github.com/cordiverse/paper
...
**发现循环：假设、实验、评估、反馈**。以 Voyager 为代表的 Agent 自我进化系统正是遵循了由假设、实验、评估、反馈构成的发现循环，这也是数百年来沉淀而成的科学方法论。最近，Jeff Dean 等人创办的 Discovery Loop 提出将发现循环推向自动化：提出实验、实现它、评估它、拿到结果，再把结果喂给下一轮[^ch1-discovery-loop]。这本质上就是 Agent 自我进化在科学领域的应用。本章所述的 Agent 自我进化要想避免自说自话、自我评价良好，就必须遵从科学方法论。
...
- **维护搜索多样性**：不应只保留当前得分最高的一条链。备选方案池还需保留若干暂时低分但不同质的分支，避免所有方案收敛成同一个易得分模板。
