...
第一章的框架对比表把 DeepSeek Harness（`dsh`）归为 “Agent 自进化框架”[^dsh-2026]。它值得单独讨论，不是因为多了几个自我修改的工具，而是因为它把前面几段的原则直接写进了架构。

**Agent 能进化到什么程度，先取决于系统有多少部分是可以被替换的。** `dsh` 基于 Cordis 插件框架，模型适配器、工具注册表、会话日志乃至 Agent 主循环本身都是插件，**不存在一个只能由人类维护的特权内核**；扩展的方式不是改动核心代码，而是在插件树旁边挂载一个新插件。如果重试策略、工具路由和上下文压缩都写死在主循环里，Agent 再聪明也只能在 Prompt 上打补丁——本节开头列出的操作层、控制层、验证层和架构层，只有在它们本来就是可替换组件时，才谈得上被修改。这也让前面强调的组件层可观测性有了落点：**可编辑组件都有文件级表示**，最小 diff 才有明确的作用位置。

**卸载必须和挂载一样干净。** 插件在运行期间做的每一次注册都是可撤销的，卸载时自动回收。这把回滚从一次需要记得执行的补偿操作，变成了结构本身的性质：不必依赖模型正确清理自己留下的痕迹，也不必事后追查某个副作用是谁加的。对比 Prompt 的堆叠——追加一句容易，判断哪一句该删很难——可以看出，**可回滚性是设计出来的，不是事后补上的**。

**写出代码和让代码生效必须是两个动作。** `dsh` 把自我修改拆成定义与激活两步：定义只做语法校验并记录源码，什么都不运行，用户在对话中看到的是一张待启动的卡片；激活才把它载入运行时，涉及界面的部分还需要用户批准。这就是前面说的“模型只生成提案”，区别在于这条边界被画进了工具本身——模型可以调用的动词里，“写”和“发布”不是同一个。

**临时生效不等于正式入库。** 模型写出的插件只存在于进程内存中：不产生插件文件，不改动配置，重启即消失，而且**不能被自动提升为正式插件**。要把一次成功的自我修改留下来，必须另走一条更慢的路，也就是前面说的 worktree 加 Pull Request。这道刻意设置的落差防止了偶然一次成功被永久固化：**进化的速度在临时层，进化的资格在正式层**。

**这类能力应当默认关闭。** `dsh` 的沙箱隔离了全局变量，但其文档明确声明这不是安全边界，并要求像对待 bash 权限一样对待整套工具集——它默认不出现在任何发行组合里，必须显式开启。让 Agent 改写自己的运行时，本质上等同于授予它执行任意代码的权限。这与本章反复出现的结论一致：**自我修改可以做得又快又可回滚，但真正的安全保证只能来自被修改者触及不到的那一层**。

自我修改还有一项容易被忽略的代价。正在运行的动态插件可以注册新的模型可见工具和提示片段，而工具集一旦变化，请求前缀也就变了，第二章讨论的 KV Cache 会从第一个发生变化的位置开始失效；`dsh` 因此为每个包的文档单独标注了 “Token effect” 和 “KV Cache effect”。
...
[^dsh-2026]: DeepSeek AI, *DeepSeek Harness: Everything is a Plugin*, 2026. https://github.com/deepseek-ai/deepseek-harness ；插件组合与补丁分层见 `docs/architecture.md`，自引用 Cordis 工具集的生命周期、沙箱语义与信任声明见 `docs/subsystems/extensions.md`、`packages/extensions/README.md` 和 `packages/extensions/tool-cordis/README.md`。
