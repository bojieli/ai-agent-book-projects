...
2026 年 7 月，Induction Labs 公布了 **Photon-1**，把这种思路称为 *imagination model*。它不是在预训练阶段逐像素生成截图，而是把每帧压缩为离散的潜在 token，自回归预测下一状态的潜在表示。预训练不需要动作标签，之后再用少于 3.5 万条 computer-use 轨迹教会模型输出动作格式，并用虚拟机上的在线 RL 优化任务完成。训练过程仅使用 3 万小时的 H200 算力，成本低于 10 万美元。给定一张种子截图和后续动作，Photon-1 能连续“想象”桌面状态；模型另外接了一个图像生成器，只用于把潜表示可视化，并非其推理必需组件。[^ch9-20]
...
> **阅读提示**：本节的核心实验都可以在本地 GPU 上完成：9-8 和 9-9 使用小型、确定性的桌面模拟器，9-10 使用本地 RGB 跨环境测试。它们验证的是控制逻辑和训练方法，不等于真实机械臂已经被致动。XLeRobot、RoboCrew 和 SO100 真机路径均保留为需要单独授权、校准和现场安全措施的扩展实验。

机器人操作比“看图回答问题”难，原因不只是多模态，而是模型必须在不同时间尺度上连续地改变物理世界。一个可用系统至少要回答四个问题：
...
本节用一个统一的控制闭环回答这四个问题，并把长程规划、VLA、世界模型和“仿真环境迁移到现实环境”放在各自正确的位置。

### 从遥操作看硬件与算法边界

如果由人通过遥操作控制，低成本机器人已经能够完成不少视觉反馈为主的抓取、放置和擦拭任务。这说明在本节讨论的任务范围内，硬件提供了一个相当高的能力上限；但它不意味着触觉传感、灵巧手、精细力控和执行器可靠性已经全部解决。硬件上限与自主策略之间的差距，正是下面算法问题的实验入口。

> **实验 9-8 ★：用理想控制者测量任务上限**
>
> [实验目录](../chapter9/xlerobot-teleoperation/)中的本地 GPU 版本在二维桌面环境中批量执行一个理想控制器：随机化物体位置，把每个物体移动到对应目标。正式验收固定 5 个随机种子、4 种物体数量，每个条件 512 个回合，共 10,240 个回合；逐格记录成功率、完成步数、95 分位步数和路径长度，并用同一随机种子重复运行检查可复现性。当前验收是非致动模拟，不把它写成 XLeRobot 真机证据；真实遥操作仍是需要另行具备硬件和安全条件的扩展。[^ch9-1]
...
| 层级 | 核心问题 | 输出 | 典型时间尺度 |
| --- | --- | --- | --- |
| 任务目标 | 人想完成什么 | “整理桌面” | 分钟级 |
| 长程规划 | 先做什么、后做什么 | 子任务和依赖 | 秒到分钟 |
| 基本技能 | 当前要完成哪个状态变化 | “抓起红色杯子” | 约 1—3 秒 |
| VLA / 技能策略 | 这个技能具体怎么动 | 一小段动作或连续轨迹 | 约 1—10 Hz 推理 |
| 低级控制与安全层 | 如何稳定、及时地执行 | 关节或末端控制量 | 约 50—1000 Hz |

这是一种常见的工程分层，不是唯一的模型架构。VLA 可以吸收一部分高层推理，规划器也可以是符号程序、VLM 或优化器。关键是把“任务顺序”和“即时动作”分开，否则高层模型的延迟和底层控制的频率会相互拖累。

### 基线：长程规划驱动基本技能

用户说“把桌面整理干净”时，规划器不应把整句话直接交给动作策略。它先建立对象与状态清单，再生成带前置条件、完成条件和风险约束的技能序列：
...
每项技能完成后都是一个可以检查的节点。若抓取失败，只重试当前技能；若物体位置或用户目标发生变化，只作废受影响的计划后半段，而不是盲目把旧计划跑完。工具调用还必须是一步一事的：动作时间短、工作范围固定、有超时限制，并在执行后立即重新观察。

> **实验 9-9 ★★：桌面操作规划与逐步检查**
>
> [实验目录](../chapter9/gemini-xlerobot-navigation/)保留 RoboCrew 风格的智能体循环，并预留 XLeRobot 执行器接入层，但把任务改为“抓起红色杯子放入托盘，再把黄色纸张放入垃圾盒”。本地 GPU 版本只开放五个职责明确、权限固定的工具：observe_scene、pick、place、verify_state 和 stop。正式验收使用 3 个随机种子、0/0.25/0.5 三档瞬时抓取失败概率，每格 128 个回合，共 3,456 个回合；比较一次性执行、每步重新观察，以及使用短期预测模型选择下一项技能三种方式。实验记录任务成功率、工具调用次数、失败后的恢复次数、动作条件世界模型的训练/测试误差和事件日志。真实运行时，pick 和 place 必须由经过标定、限速并带超时的 XLeRobot 动作原语实现，不能让模型直接输出任意关节角。
...
RT-2 和 OpenVLA 将连续动作离散成 token，再像生成文本一样自回归输出；π₀ 则代表另一条路线，用连续生成方法直接产生平滑轨迹。动作表示的真正分野不是“哪个模型更大”，而是离散动作 token 与连续轨迹生成之间的取舍。[^ch9-15]

大模型推理通常只有约 1—10 Hz，而传统控制器可能需要几十到上千 Hz。动作分块让模型一次生成一小段未来动作，控制线程高频执行，后台同时计算下一块。块越长，动作越平滑、推理延迟越容易被隐藏，但模型在块内看到的新观察越少，突发变化也越难响应。因此动作分块不是免费提速，而是用反应速度换动作平滑。

### 基线的边界：泛化、物理因果与状态陈旧

这套“长程规划 + VLA”基线很实用，但有四个结构性边界：

- **数据边界**：机器人演示远少于互联网文本和图像数据；
- **目标边界**：行为克隆学习“示范者下一步怎么做”，并不显式回答“这个动作会造成什么后果”；
- **具身边界**：不同机器人具有不同自由度、坐标系、夹爪和执行器延迟；
- **闭环边界**：预先生成的一小段动作执行期间，物体可能被移动、遮挡或碰倒。

因此，语言模型知道“杯子”是什么，并不代表它知道摩擦、接触、液体晃动和电源线会怎样改变未来状态。VLA 主要回答“现在应该做什么”，机器人还需要一个模型回答“做了之后会发生什么”。
...
只会描述视频的 VLM、只会生成好看视频的生成器，都不能自动充当可靠的机器人世界模型。V-JEPA 2 代表在潜在状态中预测未来的路线，World-Action Model 则显式把动作与未来观察联系起来；这些模型可以与 VLA 组合，不必替代 VLA。[^ch9-16]

在系统中，世界模型通常有三个用处：

1. **规划前**：比较抓取、推动、等待等候选动作；
2. **执行中**：比较真实观察和预测，发现偏差就缩短动作、停止或重新规划；
3. **训练时**：利用视频、仿真和失败轨迹学习状态变化，减少真机试错。

世界模型的价值不是幻想一个确定答案，而是提供可以比较的后果假设。长期预测会累积误差，视觉上逼真的未来也可能违反接触动力学；因此实际系统仍需要短期预测、实时观察、不确定度估计和硬件安全控制器。生成式世界模型可以用于交互式仿真或可视化，但“视频生成器”和“可控制的世界模型”不能混为一谈。[^ch9-21]

实验 9-9 的“预测式”模式是一个小型、可验证的模型式控制演示，不声称它等价于 V-JEPA 2 或真实机器人 WAM。它只展示关键关系：预测模型可以帮助给候选技能排序，但最终的安全闭环仍依赖真实检查。

### 训练与部署：从仿真环境迁移到现实环境

仿真到现实的迁移不是另一种控制器，而是把训练环境中的能力带到真实硬件时遇到的问题。训练时可以使用遥操作数据、视频数据或仿真交互数据；部署时则必须面对视觉、物理参数、相机位姿、传感器噪声和执行器延迟的差异。

> **实验 9-10 ★★★：RGB 视觉策略的跨环境测试**
>
> [实验目录](../chapter9/rgb-sim2real-grasping/)在本地 GPU 上训练四种相同结构的 RGB 策略：固定画面、只改变背景、只改变物体外观，以及同时改变背景/外观/光照/噪声。每种策略使用 3 个随机种子，每个种子每种条件训练 4,096 个样本、训练 10 轮，并在两个变化后的测试环境各测试 1,024 个样本；总训练样本为 49,152。验收同时检查源环境准确率、两个测试环境的平均准确率、逐种子波动和固定种子下的数据复现，并要求完整随机化在两个测试环境都优于固定画面训练。这个实验验证的是“扩大训练分布可以缓解视觉差距”，不是 SO100 真机成功率；LeRobot、ManiSkill 和 SO100 真机部署作为需要另行具备硬件和安全条件的扩展。[^ch9-6]

本机 Apple M2 Max 的 MPS（苹果 GPU 后端）正式运行摘要如下。这里的“成功”都指模拟器或离线视觉任务的成功，不能解读为真实机械臂成功率：

| 实验 | 覆盖规模 | 关键结果 |
| --- | --- | --- |
| 9-8 专家上限 | 5 个种子 × 4 种物体数量 × 512 回合 = 10,240 回合 | 最差条件成功率 100%，固定种子复跑一致 |
| 9-9 桌面规划 | 3 个种子 × 3 种失败概率 × 3 种执行方式 × 128 回合 = 3,456 回合 | 世界模型测试 MSE 为 0.0124–0.0170；失败概率 0.5 时，开环成功率约 53%，逐步检查和预测式执行均为 100% |
| 9-10 跨环境视觉 | 3 个种子 × 4 种训练条件 × 4,096 样本 = 49,152 样本 | 固定画面训练环境准确率 93.0%；完整随机化在两个测试环境分别为 76.1% 和 80.2%，均高于固定画面基线的 54.8% 和 56.5% |

随机化也不是越多越好。范围太窄覆盖不了现实，范围太宽会让训练目标变得含糊。实际部署前应先用真实数据校准相机、延迟和关键物理参数，再逐步扩大随机化范围，并把仿真中的同分布测试、陌生场景测试和真机结果分开报告。
...
长程规划解决“先做什么”，VLA 解决“当前技能怎么做”，世界模型预测“做了之后会怎样”，仿真到现实的迁移解决“训练能力如何落到真实硬件”。它们不是四种互斥模型，而是运行时架构、预测增强和训练部署三个不同维度。

本节三个本地 GPU 实验分别建立能力参考线、验证逐步规划、测量跨环境泛化；真实 XLeRobot、RoboCrew 和 SO100 运行则必须在有硬件、授权和安全证据时单独开启。
...
[^ch9-21]: Google DeepMind, “Genie 3: A new frontier for world models,” 2025-08-05. https://deepmind.google/blog/genie-3-a-new-frontier-for-world-models/；Zachary Lin et al. *Cosmos World Foundation Model Platform for Physical AI.* arXiv:2501.03575, 2025. https://arxiv.org/abs/2501.03575
