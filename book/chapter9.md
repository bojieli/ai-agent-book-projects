...
τ²-bench 的 telecom 政策里，转人工的规矩写得很简单：超出你能做的范围才转，转之前先尽力。第七章解剖这个环境时，这两句话看着并无不妥。换一个能力弱些的模型跑，问题就出来了——工具一报错它就重试，试到没辙就转人工了事，二十条提炼任务里十九条这样收场。

把这十九条失败轨迹丢给模型，让它自己看出毛病在哪、写几条规矩出来，追加到政策末尾。再拿一批它没见过的任务重跑，通过率从 12.3% 到 19.3%，原来能过的任务一条也没被改坏。

有意思的不在这个数字。

同样十九条轨迹，给提炼器看的材料多一点，它学到的东西就完全不同。只给失败摘要和报错文本，它写出来的是“同一个工具报错就别再调了”；把 Agent 和用户各自能调哪些工具的清单一并给它，它写出来的就变成了“查网络状态、SIM 卡、APN 这些是用户手机上的事，得让用户自己去点，不能自己调”。前者是记住教训，后者是搞清楚了谁该干什么。

模型也会把坏习惯当成经验。第一版规矩里有两条：连续失败三次就转人工，用户两次不给号码就转人工。它在轨迹里见得最多的就是转人工，于是当成了正常兜底。可这套评估里转人工必然判失败，这两条等于把失败写进了规范。提炼出来的东西不能直接上线，得过一道跟提炼者无关的验证。

真正被修好的地方朴素得出人意料。基线里有一条轨迹，Agent 要用户的电话号码，就去调查询工具，参数里填的是“请您提供您的电话号码”，填了五次，报错五次，然后转人工。它知道该问用户，只是把这句话说给了工具听。加了规矩之后，它先在对话里开口问，拿到号码再查；后面想查 SIM 卡状态被系统拒了，就改口让用户自己把卡拔下来重插一次。任务过了。

> **实验 9-2 ★★：从 τ²-bench 失败轨迹提炼转接与工具使用规则**
>
> 沿用第七章的 τ²-bench telecom 环境。提炼用的任务和迁移用的任务在上游仓库里本来就是两套，提炼过程碰不到迁移集。
>
> 先用弱模型跑提炼集，把失败轨迹收下来；规则由模型归纳，不要人工代笔，写好追加到原始政策末尾；再在迁移集上比原始策略和两个进化版本。三臂之间只换策略文件，用户模拟器不动。
>
> 除了通过率，还要盯三个跟规则直接对应的行为：转人工的比例，Agent 越界去调用户侧工具的次数，参数没填就发出的调用次数。后两项在进化版本里都掉了八成——说明提升是规则修好了具体动作换来的。

同样的做法换个领域也成立。航空客服 Agent 常见的问题案例是：用户嫌退票费、改签费或行李政策不合理，Agent 政策也不查、规则也不解释、替代方案也不找，直接调 `transfer_to_human`。普通的政策争执用不着转人工，**用户点名要人工，或者出了安全方面的事，才是必须转的**。诊断落点还是转接边界没写清，改法还是把它变成一条带出处的最小规矩。
