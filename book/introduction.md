...
书里出现的实验数字分两类，读者可以按不同的力度采信。**一类有可复现的出处**：给出了样本量、运行次数或置信区间，或附有配套仓库中的运行记录与原始输出，第七至九章的实验大多属于这一类。**另一类是示意性观察**：来自一次小规模的对照运行，用来说明方向和机制，不足以支撑“某方案比另一方案高多少”的量级结论；这类数字会在实验框里明确标注。第七章会专门讨论为什么区分这两者很重要——一个单次跑出来的百分点差异，往往落在统计噪声之内。
