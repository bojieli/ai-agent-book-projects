...
实验标题里的星级只表达**难度**，不表达**投入**。默认情况下，实验都配有本仓库自带的可运行代码，配好凭证即可跑；三种例外会在标题里显式标出：`[外部仓库]` 表示需要自行 clone 训练框架或评测基准等外部仓库，`[设计方案]` 表示正文只给出架构与验收标准、可运行代码需要读者自己完成，`[需要真机]` 表示必须有真实机械臂等硬件才能执行。每章配套 README 里还有逐项的项目类型表和外部依赖清单。
