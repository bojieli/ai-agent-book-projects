...
#### Uygulama notu: standart ve wire formatı
...
Codex CLI farklı bir Harness kullanır. OpenAI'nin [Skills belgelerine](https://developers.openai.com/codex/skills/) göre başlangıç kataloğu her Skill'in adını, açıklamasını ve yolunu içerir. Güncel açık kaynak kodu bu kataloğu `developer` context parçası, seçilen Skill gövdesini ise `<skill>` ile sarılmış bir user parçası olarak oluşturur ([`fragments.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/fragments.rs), [`extension.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/extension.rs)). Bazı gateway'ler `developer` içeriğini system benzeri bir alanda gösterebilir. Bu nedenle DeepSeek'e atfedilen karşılaştırma tablosu yalnızca belirli istemci sürümlerinin anlık görüntüsü olarak değerlidir; rol, sarmalayıcı ve tur başına yeniden oluşturma Harness ayrıntılarıdır. Önbellek maliyetleri de Claude Code modelini tüm istemcilere aktarmak yerine her runtime için ayrı değerlendirilmelidir.
