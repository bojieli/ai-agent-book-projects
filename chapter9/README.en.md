...
| 9-6 | [Anthropic native Computer Use record](claude-computer-use-native/) + `claude-quickstarts/computer-use-demo/` | 📖 | The pinned source and Dockerfile hash match and Docker is available, but the 2026-08-03 official Anthropic endpoint probe returned HTTP 401 `authentication_error`. The native `computer`-tool arm is externally blocked before a model call and is not claimed complete. |
