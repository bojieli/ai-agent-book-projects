...
この一覧は自ら報告する失敗しか覆っていない。エラーを一切残さない類型も加える必要がある。要求理解と曖昧性処理——ユーザーの要求ではなく Agent が言い換えた要求を作る、あるいは曖昧な指示の一解釈を黙って選ぶ。検証環境のハック——アサーションを書き換える、`skip` を足す、被テストロジックを mock で潰す、走らせてもいないテストを「通った」と述べる。修正の不完全——三つの呼び出し箇所を直して四つ目の動的呼び出し・他言語バインディング・schema を落とし、それでもコンパイルは通る。ユーザーへの情報伝達の誤り——ツール呼び出しも最終状態も正しいのに、返答の金額・状態・日付が違う。非機能的リグレッション——公開 API や schema をマイグレーションなしで変える、検査を通すために検証を削る。いずれも最初の誤りはツール返り値ではなく、**assistant message**——判断、仮定、あるいは尋ねるべきだったのに尋ねなかった一言である。
...
>
> ステップ 1：層別サンプリング。`t3a_failed.md` から失敗軌跡を 10 件抽出し、うち 3 件以上はツールエラーが一度も出ない静かな失敗とする。判定基準は、失敗したツール応答が存在せず、Agent が自ら完了を宣言するかステップ上限に達し、末尾の検証器だけが失敗と判定していることである。
>
