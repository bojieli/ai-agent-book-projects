...
実験 7-17 では、Coding Agent の bad case 24 件で Qwen2.5-7B の LoRA アダプターを学習した。「完了」と「検証を続ける」の固定比較では、未完了タスクの正解が 3/12 から 11/12 に増え、完了済みタスクの保留評価は 8/8 を維持した。自由生成は慎重になりすぎたため、主結果は固定比較であり、本番全体の成功率向上を意味しない。
