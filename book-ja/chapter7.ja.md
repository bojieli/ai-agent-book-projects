...
> **実験 7-17 ★★：「早すぎる完了」bad case から DPO と GRPO へ**
>
> 第6章の失敗を二つの訓練経路につなぐ。`build_preference_data.py` は24件の bad case を軌跡接頭辞の選好対に変換し、元の完了宣言を rejected、先に検証してから結論を出す行動を chosen とする。`train_dpo.py` は Qwen2.5-7B-Instruct の LoRA-DPO 経路を提供し、任意の `train_grpo_optional.py` は見えない受入テストを使う。
>
