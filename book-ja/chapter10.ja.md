...
ここまでの議論はすべて工学の視点でした——一群の Agent にどう協調してタスクを終わらせるか。続く節は**本書の他の部分とは体裁が異なります**。これまでの九章半が答えてきたのは「どう作るか」ですが、この節が答えるのは「いま何が試みられているか」です。現在のマルチ Agent 社会実験に対する概観であり、そのまま真似できる工学的手順ではなく、観察と未解決の問いを提示します。それでも本章に置くのは、これらの実験がまさに前述のコンテキスト共有・協調トポロジー・失敗モードを極端な規模まで押し進めているからであり、しかもそのうち二つ（Stanford AI Town と音声人狼）については本書が再現可能・実行可能な付属プロジェクトを用意しているからです。この節は、前述の工学的結論をはるかに長い時間スケールに置いたときのストレステストとして読んでください。
...
本章は本文の最終章でもあります。第 1 章の Agent = LLM + コンテキスト + ツールから、構築・評価と進化を経て、本章の協調まで、工学側の道筋はひととおり走りきりました。しかし工学そのものでは答えられない問いが二つ残ります。モデルが強くなり続けたとき、今日の Harness はどれだけ残るのか。そして、特定のユーザー・特定の企業に属する知識は「もう一度訓練する」ことで埋められるのか。後記はこの二つを引き受けます。
