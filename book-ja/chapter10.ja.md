...
**コンテキスト非共有**とは、各 Agent が完全に独立したコンテキストと対話履歴を維持し、互いに相手の可視メッセージ、ツール記録、プロバイダー返却状態へ直接アクセスできないことを意味します。これは異なる部署間の協業に似ています。各人が自分の席で独立して働き、共有文書と議事録を通じて情報を交換します。この方式はモジュール性と隔離性に優れ、各 Agent は自身の職責に関連する情報だけに集中できます。新しい Agent を追加しても既存 Agent の内部ロジックを変更する必要はなく、インターフェースとデータ形式を定義するだけで済みます。
...
明確にしておくべきなのは、どちらのアーキテクチャも本物のマルチ Agent システムだということです。違いは協調方式にあります。**コンテキスト共有**は暗黙的な協調に依存し、後続の Agent は前の Agent の可視メッセージ、ツール記録、およびプロトコルが継続を求めるプロバイダー返却状態を継承します。ただし、API が公開しなかった生の思考チェーンを見られるわけではありません。**コンテキスト非共有**は明示的な協調に依存し、Agent 間はファイル、メッセージ、構造化データインターフェースを通じて情報を交換します。
...
| 情報隔離 | 不要（すべての役割が利用可能な軌跡を共有） | 必要（セキュリティ審査が前段の自由形式分析や機微な中間出力を継承すべきでない場合など） |
...
**共有ファイルシステムで状態を取得する**。最も徹底した形態は**軌跡の永続化**（trajectory persistence）です。サブ Agent は実行中、自身の軌跡（ユーザーメッセージ、モデルの可視出力、ツール呼び出しと結果、およびプロトコルが保存を求めるプロバイダー返却状態）を JSONL ログへ追記します。メイン Agent はこのファイルから、直近のツール呼び出し、返された結果、失敗するリトライに嵌まっていないかといった観測可能な実行記録を確認できますが、API が公開しなかった生の思考チェーンは見えません。軌跡は細かな観測を提供しますが、別プロセスの完全なメモリではありません。軌跡は数万 token に達することもあるため、多くの場面では、サブ Agent が `progress.md` を更新する**取り決め済みの進捗ファイル**のほうが実用的です。これは共有ストレージに小さな定型領域を設け、完全なランタイム状態ではなく要約済みの進捗を公開する形です。最終更新時刻を使えば、従来どおりスタック検知も行えます。

軌跡の永続化の価値は監視にとどまりません。第 1 章の「Agent のコンテキスト = 静的プレフィックス + 軌跡」を踏まえると、軌跡を再読み込みして静的プレフィックスを加えることで、Agent の**対話状態とモデル状態**を再構築できます。これはセッション復元の重要な基礎であり、プロバイダー契約が思考状態の返送を求める場合は、それも永続化しなければなりません。しかし、軌跡は完全なランタイムチェックポイントではありません。ファイルやデータベースの内容、ブラウザとツールのセッション、ロックとリース、認証情報と認可状態、実行中のリクエスト、すでに発生したメール・支払い・注文などの外部副作用は軌跡の外にあり、外部サービスの状態は非決定的な場合もあります。

したがって、信頼できる復旧では軌跡をより大きな復旧プロトコルに組み込みます。副作用のある操作には永続的な操作記録と冪等性キーを用意し、外部リソースをチェックポイント化し、再起動時に各操作が未開始・実行中・完了・結果不明のどれかを照合してから、再開・再試行・補償を決めます。対話記録を盲目的に再生すると、メール送信、支払い、注文を重複させるおそれがあります。先行書き込みログ（write-ahead log）の比喩は有用ですが、軌跡は復旧プロトコル内の一つのログにすぎず、外部状態チェックポイントと操作記録を併用しなければ安全な復旧は保証できません。
...
マルチ Agent システムには 2 つの直交する中核的な設計次元があります。コンテキストを共有するか、そして協調トポロジーをどう組織するかです。コンテキスト共有は「継承式」のマルチ Agent 協調です——後続の Agent が前の Agent の完全なコンテキストを継承し、情報はゼロ損失ですがコンテキストの膨張が速い。コンテキスト非共有は完全に独立したマルチ Agent 協調で、精製された移譲パック、ファイルシステム、あるいはメッセージ受け渡しを通じて情報を交換します。協調トポロジーでは、対等パターンは少数の Agent の反復改善に適し、管理者パターンは動的な調度を要する複雑タスクに適し、去中心化パターンは職責が対等で、制御権を Agent の間で自律的に流転させる必要のある場面に適します。これらすべては、トポロジーに依存しない 2 つのインフラの上に架かっており、その設計の青写真はオペレーティングシステムに由来します——Agent がランタイムに対して持つ関係は、ちょうどプロセスがカーネルに対して持つ関係と同じです。静的プレフィックスはプログラム、軌跡は対話状態とモデル状態を担い、LLM は時分割で多重化される CPU です。データプレーンとしての**共有ファイルシステム**は、本質的に Agent 専用ワークスペース・マルチ Agent 共有空間・外部リソース・システム組み込みリソースの 4 種類の領域をマウントした仮想ディレクトリツリーで、Agent 間はファイルパスを受け渡して生成物を交換します。コントロールプレーンとしての**通信・制御メカニズム**は、メッセージ受け渡し・状態照会・実行終了・リソース調度をサポートします。状態照会も同じく 2 つの大きな通信パラダイムの内に収まります。メッセージによる非同期の問答か、あるいは共有ファイルシステムを経たバイパス観測——サブ Agent がリアルタイムに永続化した軌跡ファイルを読むか、双方が事前に取り決めた軽量な進捗ファイルを読むかです。軌跡を読み込めば対話状態とモデル状態を再構築できますが、完全なランタイムチェックポイントではありません。クラッシュ復旧には永続的な操作記録、冪等性キー、外部状態チェックポイント、再起動後の照合も必要であり、再生による副作用の重複を防がなければなりません。メッセージバスはコントロールプレーンのよくある実装で、リアルタイム・非同期・多者のメッセージ協調に適します。組織の境界をまたぐときは、A2A のような標準化された相互運用プロトコルが必要になります。
