...
>
> 上の三つの次元の数字は一度きりの小規模な対照実行によるもので、**示唆的な観察**です。示しているのは方向——組織構造とツール説明の影響が語調やスタイルよりはるかに大きいこと——であって、再現可能な量ではありません。第 7 章の基準に照らせば、この種の単発の差が「どれだけ高いか」を支えるにはサンプル数と反復回数が要ります。読者自身がこの実験を走らせるときは、三つの次元の相対順位が安定するかどうかに注目してください。同じパーセントポイントを再現できるかどうかではありません。
>
>
...
実験 2-8 は、直感を得るための小規模な定性的デモです。この「先に計算して、直接ひと目で確認する」方法がどれほど役立ち、どこに限界があるのかを、筆者らは専用のベンチマークで定量化しました[^ch2-7]（この方法の総称は **コンテキスト蒸留、Context Distillation** であり、Agent ステータスバーはその日常的な形です）。結論は次のとおりです。
...
>
> **詳細なエラー情報**：4 層の内容を含みます——エラーの種類と記述、完全なパラメータの JSON、コールスタック情報、そして的を絞った修正の提案（FileNotFoundError に遭遇したらパスの検証、作業ディレクトリのチェック、絶対パスの使用を提案するなど）です。有効にすると、Agent がエラーのシーンで代替案を見つける成功率が 60% から 95% に向上し、やみくもな再試行から分析的な問題解決へと変わります（これも一度きりの小規模な実行による**示唆的な観察**で、示しているのは「エラー情報を詳しく書く」という方向が有効だということであって、この幅が再現されるということではありません）。
>
