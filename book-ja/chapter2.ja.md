...
>
> **モデルの内部思考過程**：思考の連鎖をサポートするモデル（Qwen3 など）は、ツール呼び出しを生成する前に、まず `<think>` タグ内で思考を行います。ユーザーの意図を分析し、どのツールが適用できるかを評価し、呼び出し順序を計画するのです。この思考過程は Agent の振る舞いをデバッグするのに非常に価値があります。なお、`<think>` はモデルの生の出力ストリームに現れるタグであり、API のレスポンスにそのまま現れるとは限りません。本実験では Ollama に `think=True` を渡しているため、Ollama は返す前にこの思考部分を解析し、独立した `thinking` フィールド（ストリーミング時は `thinking` の差分チャンク）に格納します。そのため `content` には `<think>` タグが現れません。両者は同じ思考内容の表現形式が違うだけで、実験コードはどちらの場合にも対応しています。
>
