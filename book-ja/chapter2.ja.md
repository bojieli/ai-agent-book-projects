...
本節では **OpenAI-compatible Chat Completions** プロトコルを例に、Agent が大規模モデルを呼び出す際のリクエスト構成を詳しく分解します。このプロトコルは OpenAI だけでなく、DeepSeek、Kimi、Qwen、さらに vLLM や Ollama などのオープンソースモデル提供基盤でも広く実装されており、プロバイダーをまたぐ最も一般的な共通インターフェースです。Anthropic や Google などのネイティブ API は、それぞれ異なるメッセージ・ツール構造を持つため、本節では個別に扱いません。

OpenAI には、より新しい Responses API もあります。これは `instructions` と入出力 item を中心にリクエストを構成し、`developer` 指示を利用でき、`previous_response_id` または Conversations によって会話状態を管理できます。ただし、オープンソースモデルと互換サービスへそのまま適用できるよう、本章の例は引き続き移植性の高い Chat Completions を中心とします。以下の system、user、assistant、tool は、実装をまたいで最も一般的な 4 つのロールです。
...
このリクエストには、開発者が書いたルールを含む system メッセージと、ユーザー入力を含む user メッセージの 2 条だけがあり、モデルは assistant メッセージを返します。本章が用いる基本的な Chat Completions パターンでは、**各リクエストをステートレスに構築し、モデルが必要とする情報をクライアントがメッセージリストに含めます**。これはすべての API に共通する制約ではなく、Responses API は前のレスポンスや永続的な Conversation を参照できます。
...
1. **2 回目のリクエストには 1 回目のすべての対話履歴が含まれています**——system メッセージ、user メッセージ、1 回目の assistant の返答（ツール呼び出しを含む）、そして新たに加わった tool の結果です。これは基本的な Chat Completions のステートレスなリクエストパターンを示しており、Agent フレームワークは次のリクエストに必要な履歴を含めなければなりません。
...
> 4. **ストリーミング応答の利点**：このローカル実験では、選択した Qwen モデルとテンプレートが `<think>` token を出力するため、開発者は公開された思考テキスト、ツール呼び出しの判断、結果処理をリアルタイムに観察できます。これはローカルモデル実験の可観測性であって、ホスト型 API に共通する性質ではありません。本番システムでは通常、このような生のテキストをエンドユーザーに公開すべきではありません。
...
> **ReAct ループの実際の事例。**
>
> プロジェクト内のマルチターンのツール呼び出しは、第 1 章で紹介した ReAct の思考-行動-観察ループに従います。ここではその原理を繰り返しません。前節では OpenAI-compatible Chat Completions の JSON フォーマットで、この過程の完全なメッセージ構造を示しました。ローカルにデプロイした実験では、これらの API メッセージはサーバー側（vLLM、Ollama など）によってモデル内部の token フォーマットに変換されます。本実験の `local_llm_serving` プロジェクトでは、通常の API 層では見えない生の入出力 token 流を直接観察できます。
>
> **モデルテンプレートが公開する思考 token**：本実験の Qwen3 は、ツール呼び出しを生成する前に `<think>` タグ内へ分析を出力します。たとえば、適用可能なツールを評価し、呼び出し順序を計画します。これらの token は特定のローカルモデルの生成挙動を研究する助けになりますが、完全かつ忠実な意思決定の説明と仮定したり、セキュリティ監査の証拠にしたりすべきではありません。
>
...
**第一に、なぜ標準的な API フォーマットを使わなければならないかを説明します**。開発者が API を迂回してメッセージを自分で連結し、たとえばツール結果を tool ではなく通常の user メッセージとして渡すと、Chat Template は会話を誤って表現します。Qwen3 のテンプレートでは、1 回のツール呼び出しラウンドで `<think>` タグ内の内容を保持する一方、新しいユーザーターンを検出すると規則に従ってそれを消去する場合があります。ツール結果を user と誤ってマークすれば、意図しないリセットが起き、複数ステップの連続性が損なわれます。

思考状態の返送規則は、モデル間で統一されていません。DeepSeek R1 の公式プロトコルは後続ターンで `reasoning_content` を除去するよう求めますが、別の DeepSeek、Kimi、GLM のモデルやバージョンは、API が返した `reasoning_content` を対応する assistant メッセージへそのまま戻すよう求めます。Claude のネイティブ API は署名付き thinking block を使い、返送範囲を定めています。OpenAI の Responses API は生の思考チェーンを公開しませんが、保持すべき不透明または暗号化された reasoning item と任意の要約を返す場合があります。ステートレスに継続するときは、関連する出力 item をまとめて返す必要があります。一般化できる教訓は「可視の思考を必ず保持する」ことではなく、**思考関連の状態はプロバイダー契約の一部であり、返送の要否と形式は、各モデルと API の最新契約に従う**ということです。不透明な状態を捏造・解析・公開せず、可読な思考テキストも忠実な説明として扱わないでください。
