...
- **モデル返答**（Assistant Messages）：モデルが以前に返した可視の出力には、通常、テキスト内容（`content`、ユーザーへの返答）とツール呼び出しリクエスト（`tool_calls`、Agent が行動を起こす方法）が含まれます。一部の API は、プロバイダー固有の思考状態も返します。オープンソースまたはローカルモデルは `<think>` や `reasoning_content` で思考 token を公開することがありますが、別の API は、そのまま返送すべき不透明・署名付き・暗号化済みの状態、または任意の思考要約だけを返します。OpenAI のホスト型 reasoning モデルは、生の内部思考チェーンを公開しません。Responses API は、不透明または暗号化された reasoning item と任意の要約を返す場合があります。フレームワークは各モデルの契約に従ってこれらの状態を保存・返送すべきですが、生の思考チェーンを汎用的で可読な `reasoning` フィールドと仮定したり、忠実な説明や監査証拠として扱ったりしてはなりません。
...
特定のシステムにおける各コンポーネントの役割を検証する最も直接的な方法は、**アブレーション実験**（Ablation Study）です。医師が診断のときに病因を一つずつ排除していくように、まず A のコンポーネントを取り除いてシステムがなお正常かを見て、次に B のコンポーネントを取り除き、以下同様にして、各コンポーネントの寄与を判断します。実験 1-1 はこの考え方に沿って、上記の 5 つのコンポーネントを体系的にテストしました。ツール定義を取り除くと Agent は行動能力を完全に失い、ツール実行結果が欠けると同じツールを繰り返し呼び出して無限ループに陥ります。本実験で使用したモデルと API では、インターフェースが返し、契約上返送を求める思考状態を破棄すると、前後の意思決定が矛盾し始めました。履歴メッセージがなければ、Agent はタスクを最初からやり直し、完了済みの手順を繰り返します。この結果はテストしたモデルとインターフェースの挙動を示すものであり、「すべての Agent が可読な思考チェーンを公開または返送すべきだ」と一般化することはできません。
...
実験結果は、この実験構成における各コンテキストコンポーネントの役割を示しました。**ツール定義**（Tool Definitions、静的プレフィックスの一部）は Agent の行動能力の基礎であり、それがなければ Agent はいかなるツールも識別・呼び出しできません。**ツール実行結果**（Tool Results）は閉ループ制御の鍵であり、それが欠けると Agent は「盲目的に」実行し、無限ループに陥ります。**プロバイダーが返した思考状態**は、本実験のモデル契約では assistant メッセージとともに保持する必要があり、削除すると複数ステップの意思決定の連続性が損なわれます。ただし、その状態が必ず可読であることや、意思決定の完全な理由を含むことを意味しません。**履歴メッセージ**は冗長な操作を防ぎ、タスク実行の一貫性を保ちます。
...
多通貨の収入集計という具体的な例を通じて、Agent の**軌跡**（trajectory）を理解しましょう。軌跡とは、Agent がタスクを実行する過程で蓄積していくメッセージ履歴、すなわちユーザーメッセージ、モデル返答（可視テキスト、ツール呼び出し、およびプロトコルが保存を求めるプロバイダー返却状態）、ツール実行結果です。LLM を呼び出すたびに、それが受け取る完全なコンテキストは、**静的プレフィックス**（システムプロンプト + ツール定義）と**軌跡**（動的なメッセージ履歴）の 2 つの部分から成ります（図1-3）。これは一つの重要な事実を明らかにします。**Agent のコンテキスト = 静的プレフィックス + 軌跡**。具体的に言えば、静的プレフィックスは前述の 5 つのコンポーネントのうち前の 2 項目に対応し、軌跡は後の 3 項目に対応します。この完全なコンテキストにもとづいて、LLM は次の応答を生成し、それが軌跡に追加されます。

![図1-3 Agent の軌跡——多通貨集計タスクの ReAct ループ](images/fig1-3.svg)

疑似コードを通じて Agent の軌跡の構造を理解しましょう。以下の `reasoning_state` は、プロバイダーの契約に従って保存・返送する状態を表す抽象的なプレースホルダーにすぎません。可読な `reasoning_content`、不透明なブロック、または状態なしの場合があります。タスク分析のコメントは読者向けに著者が付けた要約であり、API が公開した生の思考チェーンではありません。
...
軌跡 = [
  ...
  {role: “assistant” , 
   reasoning_state: “<プロバイダー契約に従って保持、内容は非表示>” ,
   # 著者による要約：すべての通貨を USD に変換する
...
この設計の巧妙さは**コンテキストの累積性**にあります。LLM を呼び出すたびに完全な軌跡を見られるため、今タスクのどの段階にいるのか、以前に何を試したのか、どんな結果を得たのかを理解できます。Agent は軌跡を通じてタスク全体に対する大局的な認識を保ちます。同時に、構造化された軌跡は高いデバッグ性をもたらします。ユーザーメッセージ、可視の返答、ツール呼び出し、ツール実行結果を明確に区別し、観測可能な挙動を再現できるためです。ただし、軌跡はモデル内部の推論を忠実に説明するものではありません。API が思考要約を提供する場合も、それは生の思考チェーンや監査証拠ではなく、要約として扱うべきです。
