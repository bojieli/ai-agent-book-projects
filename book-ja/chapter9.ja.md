...
観察インターフェースは、動的な変化を早く届けて記憶に残すことで「二つのスクリーンショットの間に何が起きたか」に答える。ただし計画コストまでは消えないため、Agent は依然として「スクリーンショット、思考、クリック」を繰り返し、各行動の後に次の手を考え直し得る。OSWorld-Human は、人間並みの正確さでも人間より多くの手順と待ち時間を要し得ることを示す。

人間はデスクトップを予測的に操作する。行動の結果を予測し、観測状態が予測と一致すれば最初の計画をそのまま続け、ずれたときだけ観察と再計画に戻る。これが speculative execution であり、世界モデルによって Agent にも可能になる。**世界モデルは問題のもう半分を解決する**。次状態を予測し、一致すれば続行し、不一致なら再計画または停止する。
...
> **実験 9-8 ★：XLeRobot のテレオペレーションで机を片付ける**
>
...
> **実験 9-9 ★：同じ課題の理想制御上限をシミュレーションで測る**
>
...
> **実験 9-10 ★★：Gemini Robotics-ER 1.5 で実機 XLeRobot を自律制御する**
>
> **目的:** 人間を、机を観察して pick・place・verify の限定スキルを呼び出す Agent に置き換え、実験 9-8 と同じ機体・課題・成功条件を保つ。
>
...
> **実験 9-11 ★★：シミュレーションで三つの自律ループを比較する**
>
