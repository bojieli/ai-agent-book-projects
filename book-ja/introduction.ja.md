...
実験タイトルの星は**難易度**を表すもので、**必要な手間**を表すものではありません。既定では、実験には本リポジトリ同梱の実行可能なコードが付き、認証情報を設定すれば動きます。三つの例外はタイトルに明示します。`[外部リポジトリ]` は訓練フレームワークやベンチマークなど外部リポジトリを自分で clone する必要があること、`[設計のみ]` は本文が構成と受け入れ基準だけを示し、実行可能なコードは読者が書くこと、`[実機必須]` は実物のロボットアームなどのハードウェアがないと実行できないことを示します。各章の付属 README には、実験ごとのプロジェクト種別表と外部依存の一覧もあります。
