...
本書に出てくる実験の数字は二種類あり、読者はそれぞれ異なる強さで受け取ってかまいません。**一つは再現可能な出所を持つもの**——サンプル数、実行回数、信頼区間のいずれかが示されているか、付属リポジトリに実行記録と生の出力が残されているもので、第 7〜9 章の実験の多くはこちらです。**もう一つは示唆的な観察**——一度きりの小規模な対照実行から得られたもので、方向と仕組みを示すためのものであり、「ある案が別の案より何ポイント高い」という量的な結論を支えるには足りません。この種の数字は実験ボックス内で明示的に断ってあります。なぜこの区別が重要かは第 7 章で詳しく論じます——一度の実行で出た数ポイントの差は、しばしば統計的なノイズの内側に収まるからです。
