...
- **変更頻度**：頻繁に変化する能力は、Skill として記述・配布するほうが通常は軽量です。一方、安定した低レイヤの操作は専用ツールにするほうが適しています。ただし、「テキストを変更しやすい」ことは配布形式が軽いという意味であり、変更リスクが低いという意味ではありません。
...
**Skill の変更はコード変更と同じ規律で管理すべきです。** Skill は自然言語の文書であっても、Agent に shell コマンド、サービスのデプロイ、データベース変更、高権限ツールの呼び出しを指示でき、その運用上の影響はプログラム変更と同等になり得ます。したがって、Skill はバージョン管理し、出所・所有者・適用権限を明示し、ピアレビューと自動 lint/schema チェックを受けるべきです。マージ前にサンドボックスで代表タスクと回帰テストを実行し、段階的またはカナリア方式で公開し、直ちに使えるロールバック版を保持します。本番への書き込み、資金、認証情報、その他重大な操作には追加承認を設けます。コード同等のガバナンスは Skill の作成・配布の速さを損なうのではなく、その速さに制御可能な安全境界を与えます。
...
>
>
> ![図4-4 実験 4-4 イベント駆動 Agent アーキテクチャ](images/fig4-4.svg)
>
>
...
>
>
> ![図4-6 実験 4-5 非同期 Agent の割り込みと復帰](images/fig4-6.svg)
>
>
...
ツール設計の面では、粒度のトレードオフ、汎用性の設計、記述の規範などの ACI 原則がすべてのツールに適用されます。MCP プロトコルはツール相互運用の標準を統一し、階層的な組織化、動的なツール発見、Skills がツール過多の課題に応えます。同時に、サードパーティの MCP サーバーは新しい信頼境界を導入するため、接続前の審査と実行時の防御が必要です。実際の操作を指示できる Skill の変更も、コード変更と同様にバージョン管理、レビュー、テスト、段階的公開、ロールバックを受けなければなりません。すべてのツール設計を貫く底線は、パラメータ渡しの忠実性です。モデルが知覚する世界と、ツールが操作する世界との間に体系的な乖離が存在してはなりません。
