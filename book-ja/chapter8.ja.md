...
>
> **挑戦**：専用の問題ではなく、本書全体と Hermes 自身のコードを渡した。「読んだあと、自分を見直し、本当に一つ改善してほしい」。読者から ablation、ステータスバー、記憶の忘却、Reviewer という4つのヒントは与えたが、どれが自分に合うかは Hermes が本文を読んで判断する。
>
> **提案だけで終わらなかった**：全10章を読んだ Hermes は、本の考え方を自分の設計と比較し、まず現在の作業状態をモデル自身に見せる改善を選んだ。そして自分のソースコードを編集し、検証まで実行した。
>
> **拒否も次の教材になる**：新しい Reviewer が変更を確認し、不合格なら意見を元の Hermes に返す。Hermes はコードを読み直し、問題を直して再挑戦した。受理されるまで続いた流れは、**読む → 比べる → 変える → レビュー → 学ぶ → もう一度変える** である。
>
> **面白い点**：本は人だけでなく Agent のアップグレード手引きにもなる。Hermes は原則を理解し、自分の実装へ結びつけ、フィードバックから実際の変更を完成させた。ただし全タスクで性能が上がるかは別の ablation 実験が必要である。完全な記録は [`hermes-self-evolution`](../chapter8/hermes-self-evolution/) にある。
