...
RL の方策は参照回答の token を直接まねるのではなく、モデルが**自分で生成した**回答を報酬で評価します。報酬の計算に参照解答や選好データを使うことは依然として可能です。この信号から学ぶには、少なくとも二つの前提が要ります。出力が検証できること、そして現在の方策がときおり価値ある行動を探索できることです。

第一の前提は**形式のサポート**です。タスクが JSON やツール呼び出しを求めているのにモデルが解析不能なテキストを吐くなら、報酬関数は「成功か失敗か」すら判定できません。ここで SFT は「まず話をなめらかにする」役割を果たせます。少量の示範で形式と基本手順を安定させ、報酬を計算可能にしてから、RL で方策を最適化するのです。これがよく知られた「まず SFT、次に RL」です。

第二の前提はより根本的で、**能力のサポート**です。まず hold-out タスクに対し訓練時に近い温度でサンプリングし、`pass@1` と `pass@k` を測ります。一回の成功確率を $p$ とすると、近似的に独立な $k$ 回のサンプリングで少なくとも一度成功する確率は

$$
\operatorname{pass@}k = 1-(1-p)^k.
$$

`pass@1` が低くても `pass@k` が $k$ とともに明確に上昇するなら、正しい方策はすでにモデルの分布の中にあり、確率質量が小さすぎるだけです。RL も棄却サンプリングも蒸留も、増幅すべき対象を持っています。逆に、妥当な $k$、サンプリング温度、タスク被覆のもとで実測の `pass@k` がなお 0 に近いなら、基盤モデルは成功軌跡をほとんど生成できません。終点の 0/1 報酬しか与えない場合、GRPO の rollout 群は全部 0 になりがちで、群内アドバンテージがそのまま消えます。PPO も「どちらへ動くべきか」を示す正例を見られません。サンプル量を増やし続けても、およそ $1/p$ の速度で偶然の成功を待つだけで、効率はたちまち実用性を失います。

このときはまず何が欠けているのかを問うべきです。ドメイン言語、事実、コードのパターン、長文脈の基礎能力が欠けているなら、まず Mid-training で土台を補います。能力はあるがインタフェースどおりに表現できないなら、まず SFT を使います。部分的には前進できるが終点に届かないなら、検証可能な部分報酬やカリキュラム学習を加えられます。RL が得意なのは、**すでにあるが確率の低い**成功行動を押し上げることであって、全ゼロ報酬から、モデルが学んだことのない知識や能力を無から作り出すことではありません。

重要な境界が一つあります。「必ず先に SFT」が成り立つのは、出力形式や基本行動がまだ確立していないときだけです。実験 8-11 で見るように、Llama-3.2-Vision-11B は厳格な構造化出力の設定では SFT を経ずに直接 RL を行うと失敗します。しかし十分に強く、すでに非ゼロの成功率を持つ基盤モデルなら SFT を飛ばせます。DeepSeek-R1-Zero がその例です。同モデルが後にコールドスタート SFT を加えたのは、主に可読性と言語の一貫性を改善するためであって、RL のためにタスク知識を注入するためではありません。より完全な Mid-training／SFT／RL の選択フローは、後の独立した意思決定の節で述べます。
...
**ポストトレーニングは、モデルがいつ行動するかも形作ります。** Coding モデルを例に取ると、GPT 系と Claude 系はしばしば異なる既定の行動閾値を示します。前者はより多くのリポジトリ情報を読んでから修正しがちで、後者はより少ないファイルで問題箇所を特定し、まず実装してからテストのフィードバックで直しがちです。これは一方のモデルを「慎重」、他方を「直感的」と擬人化する話ではありません。パラメータの中の方策が、「もう一つファイルを読むことの期待価値は、いまのパッチを提出して検証することの期待価値をまだ上回っているか」を見積もっているのです。SFT の示範に、広く調査してから編集する軌跡が繰り返し含まれていれば、モデルは高めの行動閾値を模倣します。RL のプロセス報酬や結果報酬が、素早い特定と早期の検証可能なループへの移行を一貫して評価すれば、確率質量は早く行動する軌跡へ寄っていきます。第 7 章の実験 7-9 は、まったく同一の中立な Coding Harness の中でモデルを差し替え、この差がモデルによって変わることを実際に測っています。つまり Harness が手順を強制しなくても、モデル自身が安定したツール使用の方策を携えているということです。Harness はそれを調整できますが、行動の主たる出所はポストトレーニング後のパラメータにありうるのです。ベンダーが完全なデータと報酬のレシピを公開していない以上、この実験が示せるのはモデル側の行動の差であって、特定の非公開アルゴリズムがそれを引き起こしたと断ずることはできません。
...
Mid-training が主に埋めるのは二種類の欠落です。

- **知識の欠落**：汎用の事前学習が対象言語、金融／医療／法務の領域、企業内部の文書、あるいは特定のコードベースを十分に覆っておらず、モデルが概念や用語すら理解できない。
- **基礎能力の欠落**：目標タスクが、基盤モデルにまだ形成されていない長文脈、コードのパターン、数学的導出、あるいはマルチモーダル表現を要求する。このとき問題は回答の形式ではなく、十分な回数サンプリングしてもモデルがほとんど正解に到達しないことにある。

これは、SFT を知識注入の主な手段と見なすべきでない理由でもあります。SFT はもちろん少量の事実を記憶できますし、Mid-training の後に置いてドメインの質問への答え方を教える使い方もよくあります。しかし少量の QA 対が覆う問い方は限られており、SFT が得意なのは「どう取り出し、どう表現するか」の訓練であって、大規模で相互に関連する生の知識を担うことではありません。逆に、Mid-training がドメインテキストの言語モデル損失を下げたとしても、モデルがユーザーの質問に応じて自動的に知識を取り出すとは限りません。継続事前学習と指示学習の順序やデータの構成が、知識を QA の形でアクセスできるかどうかに大きく影響することは、既存研究でも見出されています[^ch8-31]。堅牢なレシピは通常、Mid-training で知識と能力を吸収する → 小規模な SFT で出力形式を確立する → 成功率が非ゼロになってから RL で成功率と汎化を高める、という順序になります。
...
「事前学習から RL まで：4 段階の全景」の節では三種類の訓練の仕組みを整理しました。この節が与えるのは実務上の診断です。**まず欠けているのが基盤なのか、プロトコルなのか、方策なのかを見極め、「モデルがうまくできない」をすべて RL が必要だという結論に押し込めないでください。**
...
実際の意思決定は、次の順序で進められます。

1. **まず重みを変えずに済む方策を除外する。** プロンプト、ツール、コード上の制約、コンテキスト管理で振る舞いの問題が解けるなら訓練は要りません。事実を頻繁に更新・引用・削除する必要があるなら RAG を優先します。
2. **目標の hold-out セットで能力サポートを測る。** 貪欲な `pass@1` だけを見ず、サンプリング設定を固定したうえで `pass@k`、部分進捗率、形式の解析成功率も見て、失敗原因を人手で監査します。`pass@k` がなお 0 に近く、失敗が知識・基礎能力に集中しているなら、まず Mid-training を行い、再評価してから次の段階を決めます。
3. **SFT はプロトコルを立てるために使い、知識ベースを詰め込むために使わない。** 「できるが要求どおりに出せない」ときは、高品質な示範で JSON schema、ツール呼び出し、用語の使い方、手順、文体を固めます。少量の事実は示範とともにパラメータへ入ってもかまいませんが、大量の事実知識を少数の QA 対に背負わせてはいけません。
4. **探索の余地があるときにだけ RL を載せる。** 現在の方策がすでに採点可能で、ときおり成功する rollout を出せて、報酬がデプロイ目標を忠実に映しているなら、低確率の成功方策を押し上げ、示範の外の経路を探索するのに RL が向いています。`pass@k` が 0 に近いときは、まず Mid-training／SFT を補うか、到達可能なカリキュラムと部分報酬を設計してください。全ゼロの rollout に PPO/GRPO を直接かけても、たいていはサンプリング予算を食いつぶすだけです。

この流れは、どのプロジェクトも三種類の訓練を順に走らせよという要求ではありません。強い基盤モデルはそのまま RL に入れるかもしれませんし、形式が中心のタスクは SFT だけで足りるかもしれません。安定したドメイン知識なら Mid-training だけを行い、既存のアラインメント能力をそのまま再利用することもあります。肝心なのは、各ステップに測定可能な進入条件があることであって、「Mid-training → SFT → RL」を儀式化されたパイプラインとして扱わないことです。
