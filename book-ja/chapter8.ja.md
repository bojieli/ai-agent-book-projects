...
実験 8-8 と 8-9 も実際のモデル/API 呼び出しで実行した。8-8 では、外部の安全検査がモデルの危険な候補を正しく拒否した。8-9 の初回実行は検出 0/8、正常文への誤判定 7/8 だったが、モデル外の候補フィルタと決定的なフォールバックを加えた最終実行では検出 8/8、誤判定 0/8、21 候補を 8 ルールに統合できた。
