...
> **実験 8-6 ★★★：Hermes に本書を読ませ、レビュー駆動の自己更新を完了させる**
>
> **実験目標**：本章のループを実在する Agent リポジトリへ適用する。固定した Hermes を clone し、英語版全10章を読ませ、読者が挙げた4つの欠落――製品レベルの ablation 基盤、モデル可視の Agent Status Bar、永続記憶の忘却・統合、独立実行証拠を用いる汎用 Proposer–Reviewer ループ――を監査させる。4項目は Prompt で与えたため、測るのは監査、候補生成、修正であり、自律的な問題発見ではない。
>
> **実行とループの完了**：2026年8月2日、commit `85c8956e` の Hermes は OpenRouter 経由で `openai/gpt-5.6-luna` を使い、任意有効化の `<agent_status>` を提案した。proposer の会話履歴を持たない新しい Hermes reviewer が候補を検査し、各 `VERDICT: REJECT` を元の Hermes セッションへ返した。Hermes は自分の checkout を修正し、新しい reviewer が再検査した。5回の終局拒否が cache、tool loop、境界、再起動後の永続化、retry の冪等性、既存 memory/plugin sidecar との合成の欠陥を発見し、6回目が `VERDICT: ACCEPT` を返した。
>
> **結果の境界**：受理版は **新規8件と既存36件のテスト**、compile、空白、credential scan、clean clone への patch 適用を通過した。これは「読む → 自己監査 → 自己修正 → 拒否 → フィードバック修正 → 独立受理」を完了しており、途中結果ではない。ただし patch は upstream 未統合で downstream ablation も未実施なので、status bar がタスク成功率を高めるとは主張しない。runner、9回の proposer 対話、6回の acceptance review、report、patch、credential-free manifest は [`hermes-self-evolution`](../chapter8/hermes-self-evolution/) にある。
