...
# Usage: ./build_epub.sh [all|zh-CN|zh-TW|en|es|id|ru|ta|vi|tr|ko|ja|ar]
...
case "$SELECTION" in
    all|zh-CN|zh-TW|en|es|id|ru|ta|vi|tr|ko|ja|ar) ;;
    *)
        echo "Usage: $0 [all|zh-CN|zh-TW|en|es|id|ru|ta|vi|tr|ko|ja|ar]" >&2
        exit 2
        ;;
esac
...
build_edition() {
    ...
    case "$language" in
        ...
        id)
            ...
            pdf="AI-Agents-in-Depth-Bojie-Li-v1.3-id.pdf"
            output="AI-Agents-in-Depth-Bojie-Li-v1.3-id.epub"
...
if [ "$SELECTION" = "all" ]; then
    for language in zh-CN zh-TW en es id ru ta vi tr ko; do
