...
- **Ответы модели** (Assistant Messages): видимый вывод модели обычно содержит текст (`content`, ответ пользователю) и запросы на вызов инструментов (`tool_calls`, действия агента). Некоторые API также возвращают состояние рассуждений, специфичное для поставщика. Открытые или локальные модели могут раскрывать токены рассуждений через `<think>` или `reasoning_content`; другие API возвращают лишь непрозрачное, подписанное или зашифрованное состояние, которое нужно передавать обратно без изменений, либо необязательное резюме рассуждений. Размещённые у OpenAI reasoning-модели не раскрывают исходную внутреннюю цепочку рассуждений; Responses API может возвращать непрозрачные или зашифрованные reasoning item и необязательные резюме. Фреймворк должен сохранять и возвращать это состояние согласно контракту модели, но не должен считать исходную цепочку рассуждений универсальным читаемым полем `reasoning` или использовать её как достоверное объяснение либо аудиторское свидетельство.
...
Чтобы измерить роль каждого компонента в конкретной системе, самый прямой способ — **абляционное исследование** (Ablation Study): поочерёдно убирать компоненты и наблюдать результат. Эксперимент 1-1 систематически протестировал пять описанных выше компонентов. Без определений инструментов агент полностью теряет способность действовать; без результатов инструментов он повторяет один и тот же вызов и попадает в бесконечный цикл. Для модели и API этого эксперимента удаление возвращённого интерфейсом состояния рассуждений, которое контракт требовал передавать обратно, привело к противоречиям между последовательными решениями. Без истории сообщений агент начинает задачу заново и повторяет завершённые шаги. Этот результат описывает поведение конкретной модели и интерфейса; из него нельзя заключить, что каждый агент обязан раскрывать или возвращать читаемую цепочку рассуждений.
...
Результаты показывают роль компонентов в этой экспериментальной конфигурации. **Определения инструментов** — основа способности агента действовать, а **результаты выполнения инструментов** замыкают контур обратной связи. **Возвращённое поставщиком состояние рассуждений** по контракту использованной модели должно сохраняться вместе с сообщениями assistant; его удаление нарушает связность многошаговых решений. Однако это не означает, что состояние обязательно читаемо или содержит полные причины решения. **История сообщений** предотвращает лишние операции и поддерживает непрерывность выполнения задачи.
...
Давайте разберём **траекторию** (trajectory) агента на примере сведения доходов в нескольких валютах. Траектория — это накапливаемая история сообщений: сообщения пользователя, видимый вывод модели, вызовы инструментов, состояние поставщика, которое протокол требует сохранять, и результаты инструментов. При каждом вызове LLM полный контекст состоит из **статического префикса** (системный промпт + определения инструментов) и **траектории** (динамической истории сообщений) (рис. 1-3). Отсюда следует ключевой факт: **контекст агента = статический префикс + траектория**. Статический префикс соответствует первым двум компонентам, а траектория — последним трём. Ответ LLM добавляется в траекторию для следующего вызова.

![Рис. 1-3 Траектория агента — цикл ReAct в задаче сведения доходов в нескольких валютах](images/fig1-3.svg)

Давайте разберём структуру траектории агента на псевдокоде. `reasoning_state` ниже — лишь абстрактная заглушка состояния, которое сохраняется и возвращается по контракту поставщика: это может быть читаемый `reasoning_content`, непрозрачный блок или вообще отсутствующее поле. Комментарии с анализом задачи добавлены автором для читателя и не являются исходной цепочкой рассуждений, раскрытой API.
...
траектория = [
  ...
  {role: “assistant” , 
   reasoning_state: “<сохранено по контракту поставщика; содержимое не показано>” ,
   # Резюме автора: перевести все валюты в USD
  ...
  {role: “assistant” ,
   reasoning_state: “<сохранено по контракту поставщика; содержимое не показано>” ,
   # Резюме автора: свести и рассчитать результаты конвертации
   content: “” ,
   tool_calls: [
  ...
  {role: “assistant” ,
   reasoning_state: “<сохранено по контракту поставщика; содержимое не показано>” ,
   # Резюме автора: расчёты завершены, подвести итог
...
Изящество такой конструкции — в **накопительности контекста**. При каждом вызове LLM видит полную траекторию и понимает текущий этап, прежние попытки и результаты. Структура траектории также упрощает отладку: сообщения пользователя, видимые ответы, вызовы инструментов и их результаты чётко разграничены, поэтому наблюдаемое поведение можно воспроизвести. Но траектория не является достоверным описанием внутреннего вывода модели; даже предоставленное API резюме рассуждений следует считать резюме, а не исходной цепочкой рассуждений или аудиторским свидетельством.
