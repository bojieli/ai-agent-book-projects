...
Начнём с того, что целиком разберём одну настоящую задачу из домена telecom в τ²-bench. τ²-bench — открытый проект Sierra, и в сопутствующий репозиторий он намеренно не включён; склонируйте его в `chapter7/tau2-bench` командой из `chapter7/tau2-bench-eval/README.md`, после чего файл задач будет лежать по пути `data/tau2/domains/telecom/tasks_small.json`.
