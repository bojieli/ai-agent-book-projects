...
Этот перечень охватывает только те отказы, которые заявляют о себе сами. Нужно добавить категории, не оставляющие ошибок: понимание требований и работа с неоднозначностью, когда Агент строит то, что сам же пересказал, а не то, о чём просили, или молча выбирает одно прочтение неоднозначного запроса; взлом среды проверки, когда правится утверждение, добавляется `skip`, тестируемая логика подменяется моками или объявляется пройденным тест, который не запускался; неполная правка, когда обновлены три места вызова, а четвёртое — динамический вызов, привязка на другом языке, schema — пропущено, и всё равно компилируется; неверная информация, сообщённая пользователю, когда каждый вызов инструмента и финальное состояние верны, а сумма, статус или дата в ответе — нет; и нефункциональные регрессии, когда публичный API или schema меняются без миграции либо валидация удаляется, чтобы проверка прошла. Во всех этих случаях первая ошибка — не возврат инструмента, а **assistant message**: суждение, допущение или вопрос, который следовало задать и не задали.
...
>
> Шаг 1: стратифицированная выборка. Возьмите десять неудачных трасс из `t3a_failed.md`, из них не менее трёх должны быть тихими отказами без единой ошибки инструмента: признак — ни один вызов не вернул ошибку, Агент либо объявил задачу выполненной, либо исчерпал шаги, и только финальный вердикт валидатора фиксирует провал.
>
