...
Глава начинается с обзора пяти категорий инструментов, затем рассматривает общие принципы их проектирования и то, как протокол MCP унифицирует экосистему. На этой основе иерархическая организация, динамическое обнаружение и Skills решают задачу выбора. Далее подробно разбираются активно вызываемые агентом инструменты восприятия, выполнения и сотрудничества, после чего рассматриваются событийно-ориентированная асинхронная архитектура, инструменты срабатывания событий и коммуникации с пользователем. Завершает главу проактивное обнаружение среди сотен и тысяч инструментов.
...
Обновлённая схема безопасности использует изоляцию процесса для низкорисковых агентов, контейнеры или microVM для недоверенных входных данных и квоты ресурсов на каждом уровне. Лёгкий Sidecar проверяет структурированные поля вызова как шлюз перед выполнением; после повторных отказов нужен автоматический выключатель и решение пользователя. Неидемпотентные операции выполняются в два этапа: предварительная проверка и подтверждение.
...
![Рис. 4-2 Архитектура событийно-управляемого асинхронного агента](images/fig4-2.svg)

### Реализация событийных механизмов в OpenClaw

В обновлённом тексте уточняется: Hooks происходят из внутреннего жизненного цикла OpenClaw, а Cron и Heartbeat управляются временем. Для внешней почты и API-вызовов нужен немедленный входной канал вроде Channel в PineClaw.
...
![Рис. 4-3 Три стратегии асинхронной обработки событий](images/fig4-3.svg)
...
> **Эксперимент 4-5 ★★★: событийно-ориентированный агент обработки писем**
>
>
> ![Рис. 4-4 Эксперимент 4-5. Архитектура событийно-ориентированного агента](images/fig4-4.svg)
>
>
...
![Рис. 4-5 Парадигма синхронного обучения и реальность асинхронного развёртывания](images/fig4-5.svg)
...
![Рис. 4-7 Иерархическое сопоставление инструментов (двухуровневый семантический поиск: сервер → инструмент)](images/fig4-7.svg)
...
![Рис. 4-8 Оптимизация KV Cache при динамической загрузке инструментов](images/fig4-8.svg)
...
![Рис. 4-9 Структура контекста после динамического обнаружения: схемы инструментов рассеяны по всей траектории](images/fig4-9.svg)
...
> **Эксперимент 4-7 ★★★: проактивное обнаружение инструментов**
>
...
Семь экспериментов последовательно продвигаются от основ к архитектуре: эксперименты 4-1–4-4 строят три базовых набора инструментов — для восприятия, выполнения и сотрудничества; эксперимент 4-5 вводит событийную модель на примере агента обработки почты; эксперимент 4-6 реализует параллельное выполнение, восстановление после прерывания и управление состоянием; эксперимент 4-7 проверяет ценность активного обнаружения инструментов при большой библиотеке инструментов. Границы этой главы охватывают описание, обнаружение и безопасное использование **существующих инструментов**; в главе 8 будет рассмотрено, как агент на основании неудач и повторяющихся операций определяет, когда следует создать, изменить, повторно проверить или вывести инструмент из эксплуатации.
