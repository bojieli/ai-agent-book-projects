...
Политика RL не имитирует напрямую токены эталонного ответа, а оценивает наградой ответы, которые модель **порождает сама**; при этом сам расчёт награды по-прежнему может опираться на эталонные ответы или данные о предпочтениях. Чтобы учиться на таком сигнале, нужно выполнить как минимум два условия: вывод должен поддаваться проверке, а текущая политика должна хотя бы изредка нащупывать ценное поведение.

Первое условие — **поддержка формата**. Если задача требует JSON или вызова инструмента, а модель выдаёт неразбираемый текст, функция награды не может определить даже «успех это или провал». Здесь SFT берёт на себя роль «сначала научить говорить внятно»: небольшое число демонстраций стабилизирует формат и базовую процедуру, награда становится вычислимой, и уже после этого RL оптимизирует политику. Это и есть привычная схема «сначала SFT, потом RL».

Второе условие более фундаментально — это **поддержка способности**. Сначала на отложенных задачах выполните выборку при температуре, близкой к обучающей, и измерьте `pass@1` и `pass@k`. Если вероятность успеха одной попытки равна $p$, то при приближённо независимой выборке вероятность хотя бы одного успеха за $k$ попыток равна

$$
\operatorname{pass@}k = 1-(1-p)^k.
$$

Если `pass@1` низок, но `pass@k` заметно растёт с ростом $k$, значит верная стратегия уже присутствует в распределении модели, просто её вероятностная масса слишком мала; тогда и RL, и rejection sampling, и дистилляции есть что усиливать. И наоборот, если при разумных $k$, температуре выборки и покрытии задач измеренный `pass@k` по-прежнему близок к нулю, базовая модель практически не способна породить успешную траекторию. При одной лишь терминальной награде 0/1 группа rollout'ов в GRPO с большой вероятностью окажется полностью нулевой, и внутригрупповое преимущество попросту исчезнет; PPO тоже не увидит положительного примера, показывающего, «куда следует двигаться». Дальнейшее увеличение объёма выборки означает лишь ожидание случайного успеха со скоростью порядка $1/p$, и эффективность быстро теряет практический смысл.

В этот момент следует спросить, чего именно не хватает. Если не хватает языка предметной области, фактов, кодовых паттернов или базовой способности к длинному контексту — сначала укрепите фундамент с помощью Mid-training. Если способность есть, но модель не умеет выразить её в требуемом интерфейсе — начните с SFT. Если модель продвигается частично, но не доходит до конца — добавьте проверяемые частичные награды или обучение по учебному плану. RL хорош в том, чтобы поднимать вероятность **уже существующего, но маловероятного** успешного поведения, и плох в том, чтобы из сплошь нулевой награды создавать с нуля знания и способности, которым модель никогда не училась.

Важная граница: правило «сначала обязательно SFT» справедливо лишь тогда, когда формат вывода или базовое поведение ещё не сформированы. В эксперименте 8-11 видно, что Llama-3.2-Vision-11B при строгих требованиях к структурированному выводу проваливается, если сразу перейти к RL без SFT; но достаточно сильная базовая модель с ненулевой долей успехов может обойтись без SFT — именно таков случай DeepSeek-R1-Zero. Добавленный позже холодный старт на SFT улучшал главным образом читаемость и языковую согласованность, а не внедрял знания о задаче ради RL. Более полная процедура выбора между Mid-training, SFT и RL приведена ниже, в отдельном разделе о принятии решения.
...
Mid-training закрывает главным образом два вида пробелов:

- **Пробелы в знаниях**: общее предобучение недостаточно охватило целевой язык, финансовую, медицинскую или юридическую предметную область, внутренние документы компании или определённый класс кодовых баз, так что модель не понимает даже понятий и терминологии.
- **Пробелы базовых способностей**: целевая задача требует длинного контекста, кодовых паттернов, математических выкладок или кросс-модальных представлений, которые базовая модель ещё не сформировала. Дело здесь не в формате ответа: даже при достаточно большом числе выборок модель почти никогда не получает верное решение.

Отсюда понятно, почему SFT не стоит считать основным средством внедрения знаний. SFT, конечно, способен запомнить небольшое число фактов и часто ставится после Mid-training, чтобы научить модель отвечать на предметные вопросы; но малое число пар «вопрос — ответ» покрывает лишь ограниченный набор формулировок, и SFT лучше обучает тому, «как достать и как выразить», чем несёт крупный, взаимосвязанный массив исходных знаний. Верно и обратное: снижение языковой ошибки на предметных текстах в ходе Mid-training не гарантирует, что модель сама извлечёт нужное знание в ответ на вопрос пользователя. Исследования показывают, что порядок продолженного предобучения и обучения на инструкциях, а также организация данных существенно влияют на то, доступно ли знание в форме вопросов и ответов[^ch8-31]. Надёжный рецепт обычно таков: Mid-training впитывает знания и способности → небольшой SFT задаёт формат вывода → когда доля успехов становится ненулевой, RL повышает успешность и обобщение.
...
Крупномасштабный LLM RL обычно порождает rollout'ы движком вывода вроде vLLM/SGLang, а затем пересчитывает log probability и градиенты обучающим движком вроде FSDP/Megatron. Даже когда обе стороны загружают одни и те же веса, различия в точности вычислений, порядке редукции, схеме тензорного параллелизма, размере батча, KV cache и слитых ядрах способны дать чуть разные log probability для одного и того же токена. В итоге $\rho_t$, который до обновления должен равняться единице, уже от неё отклоняется: формально система синхронизировала веса, а численно превратила on-policy обучение в off-policy. Контролируемые эксперименты показали, что даже изолированное крошечное расхождение обучения и вывода на уровне токена способно обрушить обучение[^ch8-33].

Чувствительность порождается цепочкой усиления: **малая ошибка log probability → отклонение экспоненцированного отношения вероятностей → накопление на длинном префиксе → изменение clipping и взвешивания по преимуществу → изменение направления градиента и эффективного числа примеров**. Например, если у 4 000 токенов log ratio смещён на $10^{-3}$ в одну сторону, отношение на уровне траектории накопится до $e^4\approx54.6$; реальные ошибки не обязаны быть однозначными по знаку, но пример показывает, почему длинные последовательности усиливают ошибку, «маленькую на каждом токене». Крошечная разница вероятностей на ранних токенах способна к тому же изменить сам выбранный токен, и тогда вся последующая траектория состояний расходится. Проявляется это не только как «на один и тот же промпт иногда получаются разные ответы»: возможны всплески отношения важности, массовое отсечение токенов, скачки градиента или длины ответа, а следом совместный обвал награды и энтропии. Замечено и то, что изменение размера батча меняет порядок редукции и нарушает численную batch invariance, превращая номинально on-policy RL в неявный off-policy; помогают либо согласованная численная схема выборки и обучения, либо явная off-policy коррекция[^ch8-34].

Инженерно это следует считать ключевой проблемой, а не обычным шумом плавающей точки:

- **До любого обновления параметров** сравните на одной и той же партии траекторий token log probability sampler'а и trainer'а, отслеживая среднее, квантили, максимум $\rho_t$, приближённый KL и долю отсечённых токенов; это самый прямой юнит-тест на on-policy.
- Синхронизировать нужно не только веса, но и LoRA-адаптеры, токенизатор, chat template, ревизию модели и конфигурацию позиционного кодирования; rollout должен сохранять behavior log probability момента генерации, а не подменять её текущей моделью задним числом.
- По возможности выравнивайте точность, схему параллелизма и ключевые вычислительные ядра между выборкой и обучением; если это недостижимо, честно считайте расхождение off-policy, применяйте коррекцию важности и следите за эффективным числом примеров, а не полагайтесь на то, что PPO clipping всё исправит сам.
- Держите rollout'ы свежими, ограничивайте число проходов обновления по одной партии данных и асинхронную staleness. Переиспользование старых данных покупает пропускную способность, но должно быть измеренным компромиссом между смещением и эффективностью, а не бесплатным ускорением.
