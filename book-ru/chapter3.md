...
TF-IDF основан на простой интуиции: чем выше частота, с которой слово встречается в документе (TF, частота термина, Term Frequency), и чем меньше документов во всей коллекции содержат это слово (чем ниже документная частота, или DF, Document Frequency), — то есть чем выше его обратная документная частота (IDF, Inverse Document Frequency), — тем важнее это слово. В примере выше для "модели" `df/N = 60%` и значение IDF низкое; для "дистилляции" `df/N = 3%` и значение IDF высокое, поэтому вклад "дистилляции" в ранжирование намного больше, чем у "модели". Однако TF-IDF не учитывает длину документа (длинные документы естественным образом имеют более высокую частоту слов), а рост частоты термина считается линейным (действительно ли слово, встретившееся 10 раз, вдвое важнее слова, встретившегося 5 раз?). BM25 вводит два ключевых параметра для исправления этих проблем. `k1` управляет "насыщением" частоты термина: интуитивно понятно, что если статья упоминает "дистилляцию" 20 раз против 10 раз, её релевантность запросу о "дистилляции" на самом деле не отличается вдвое. `k1` заставляет вклад частоты термина постепенно выходить на плато по мере её роста, не позволяя длинным документам несправедливо выигрывать за счёт накопления повторов слов; `b` же управляет нормализацией по длине документа, позволяя алгоритму более справедливо обрабатывать документы разной длины. Это делает BM25 более устойчивой и эффективной функцией ранжирования, которая до сих пор остаётся незаменимым ключевым компонентом в крупнейших поисковых системах.
