...
#### Примечание о реализации: стандарт и wire-формат
...
В Codex CLI используется другой Harness. В [документации OpenAI по Skills](https://developers.openai.com/codex/skills/) сказано, что исходный каталог содержит имя, описание и путь каждого Skill. Текущий открытый исходный код представляет этот каталог как фрагмент контекста `developer`, а тело выбранного Skill — как user-фрагмент, обёрнутый в `<skill>` ([`fragments.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/fragments.rs), [`extension.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/extension.rs)). Некоторые шлюзы могут показывать содержимое `developer` в области, похожей на system. Поэтому таблица сравнения, приписываемая DeepSeek, полезна лишь как снимок отдельных версий клиентов; роль, обёртка и пересборка на каждом ходу — детали Harness. Затраты на кэш также следует оценивать для каждой среды выполнения, а не переносить модель Claude Code на все клиенты.

Именно такая двухуровневая схема — небольшой постоянно доступный каталог и полные инструкции, загружаемые по требованию, — позволяет Skills сочетать обнаруживаемость с экономным расходованием контекста.
...
Эксперимент 2-7 — небольшая качественная демонстрация, дающая интуитивное представление. Чтобы количественно оценить пользу и границы подхода «заранее посчитать и сразу посмотреть», автор и соавторы использовали специальный бенчмарк[^ch2-7] (единое название подхода — **дистилляция контекста, Context Distillation**; строка состояния агента — его повседневная форма). Выводы:
