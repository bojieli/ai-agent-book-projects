...
В этом разделе мы подробно разберём структуру запроса на примере протокола **OpenAI-compatible Chat Completions**. Его реализуют не только OpenAI, но и DeepSeek, Kimi, Qwen, а также системы обслуживания открытых моделей вроде vLLM и Ollama, поэтому это самый распространённый общий интерфейс между поставщиками. Нативные API Anthropic, Google и других компаний имеют собственную структуру сообщений и инструментов и здесь отдельно не рассматриваются.

У OpenAI также есть более новый Responses API. Он организует запрос вокруг `instructions` и входных/выходных item, поддерживает инструкции `developer` и может управлять состоянием диалога через `previous_response_id` или Conversations. Однако ради прямой переносимости примеров на открытые модели и совместимые сервисы основным интерфейсом главы остаётся Chat Completions; system, user, assistant и tool — четыре роли, наиболее широко поддерживаемые разными реализациями.
...
Этот запрос содержит всего два сообщения: system с правилами разработчика и user с вводом пользователя. Модель возвращает сообщение assistant. В базовом режиме Chat Completions, используемом в этой главе, **каждый запрос строится без сохранения состояния, поэтому клиент должен передать нужную модели информацию в списке сообщений**. Это не универсальное ограничение всех API: Responses может ссылаться на предыдущий ответ или постоянный Conversation.
...
1. **Второй запрос содержит всю историю диалога первого запроса** — сообщение system, сообщение user, первый ответ assistant (включающий вызов инструментов), а также новые результаты tool. Это базовый режим запросов Chat Completions без сохранения состояния: фреймворк агента должен включить необходимую историю в следующий запрос.
...
> 4. **Преимущества потокового ответа**: в этом локальном эксперименте выбранная модель Qwen и её шаблон выводят токены `<think>`, поэтому разработчик может в реальном времени наблюдать раскрытый текст рассуждений, решения о вызове инструментов и обработку результатов. Это экспериментальная наблюдаемость локальной модели, а не общее свойство размещённых API; в производственной системе такой сырой текст обычно не следует показывать конечным пользователям.
...
>
> Многораундовый вызов инструментов в проекте следует циклу «размышление — действие — наблюдение» ReAct, описанному в первой главе. В предыдущем разделе полная структура сообщений была показана в JSON-формате OpenAI-compatible Chat Completions. При локальном развёртывании сервер (например, vLLM или Ollama) преобразует эти сообщения API во внутренний формат токенов модели. Проект `local_llm_serving` позволяет наблюдать исходный поток входных и выходных токенов, обычно невидимый на уровне API.
>
> **Токены рассуждений, раскрытые шаблоном модели**: в этом эксперименте Qwen3 перед вызовом инструментов выводит внутри `<think>` анализ — например, оценивает подходящие инструменты и планирует порядок вызовов. Эти токены помогают изучать генеративное поведение конкретной локальной модели, но их нельзя считать полным и достоверным объяснением решения или использовать как свидетельство для аудита безопасности.
>
...
**Во-первых, оно объясняет, почему нужно использовать стандартный формат API**. Если разработчик обходит API и самостоятельно склеивает сообщения — например, передаёт результат инструмента как user вместо tool, — Chat Template неверно представляет диалог. Шаблон Qwen3 может сохранять содержимое `<think>` в рамках раунда вызова инструментов и очищать его по своим правилам при обнаружении нового пользовательского хода. Ошибочная маркировка результата инструмента как user способна вызвать несвоевременный сброс и нарушить связность многошаговой задачи.

Правила возврата состояния рассуждений не унифицированы между моделями. Официальный протокол DeepSeek R1 требует удалять `reasoning_content` в последующих раундах; другие модели или версии DeepSeek, Kimi и GLM требуют без изменений помещать возвращённый `reasoning_content` в соответствующее сообщение assistant. Нативный API Claude использует подписанные thinking block и задаёт область их возврата. Responses API OpenAI не раскрывает исходную цепочку рассуждений, но может вернуть непрозрачные или зашифрованные reasoning item, которые нужно сохранить, а также необязательное резюме; при продолжении без серверного состояния связанные выходные item следует вернуть вместе. Общий вывод не в том, что «видимые размышления нужно всегда сохранять», а в том, что **состояние, связанное с рассуждениями, является частью контракта поставщика, и его возврат должен следовать актуальному контракту конкретной модели и API**. Не следует придумывать, разбирать или раскрывать непрозрачное состояние, а читаемый текст рассуждений нельзя считать достоверным объяснением.
