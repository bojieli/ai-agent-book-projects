...
>
> **Внутренний процесс размышления модели**: модели, поддерживающие цепочку рассуждений (например, Qwen3), прежде чем сгенерировать вызов инструмента, сначала размышляют внутри тегов `<think>` — анализируют намерение пользователя, оценивают, какие инструменты подходят, планируют порядок вызовов. Этот процесс размышления очень ценен для отладки поведения агента. Обратите внимание: `<think>` — это тег в сыром потоке вывода модели, и он не обязательно появится в ответе API в исходном виде. В этом эксперименте в Ollama передаётся `think=True`, поэтому Ollama разбирает размышления перед возвратом ответа и помещает их в отдельное поле `thinking` (при потоковой передаче — инкрементальными фрагментами `thinking`), так что тег `<think>` в `content` не появляется. Это лишь два разных представления одного и того же содержимого размышлений, и код эксперимента обрабатывает оба случая.
>
