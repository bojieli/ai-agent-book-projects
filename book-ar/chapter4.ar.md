...
في فيلم الخيال العلمي *Her*، تستطيع سامانثا، مساعدة الذكاء الاصطناعي، تنظيم رسائل البريد الإلكتروني بشكل استباقي، وتحديد الرسائل المعقدة عاطفيًا واقتراح ردود منقحة، وتمثيل بطل الرواية في مسائل النشر، والتبديل بسلاسة بين قنوات الاتصال المختلفة. إن ذكائها مقنع لأنها تمتلك **أدوات** قوية — "الأيدي والأقدام والحواس" التي تربط "دماغ" اللغة بالعالم الرقمي الحقيقي.

ومع ذلك، فإن بناء مثل هذا المساعد باستخدام تكنولوجيا اليوم يعني حل تحديين أساسيين:

1.  **التحدي المتمثل في اختيار الأداة**: عندما يمكن أن تتجاوز وثائق آلاف الأدوات نافذة السياق، كيف يمكن للوكيل العثور بدقة وكفاءة على الأداة المطلوبة لمهمة ما؟ كيف يمكن أن تتطور من "اختيار" الأدوات بشكل سلبي إلى "اكتشافها" بشكل فعال؟ يركز هذا الفصل على مبادئ تصميم الأدوات، والنظام البيئي الحالي، والاكتشاف الاستباقي على نطاق واسع؛ يتم تأجيل كيفية قيام الوكيل بإنشاء الأدوات وتعديلها وإيقافها بشكل مستقل على أساس الخبرة التشغيلية إلى الفصل 8.
2.  **تحدي عدم التزامن والأحداث**: كيف يمكن للوكيل إدارة المهام طويلة الأمد، والتعامل مع الانقطاعات من المستخدم أو النظام في أي لحظة، والاستجابة للأحداث الخارجية من قنوات مثل البريد الإلكتروني والتقويمات وتنبيهات النظام دون الوقوع في فخ الانتظار المتزامن؟

ويتناول هذا الفصل هذين التحديين. يبدأ بنظرة عامة على خمس فئات من الأدوات، ثم يناقش مبادئ التصميم المشتركة بين جميع الأدوات وكيف يعمل بروتوكول MCP على توحيد النظام البيئي للأداة. وعلى هذا الأساس، فإنه يستخدم التنظيم الهرمي، والاكتشاف الديناميكي، والمهارات لمواجهة تحديات اختيار الأدوات. ثم يفحص بالتفصيل الفئات الثلاث من الأدوات التي يستدعيها الوكيل بشكل استباقي - الإدراك والتنفيذ والتعاون - قبل التحول إلى بنيات الوكيل غير المتزامنة المستندة إلى الحدث وأدوات الاتصال بالمستخدم المبنية عليها. ويختتم بـ "الاكتشاف الاستباقي للأدوات"، الذي يتناول الاكتشاف بشكل منهجي عندما يصل عدد الأدوات إلى المئات أو الآلاف. تتم مناقشة كيفية قيام الوكيل بتحويل مسارات استخدام الأداة المقدرة إلى قدرات جديدة بشكل منهجي في الفصل الثامن، "التطور المستمر للوكيل".

## تصنيف الأداة

قدم الفصل الأول الفئات الخمس لأدوات الوكيل (الإدراك، والتنفيذ، والتعاون، وتشغيل الأحداث، واتصالات المستخدم). لمعرفة مدى اختلاف تصميماتها، افحص كل فئة وفقًا لخاصيتين: **توجيه الاستدعاء** (من يبدأ التفاعل) و **هدف الإجراء** (ما الذي يعمل عليه التفاعل). لاحظ أن هذين العمودين لا يشكلان إطار عمل للتصنيف المتبادل - فكل فئة لها قيمتها المحددة الخاصة بـ "هدف الإجراء"؛ إنهم ببساطة يساعدون القراء على وضع كل فئة في لمحة سريعة. ويلخص الجدول 4-1 كلتا الخاصيتين للفئات الخمس، ويحدد مناقشات التصميم التالية.

الجدول 4-1 توجيه الاستدعاء وهدف الإجراء لفئات الأدوات الخمس

| نوع الأداة | اتجاه الدعوة | هدف العمل |
|-------------------------|-----------------------------------|-----------------------------------|
...
| أدوات تشغيل الحدث | سجلات الوكيل، المشغلات الخارجية | قم بقيادة الوكيل لبدء التنفيذ |

**أدوات الإدراك** هي الوسائل التي يكتسب الوكيل من خلالها المعلومات بشكل فعال ويدرك العالم. تتضمن الأمثلة أدوات بحث الويب (`web_search`)، وأدوات استرداد قاعدة المعرفة الداخلية (`knowledge_base_search`)، وأدوات قراءة صفحات الويب (`fetch_url`)، وأدوات البحث عن أسماء الملفات (`find_file`)، وأدوات البحث عن محتوى الملف (`grep_file`)، وأدوات قراءة الملفات (`read_file`). تتمثل اعتبارات التصميم الرئيسية لأدوات الإدراك في مفاضلات التفاصيل والتحكم في كمية معلومات المخرجات.

**أدوات التنفيذ** هي الوسيلة التي يقوم الوكيل من خلالها بتغيير العالم الخارجي. تتضمن الأمثلة أدوات سطر الأوامر (`shell_exec`)، وأدوات مترجم التعليمات البرمجية (`code_interpreter`)، وأدوات كتابة الملفات (`write_file`)، وأدوات تحرير الملفات (`edit_file`)، وأدوات إرسال البريد الإلكتروني (`send_email`). على عكس أدوات الإدراك، يمكن أن تكون تكلفة الأخطاء في أدوات التنفيذ مرتفعة للغاية، مما يجعل القيود الأمنية هي جوهر تصميمها.

**أدوات التعاون** هي الوسائل التي يتعاون الوكيل من خلالها مع العملاء الآخرين والبشر. تتضمن الأمثلة إنشاء وكيل فرعي (`spawn_subagent`)، وإرسال رسالة إلى وكيل فرعي (`send_message_to_subagent`)، وإلغاء وكيل فرعي (`cancel_subagent`)، واكتشاف الوكلاء المتوفرين في النظام (`list_agents`). أبسط سبب لحاجة الوكيل إلى التعاون هو التوازي - على سبيل المثال، البحث عن العديد من المؤسسين المشاركين لـ OpenAI في وقت واحد. السبب الأعمق هو التخصص: إعطاء المهام المختلفة نماذج وأدوات ومطالبات وسياقات مختلفة للحصول على نتائج أفضل. سيناقش الفصل العاشر بشكل أكبر البنى متعددة الوكلاء.

**أدوات اتصال المستخدم** هي الوسائل التي يقوم الوكيل من خلالها بنقل المعلومات إلى المستخدم بشكل فعال. تتضمن الأمثلة الرد على رسالة مستخدم (`reply_to_user`)، وإرسال رسالة بطاقة منظمة (`send_card_to_user`)، وإرسال تنبيه إعلام المستخدم (`send_user_notification`). عندما يتوسع الاتصال بين الوكيل والمستخدم من سؤال وجواب بسيط خلال جلسة واحدة إلى مراسلة غير متزامنة متعددة القنوات، فإن "التحدث" في حد ذاته يحتاج إلى أن يصبح استدعاء أداة صريحًا.

**أدوات تشغيل الأحداث** هي الوسائل التي يحرك بها العالم الخارجي تصرفات الوكيل. تتضمن الأمثلة إعداد مؤقت (`set_timer`)، ومراقبة مهام سطر الأوامر في الخلفية (`monitor_shell`)، والاتصال بمصادر الأحداث الخارجية (`connect_channel`). تتضمن هذه الأدوات لحظتين: **التسجيل**، حيث يقوم الوكيل باستدعاء الأداة بشكل نشط للإعلان عن الأحداث التي يهتم بها؛ و**التشغيل**، حيث يتم استدعاء حدث خارجي بشكل غير متزامن لإيقاظ الوكيل حتى يتمكن من بدء المعالجة - وهذا هو معنى "سجلات الوكيل، المشغلات الخارجية" في الجدول 4-1. بدون أدوات تشغيل الأحداث، لا يمكن للوكيل الاستجابة إلا بشكل سلبي عندما يبدأ المستخدم محادثة، ويكون غير قادر على التصرف بشكل مستقل في وقت محدد أو التفاعل مع الأحداث الخارجية مثل رسائل البريد الإلكتروني الجديدة أو تنبيهات النظام.

يتم استدعاء الفئات الأربع الأولى من الأدوات بشكل نشط بواسطة الوكيل، وستتم مناقشة تصميمها بالتفصيل أدناه. لا يمكن فصل تصميم الأدوات التي يتم تشغيلها بواسطة الأحداث عن البنية غير المتزامنة التي تعتمد على الأحداث، والتي سيتم تناولها في قسم "الوكلاء غير المتزامنين الذين يحركهم الأحداث" لاحقًا في هذا الفصل. أولاً، نقدم مبادئ التصميم العالمية التي تنطبق على جميع الأدوات.
...
- **المهارات + المنفذون العامون**: تصف وثائق المهارات المكتوبة باللغة الطبيعية سير العمل التشغيلي، الذي ينفذه الوكيل عبر محطة طرفية أو مترجم كود. ولا يتطلب هذا سوى عدد قليل من الأدوات العامة لتغطية نطاق واسع من السيناريوهات (كما سيناقش الفصل الخامس مع سبع أدوات أساسية).
...
مبدأ عملي لتصحيح الأخطاء: عندما يستمر العميل في اختيار الأداة الخاطئة، **تحقق من أوصاف الأداة أولاً** بدلاً من الشك في النموذج. تعود معظم أخطاء اختيار الأداة إلى أوصاف غير دقيقة - حدود غير واضحة، وأمثلة سلبية مفقودة، ومعاني معلمات غامضة. عادةً ما يكون إصلاح الأوصاف أفضل بكثير من التبديل إلى نموذج أقوى.
...
**فصل الموارد والأدوات**. بالإضافة إلى الأدوات القابلة للتنفيذ، يحدد MCP موارد القراءة فقط (على سبيل المثال، محتويات الملف وسجلات قاعدة البيانات) التي يمكن للعملاء تصفحها وقراءتها دون استدعاء الأدوات. يسمح هذا الفصل للوكلاء بالتمييز بين "الحصول على المعلومات" و"تنفيذ الإجراءات". هناك أيضًا بدائي ثالث - المطالبات: قوالب المطالبات القابلة لإعادة الاستخدام التي يوفرها الخادم للعملاء والمستخدمين لاستدعاءها عند الطلب. تتوافق الأدوات والموارد والمطالبات مع "العمليات التي يمكن للنموذج تنفيذها"، و"البيانات التي يمكن للتطبيق قراءتها"، و"النماذج التي يمكن للمستخدم الاختيار من بينها"، على التوالي.
...
إذا كانت أدوات الإدراك هي "حواس" الفاعل، فإن أدوات التنفيذ هي "يديه وقدميه". ولكن على عكس أدوات الإدراك، يمكن أن تفشل أدوات التنفيذ بشكل مكلف: فالملف المحذوف عن طريق الخطأ يختفي إلى الأبد، ويمكن لأمر نظام سيء أن يزيل الخدمة، ويمكن أن تكلف مكالمة API غير المدروسة أموالًا حقيقية. ولذلك، يجب أن يحقق تصميمها توازنًا دقيقًا بين **انفتاح القدرات** و**القيود الأمنية**.
...
يحتاج Sidecar الأمني أيضًا إلى **قاطع دائرة الرفض**: عندما يرفض المصنف عملية بعد عملية، يجب ألا يقوم النظام بإعادة المحاولة إلى أجل غير مسمى - مما يؤدي إلى إهدار الموارد ويمكن أن يحبس المستخدم في حلقة - ولكن يلجأ إلى مطالبة المستخدم بالحكم يدويًا. يعد هذا مثالًا نموذجيًا لوظيفة "تصحيح" الحزام من الفصل الأول.
...
- **العزل على مستوى نظام التشغيل**: يستخدم آليات أمان نظام التشغيل لتقييد سلوك العملية، مثل حزام الأمان لنظام التشغيل macOS (sandbox-exec)، وseccomp ومساحات الأسماء لنظام التشغيل Linux. يمكنه تقييد نطاق الوصول إلى الملفات وتعطيل الشبكات وحظر مكالمات النظام الخطيرة. هذا هو الحل المحلي خفيف الوزن المفضل.
...
النهج الأساسي للتعامل مع هذا هو **العجز**: تنفيذ نفس العملية مرة واحدة وتنفيذها عدة مرات له نفس التأثير تمامًا على العالم الخارجي، مما يسمح بإعادة المحاولة بشكل آمن. هناك طريقتان شائعتان للتصميم: أولاً، جعل العملية تحمل **معرفًا فريدًا** (على سبيل المثال، مفتاح عدم القدرة الذي أنشأه العميل)، والذي يستخدمه الخادم لإلغاء البيانات المكررة، مما يؤدي إلى إرجاع النتيجة الأولى للطلبات المكررة بدلاً من تنفيذها مرة أخرى؛ ثانيًا، **الاستعلام قبل الطفرة** — قبل إعادة المحاولة، استعلم عن الحالة الحالية للمورد الهدف (سواء تم إنشاء الطلب، أو ما إذا كان الملف قد تمت كتابته)، ولا يتم التنفيذ إلا إذا لم تكن العملية قد اكتملت بالفعل. العمليات التي تتسم بالعجز تجعل التعامل مع المهلات والانقطاعات أسهل بكثير.

ولكن ليس كل العمليات يمكن جعلها عاجزة. تؤدي عمليات مثل **إرسال بريد إلكتروني، أو إجراء مكالمة هاتفية، أو تحويل الأموال** إلى حدوث حدث واقعي لا رجعة فيه في كل مرة يتم تنفيذها. علاوة على ذلك، غالبًا ما يكون الخادم خارج نطاق سيطرتك، مما يجعل من المستحيل إلغاء التكرار باستخدام معرف فريد. بالنسبة لمثل هذه العمليات غير الفعالة، يجب استخدام نهج **"الفحص المسبق ثم التأكيد" المكون من مرحلتين**: المرحلة الأولى تؤدي فقط التحقق من الصحة والتشغيل التجريبي (التحقق من الرصيد، وتأكيد المستلم، وإنشاء المحتوى المراد إرساله)، وإرجاع النتيجة مع رمز التأكيد؛ تستخدم المرحلة الثانية الرمز المميز للتنفيذ الفعلي، وإذا فشل التنفيذ، فلا ينبغي إعادة المحاولة بشكل أعمى في نفس المرحلة، ولكن يجب إعادة التحكم إلى الطبقة العليا لتكرار الفحص المسبق. هذا جزء من الموافقة المسبقة للمقترح والمراجع التي تمت مناقشتها سابقًا، ومع الفصل "البدء/الكامل" لواجهات الأدوات غير المتزامنة التي تمت مناقشتها لاحقًا.
...
> - **مترجم الكود**: تنفيذ Sandboxed Python، ودعم الموافقة على العمليات الخطيرة وتلخيص المخرجات الطويلة
...
عندما تتجاوز مهمة ما حدود قدرة وكيل واحد، تسمح أدوات التعاون لها بتفويض المهام الفرعية إلى عملاء آخرين أو بشر، ثم دمج النتائج من جميع الأطراف.

**فلسفة تصميم الوكلاء الفرعيين.**

تكمن القيمة الأساسية للوكلاء الفرعيين في **التخصص من خلال تقسيم العمل** - بدلاً من بناء وكيل واحد يقوم بكل شيء، قم ببناء مجموعة من المتخصصين الذين يحلون المشكلات من خلال التعاون. يمكن لكل وكيل فرعي تحسين الموجه ومجموعة الأدوات وقاعدة المعرفة الخاصة به بشكل مستقل، دون القلق بشأن التعارضات مع الآخرين.

**العناصر الأساسية لمطالبات الوكيل الفرعي.**

**يجب أن يكون تعريف الدور واضحًا.** اذكر مقدمًا، "أنت وكيل مساعد مسؤول بشكل خاص عن XXX."

**يجب تسمية مصادر السياق بشكل واضح.** قد يتلقى الوكيل الفرعي معلومات من مصادر متعددة. يجب أن يميز الموجه بوضوح بين كل مصدر: "`[FROM_MAIN_AGENT]` هي تعليمات المهمة من وكيل التنسيق الرئيسي؛ `[FROM_USER]` هي المعلومات المقدمة مباشرة من قبل المستخدم؛ `[TOOL_RESULT]` هي النتيجة التي يتم إرجاعها بعد استدعاء الأداة." يمنع هذا التصنيف الوكيل الفرعي من إرباك مصادر المعلومات ويتجنب هجمات **الحقن الفوري** (التي تم تقديمها في قسم Sidecar سابقًا).
...
- **Heartbeat (Heartbeat Daemon)**: إيقاظ العميل كل N دقيقة للتحقق مما إذا كان هناك أي شيء يتطلب الاهتمام، وذلك باستخدام الحكم لتجنب إرهاق التنبيه

تمنح هذه الآليات الثلاث وكلاء OpenClaw مظهر الاستقلالية - حتى عندما يكون المستخدم غير متصل بالإنترنت، يمكن للوكيل إنشاء تقارير في الموعد المحدد، والتحقق من حالة النظام، والتعامل مع المهام الروتينية. ومع ذلك، انظر عن كثب، وستظهر قيودًا أساسية. على وجه الدقة: تتعامل البوابة بالفعل مع الرسائل الواردة من القنوات المدمجة (المراسلة الفورية، واجهة الويب) بطريقة **الدفع** — حيث يتم توجيهها إلى العميل لحظة وصولها. ومن بين آليات الأتمتة الثلاث، فإن Cron وHeartbeat فقط هما اللذان يسمحان للوكيل بالتصرف دون رسالة مستخدم، وكلاهما **يعتمدان على الوقت** - يتحققان من نبضات القلب على فترات زمنية محددة، ويطلق Cron في أوقات محددة مسبقًا. تتفاعل الخطافات فقط مع أحداث دورة الحياة الداخلية لإطار العمل ولا يمكنها جلب تغييرات جديدة من العالم الخارجي. الفجوة الحقيقية هي كما يلي: بالنسبة لأي مصدر حدث تابع لجهة خارجية خارج القنوات المدمجة - بريد إلكتروني جديد، رد اتصال خارجي API يدفع البيانات، إشعار عاجل يتطلب اهتمامًا فوريًا - لا يوجد لدى OpenClaw مسار دخول فوري. لا يمكن للوكيل الاستجابة لحظة وقوع الحدث؛ في أحسن الأحوال، يلاحظ ذلك عند علامة Cron/Heartbeat التالية.
...
من منظور التصميم، يجب أن تحدد الأدوات التي يتم تشغيلها بواسطة الأحداث شروط تشغيل واضحة وقواعد تصفية لمنع الأحداث غير ذات الصلة من إيقاظ العامل وإهدار الموارد الحسابية. يجب أن تحتوي حمولة الحدث على معلومات سياق كافية لتقليل عدد الاستعلامات الإضافية التي يحتاج الوكيل إلى إجرائها بعد استيقاظه.
...
كما ذكرنا في بداية هذا الفصل، تتمتع سامانثا في *Her* بهوية وبيئة عمل مستقلة. إن تحقيق مثل هذا المساعد للأغراض العامة يفرض خيارًا معماريًا رئيسيًا: هل يجب على الوكيل إدارة الحسابات الشخصية للمستخدم مباشرة، أو الاحتفاظ بهوية افتراضية خاصة به؟ تبدو الإدارة المباشرة ملائمة، ولكن خطأ العميل الواحد أو التسوية التي تعرض لها تكشف الهوية الرقمية الكاملة للمستخدم. يتمثل النهج الأكثر أمانًا في منح الوكيل هوية افتراضية مستقلة - بالطريقة التي يمتلك بها السكرتير هاتف مكتبه وصندوق بريد خاص به - تشتمل على حسابات اتصال مخصصة وبيئات تخزين وحوسبة، بحيث يمكن للوكيل العمل نيابة عن المستخدم بموجب هوية شفافة ومعلنة بوضوح. وهذه الشفافية لا تضعف الثقة؛ يمكن أن يجعل التواصل أكثر أصالة.

يجب أن ترتكز الهويات الافتراضية على بيئات تنفيذ معزولة. **أجهزة الكمبيوتر الافتراضية** (أجهزة افتراضية/حاويات) و**هواتف افتراضية** (محاكيات Android) توفر للوكيل عزلًا على مستوى نظام التشغيل وإمكانات تشغيل كاملة لسطح المكتب/الهاتف المحمول: يمتلك الوكيل حساب المستخدم الخاص به والدليل الرئيسي وبيانات اعتماد تسجيل الدخول داخلها، مما يجعل جميع العمليات قابلة للتتبع والتدقيق؛ حتى في حالة إجراء عمليات خاطئة، يظل النظام المضيف والجهاز الحقيقي للمستخدم غير متأثرين. يعد هذا امتدادًا لمفهوم وضع الحماية الذي تمت مناقشته في قسم أدوات التنفيذ في بُعد "الهوية الرقمية" - حيث تعمل صناديق الحماية على عزل تنفيذ التعليمات البرمجية، بينما تعزل أجهزة الكمبيوتر والهواتف الافتراضية الهوية الرقمية بأكملها.
...
إذا أخذنا رسالة البريد الإلكتروني الخاصة بطلب استرداد العميل كمثال، يبدو الحدث المنظم كما يلي:
...
**المعالجة المبنية على الإلغاء** تُستخدم للأحداث العاجلة؛ جوهرها هو **فرض نقطة آمنة مبكرًا** للحدث العاجل: مقاطعة الخطوة الحالية بشكل استباقي لتحويل هذه اللحظة إلى حد يمكن عنده استهلاك الحدث الجديد. عند وصول حدث عاجل (على سبيل المثال، ينقر المستخدم على "إيقاف" أو يرسل نظام إشرافي تعليمات ذات أولوية عالية): (1) أوقف العملية الحالية - إذا كان LLM يفكر، قم بإلغاء استجابة البث على الفور؛ في حالة تنفيذ أداة متزامنة، أرسل إشارة إلغاء؛ (2) استنزاف قائمة الانتظار المعلقة عن طريق إزالة جميع الأحداث المعلقة؛ (3) إلحاق تلك الأحداث مع الحدث العاجل بنهاية المسار؛ (4) أعد استدعاء LLM على الفور بالمسار الكامل المحدث كمدخل لتقييم الموقف. على سبيل المثال، إذا قام المستخدم بإدخال "توقف! لقد قلت الشيء الخطأ" بينما يكون الوكيل على وشك تنفيذ عملية يحتمل أن تكون خاطئة، فسوف يرى الوكيل على الفور هذا الإدخال الجديد، ويعيد فهم النية الحقيقية، وبالتالي يتجنب تنفيذ الإجراء الخاطئ.
...
القواعد المضمنة لها قيود؛ دلالات الحدث تملي طريقة التعامل - "توقف فورًا!" يستخدم المعالجة القائمة على الإلغاء، "كيف حال الطقس اليوم؟" يستخدم المعالجة المتوازية، ويستخدم "إرسال التقرير باللغة الصينية" المعالجة في قائمة الانتظار. **يوصى باستخدام تصنيف خفيف الوزن LLM كموجه للحدث**، لتحديد الإستراتيجية التي سيتم اعتمادها بسرعة عند وصول الحدث.
...
>
>
> ![الشكل 4-4: التجربة 4-4 بنية الوكيل الموجه بالحدث](images/fig4-4.svg)
>
>
...
**القاعدة 3**: تتطلب الانقطاعات أثناء تنفيذ الأداة وجود عناصر نائبة. أنشئ استجابة عنصر نائب للأداة غير المكتملة (على سبيل المثال، "يتم تنفيذ الأداة في الخلفية، يرجى إعطاء الأولوية للحدث الجديد")، وألحق حدث المقاطعة، وأعد استدعاء LLM. من منظور LLM، لا تزال رسالة المساعد تحتوي على نتيجة أداة مقترنة.
...
2. قبل أن تقوم أداة البحث بإرجاع النتائج، يرسل المستخدم "تحقق أولاً من طقس الغد بالنسبة لي". نظرًا لأن هذه مقاطعة للمستخدم، يقوم النظام بإنشاء نتيجة أداة نائب لـ `search_contacts` غير المكتملة ("يتم تنفيذ الأداة في الخلفية، يرجى تحديد أولوية الحدث الجديد"، القاعدة 3)، ثم يُلحق استعلام الطقس الخاص بالمستخدم بالمسار ويعيد استدعاء LLM. عند هذه النقطة، يكون تنسيق المسار الذي يراه LLM صالحًا تمامًا - حيث يتم إقران رسالة المساعد ونتيجة الأداة بشكل مثالي.
3. بعد أن يجيب الوكيل على استعلام الطقس، تصل نتيجة `search_contacts` الأصلية ويتم إلحاقها بالمسار كحدث جديد (القاعدة 2). يقرأ الوكيل معلومات الاتصال ويستمر في صياغة البريد الإلكتروني.
...
**مستوى المطالبة**: أخبر النموذج، "عندما تتلقى عدة أحداث متتالية، يرجى التأكد من أنك تأخذ في الاعتبار جميع المعلومات بشكل شامل."
...
إن التقنيات الهندسية المذكورة أعلاه هي في الأساس **استخدام الهندسة السريعة للتعويض عن أوجه القصور في التدريب النموذجي**، وهي وسيلة مؤقتة خلال الفترة الانتقالية. الحل الحقيقي يتطلب نقلة نوعية على مستوى التدريب النموذجي.
...
ومع ذلك، فإن التفكير المستمر لا يحتاج إلى انتظار الجيل القادم من النماذج. يمكن لطبقة رقيقة من منطق التنسيق (حوالي مائتي سطر) أن تحول نموذج التفكير النصي **الجاهز** إلى وكيل **الوقت المستمر** على الفور[^ch4-async-1] - وهو ما يربط بشكل أنيق بين "الوسيلة الهندسية" و"تطور النموذج" إلى النصف أعلاه. الآلية هي القاعدة 4، تمت ترقيتها: بدلاً من **تجاهل** فكرة نصف مكتملة بشأن المقاطعة، قم ببناء التفاعل بأكمله باعتباره **تيارًا متواصلًا من الأفكار** - في أي لحظة، قم بإغلاق كتلة `<think>` بالقوة التي يكتبها النموذج، وأدخل الملاحظة التي وصلت حديثًا (إرجاع أداة، مقاطعة مستخدم، نتيجة تعرف جديدة) كرسالة عادية، واترك النموذج يواصل فك التشفير. يستغل هذا موردًا يُهدر عادةً: يمكن للنموذج إنشاء آلاف الرموز المميزة في الثانية، بينما يستغرق استدعاء الأداة أو نطق المستخدم عدة ثوانٍ - تكون فترات الانتظار هذه **حسابات مجانية**، ويمكن استخدامها للتفكير في المستقبل. يظهر سلوكان: **التفكير أثناء الانتظار** - بدلاً من انتظار عودة الأداة أو انتهاء المستخدم من التحدث، يفكر النموذج في المعلومات الجزئية التي لديه بالفعل، حتى أنه يطلق استدعاء الأداة التالي مبكرًا (تم إعادة إنتاج هذا الاتجاه "التفكير الاستباقي" بدون أي فرصة عبر مجموعات نموذجية متعددة؛ راجع الورقة المذكورة في الحاشية السفلية للبيانات)؛ و **التفكير أثناء العمل** — الاستمرار في التفكير أثناء إنتاج النتائج، والقدرة على تصحيح نفسه في منتصف العمل.

لكن النصف الأكثر أهمية من هذا البحث يتعلق بـ **التدريب**، وهو يجيب على دعوة "توقع تطور النموذج" أعلاه: فالتنسيق وحده يجعل التفكير المستمر **ممكنًا**؛ سواء أصبح **مفيدًا** يعتمد على إشارة التدريب. وجد البحث أنه من خلال أسلوب المكافأة "LLM كقاضي"، يتعلم النموذج إخفاء أفكاره - مقايض الصمت بموافقة القاضي - بينما تسوء المقاييس الموضوعية فعليًا؛ فقط الأهداف التي يمكن التحقق منها والتي تحمي تغطية المعلومات هي التي تجعل التفكير المستمر يؤتي ثماره. باختصار: **التنسيق يجعل السلوك ممكنًا؛ التدريب يجعل السلوك جيدًا** - وهو ما يؤكد حكم هذا القسم بأن القدرة غير المتزامنة يجب تعزيزها في النهاية من خلال التدريب المناسب، وليس تصحيحها إلى الأبد بالهندسة السريعة.
...
>
>
> ![الشكل 4-6: التجربة 4-5 مقاطعة العامل غير المتزامن واسترداده](images/fig4-6.svg)
>
>
...
>
> **4. الإلغاء والاستعلام عن الحالة للأدوات الموازية**: بعد اكتمال الأداة غير المتزامنة، يتم إدخال النتيجة الحقيقية في المحادثة عبر حدث جديد. يدعم الإلغاء أو الاستعلام عن التقدم عبر معرف المهمة. **سيناريو التحقق من الصحة**: يطلب المستخدم، "قم بتشغيل هذه البرامج النصية الثلاثة في وقت واحد بالنسبة لي. أيهما ينتهي أولاً، تحقق من تقدم البرامج النصية المتبقية. إذا لم يتجاوز أي منها 50%، فقم بإلغائه." تحاكي البرامج النصية الثلاثة عمليات التحليل، وتنتج التقدم بشكل مستمر بسرعات 3% و2% و1% في الثانية على التوالي. يبدأ الوكيل ثلاثة أوامر طرفية غير متزامنة في وقت واحد. عندما ينتهي البرنامج النصي بمعدل 3% في الثانية في حوالي 33 ثانية، يستعلم العميل عن حالة المحطتين المتبقيتين، ويجد أحدهما عند حوالي 66% والآخر عند حوالي 33%. ثم يلغي الذي لم يتجاوز 50%. بعد اكتمال كلا المحطتين، فإنه يدمج النتائج لإنشاء تقرير كامل.
>
...
يقوم النهج التقليدي بإدخال مخطط كل أداة في موجه النظام مرة واحدة، ويتعطل بسرعة بمجرد أن يصل عدد الأدوات إلى الآلاف: ينسد السياق بأدلة الأدوات، وتنخفض دقة التحديد. التصفية المسبقة القائمة على الاسترجاع (تمت مناقشتها في قسم "النظام البيئي للأداة" أعلاه)، والتي تفحص المرشحين حسب التشابه الدلالي أولاً، تخفف المشكلة ولكنها تحمل حدًا متأصلًا - فهي تطابق **مرة واحدة**، مقابل الاستعلام الأولي للمستخدم. قد يتطلب طلب يبدو بريئًا مثل "تصحيح أخطاء الملف" سلسلة أدوات متعددة الخطوات ومتعددة المجالات - الوصول إلى الملفات، وتحليل التعليمات البرمجية، وتنفيذ الأوامر - والتي لا يمكن لأحد توقعها عندما تبدأ المهمة.

**من التحديد السلبي إلى الاكتشاف الاستباقي.** الخطوة التالية هي تحويل الوكيل من مستلم سلبي إلى مكتشف نشط: عندما يصل إلى فجوة القدرة في منتصف التنفيذ، فإنه يعلن بلغة طبيعية عن الإمكانية التي يحتاجها، ويقوم النظام بمطابقة الأداة وإدخالها بسرعة. MCP-Zero[^mcp-zero-2025] هو العمل التمثيلي. لم يتم تحميل أي مخطط أداة مسبقًا في موجه النظام؛ يُصدر الوكيل كتل طلب منظمة في تفكيره (على سبيل المثال، "خادم GitHub: مستودعات البحث وإرجاع البيانات التعريفية")، ويوجه النظام عبر مستويين من المطابقة الدلالية (مستوى الخادم → مستوى الأداة) عبر آلاف المرشحين قبل الحقن. تشير الورقة إلى انخفاض بنسبة 98٪ تقريبًا في استخدام الرمز المميز مقارنة بالحقن الكامل عبر حوالي 2800 أداة. المعادل الهندسي الأكثر شيوعًا يحتفظ فقط بعدد قليل من الأدوات الأساسية (بحث الويب، مترجم التعليمات البرمجية) بالإضافة إلى "أداة بحث عن الأدوات" في موجه النظام، ويتيح للوكيل وصف احتياجاته باللغة الطبيعية لاسترداد وتحميل الباقي - أداة البحث عن الأدوات Anthropic في Claude API هي إحدى هذه الأدوات. ما يتشاركونه: يعلن الوكيل عن الفجوة؛ يقوم النظام بالحقن عند الطلب.
...
يوضح الشكل 4-9 الصورة الكاملة بعد عدة جولات من الاكتشاف الديناميكي: البادئة الثابتة تحمل فقط موجه النظام، والأدوات الأساسية، والأداة الوصفية للبحث عن الأدوات، في حين أن المخططات المكتشفة على طول الطريق متناثرة عبر المسار، ويتم تثبيتها حيث تم إدخالها لأول مرة ويتم تقديمها من ذاكرة التخزين المؤقت كسجل عادي في المنعطفات اللاحقة. وهذا يعني أيضًا أن "تعريفات الأداة يجب أن تكون في مقدمة السياق" لم تعد قاعدة ثابتة - فالبادئة لا تزال ثابتة وملحقة فقط؛ لقد اكتسبت تعريفات الأداة ببساطة القدرة على دخول المسار عند الطلب. والتكلفة هي أن النموذج يجب أن يتم تدريبه لاحقًا لفهم تعريفات الأداة المنتشرة في جميع أنحاء السياق.

من الواضح أن آلية الإعلان عن المطابقة والحقن تعمل بالكامل، ولكنها تتطلب هندسة كبيرة: فهرس تضمين للمحافظة على عدم الاتصال بالإنترنت، وإبطال KV Cache لإدارة، وتدريب مخصص للنماذج الأضعف. الفرضية المشتركة الكامنة وراء كل ذلك هي التعامل مع كل أداة باعتبارها **تعريفًا رسميًا موجهًا إلى النموذج** — مسجلاً، ومسترجعًا، ومُحقنًا. تسقط آلية المهارات في القسم التالي هذه الفرضية لشيء أخف.
...
> - "تحليل إحصائيات المساهمين في مستودع GitHub، وإنشاء تقرير مرئي" (يتطلب GitHub + مترجم الكود)
>
> **مجموعة التحكم**: أدخل المخططات الكاملة لجميع الأدوات التي يزيد عددها عن 120 أداة في موجه النظام مرة واحدة (أكثر من 50 ألف رمز مميز). تتدهور قدرة نموذج 4B على متابعة التعليمات بشدة مع مثل هذا السياق الطويل، مما يعرض مشاكل نموذجية: عند مواجهة "سعر سهم الاستعلام"، قد يحدد بشكل غير صحيح بحث الويب بدلاً من أداة Yahoo Finance المتخصصة، أو "ينسى" أدوات معينة في القائمة، مما يؤدي إلى فشل المهمة.
>
> **مجموعة التجارب**: تنفيذ المخطط المختلط الموضح سابقًا (مفهوم الاكتشاف الاستباقي MCP-Zero + تنفيذ أداة البحث عن الأداة): (1) يحتفظ موجه النظام فقط بالأدوات التعريفية `web_search` و`code_interpreter` و`discover_tools`؛ (2) يقبل `discover_tools` طلبات اللغة الطبيعية (على سبيل المثال، "أحتاج إلى القدرة على الاستعلام عن أسعار الأسهم")، ويعرض 3-5 أدوات مرشحة مع مخططات كاملة باستخدام مطابقة تشابه التضمين والمتجه؛ (3) يتم إلحاق تعريفات الأداة الجديدة بمحفوظات المحادثة (كرسالة مستخدم)، ويقوم شريط حالة الوكيل بتحديث قائمة أسماء الأداة؛ (4) قم بتوجيه النموذج للاتصال بـ `discover_tools` بشكل استباقي عند مواجهة فجوات في القدرات.
>
...
الاستنتاج الأساسي لهذا الفصل: جودة تصميم الأداة تحدد سقف قدرات العميل، وتحدد البنية غير المتزامنة ما إذا كان العميل يمكنه العمل بشكل موثوق في العالم الحقيقي.
...
- **أدوات تشغيل الحدث**: تشمل الاعتبارات الرئيسية تصفية شروط التشغيل وتصميم حمولات الحدث، مما يمكّن العالم من تنبيه العميل بشكل استباقي.
...
تتقدم ست تجارب من الأساسيات إلى الهندسة المعمارية: التجارب من 4-1 إلى 4-3 تبني مجموعات الأدوات الأساسية الثلاث - الإدراك والتنفيذ والتعاون؛ تقدم التجربة 4-4 المعالجة المستندة إلى الأحداث باستخدام وكيل التعامل مع البريد الإلكتروني؛ تنفذ التجربة 4-5 التنفيذ المتوازي، واستعادة المقاطعة، وإدارة الحالة؛ والتجربة 4-6 تتحقق من صحة قيمة الاكتشاف الاستباقي للأداة على نطاق المكتبة. حدود هذا الفصل هي الوصف والاكتشاف والاستخدام الآمن للأدوات الموجودة **. يناقش الفصل الثامن بدلاً من ذلك كيف يحدد الوكيل من حالات الفشل والعمليات المتكررة متى يقوم بإنشاء أداة أو تعديلها أو إعادة التحقق من صحتها أو إيقافها.

يطرح الفصل التالي سؤالاً أكثر أهمية من "كيف يستخدم الوكيل الأدوات؟": هل يستطيع الوكيل **إنشاء** الأدوات عن طريق كتابة التعليمات البرمجية؟ يعد وكيل التشفير بالإضافة إلى نظام الملفات هو الأساس الأساسي لكل وكيل للأغراض العامة، كما أنه يوفر إمكانية التنفيذ المطلوبة لمناقشة الفصل الثامن حول التعديل الذاتي للنظام المتحكم فيه.
...
7. ★★ يثير هذا الفصل مشكلة "انفجار الأداة" - حيث تقل دقة اختيار العميل عند مواجهة آلاف الأدوات. إلى جانب الاكتشاف الاستباقي للأداة، ما هي الأساليب الأخرى الموجودة؟ فكر في الاعتماد على كيفية تعامل الخبراء البشريين مع مجموعة واسعة من الأدوات المتاحة.
