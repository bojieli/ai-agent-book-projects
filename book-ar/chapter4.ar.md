...
في فيلم الخيال العلمي *Her*، تستطيع سامانثا، المساعدة الذكية، تنظيم البريد الإلكتروني استباقيًا، وتمييز الرسائل المشحونة عاطفيًا واقتراح ردود مصقولة، وتمثيل بطل القصة في شؤون النشر، والتنقل بسلاسة بين قنوات الاتصال. ويبدو ذكاؤها مقنعًا لأنها تملك **أدوات** قوية؛ أي «الأيدي والأقدام والحواس» التي تصل «الدماغ» اللغوي بالعالم الرقمي. وقد حققت الوكلاء العامة اليوم، مثل Manus وOpenClaw، معظم القدرات التي تحتاج إليها سامانثا في *Her*.

يبدأ الفصل بنظرة عامة على خمس فئات من الأدوات، ثم يناقش مبادئ التصميم المشتركة ودور بروتوكول MCP في توحيد منظومة الأدوات. وبعد ذلك يعرض التنظيم الهرمي والاكتشاف الديناميكي والمهارات بوصفها حلولًا لمشكلة الاختيار. ثم يفصل في أدوات الإدراك والتنفيذ والتعاون، قبل الانتقال إلى بنى الوكلاء غير المتزامنة القائمة على الأحداث وأدوات التشغيل بالأحداث والتواصل مع المستخدم. ويختتم بالاكتشاف الاستباقي حين يبلغ عدد الأدوات المئات أو الآلاف.
...
**حدود MCP**. ينصب تركيز MCP على توحيد التفاعل بين الوكلاء والقدرات الخارجية، لا على توفير بيئة تشغيل كاملة للأحداث. يستطيع البروتوكول بالفعل دعم التفاعلات متعددة الجولات، والاشتراك في التغييرات، والمهام طويلة الأمد، لكن هذه الآليات تجيب عن سؤال «كيف يستمر سير عمل واحد»، ولا تتولى إبقاء الوكيل متصلًا باستمرار. أما البنى المعتمدة على الأحداث التي تمتد عبر الجلسات وتجمع مصادر أحداث متعددة وتوقظ وكيلًا غير نشط — مثل تشغيله عند وصول رسالة بريد جديدة أو استئناف مهمة بعد رد نظام خارجي — فلا يزال من الضروري بناؤها فوق البروتوكول[^ch4-mcp-current]. تتوزع المسؤوليات على طبقتين: يوحّد MCP استدعاءات القدرات، بينما يتولى إطار عمل الوكيل استقبال الأحداث وجدولتها وتشغيلها بالتوازي وإيقاظ الوكيل. ويناقش النصف الثاني من هذا الفصل هذه الطبقة الأخيرة.
...
لفهم الصور والفيديو والصوت وملفات PDF، يحتاج الوكيل إلى إدراك متعدد الوسائط. توجد ثلاثة مسارات: معالجة أصلية متعددة الوسائط داخل النموذج، واستخراج المحتوى إلى نص، وتغليف نموذج متعدد الوسائط داخل أداة.

المعالجة الأصلية هي الأعلى قدرة؛ إذ ترسم المشفرات أنواع البيانات المختلفة في فضاء دلالي مشترك، كما في Qwen-VL وLLaVA المعتمدين على Vision Transformer. أما الاستخراج إلى نص فيلائم النماذج التي لا تدعم الوسائط الأصلية، ويكون أوفر للملفات النصية الكثيفة، لكنه يفقد التخطيط والمخططات والصور. وعندما لا يكون النموذج الرئيسي متعدد الوسائط، يمكن لأدوات مثل `analyze_image` و`analyze_pdf` و`analyze_audio` تمرير ملف وسؤال إلى نموذج متخصص وإعادة وصف طبيعي موجز، مع إبقاء عدد الرموز في السياق منخفضًا.

## أدوات التنفيذ

تتضمن المراجعة المنقحة أيضًا عزلًا على مستوى العملية للمهام منخفضة المخاطر، وعزل الحاويات أو microVM للمدخلات غير الموثوقة مع حصص للموارد. ويعمل Sidecar كنموذج خفيف يراجع حقول استدعاء الأداة المنظمة كبوابة قبل التنفيذ؛ وبعد الرفض المتكرر يجب استخدام قاطع دائرة والرجوع إلى حكم المستخدم. أما العمليات غير القابلة للتكرار فتحتاج إلى مرحلتي «الفحص المسبق ثم التأكيد».
...
### تنفيذ آليات تعتمد على الأحداث في OpenClaw

توضح الصياغة المحدثة أن Hooks مصدرها دورة حياة OpenClaw الداخلية، بينما Cron وHeartbeat آليتان مدفوعتان بالوقت؛ أما البريد وواجهات API الخارجية فتحتاج قناة فورية مثل Channel في PineClaw.
...
في OpenClaw تكون الجلسة شفافة للمستخدم، ويمكن للطرفين إرسال الرسائل في أي وقت. وتستخدم الأداة رسائل مخصصة قد تتضمن صورًا وملفات وإشعارات دفع، مع دعم واجهات متعددة الوسائط وGenerative UI.
...
يمكن للكمبيوتر الافتراضي أن يعمل على مدار الساعة، ويحدّ من وصول الوكيل إلى الملفات المحلية ويحمي جهاز المستخدم حتى عند حدوث خطأ. وتُنقل البيانات عبر نظام ملفات مشترك على شكل مسارات لا محتوى كامل.
...
## الاكتشاف الاستباقي للأداة والإفصاح التدريجي القائم على المهارات
...
### اكتشاف الأدوات الأصلية للنموذج

تختلف طريقة الاكتشاف بحسب تمثيل إطار الوكيل للأدوات: بعض الأطر تستخدم أدوات أصلية للنموذج، وأخرى تستخدم تمثيلًا قائمًا على المهارات. عند وجود فجوة في القدرة، يعلن الوكيل حاجته بلغة طبيعية، ثم يطابق النظام الأداة ويحقن تعريفها عند الطلب.
...
**الإفصاح التدريجي.** لا تعرض المهارات كل المخططات دفعة واحدة؛ يرى الوكيل عند البدء فهرسًا رقيقًا يحوي `name` و`description` لكل مهارة، ثم يقرأ المهارة الفرعية والملفات المشار إليها عندما يحتاجها السياق. يشبه ذلك الرجوع إلى كتاب مرجعي أو ويكيبيديا عند الحاجة. الأدوات الأصلية أسهل للنموذج بفضل JSON، بينما المهارات أسهل للكتّاب البشريين ولا تؤدي أخطاؤها المحلية عادةً إلى تعطيل الوكيل كله.
