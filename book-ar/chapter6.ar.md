...
- كيف ينبغي تنظيم مطالبات النموذج ومهاراته؟
- ما هي القيود التي يجب إضافتها إلى الحزام؟
...
من وجهة نظر هندسة الحزام التي تم تقديمها في الفصل الأول، يلعب التقييم الدور الأساسي "للتحقق" داخل الحزام. الفكرة الأساسية هي: **يجب ألا يكون موضوع التقييم هو النموذج فقط، بل مزيج من النموذج والأداة**. يمكن أن يؤدي النموذج نفسه بشكل مختلف تمامًا في الأدوات المختلفة - فقد قامت بعض الفرق بتحسين أداء نفس النموذج بشكل كبير في المهام الطرفية فقط عن طريق تحسين الأدوات (انظر الفصل 5). لذا، عندما يتم تقييم الوكيل بشكل سيئ، فقد لا يكون الإصلاح نموذجًا مختلفًا ولكن مكونًا أفضل (المطالبات، تصميم الأداة، حلقات التعليقات). إن نظام التقييم السليم لابد أن يكون قادراً على التمييز بين مشكلتين مختلفتين جوهرياً: "القدرة النموذجية غير الكافية" و"استغلال عيوب التصميم". **من الطرق الشائعة للتمييز بينها هي تجربة تبديل النماذج**: أصلح الحزام، وقم بالتبديل بنموذج أقوى أو أضعف، وراقب مدى تحرك النتيجة. إذا لم يرفع النموذج الأقوى النتيجة، فإن عنق الزجاجة هو الحزام. إذا كان النموذج الأضعف يخفض النتيجة وتتأرجح النتائج بشكل حاد مع قدرة النموذج، فإن القراءة الأكثر مباشرة هي أن النموذج نفسه هو عنق الزجاجة وأن النموذج يهيمن على الأداء الحالي. سواء كان ذلك بسبب أن المهمة صعبة بطبيعتها أو لأن الحزام يعتمد بشكل كبير على المعرفة السابقة للنموذج، فإنه يتطلب المزيد من التحليل. لاحظ أن هذا يختلف عن تجربة الاستئصال المذكورة أعلاه: يعمل الاستئصال **على تعطيل مكون الحزام** لمعرفة كيفية تغير الأداء العام؛ تبديل النموذج **يصلح الحزام ويغير النموذج فقط**. يحدد الأول الجزء المهم داخل الحزام؛ يخبرك الأخير ما إذا كان عنق الزجاجة هو النموذج أم الحزام.
...
**حالة البيئة**: تتتبع الحالة القابلة للتغيير أثناء تنفيذ المهمة ويجب أن توازن بين الواقعية وإمكانية التحكم. على سبيل المثال، في تقييم خدمة العملاء، تتضمن حالة البيئة سجلات الطلبات في قاعدة البيانات وأرصدة حسابات المستخدم. بعد أن يتصل الوكيل بـ `process_refund`، تتغير حالة الطلب من "تم التسليم" إلى "تم استرداده" ويزداد الرصيد. تتطلب "الواقعية" أن تتبع تغييرات الحالة منطق الأعمال (لا يمكن أن يتجاوز مبلغ الاسترداد مبلغ الطلب)، وتتطلب "إمكانية التحكم" إمكانية إعادة تعيين كل اختبار إلى نفس الحالة الأولية.
...
>
> - **تم تقديم تعليمات مهمة أكثر تفصيلاً**: بما في ذلك "متطلبات التأريض"، مما يعني أن الاستجابات يجب أن تستند إلى الحالة الفعلية للبيئة
...
## تصميم مجموعات بيانات مهمة التقييم
...
لا تستنفد هذه القائمة مشهد تقييم الوكيل. حتى ضمن فئة الويب/واجهة المستخدم الرسومية، هناك العديد من المعايير ذات التركيز المختلفة: تقوم WebArena ببناء مواقع ويب قابلة للتكرار بالكامل (التجارة الإلكترونية، والمنتديات، واستضافة الأكواد، وما إلى ذلك)، والتي تحتوي على عدم القدرة على التنبؤ بصفحات الويب الحقيقية داخل صندوق الحماية؛ ويذهب Mind2Web في الاتجاه المعاكس، حيث يختبر التعميم مباشرة على مئات المواقع الحقيقية؛ يتخصص BrowseComp في الاسترجاع العميق - الإجابات مدفونة بعمق بحيث لا يمكن عرضها إلا من خلال التصفح متعدد القفزات والتحقق المتبادل. على جانب استدعاء الأدوات، توجد لوحات صدارة مخصصة لاستدعاء الوظائف مثل BFCL (لوحة متصدرين لاستدعاء الوظائف في بيركلي). ولا يحاول هذا الفصل تصنيفها جميعًا. وبدلاً من ذلك، فإنه يأخذ نموذجي البيئة الأساسيين (استدعاء الأدوات والتفاعل بين الإنسان والحاسوب)، بالإضافة إلى سيناريوهات تشغيل واجهة المستخدم الرسومية التي يتم تشغيلها من خلال دراسات حالة مجموعة البيانات، ويتعمق في مقايضات التصميم الخاصة بها. بمجرد فهم النماذج، يمكنك الحكم بسرعة على أي مقاييس مرجعية جديدة، ومدى نجاحها في منع تسرب البيانات، وإلى أي مدى يمكن استقراء استنتاجاتها.
...
يقدم τ²-bench تصميمًا سياقيًا، حيث تحتوي كل مهمة على طبقات متعددة من المعلومات: المشكلة السطحية ("بيانات الهاتف المحمول لا تعمل")، وتوقع الأداء ("يتطلب تصنيف سرعة ممتاز")، والقيد ("لن نقبل أي تصنيف آخر")، والعاطفة الضمنية. أحد التحسينات الرئيسية هو فصل "المعلومات المعروفة" عن "تعليمات المهمة": المعلومات المعروفة هي ما يعرفه المستخدم حاليًا، بينما تقوم تعليمات المهمة بتوجيه المحاكي حول كيفية الكشف عن المعلومات تدريجيًا، بما في ذلك "متطلبات التأريض" (يجب أن تستند الاستجابات إلى النتائج الفعلية التي يتم إرجاعها بواسطة استدعاءات الأداة، وليست ملفقة).
...
- **Pass@k**: احتمال نجاح **واحدة على الأقل** من محاولات k، والإجابة بـ "هل يستطيع العميل القيام بذلك؟"
...
|----------------------------------|---------------|-----------------------------------------------|
| التحقق من الاستقرار (اختبار الانحدار) | تمرير ^ ك | استخدام أقنعة Pass@k لعدم الاستقرار - سيظل العميل الذي ينجح مرة واحدة فقط في خمس محاولات يظهر على أنه "تمرير" |
...
الممارسة الأساسية: تحديد مستويات تسجيل يمكن التحقق منها بشكل موضوعي لكل بُعد، مع أمثلة ملموسة و**حالات هامشية** لحل المواقف الغامضة. احذر بشكل فعال من **اختراق المكافآت** - حيث يجد العميل "طريقًا مختصرًا" للحصول على أعلى الدرجات دون إكمال المهمة فعليًا - من خلال معاقبة الهلوسة والتملق وحشو الكلمات الرئيسية والتهرب من الأسئلة الصعبة بشكل صريح. يعتبر نموذج التقييم منتجًا متكررًا: حيث يكشف الاستخدام التجريبي عن خلافات بين المقيمين، ويتطور نموذج التقييم تدريجيًا من خلال هذه التعليقات من المبادئ المجردة إلى كتاب حالة مفصل.
...
أرسل نموذج التقييم هذا مع الرد الفعلي للوكيل على نموذج التحكيم، والذي سيسجل كل بُعد ويقدم المنطق. من خلال تشغيل ذلك عبر العشرات من حالات الاختبار، يمكنك تحديد فجوات قدرة العميل بشكل منهجي - على سبيل المثال، يشير متوسط ​​الدرجة 2.1 في بُعد "الاقتران عبر الجلسات" بوضوح إلى أوجه القصور في استرجاع الذاكرة أو ارتباط المعلومات.

> **التجربة 6-3 ★★: إنشاء نظام لتقييم ذاكرة المستخدم قائم على القواعد**
>
> **المتطلبات الأساسية**: يجب إكمال الفصل الثالث من تجربة ذاكرة المستخدم (`ch3/user-memory-evaluation`).
>
> تتطلب هذه التجربة تعديل إطار عمل `ch3/user-memory-evaluation` من الفصل 3، وترقية آلية تسجيل LLM البسيطة الحالية إلى نظام تقييم منظم ومتعدد الأبعاد. يستخدم النظام الحالي استدعاء LLM واحد لإرجاع نتيجة النجاح/الفشل بالإضافة إلى أسباب التقييم، ويفتقر إلى إمكانيات التشخيص المنظمة.
>
...
>
> **المتطلبات الأساسية**: يجب إكمال الفصل الثالث من ذاكرة المستخدم وتجارب RAG (`ch3/user-memory`, `ch3/agentic-rag-for-user-memory`).
>
> **الهدف**: مقارنة مزايا وحدود الذاكرة المنظمة بشكل عادل مقابل الاسترجاع غير المنظم في نفس مجموعة التقييم. أعد استخدام مشروعي الفصل 3 وقارن ثلاثة تكوينات في 60 حالة اختبار من `ch3/user-memory-evaluation` — بطاقات JSON المتقدمة النقية (البطاقات المنظمة التي يتم الاحتفاظ بها في السياق، دون الحاجة إلى استرجاعها)، وRAG النقي (أجزاء المحادثة المضمنة في مخزن المتجهات، والاسترداد مطلوب)، والنظام الهجين (الحقائق الأساسية المقيمة + المحادثات الأصلية المستردة عند الطلب).
>
...
**التكلفة**: تسعير الرموز المميزة للإدخال/الإخراج/ذاكرة التخزين المؤقت. لا ينبغي تقييم التكلفة بشكل منفصل - فالنموذج الرخيص ذو معدل النجاح المنخفض قد يؤدي في الواقع إلى تكبد تكاليف أعلى بسبب إعادة المحاولة المتكررة. يجب حساب متوسط ​​التكلفة لكل مهمة ونسبة التكلفة إلى الأداء.
...
|-------|--------------------------------------------|------------------------|------------|---------|
| 1 | موجه النظام + سؤال المستخدم → قرر الاستعلام عن الطلب | 2,500 (2,000 موجه النظام) | 150 | 0.0098 دولار |
...
من منظور كمي، فإن أدوات جانب الإدخال الأكثر فعالية هي **KV Cache Reuse** (الحفاظ على بادئة مستقرة بحيث تتم محاسبة مطالبات النظام المتكررة وتعريفات الأداة والجولات التاريخية بسعر ذاكرة التخزين المؤقت، مما يقلل تكاليف رمز الإدخال بنسبة 30%-60% — في المثال الثلاثي أعلاه، يوفر التخزين المؤقت ما يقرب من نصف تكلفة الإدخال)، **ضغط السياق** (ضغط المسارات التاريخية، واقتطاعها) مخرجات الأدوات المتكررة، التي تتحكم بشكل مباشر في معدل نمو السياق، وخاصة الفعالة في المهام الطويلة)، و**توجيه النموذج المتدرج** (تنتقل الطلبات البسيطة إلى النماذج خفيفة الوزن والتفكير المعقد يذهب إلى نماذج قوية). تمت مناقشة التطبيقات المحددة لهذه الطرق الثلاث - تصميم استقرار البادئة، وتوقيت الضغط وإستراتيجيته، وآليات التوجيه - بالتفصيل في الفصل الثاني ولن يتم تكرارها هنا. ويكملها هذا الفصل بطريقتين من التقييم والعمليات.
...
>
> **المنهج الفني**: حدد العديد من المهام النموذجية، واستخدم LangSmith أو نظام تتبع مدمج لتسجيل عدد الرموز المميزة للإدخال/الإخراج، وعدد الرموز المميزة للتفكير، وعدد استدعاءات الأداة وأحجام الإرجاع، وزمن الاستجابة الشامل لكل مكالمة LLM. احسب متوسط ​​التكلفة وتوزيع التكلفة (p50/p95/p99) ونسبة تكوين التكلفة لكل نوع مهمة.
>
...
>
> **الهدف**: إجراء تقييم شامل لاختيار النموذج لوكيل استرداد ذاكرة المستخدم، وفحص كيفية تأثير نموذج التضمين وإعادة الترتيب والنموذج الرئيسي للوكيل بشكل مشترك على جودة الاسترداد وزمن الوصول والتكلفة. أعد استخدام `ch3/contextual-retrieval-for-user-memory` أو `ch3/agentic-rag-for-user-memory`، وقارن التكوينات في 60 حالة اختبار.
>
...
تم بناء إمكانية ملاحظة الوكيل على أساس **الآثار**، التي ترث بنية بياناتها مباشرة نموذج شجرة الامتداد من الأنظمة الموزعة: يتوافق تنفيذ مهمة واحدة مع تتبع واحد، حيث يكون كل استدعاء LLM، وكل استدعاء أداة، وكل عملية استرجاع **span** (وحدة تنفيذ تسجل الإدخال/الإخراج، وأوقات البدء/الانتهاء، واستهلاك الرمز المميز، ومعلومات الخطأ). تشكل العلاقات بين الأصل والفرع بين الامتدادات شجرة تنفيذ - على سبيل المثال، قد يحتوي نطاق "Agent Main Loop" على عدة امتدادات فرعية "LLM Call" و"Tool Call" معلقة تحتها. البروتوكولات القياسية متاحة بالفعل لهذه الطبقة: **OpenTelemetry** هو معيار التتبع الموزع للأغراض العامة، في حين أن المواصفات مثل **OpenInference** تحدد الاصطلاحات الدلالية الخاصة بـ LLM فوقها (كيفية تسجيل المطالبات، ومعلمات النموذج، واستخدام الرمز المميز، وما إلى ذلك). وتتمثل ميزة اعتماد البروتوكولات القياسية في فصل الجمع والتحليل - حيث يمكن ربط نفس بيانات التتبع بواجهات خلفية مختلفة للتحليل، مما يؤدي إلى تجنب تقييد البائع.
...
من وجهة نظر هندسة الحزام، يدور هذا القسم بشكل أساسي حول منهجية تحسين الحزام بشكل متكرر - باستخدام بيانات التقييم لتحديد نقاط الضعف في الحزام (سياق غير كافٍ؟ القيود المفقودة؟ التحقق غير الكافي؟ ردود الفعل في غير الوقت المناسب؟)، إجراء تحسينات مستهدفة، ثم إعادة التقييم، وتشكيل حلقة مغلقة للتطور المستمر للحزام.

قبل تحليل أي تقرير مرجعي، لاحظ مبدأ يتم التغاضي عنه بسهولة: **عندما ينخفض أداء الوكيل، تحقق من نظام التقييم أولاً، ثم الوكيل**. الخطأ الشائع هو البدء في تعديل كود العميل في اللحظة التي تنخفض فيها النتيجة، متجاهلاً احتمال تعطل نظام التقييم أولاً - التوجيه بإشارة مشوهة ويكون التصحيح خاطئًا منذ الخطوة الأولى. تشمل حالات الفشل النموذجية في جانب التقييم ما يلي: نفاد الموارد في بيئة التشغيل وعمليات القتل (التي تظهر كإخفاقات عشوائية)، والأخطاء الموجودة في المسجل التي تحدد الإجابات الصحيحة على أنها فاشلة، وحالات الاختبار التي تنحرف عن المزامنة مع سيناريوهات الإنتاج. في الأرقام الرئيسية، تبدو كل هذه الأمور متطابقة مع تدهور النموذج؛ فقط مراجعة الآثار الكاملة يمكن أن تفرق بينهما.
...
بعد تنفيذ التحسينات الثلاثة H1 وH3 وH6 (لم يتم نشر H4)، يرتفع معدل نجاح العميل على AndroidWorld من 88% إلى 94%. وبإعادة تشغيل المعيار الكامل، يكشف التقرير الجديد عن نمط فشل مختلف: فقد تحسنت النسخ والإعدادات ومهام واجهة المستخدم المعقدة بشكل ملحوظ. يتركز معدل الفشل المتبقي، حوالي 6%، في مهام العد التي لم يتم حلها، والتحقق من حالة Wi-Fi غير المستقرة (يصل من 0% إلى 60% ولكنه لا يزال غير مستقر)، وعدد قليل من حالات الفشل الجديدة، التي ربما تكون ناجمة عن مطالبات أطول أو الكثير من معلومات شجرة العناصر التي تشتت انتباه النموذج.
...
>
> هذه التجربة هي ممارسة كاملة في حلقة مغلقة، بدءًا من تقرير التقييم وحتى تحسين النظام. ابدأ بتقرير تقييم AndroidWorld في `ch6/android-world`.
>
...
>
> الخطوة 3: التجريب المرحلي. تصميم تجارب مضبوطة لاختبار الفرضيات. تختبر المرحلة الأولى الفرضيات السطحية منخفضة التكلفة، مثل التحسين السريع وأوصاف الأدوات الأكثر ثراءً. تختبر المرحلة الثانية فرضيات القدرة متوسطة المستوى (تعديل مسار الإدخال، وتبديل وضع التفكير). ركز على حجم التحسين في المهام المرتبطة بعلامات قدرة محددة، مع قياس الآثار الجانبية أيضًا.
>
...
## من التقييم الخارجي إلى التقييم الداخلي: البنية التحتية للتقييم لوكلاء درجة الإنتاج
...
**تعيين مقاييس الدرابزين.** حتى لو تحسن المقياس المستهدف، يجب إيقاف التجربة في حالة انخفاض رضا المستخدم، أو زيادة عدد العمليات، أو ارتفاع معدل الخطأ. مقاييس الدرابزين هي عتبات غير قابلة للتفاوض ويجب ألا تتراجع.
...
### تقييم الحساسية الفوري

يعد موجه النظام هو "الكود" الأساسي لسلوك الوكيل، ولكنه غالبًا ما يفتقر إلى التحكم في الإصدار واختبار الانحدار المتوفر للكود العادي. يتمثل نهج OpenClaw في توفير أداة مخصصة يمكنها استخراج مطالبة النظام المقدمة بالكامل عند مراجعة Git أو التزام محدد - بما في ذلك النص النهائي بعد توسيع جميع الشروط الديناميكية. يتيح ذلك للفريق الإجابة بدقة: **ما الالتزام الذي غيّر المطالبة؟ ما هو التأثير على مجموعة التقييم؟**

بالنسبة لأي فريق وكيل، فإن الممارسات الموصى بها هي: (1) يجب أن يكون موجه النظام قابلاً للعرض بشكل حتمي (بالنظر إلى نفس مدخلات التكوين، فإنه ينتج دائمًا نفس المخرجات)؛ (2) إنشاء آلية لقطة ذات إصدار للمطالبات؛ (3) يجب أن يقوم كل تغيير سريع بإجراء اختبارات الانحدار على مجموعة التقييم - تمامًا كما تتطلب تغييرات التعليمات البرمجية CI.

### تحليلات الخصوصية المدركة كأساس للتقييم
...
الرسالة الأساسية لهذا القسم هي: **علمتك الأقسام السابقة كيفية تقييم الوكيل خارجيًا؛ يكشف هذا القسم عن كيفية تقييم أفضل منتجات الوكيل لنفسها داخليًا.** يخبرك التقييم الخارجي "مدى جودة الوكيل"؛ تخبرك البنية الأساسية للتقييم الداخلي "بالتغيير الذي جعله أفضل". تكتشف تجارب الاستئصال الميزات المهمة حقًا، ويحدد اختبار A/B تأثير كل تغيير، وتوفر علامات الميزات البنية التحتية للتجريب والتراجع، ويدمج تقييم الحساسية الفوري موجه النظام في نظام CI، وتضمن التحليلات المدركة للخصوصية الامتثال في جمع البيانات. تشكل هذه المكونات الخمسة معًا هندسة منتج تعتمد على التقييم، ولا يتم التقييم من حين لآخر، ولكنها تدمج التقييم في كل قرار يتعلق بالمنتج.
...
من منظور هندسة الحزام المقدمة في الفصل الأول، فإن منهجية التقييم في هذا الفصل هي التنفيذ المنهجي لوظيفة "التحقق من صحة" الحزام، في حين أن الحلقة المغلقة "من التقرير المعياري إلى تحسين النظام" هي الآلية الأساسية لتحسين الحزام التكراري. يجيب هذا الفصل على "كيفية القياس بشكل موثوق"؛ وبناءً عليه، يجيب الفصل الثامن على "كيفية تحويل تقييمات المسار متعددة الأبعاد إلى تحديثات نظام قابلة للتنفيذ وقابلة للعكس".
...
6. ★★ يقترح هذا الفصل المنهج العلمي "الملاحظة → الافتراض → التجربة → التحقق من الصحة". ومع ذلك، من الناحية العملية، فإن مساحة سلوك العميل واسعة، وقد يتطلب التحقق من صحة فرضية واحدة مئات من عمليات التقييم. كيف يمكن تعظيم المعلومات المكتسبة من التقييم في ظل ميزانية حسابية محدودة؟
7. ★ في الحالة الافتراضية في هذا الفصل، أدى التفكير التمكيني عالميًا (H4) إلى تحسين معدل النجاح الإجمالي ولكن تم رفضه بسبب زمن الوصول والتكلفة، مما أدى في النهاية إلى التنشيط المشروط (H7). ما هي الإشارات (ميزات وصف المهمة، وأنماط الفشل التاريخية، وعدم اليقين في وقت التشغيل) المناسبة كمعايير توجيه "لتمكين وضع التفكير"؟ هل هناك سيناريوهات للوكلاء حيث يكون التفكير ضارًا بالفعل؟
