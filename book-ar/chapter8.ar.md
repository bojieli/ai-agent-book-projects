...
> **تحديث 2026.** توضح النسخة المنقحة أن عبارة «SFT يحفظ، وRL يعمم» ملاحظة من مقارنات GeneralPoints/V-IRL المضبوطة وليست قانونًا عامًا. كما تميز بين محاكاة مخرجات الأدوات ومحاكاة ديناميكيات البيئة كاملة، وتعدّ انحياز المحاكي سقفًا للتدريب.
>
> وتبرز النسخة مسارين لرفع كفاءة العينات: يحوّل On-Policy Distillation مكافأة نهاية rollout إلى إرشاد على مستوى الرمز، بينما يحوّل RLVP ملاحظات المسار المهدرة عادةً إلى إشارة قابلة للتعلم. وعند غياب معلم أقوى، يستخدم OPSD معلومات مميزة ويجعل النموذج نفسه معلمًا وطالبًا.
>
> ترتيب التجارب في هذه النسخة: 7-13 SimpleVLA-RL؛ 7-14 ReTool؛ 7-15 AWorld-train؛ 7-16 RLVP.
...
> **التجربة 8-1 ★ `[تصميم فقط]`: أداء التعلم Q في لعبة البحث عن الكنز**
>
...
> **التجربة 8-2 ★★ `[تصميم فقط]`: دراسة مقارنة للوكيل التقليدي RL وLLM**
>
>
...
>
> **التجربة 8-7 ★★★ `[تصميم فقط]`: التفكير متعدد اللغات - تمكين النموذج من التفكير بأي لغة `[Extended Experiment]`**
>
...
يمكن تحويل حالات bad case في الفصل السابع إلى بيانات تدريب. في حالة وكيل برمجي ينهي المهمة مبكرًا، نقتطع بادئة المسار قبل إعلان الاكتمال، ونجعل الإعلان الأصلي rejected، بينما يكون «شغّل الاختبارات، وافحص شروط القبول واحدًا واحدًا، ثم استنتج» هو chosen. هذا أنسب لـ DPO أو لعرض حدّ القرار من SFT العادي. يجب حفظ سبب الفشل وشروط التطبيق وأداة التحقق مع كل عينة؛ ويوفر `build_preference_data.py` مسارًا حتميًا ومسارًا بنموذج معلّم مع فصل بيانات التدريب عن التقييم.

يمكن استخدام المهام نفسها كبيئة تدريب لـ RL: يستخدم SFT مسارات تم التحقق منها، بينما يعيد RL تنفيذ المهمة بالسياسة الحالية ويترك للحاقق الخارجي الحكم على النتيجة. وهكذا تحدد bad case حد القرار الذي يجب تحسينه، لا مثالًا للحفظ فقط.
...
>
> **التجربة 8-11 ★★ `[مستودع خارجي]`: النقاط العامة — مقارنة "الذاكرة والتعميم" في دورة واحدة RL**
>
...
> **التجربة 8-12 ★★★ `[مستودع خارجي]`: V-IRL-VL — الملاحة البصرية متعددة الجولات**
>
> يجعل V-IRL[^ch8-24] الوكيل يتنقّل باستمرار في مشاهد شوارع مدينية حقيقية: يجري التدريب على مسارات نيويورك، ثم يُنقل الاختبار إلى مدن مختلفة مع تغيير صياغة الاتجاهات والمظهر البصري في آنٍ واحد. ويتفوّق التعلم المعزز تفوّقًا واضحًا على SFT في الخروج عن التوزيع قاعديًّا وبصريًّا معًا، وهو ما يبيّن أن على السياسة في المهام متعددة الجولات أن تتعلّم إعادة التخطيط بحسب الملاحظة الحالية، لا أن تعيد إنتاج مسارات التدريب. وتستخدم التجربة خوارزمية PPO بشبكة قيمة، ولوحظ فيها أن التغذية الراجعة خطوةً بخطوة تخفّف من صعوبة تخصيص الائتمان على المدى الطويل.

> **التجربة 8-13 ★★★ `[مستودع خارجي]`: SimpleVLA-RL — الاستكشاف المفتوح تحت مكافأة النتيجة `[تجربة موسّعة]`**
>
> لا تستخدم SimpleVLA-RL في مهام الروبوتات ضمن LIBERO سوى مكافأة نتيجة نجاح/فشل. ولكل مهمة مسار توضيحي واحد فقط يُستخدم في تهيئة SFT الباردة، ثم يرفع التعلم المعزز معدّل النجاح من 17.3% إلى 91.7%، ويكتشف حركة "الدفع والقطع" التي لم ترد في العروض التوضيحية أصلًا. وهي تقابل V-IRL تقابلًا مفيدًا: فحين يسهل تعريف إشارة العملية تتسارع بها عملية التعلم، وحين يكون المسار الأمثل مجهولًا تحفظ مكافأة النتيجة المتفرّقة مساحة استكشاف أوسع.
...
**خط أنابيب InstructGPT ثلاثي المراحل.** أنشأ InstructGPT[^ch8-4] الخاص بـ OpenAI العملية القياسية التي لا تزال قيد الاستخدام حتى اليوم:
...
قدّمت DeepSeek خوارزمية **GRPO (Group Relative Policy Optimization)**، وهي اليوم إحدى أكثر الخوارزميات استخدامًا في تدريب RL. تقوم فكرتها الأساسية على تقدير الميزة النسبية بمقارنة مجموعة من عمليات rollout للمسألة نفسها، من دون تدريب شبكة قيمة مستقلة.
...
![الشكل 8-13: تدفق خوارزمية GRPO](images/fig8-13.svg)
...
جدول 8-4 مقارنة بين أساليب ما بعد التدريب وأساليب تحسين وقت الاستدلال
...
بالعودة إلى التجارب في هذا الفصل، فلنكن شفافين بشأن الخوارزميات المستخدمة في كل منها: GeneralPoints وV-IRL (التجارب 7-11، 7-12) تأتي من نفس الدراسة وتستخدم PPO مع شبكة القيمة؛ يستخدم AdaptThink (التجربة 8-10) هدف تحسين مقيدًا مخصصًا مع أخذ عينات الأهمية؛ لاحقًا، يستخدم ReTool (التجربة 8-14) تطبيق PPO معدّل مبني على verRL (تأتي بيانات التدريب الخاصة به من DAPO-Math-17k، لكن خوارزمية التحسين تظل PPO)؛ تعتمد SimpleVLA-RL (التجربة 8-13) وRLVP (التجربة 8-16) على GRPO. في السيناريوهات متعددة المنعطفات، تكون مشكلة تعيين الائتمان أكثر تعقيدًا، وتتمتع الخوارزميات المختلفة بنقاط قوة ونقاط ضعف خاصة بها.
...
ماذا تعني جودة البيانات على وجه التحديد؟ ثلاثة أبعاد على الأقل: **التغطية** (هل تغطي المواقف المختلفة التي تمت مواجهتها أثناء النشر، خاصة الحالات الطويلة والحواف؟)، **التنوع** (هل المتحدثون والأنماط والحلول في العروض التوضيحية غنية بما يكفي؟ وإلا، فإن النموذج سوف ينهار في وضع واحد، مثل "كل شخص يتحدث بنفس النغمة" في التجربة 8-6)، و**دقة التعليق التوضيحي** (هل إجابة العرض التوضيحي نفسها صحيحة؟ خاصة في عملية تقطير سلسلة الأفكار، سيتم تقليد عمليات التفكير الخاطئة من قبل الطالب - ومن ثم تستخدم التجربة 8-9 أداة التحقق من القواعد لتصفية المسارات ذات الإجابات غير الصحيحة أولاً). عادة ما يكون عائد الاستثمار لهذه النقاط الثلاث أعلى بكثير من التحول إلى خوارزمية أكثر روعة.
...
«الإكمال المبكر» مثال واضح. عندما يقول النموذج إن المهمة انتهت، يشغّل Harness اختبارات القبول في مساحة عمل معزولة لا يراها النموذج؛ ولا تُمنح المكافأة الإيجابية إلا عند النجاح. يجب أن تقرأ الاختبارات الملفات أو حالة البيئة الحقيقية، لا كلمة «تم». افصل مجموعة حدود المهام غير المكتملة عن مجموعة الاحتفاظ للمهام المكتملة فعلاً، حتى تقيس الأولى الإنهاء المبكر وتحافظ الثانية على الإنهاء الطبيعي.

### التحدي الأساسي في المهام متعددة الجولات

![الشكل 8-14: مقارنة RL بدورة واحدة وRL](images/fig8-14.svg)

![الشكل 8-15: تعيين الاعتماد في التفاعلات متعددة المنعطفات](images/fig8-15.svg)

إن الانتقال من المنعطف الواحد إلى المنعطفات المتعددة ينطوي على قفزة نوعية في التعقيد. يجب ألا تقتصر السياسة على اختيار الإجراء الأمثل للخطوة الحالية فحسب، بل يجب أيضًا أن تأخذ في الاعتبار قيمة الحالة المستقبلية؛ يجب ألا يتعامل مع التعليقات الفورية فحسب، بل يجب عليه أيضًا إجراء **تخصيص الائتمان** بموجب المكافآت المتأخرة - وتحديد الخطوة في تسلسل متعدد الخطوات التي ساهمت بشكل أكبر في النتيجة النهائية. على سبيل المثال، يقوم وكيل خدمة العملاء بحل مشكلة المستخدم بعد 10 دورات من الحوار ويتلقى مراجعة إيجابية - ولكن هل يجب أن تُعزى هذه المراجعة الإيجابية إلى الاستجواب الدقيق في الدورة 2 أو تفسير المريض في الدورة 7؟
...
> **التجربة 8-16 ★★★ `[مستودع خارجي]`: RLVP — كافئ النتيجة وعاقب المسار**
>
...
![الشكل 8-16: حلقة المكافأة RL لاستدعاء الأداة](images/fig8-16.svg)
...
**قناع المكافأة على مستوى المسار:**
...
> **التجربة 8-14 ★★★ `[مستودع خارجي]`: ReTool — مترجم الأكواد المحسّن لحل المشكلات الرياضية**
>
>
> ![الشكل 8-17: إعادة تشذير تفكير كود النص وحلقة الملاحظات الخاصة بتنفيذ وضع الحماية](images/fig8-17.svg)
>
>
...
>
> **التجربة 8-15 ★★★ `[مستودع خارجي]`: القطار العالمي - تعلم استخدام الأدوات في وضع الحماية**
>
>
> ![الشكل 8-18: AWorld-train MCP بنية التدريب على وضع الحماية والنظام البيئي للأدوات](images/fig8-18.svg)
>
>
...
يعد التقطير على أساس السياسة، الذي تمت صياغته ونشره بشكل منهجي بواسطة Thinking Machines Lab في عام 2025[^ch8-10]، الآن طريقة سائدة بعد التدريب ويستحق شرحًا مناسبًا. لفهم المشكلة التي يحلها، ابدأ بنقطة ضعف قاتلة في كل من SFT وRL: يجمع التقطير حسب السياسة بين نقاط القوة في كليهما.
...
وبطبيعة الحال، فإن حدود هذا النموذج واضحة أيضًا، وتنبع بشكل أساسي من حقيقة أن سقف قدرة المعلم مقيد بالطالب نفسه: **يعتمد حجم المكاسب على "مقدار القدرة الإضافية التي يمكن أن تجلبها المعلومات المميزة."** إذا كان النموذج، حتى مع وجود الإجابة في متناول اليد، لا يمكنه شرح عملية الحل بوضوح (على سبيل المثال، عندما تأتي الإجابة من بحث شامل بدلاً من التفكير الذي يمكن التعبير عنه باللغة)، فإن التقطير الذاتي ليس له مصدر للإشارة. لاحظت الأبحاث الحالية أيضًا أنماط فشل OPSD الساذجة - على سبيل المثال، يفقد النموذج أسلوب تفكيره الأصلي تدريجيًا أثناء التقطير الذاتي ويحتاج إلى مزيد من التنظيم ليظل مستقرًا[^ch8-16]. ولا تزال رؤية "نفس النموذج، وسياقات مختلفة، والمعلم والطالب لبعضهما البعض" تتطور بسرعة، ولكنها فتحت بالفعل الطريق أمام المأزق المشترك المتمثل في "عدم وجود معلم أقوى".

## من حالات bad case إلى التدريب اللاحق

نعود هنا إلى سؤال الفصل السابع: كيف تتحول بيانات التقييم المبنية من حالات الفشل في الإنتاج إلى مدخلات للتدريب اللاحق؟ فسجلات إسناد الفشل، واختبارات الانحدار من البداية إلى النهاية، واختبارات بادئة المسار، ودرجات Rubric، لها استخدامات تدريبية مختلفة.

الجدول 8-4. ربط بيانات تقييم الفصل السابع باستخدامات التدريب في الفصل الثامن

| بيانات تقييم الفصل السابع | استخدامها في تدريب الفصل الثامن |
|---|---|
| مهمة انحدار من البداية إلى النهاية مع أداة تحقق | مهام rollout ومكافآت قابلة للتحقق (RLVR)؛ مجموعة أخذ عينات لـ RFT |
| مهمة انحدار لبادئة المسار | أزواج تفضيل DPO، وأمثلة SFT لحدود القرار، وحالات المعلم لـ On-Policy Distillation |
| سجل إسناد الفشل (أول خطوة خاطئة وفئة الخطأ) | تسميات سلبية للإشراف على العملية (PRM)؛ قواعد عقوبات المسار في RLVP |
| درجات Rubric متعددة الأبعاد ومجموعة ذهبية بشرية | أبعاد المكافآت المتجهية؛ بيانات تدريب ومعايرة GRM |

### الحالة 1: الإكمال المبكر لوكيل برمجي

**من bad case إلى الإسناد.** قد يعلن الوكيل البرمجي «تم» قبل تشغيل الاختبارات، أو ينهي مهمة متعددة الأهداف بعد إنجاز جزء منها، أو يقرر أن المهمة مستحيلة بعد عدة إخفاقات. الخطأ الأول هو حد القرار الذي يستعد عنده للاستنتاج بلا دليل؛ أما فشل الاختبارات وإعادة المحاولة لاحقًا فهما نتيجتان. ويمكن لاستخراج تصحيحات المستخدم والتغذية الراجعة السلبية وعمليات التدقيق اللاحقة كشف هذه الفئة.

**بناء البيانات.** تشغّل مهمة الانحدار اختبارات قبول مخفية عندما يعلن الوكيل اكتمال المهمة: النجاح يحصل على مكافأة موجبة والفشل على مكافأة سالبة. وتحول مهمة بادئة المسار الإعلان المبكر إلى `rejected`، وتجعل «شغّل الاختبارات، وافحص كل شرط قبول، ثم استنتج» هو `chosen`. تُرشّح المرشحات التي يولدها المعلم بواسطة أداة تحقق حتمية، ثم تُنوّع أنواع المهام وشروطها الناقصة وصيغ إعلان الإكمال قبل خلط نسبة صغيرة منها مع بيانات التعليمات لتدريب LoRA.

**التقييم.** يجب تقييم مجموعة حدود المهام غير المكتملة إلى جانب مجموعة احتفاظ للمهام المكتملة فعلًا. تقيس الأولى ما إذا كان النموذج يتحقق بدل أن يتوقف مبكرًا؛ وتتحقق الثانية من أنه لا يزال قادرًا على الإنهاء الطبيعي. وإلا فقد يصبح حذرًا أكثر من اللازم ولا يتوقف أبدًا.

> **التجربة 8-17 ★★: من حالات «الإكمال المبكر» إلى DPO**
>
> **هدف التجربة**: تشغيل السلسلة كاملة من إسناد الفشل إلى بيانات انحدار بادئة المسار، وأزواج تفضيل DPO، وتدريب LoRA لنموذج 7B، وتقييم منفصل لمجموعتي الحدود والاحتفاظ.
>
> **بناء البيانات**: يوفر المشروع المصاحب 24 حالة واقعية تغطي أربع فئات من الفشل، إضافة إلى مجموعة held-out منفصلة (12 حالة حدود و8 حالات احتفاظ). هذه تجربة تعليمية؛ أما الإنتاج فيحتاج إلى عائلات مهام أكثر واختبارات مخفية لا يستطيع النموذج تعديلها أو الاكتفاء بادعاء تشغيلها.

### الحالة 2: علامات الاقتباس الصينية

طلب «تحويل علامات الاقتباس المستقيمة في المقالات الصينية إلى علامات منحنية» ليس قاعدة استبدال عامة. فالرمز ASCII نفسه قد يظهر في النثر الصيني، والاقتباس الإنجليزي، وكود Markdown، وكتل الكود، والتعليقات، وJSON، والمسارات. يمكن تحويل النثر والتعليقات الصينية؛ أما الكود القابل للتنفيذ، والنص الإنجليزي، وJSON/schema، والمسارات، والمعرّفات، والمناطق الملتبسة فيجب الحفاظ عليها.

**من bad case إلى الإسناد.** ينبغي لـ Harness تقسيم الوثيقة حسب النطاق، ومقارنة خرج النموذج بالمقاطع المسموح بها والمحمية، وتشغيل فحوص صياغة Markdown وJSON ولغة المصدر. إذا غيّر العرض أو التسلسل المدخل أولًا فالمشكلة في Harness. وإذا رأى النموذج البايتات الأصلية لكنه غيّر اقتباسًا محميًا أو لم يغيّر اقتباسًا صينيًا مسموحًا، فالفارق الأول خطأ في اختيار النطاق يصلح للتدريب اللاحق.

**بناء البيانات.** تحدد Skill قواعد النطاق الإيجابية والسلبية. تقرن العينات بين النص المصدر والهدف: النثر الصيني والاقتباسات المتداخلة والتعليقات الصينية تعديلات إيجابية؛ والنص الإنجليزي، والـ literals، وJSON، والمسارات، والكود المضمّن وكتل الكود أمثلة سلبية محمية. تُفصل مجموعات train وholdout وboundary حسب القوالب والأنواع وتركيبات المتغيرات واللغات، مع بوابات آلية وتدقيق يدوي طبقي قبل SFT.

**التقييم.** يجب الإبلاغ عن تحويل الاقتباس المستهدف، وحفظ المناطق المحمية، والتعديلات غير المستهدفة، وصحة الصياغة، وexact match للنص الكامل. وفي الإنتاج نحتاج أيضًا إلى مجموعة احتفاظ لوثائق صحيحة أصلًا لاكتشاف التعديل الزائد.

> **التجربة 8-18 ★★: تدريب SFT لعلامات الاقتباس المنحنية الحساسة للنطاق**
>
> **هدف التجربة**: اختبار ما إذا كان LoRA SFT يحول علامات الاقتباس المسموح بها فقط ويحافظ على الصياغة المحمية في تركيبات سياق جديدة.
>
> **الإعداد والبيانات**: Qwen3-8B مع LoRA وbf16، حقبتان و256 تحديثًا؛ 16 نوعًا من المقاطع، و10 أنواع مقالات، و9 لغات برمجة؛ 1024 تدريب، و256 holdout، و256 boundary. تُستخدم Skill كمواصفة للوسوم وبوابة الجودة واختبار الانحدار، مع تدقيق يدوي طبقي لـ48 عينة.
>
> **النتائج**: ارتفع exact في holdout من 0% للنموذج الأساسي إلى 96.9%، وبلغ boundary نسبة 97.7%، وحفظ المناطق المحمية 100%. وصلت فئات Python وJavaScript وJava وGo وRust وSQL وShell وYAML وMarkdown إلى 100%، بينما بقي JSON عند 68.8% ويحتاج إلى مسار مستقل للبيانات المهيكلة.

### الحالة 3: فشل تحرير الملفات المتكرر

تستخدم الوكلاء البرمجية عادةً `edit_file(path, old_string, new_string)`. تطابق الأداة `old_string` حرفيًا؛ لذلك يكفي تغيير byte واحد —مسافة أو سطر جديد أو شرطة مائلة عكسية أو تركيب Unicode أو token نادر— لظهور «old_string not found». إعادة المحاولة عرض وليست بالضرورة السبب الجذري.

**من bad case إلى الإسناد.** قارن أول اختلاف عبر السلسلة التالية:

```text
بايتات الملف الأصلية ← إرجاع الأداة ← تسلسل Harness ← سياق النموذج
← خرج tokens ← السلسلة المفكوكة ← تحليل JSON/tool-call ← مطابقة الأداة
```

تُنسب التغييرات قبل توليد النموذج إلى قارئ الملف أو المسلسل أو Harness. ويجب تدقيق encode→decode في tokenizer منفصلًا. ولا يصنف المثال كفشل نسخ من النموذج ويدخل التدريب اللاحق إلا إذا تلقى النموذج البايتات الأصلية وكان خرجه أول نقطة اختلاف.

**بناء البيانات.** استخدم ثلاث مهام قابلة للتحقق: النسخ الحرفي، واختيار الهدف المعلّم بين hard negatives متشابهة، ووضع الهدف الدقيق في حقل JSON للأداة `old_string`. نوّع الأطوال وتركيبات tokens والسياقات، مع مسافات وأسطر جديدة حقيقية وescape حرفي وشرطات عكسية ومحارف Unicode مركبة وصينية ومحارف zero-width. افصل التقسيمات بحسب seed والطول وتركيب tokens وتغليف السياق.

**التقييم.** افصل مقاييس byte-exact وcode-point-exact وtoken-exact وموضع الاختلاف الأول وround-trip للـ tokenizer عن نجاح الأداة من البداية إلى النهاية. إذا كان النسخ المباشر صحيحًا لكن `edit_file` يفشل، أصلح التسلسل أو بروتوكول الأداة بدل مواصلة تدريب النموذج.

> **التجربة 8-19 ★★: SFT للنسخ الدقيق للسلاسل الخاصة**
>
> **هدف التجربة**: بعد التأكد من أن خرج النموذج هو أول طبقة مختلفة، اختبار LoRA SFT على سلاسل عشوائية غير مرئية واستخدام تدقيق tokenizer مستقل لاستبعاد آثار التجزئة.
>
> **الإعداد والبيانات**: Qwen3-8B مع LoRA وbf16 لحقبتين؛ 1024 تدريب، و256 holdout، و256 boundary لمهام `verbatim` و`decoy_copy` و`tool_json`. يستخدم المولد سلاسل عشوائية قابلة لإعادة الإنتاج، وhard negatives، و10 سياقات لغوية، و8 أنواع مقالات، ومسافات وescape وUnicode وصينية ومحارف zero-width.
>
> **النتائج**: ارتفع byte-exact في holdout من 37.5% إلى 78.9%، وبلغ boundary 80.1%، وكان متوسط موضع اختلاف byte الأول 54.0 و54.2. في 512 probe، بلغ round-trip لـ Qwen3/Qwen2.5 نسبة 80.1% مقابل 100% لـ Mistral؛ ويجب إبقاء أخطاء tokenizer وHarness منفصلة عن نتائج نسخ النموذج.
...
يجيب هذا الفصل عن كيفية تحقيق التطور المستمر للوكيل من خلال تحديث المعلمات. في الفصل التالي سنرى أن المعلمات ليست سوى واحدة من أربع حوامل للتطور الذاتي للوكيل: المعرفة والتعليمات والبرامج والمعلمات.
