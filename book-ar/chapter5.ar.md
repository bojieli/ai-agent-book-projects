# وكيل الترميز وتوليد التعليمات البرمجية

تناولت الفصول السابقة هندسة السياق (الفصلان 2 و3) وتصميم الأدوات (الفصل 4). يجمع هذا الفصل هذه العناصر الأساسية معًا للإجابة على السؤال الأساسي: **كيف تبدو بنية الوكيل للأغراض العامة القادرة على التعامل مع المهام العشوائية؟**

الجواب هو: **الوكيل ذو الأغراض العامة الذي يستهدف المهام المفتوحة** يحتوي في جوهره على **وكيل الترميز** (وكيل يمكنه كتابة التعليمات البرمجية وتعديلها وتنفيذها بشكل مستقل) بالإضافة إلى **نظام الملفات** — مساحة العمل حيث يقوم الوكيل بتخزين التعليمات البرمجية والبيانات والذاكرة والنتائج الوسيطة، مثلما يدير المبرمج المشاريع باستخدام المجلدات الموجودة على جهاز الكمبيوتر. يأتي هذا الاستنتاج من ممارسات الصناعة - بدءًا من Manus إلى OpenClaw، يتبع جميع وكلاء الأغراض العامة الناجحين ذوي النهايات المفتوحة نفس النموذج: أنشئ وقت تشغيل Coding Agent باستخدام مجموعة صغيرة من الأدوات العامة (تنفيذ التعليمات البرمجية، وقراءة/كتابة الملفات، والبحث)، ثم قم بوضع طبقة على وحدات القدرات مثل أتمتة المتصفح والبحث على الويب. حيث ينطبق هذا الاستنتاج - وحيث لا ينطبق - يتم تناوله في نهاية القسم "من Manus إلى OpenClaw".

لماذا يمكن أن يحمل توليد الكود هذا الوزن؟ لأنها ليست مجرد أداة واحدة في صندوق الأدوات، ولكنها **القدرة الوصفية** — القدرة على إنشاء أدوات وقدرات جديدة ديناميكيًا في وقت التشغيل. النصف الأخير من هذا الفصل (قسم "الرمز: القدرة الوصفية لعامل الأغراض العامة") يطور هذا المفهوم بالكامل، إلى جانب الاتجاهات الستة التي ينطبق عليها.

يخدم الكود الوكيل على مستويين. باعتبارها وسيلة **للتفكير**، تفرض التعليمات البرمجية صرامة — حيث تسمح عبارة "العمر أكبر من 18 عامًا والتحقق من الهوية" بقراءات متعددة باللغة الطبيعية، ولكن عند كتابتها كـ `age > 18 and is_verified` فإنها تقبل قراءة واحدة بالضبط. كوسيط **للتعبير**، فإن التعليمات البرمجية التي يتم تشغيلها هي دليل خاص بها على الاتساق المنطقي، وتوفر نتيجة تنفيذها معيارًا موضوعيًا للصحة - وهو أمر لا تستطيع اللغة الطبيعية تقديمه.

يبدأ هذا الفصل بالقدرات الأساسية لعامل الترميز وبنية الوكيل للأغراض العامة (OpenClaw)، ثم يوضح تطبيق إنشاء التعليمات البرمجية في سيناريوهات مختلفة - بدءًا من التفكير الرياضي وإنشاء المحتوى وحتى القدرات الوصفية على مستوى النظام.

## وكيل الترميز

### الترميز باعتباره قدرة الوكيل التأسيسية

**لا يعد إنشاء التعليمات البرمجية مجالًا حصريًا لعدد قليل من الوكلاء المتخصصين، ولكنه قدرة أساسية يجب أن يمتلكها كل وكيل للأغراض العامة.** مع نماذج SOTA الحالية، لا يتطلب منح القدرة الأساسية على التشفير للوكيل أي بنية معقدة.

خذ بعين الاعتبار مهمة نموذجية: "تنظيم كافة تعليقات المهام المتبقية في المستودع، وتصنيفها حسب الأولوية، وإنشاء المشكلات." يتطلب إنجاز ذلك تصفح بنية الدليل (ls/glob)، وقراءة التعليمات البرمجية (قراءة)، وتعديل الملفات (تحرير/كتابة)، وتشغيل الأوامر (bash)، والبحث عن الأنماط (grep/search). تغطي هذه الفئات الخمس من العمليات تقريبًا كل إجراء أساسي لوكيل الترميز، وهي المكان الذي تأتي منه الأدوات السبعة أدناه. بالمعنى الدقيق للكلمة، ترتبط الفئات الخمس بشكل طبيعي بستة أدوات؛ السابع، مترجم الكود، يغطي عمليات "تنفيذ التعليمات البرمجية/الحساب" وفي بعض التطبيقات يتم دمجها ببساطة في Bash - الأدوات السبع عبارة عن مجموعة مرجعية عادية، وليست تعيينًا صارمًا واحدًا لواحد على الفئات الخمس.

يحتاج وكيل الترميز الأساسي فقط إلى أن يكون مجهزًا بالأدوات الأساسية السبعة التالية:

1. **مترجم الكود**: يوفر وضع حماية معزولًا (وقت تشغيل آمن منفصل عن النظام المضيف) حيث يمكن تشغيل كود Python بأمان دون أن تؤثر أخطاء التنفيذ على المضيف
...
تشكل هذه الأدوات السبع مجموعة أدوات كاملة ولكنها بسيطة يمكن لأي نظام وكيل تقريبًا دمجها بتكلفة منخفضة. في التنفيذ، يمكن عرضها جميعًا كخدمات أدوات موحدة عبر بروتوكول MCP المقدم في الفصل 4. لاحظ أن مجموعة الأدوات هذه عبارة عن تكوين أساسي خاص بوكلاء التشفير، يختلف عن فئات الأدوات العامة الخمس (الإدراك/التنفيذ/التعاون/إثارة الحدث/اتصال المستخدم) المصنفة حسب اتجاه الاستدعاء والدور الوظيفي في الفصل 4 - الأدوات الأساسية السبع تغطي بشكل أساسي فئات الإدراك والتنفيذ. ماذا عن التعاون وإطلاق الأحداث والتواصل مع المستخدم؟ في وكيل الترميز، تكون هذه عادةً مهمة إطار العمل، وليس طبقة الأداة - يتم التعامل مع تفويض الوكيل الفرعي، على سبيل المثال، من خلال منطق تنسيق إطار العمل بدلاً من أدوات التعاون المخصصة.
...
استخدمت العملية بأكملها أداتين فقط: Grep (محتوى البحث) والكتابة (كتابة الملف). إذا كانت المهمة أكثر تعقيدًا - مثل "حساب عدد المهام لكل وحدة ورسم مخطط شريطي" - فسيستخدم الوكيل أيضًا مترجم الكود لتنفيذ كود Python للإحصائيات والتخطيط. الأدوات السبعة بسيطة بشكل فردي؛ وهي تغطي مجتمعة مجموعة رائعة من المهام.
...
بالنسبة لوكيل الترميز، فإن التحدي الهندسي المركزي المتمثل في التشغيل بدون جلسة هو **الحفاظ على بيئة تنفيذ التعليمات البرمجية وحالة نظام الملفات عبر الرسائل**. قد تفصل بين رسالتين للمستخدم دقائق أو أيام، ويعتمد عمل الوكيل على قدر كبير من الحالة الضمنية: الحزم المثبتة في وضع الحماية، ودليل عمل الجلسة الطرفية ومتغيرات البيئة، وخوادم تطوير الخلفية، والملفات المكتوبة جزئيًا. نهج OpenClaw هو إدارة الحالة في طبقتين. **حالة نظام الملفات ثابتة بطبيعتها** — يتم تثبيت دليل مساحة العمل على مساحة تخزين ثابتة خارج وضع الحماية، بحيث تبقى التعليمات البرمجية والبيانات والعناصر الوسيطة عبر الرسائل وإعادة تشغيل وضع الحماية؛ وهذا معنى آخر لـ "نظام الملفات باعتباره المحور المركزي للوكيل". **يتم الاحتفاظ بحالة العملية على قيد الحياة أو إعادة بنائها عند الطلب** — يظل صندوق الحماية وجلسته الطرفية قيد التشغيل أثناء فترات النشاط لتجنب البدء البارد، وإعادة الدخول إلى دليل العمل، وإعادة تنشيط البيئة الافتراضية لكل رسالة؛ يتم تدميرها بعد انتهاء مهلة الخمول لاستعادة الموارد، ولكن قبل التدمير، يتم تسجيل حالة البيئة القابلة للتسلسل (دليل العمل، ومتغيرات البيئة، وقائمة مهام الخلفية) في ملفات مساحة العمل، ويقوم الوكيل بإعادة البناء من هذه السجلات عند التنبيه التالي. إن الجلسة الطرفية المستمرة التي تمت مناقشتها في قسم "استمرار الحالة في بيئة تنفيذ الأوامر" لاحقًا في هذا الفصل هي نظير هذه الآلية ضمن مهمة واحدة؛ تعمل الجلسة بدون جلسة على توسيع نفس المشكلة إلى نطاق زمني يمتد للرسائل والأيام.
...
### الأمن لوكلاء الترميز

ينظم هذا القسم دفاعات وكيل الترميز في إطار متماسك: نحدد أولاً **نموذج التهديد** — أي المخاطر هي الأكثر فتكًا؛ ثم **العزل كشبكة أمان** — خروج الشبكة ونظام الملفات وحدود الموارد في وضع الحماية؛ ثم **الدفاع في وقت التنفيذ** — التحليل الدلالي للأوامر، والتنفيذ التأملي الذي يجعل عمليات التحقق من الأمان "غير مرئية"؛ وأخيرًا **الثقة والولاء** — الذين يخدمهم الوكيل بموجب تفويض متعدد الأطراف، وكيفية نقل حدود الثقة إلى طبقة البيانات عندما لا يمكن الوثوق بالرمز المكتوب بواسطة الذكاء الاصطناعي نفسه. ينطبق نموذج التهديد والولاء ومناقشات حدود الثقة على جميع الوكلاء؛ يعتبر وضع الحماية وتحليل الأوامر خاصين بوكلاء الترميز.

ويطرح نموذج "الوكيل السيادي" هذا أيضًا تحديات أمنية خطيرة. يتمتع وكيل الترميز بأذونات قراءة وكتابة الملفات وتنفيذ الأوامر والوصول إلى الشبكات، مما يعني أنه بمجرد حقنه بتعليمات ضارة، فإنه قد يتسبب في أضرار لا يمكن إصلاحها. لخص المطور والباحث المستقل سيمون ويليسون هذا الخطر من خلال كتابه الشهير "Lethal Triad" - عندما تكون العناصر الثلاثة موجودة، فإنها تشكل حلقة هجوم كاملة، مما يعرض النظام لخطر كبير:
...
يؤدي هذا إلى إغلاق حلقة الهجوم: تدخل التعليمات الضارة المخفية في محتوى غير موثوق به إلى العميل، وتدفعه إلى قراءة البيانات الخاصة، ثم تقوم بتصفيتها عبر القنوات الخارجية. لاحظ أن وجود العناصر الثلاثة يشكل خطورة بحد ذاته، دون أي شروط إضافية. وبناءً على ذلك، يضيف المؤلف بُعدًا رابعًا —**الذاكرة الدائمة**. وهذا ليس شرطًا ضروريًا رابعًا موازيًا، ولكنه مضخم للهجمات: يمكن للمهاجم أن يكتب تحيزات تبدو غير ضارة أو تعليمات ضارة في ذاكرة العميل طويلة المدى، حيث تظل كامنة عبر الجلسات ويتم تشغيلها في لحظة مناسبة - مما يحول هجومًا لمرة واحدة إلى تهديد يكمن في الانتظار ويتفاقم بمرور الوقت.
...
وهذا يفسر أيضًا سبب اختيار الوكلاء التجاريين مغلقي المصدر (مثل Claude Cowork (وكيل Anthropic للأغراض العامة للعمل المعرفي، وإعادة استخدام البنية الوكلاءية لـ Claude، والقادرة على قراءة وكتابة الملفات المحلية وإكمال المهام متعددة الخطوات عبر تطبيقات مكتبية متعددة)) استراتيجيات أذونات متحفظة، ليس لأن التكنولوجيا غير كافية، ولكن لأن المخاطر الأمنية مرتفعة للغاية. ضد الحقن السريع، فإن تصفية المدخلات وحدها بالكاد تساعد. الهدف ليس التعرف على كل هجوم، ولكن التأكد من عدم حصول العميل الذي تم حقنه على فرصة لتنفيذ إجراء خطير. تم إنشاء نظام الدفاع طبقة تلو الأخرى في الفصلين السابقين: **الدفاع عن طبقة السياق** - تحديد مصادر المحتوى الخارجي، وعزل الأدوار المنظمة، وتعقيم المدخلات - راجع قسم الحقن الفوري في الفصل 2؛ **الدفاع عن طبقة التنفيذ** — مراجعة Sidecar المستقلة، الإنسان في الحلقة، أقل الامتيازات وفصل الامتيازات — راجع الفصل 4. ونظرًا لأن الوكيل لا يمكنه تحديد ما إذا كان السياق الخاص به قد تعرض للخطر بشكل موثوق، فيجب مراجعة العمليات الحيوية بواسطة آليات خارج هذا السياق. ويسري هذا المبدأ على كلا الفصلين. يضيف هذا القسم فقط ثلاث وسائل حماية محددة فريدة لوكلاء الترميز:
...
- **عزل Sandbox والتحكم في خروج الشبكة** — تنفيذ التعليمات البرمجية هو سطح هجوم فريد لوكلاء التشفير؛ سيتم تناول الخيارات الهندسية لمستويات العزل واستراتيجيات الخروج لاحقًا في هذا القسم؛
- **الدفاع عبر الجلسات من أجل الذاكرة الدائمة** — يوسع هذا الفصل تحليل Lethal Triad ليشمل الذاكرة الدائمة: يجب أن يخضع المحتوى المكتوب إلى الذاكرة طويلة المدى لنفس مراجعة الثقة مثل المدخلات الخارجية بحيث لا يمكن أن تظل التعليمات الضارة خاملة في `MEMORY.md` وتصبح سارية المفعول لاحقًا.

وتندرج هذه الحماية الثلاث ضمن طبقات التحقق والتنفيذ والبيانات على التوالي، مما يكمل نظام الدفاع من الفصلين السابقين. لا يمكن لهذه الاستراتيجيات القضاء على المخاطر بشكل كامل، ولكنها يمكن أن تقلل من مساحة الهجوم الخاصة بالعميل.

**العزل كشبكة أمان: الاختيارات الهندسية لصندوق الحماية لتنفيذ التعليمات البرمجية.** صندوق الحماية ليس مفتاحًا؛ إنها سلسلة من القرارات الهندسية. لقد أوضح الفصل الرابع بالفعل سبب ضرورة العزل، وحدد نطاقًا ثلاثي الطبقات لآليات العزل (العزل على مستوى العملية، والحاويات، وmicroVMs)، وقدم قاعدة الاختيار هذه: العزل على مستوى العملية للأجهزة المحلية الشخصية، وحاويات البيئات السحابية ذات المستأجر الواحد، وmicroVMs أو gVisor للبيئات متعددة المستأجرين أو التعليمات البرمجية غير الموثوق بها. بدلاً من تكرار هذا النطاق، يتناول هذا القسم أربعة مخاوف إضافية تنشأ عند تنفيذ وكيل التشفير: كيفية إدارة خروج الشبكة، ومقدار نظام الملفات الذي يجب تركيبه، وكيفية الحد من الموارد، وكيفية التوفيق بين الجلسات المستمرة والعزل.

**التحكم في خروج الشبكة.** هذا هو العنصر الأكثر أهمية الذي يتم التغاضي عنه بسهولة: لا توجد شبكة بشكل افتراضي، مع منح الوصول عند الطلب من خلال وكيل القائمة البيضاء إلى مجموعة محدودة من الوجهات (مصادر الحزم، ومواقع التوثيق، وواجهات برمجة التطبيقات التي تتطلبها المهمة صراحة). إذا نظرنا إلى البند 3 من الثالوث المميت - "القدرة على التواصل خارجيًا" - فإن التحكم في خروج الشبكة هو دفاع طبقة التنفيذ: حتى لو نجح الحقن الفوري وقرأت التعليمات البرمجية الضارة البيانات الحساسة داخل صندوق الحماية، بدون مسار خروج، لا يمكن نقل البيانات. بالمقارنة مع محاولة التعرف على كل حقنة، فإن قطع قناة استخراج البيانات يعد خط دفاع أكثر حتمية.
...
آليات الأمان المذكورة أعلاه تمنع "تنفيذ الأوامر بشكل ضار"؛ هناك مشكلة أمنية أكثر دقة —**الولاء الرئيسي**: **الجانب الذي يقف فيه العميل فعليًا**. يتم تدريب النماذج بمبدأ افتراضي ساذج - "كل من يتحدث معي، سأبذل قصارى جهدي لمساعدته" - لكن الوكلاء في العالم الحقيقي غالبًا ما يعملون بموجب **تفويض متعدد الأطراف**: يتصرفون نيابة عن مدير أثناء التعامل مع أطراف ثالثة تتعارض مصالحها. الوكيل الذي يتفاوض على السعر نيابةً عنك لا يواجه "مستخدمًا بحاجة إلى المساعدة" بل **خصمًا متفاوضًا**. هنا، يعد "مساعدة من يتحدث" أمرًا افتراضيًا خطيرًا - حيث يمكن للطرف المنافس أن يبدأ في التأثير على وكيلك ببساطة عن طريق إشراكه.
...
يرتبط هذا بشكل خاص بوكلاء التشفير: المحتوى غير الموثوق به المقروء من المستودع، والمخرجات التي يتم إرجاعها بواسطة أداة، والتعليمات المرسلة بواسطة خادم MCP لجهة خارجية - كلهم "معارضون" يحاولون تحويل الوكيل - **الحقن الفوري هو في الأساس محاولة لتحويل** (الفصلان 2 و 4). لذلك، يجب على الحزام أن يحدد صراحة من هو الوكيل المخلص: تحمل التعليمات الواردة من المدير الأولوية القصوى، في حين يتم تخفيض تصنيف كل شيء من الأطراف الخارجية افتراضيًا إلى "بيانات يمكن الرجوع إليها ولكنها لا تحمل قوة التعليمات". في موجه النظام، **قواعد سلوك الولاء** الفعالة هي: حماية المعلومات الخاصة للمدير، بما في ذلك حقيقة وجودها؛ عند الرفض، لا تسرد التفاصيل المحمية، لأن ذلك قد يؤدي في حد ذاته إلى تسريبها؛ المحصلة النهائية الخاصة ليست مناصب عامة؛ تنفيذ تعليمات مدير المدرسة الواضحة والمحددة فقط؛ تحمل الضغط المتكرر. في الأساس، يتم استخدام هذا الحزام لمنح النموذج الموقف الذي يفتقر إليه افتراضيًا: **الولاء المطلق للمدير، والحذر تجاه الأطراف الخارجية**.
...
**عندما يكون الكود المكتوب بواسطة الذكاء الاصطناعي في حد ذاته غير جدير بالثقة: نقل حدود الثقة إلى الأسفل.**

رمز الولاء أعلاه يجعل الوكيل **أكثر احتمالاً** لاتباع القواعد، ولكن بالنسبة لعمليات البيانات عالية المخاطر، فإن "احتمال أكبر" لا يكفي - يجب أن تنتقل القيود من "الأمل في أن يتصرف الوكيل" وصولاً إلى التنفيذ في طبقة البيانات. الموقف الأكثر تطرفًا[^ch5-2] هو: **تعامل ببساطة مع طبقة التطبيق على أنها غير جديرة بالثقة وتدفع إلى فرض ثوابت البيانات أسفلها**. على مدار الثلاثين عامًا الماضية، ظلت حدود سلامة البرامج موجودة في **طبقة التطبيقات** — حيث يحدد رمز المعالج من يمكنه تنفيذ كل عملية والقيم الصالحة، وتثق قاعدة البيانات في هذا الرمز دون قيد أو شرط؛ لكن المعالجات التي تم إنشاؤها بواسطة LLM غالبًا ما تتجاهل الأذونات وعمليات التحقق من النزاهة التي قد يدرجها المؤلفون البشريون كأمر طبيعي، ويعمل الوكلاء المستقلون مباشرة على بيانات الإنتاج، مما يكسر هذه الفرضية. النهج الجديد (الذي يمكن أن يسمى كائنات البيانات المضمنة بالإذن) يجعل كل كيان بيانات يحمل قواعد إذن تصريحية، وأدوات التحقق من الصحة، وبيانات العواقب ضمن **مخطط يراجعه الإنسان**، ويتم فرضه بواسطة خط أنابيب وقت التشغيل على **كل كتابة**. المفتاح الأساسي هو **سياق الوصول** المرفق بكل عملية: يعمل المعالج المُعاد إنشاؤه بأذونات المستخدم الذي يخدمه، بينما يعمل الوكيل المستقل تحت هويته المقيدة (مبدأ النطاق) - بدلاً من مجرد الأمل في أن يظل الوكيل مخلصًا، تعامله البنية كمدير مقيد، بحيث لا يمكنه تجاوز أذوناته حتى إذا تم اختراقه.

في المقارنات باستخدام نفس مجموعة المطالبات، أنتجت هذه الآلية **صفرًا من عمليات الكتابة التي تنتهك الثوابت المعلنة**، في حين أن SQL، والشيكات المكتوبة بـ LLM، والمطالبات الدستورية، واعتراضات حدود الإجراء، تسمح كل منها بالمرور في أي مكان من حفنة إلى عشرات الانتهاكات. ليس "من المرجح أن يكون صحيحًا" ولكن "من المستحيل أن يكون مخطئًا" بتكلفة حوالي 2 ميلي ثانية إضافية لكل كتابة. وبطبيعة الحال، فإن الضمان مشروط: يجب أن يلتقط المخطط جميع الثوابت المطلوبة، ويجب أن يحظر النشر كل مسار يمكن من خلاله للطبقة غير الموثوقة تجاوز التخزين والاتصال مباشرة بقاعدة البيانات. بالنسبة لوكلاء الترميز، يؤدي هذا إلى مبدأ معماري مهم: **عندما يكون كل من كاتب الكود ومشغل الكود غير موثوقين، لا يمكن أن توجد قيود موثوقة حقًا في الكود الذي تم إنشاؤه، ولكن يجب وضعها في الأساس الذي يراجعه الإنسان تحته** - هذا هو الشكل النهائي لمبدأ "القيود على التوجيه" من الفصل الأول، المطبق على طبقة البيانات.
...
بعد التنفيذ، يدخل الوكيل على الفور في مرحلة ضمان الجودة القائمة على الاختبار - حيث يقوم بكتابة حالات اختبار للوظيفة الجديدة أو المعدلة، والتي تغطي المسارات العادية وشروط الحدود وسيناريوهات الخطأ. بعد كتابة الاختبارات، يقوم الوكيل بتنفيذ مجموعة الاختبار. إذا فشلت الاختبارات، فلا يجب على الوكيل إبلاغ المستخدم بالفشل فحسب، بل يجب عليه تحليل السبب وتحديد المشكلة وتعديل التعليمات البرمجية حتى تنجح جميع الاختبارات. قد تتطلب حلقة "الإصلاح الاختباري" هذه عدة تكرارات، وهذه القدرة على التصحيح الذاتي هي التي ترفع مستوى وكيل التشفير من مولد التعليمات البرمجية إلى مساعد هندسي موثوق. على العكس من ذلك، فإن الطريقة الأكثر شيوعًا التي يتباطأ فيها وكيل الترميز هي تخطي هذه المرحلة تمامًا - كتابة الكود والإبلاغ عن "اكتمال المهمة" دون إجراء الاختبارات على الإطلاق. تعريف "اجتياز الاختبارات" بدلاً من "كتابة التعليمات البرمجية"، حيث أن معيار الإكمال هو على وجه التحديد مبدأ Loop Engineering المتمثل في السماح للتحقق بتحديد متى يكون التوقف آمنًا، ويتم تطبيقه على الترميز (يناقش الفصل العاشر هذه الفئة من "الإنهاء المبكر" بشكل منهجي).
...
### هندسة تسخير الممارسة العملية لوكلاء الترميز

قدم الفصل الأول مفهوم هندسة الحزام والصيغة **الوكيل = النموذج + الحزام**. تتضمن الأداة المساعدة هنا السياق والأدوات من الصيغة الأساسية، بالإضافة إلى القيود والتحقق وآليات التصحيح - تشكل هذه العناصر الخمسة معًا الأداة المحددة في الفصل الأول. ربما يكون وكلاء التشفير هو المجال الذي تحقق فيه Harness Engineering معظم النتائج - كتابة التعليمات البرمجية هي **الأكثر قابلية للتحقق** من بين جميع مهام العميل، ويمكن أن تعتمد قيودها والتحقق والتصحيح على البنية التحتية الحالية. يركز هذا القسم على الممارسة الملموسة في سيناريو وكيل الترميز.

ما إذا كان النظام يعمل بشكل مستقر غالبًا ما يعتمد بدرجة أقل على قوة النموذج وبشكل أكبر على قوة البنية التحتية المبنية حول الوكيل. يقسم الفصل الأول الأدوات إلى طبقتين - **السياق والأدوات** (تمكين الوكيل من التصرف) و**القيود والتحقق والتصحيح** (مساعدة الوكيل على التصرف بأمان وبشكل صحيح). في سيناريو وكيل الترميز، تُترجم هذه العناصر إلى مكونات هندسية محددة:
...
تشغل مهام كتابة التعليمات البرمجية بشكل طبيعي ربع "الهدف الواضح + التحقق الآلي" - توفر مجموعات الاختبار معايير قبول واضحة، وتوفر أجهزة الفحص والتحقق من الكتابة التحقق الآلي الفوري، ويوفر Git تحكمًا مثاليًا في الإصدار وإمكانيات التراجع. وهذا ما يفسر سبب كون وكلاء التشفير هم الأكثر نضجًا حاليًا بين جميع أنواع الوكلاء: ليس لأن نماذج توليد التعليمات البرمجية قوية بشكل خاص، ولكن لأن عقودًا من البنية التحتية لهندسة البرمجيات تشكل بطبيعة الحال أداة قوية.

**ممارسة الصناعة.**

تؤكد ثلاث دراسات حالة لممارسة الحزام المبادئ المذكورة أعلاه:
...
- **LangChain**: تم تحسين أداء المهام المعيارية بشكل ملحوظ فقط من خلال تحسين الأداة (مطالبات النظام، والبرمجيات الوسيطة للأداة، وحلقات التحقق الذاتي). تجدر الإشارة بشكل خاص إلى منهجية "استخدام وكيل لتحليل مسارات الفشل لتحسين الحزام"، وتحويل هندسة تسخير من تعتمد على الخبرة إلى تعتمد على البيانات.
...
**من وكيل الترميز إلى المبادئ العامة لتصميم الأدوات.**

توفر ممارسات تسخير وكلاء الترميز مبادئ تصميم قابلة للتحويل لجميع أنظمة الوكلاء:

1. **القيود المفروضة على الإرشادات**: يجب ترميز القواعد التي يمكن فرضها باستخدام التعليمات البرمجية هناك، وليس مجرد اقتراحها في الوثائق. إن قيمة قواعد linter، وقيود النوع، وعمليات فحص CI تتجاوز بكثير إرشادات "الرجاء اتباع..." في مطالبات النظام - فالأول يعني "لا يمكن القيام به"، أما الأخير فهو مجرد "ينصح به".
...
بعيدًا عن الأخطاء الفردية، اكتشف **الأنماط**. أولاً، بصمات المكالمات المتكررة: قم بتجزئة زوج "اسم الأداة + الوسائط"؛ تكرار بصمة الإصبع نفسها هو إشارة واضحة إلى حلقة عدم التقدم - كان العامل في تجربة الاستئصال في الفصل الأول الذي يستدعي نفس الأداة مرارًا وتكرارًا هو هذا النمط بالضبط. ثانيًا، عدادات الفشل المتتالية: يحتفظ كل مسار استرداد بالعداد الخاص به، مما يوفر الأساس لقواطع الدائرة التي سيتم مناقشتها لاحقًا.

لا تظهر الفئة الثالثة من حالات الفشل كأخطاء على الإطلاق وتتطلب مراقبة مخصصة للحيوية والنزاهة**. أخطر وضع فشل في اتصال التدفق ليس السقوط (الذي ينتج عنه خطأ على الفور) ولكن التوقف الصامت - يظل الاتصال قائمًا، ولكن يتوقف تدفق البيانات، مثل الأنبوب المتصل الذي لا ينتج ماء. غالبًا ما تغطي مهلات SDK الاتصال الأولي فقط، وليس عملية النقل، لذلك يحتاج وكيل الإنتاج إلى هيئة مراقبة خاملة مستقلة (مؤقت مراقبة - إذا لم يصل أي مخرج جديد خلال فترة زمنية محددة، فسيتم الحكم على الاتصال بأنه متوقف) الذي يقتل الدفق المعلق ويؤدي إلى إعادة المحاولة عند انتهاء المهلة. يتلخص هذا في مبدأ: **يحتاج كل اتصال طويل الأمد إلى إشارة حيوية، وليس مجرد مهلة اتصال**. يستهدف رصد النزاهة هيكل المسار: عندما يتبين أن استدعاء الأداة يفتقر إلى رسالة النتيجة المقترنة بها، يقوم النظام بإصلاح الاقتران قبل إدخال السياق، بدلاً من إلقاء الشذوذ الهيكلي على النموذج أو المستخدم. إحدى التفاصيل الهندسية البارزة: يقوم بعض وكلاء الإنتاج بتشغيل كل من وضع الإنتاج ووضع جمع بيانات التدريب - قد يقوم وضع الإنتاج بتصحيح الرسائل المفقودة باستخدام العناصر النائبة، بينما يرفض وضع التدريب الإصلاح، لأن العناصر النائبة الاصطناعية قد تلوث بيانات التدريب. يعكس هذا المعيار المزدوج "المتساهل في الإنتاج والصارم في التدريب" الاقتران العميق بين السرج والتدريب النموذجي.
...
1. **إعادة المحاولة الصامتة**. الإجراء الافتراضي للأخطاء القابلة لإعادة المحاولة. هناك تفصيلان يحددان ما إذا كانت إعادة المحاولة ستنجح: أولاً، استخدم التراجع الأسي مع الارتعاش العشوائي لمنع أساطيل العملاء من إعادة المحاولة بشكل متزامن والتسبب في ازدحام ثانوي، مع احترام مدة الانتظار المقترحة للخادم؛ ثانيًا، التمييز بين مكالمات المقدمة والخلفية - تتم إعادة محاولة طلب الحلقة الرئيسية الفاشل، ولكن يتم إسقاط مكالمات الخلفية المساعدة (إنشاء العنوان، واقتراحات الإدخال) عند الفشل، خشية أن تؤدي عمليات إعادة المحاولة في الخلفية إلى إزاحة حصة الحلقة الرئيسية وإنشاء "إعادة محاولة التضخيم".
...
بالعودة إلى السؤال الفكري الخاص بالفصل الأول، يمكن أن يصبح العميل محاصرًا في حلقة ليس فقط بسبب فقدان نتائج الأداة ولكن أيضًا بسبب تكرار أخطاء الأداة المتطابقة، أو الاستدعاءات الهلوسة، أو ضغط السياق الذي يفقد حالة المفتاح، أو مهمة غير قابلة للحل. يعتمد الاكتشاف على "تصنيف الأخطاء + التعرف على الأنماط"، والاسترداد على "التصعيد المتدرج"، والإنهاء على "قواطع الدائرة + الحدود القصوى الشاملة + التصعيد البشري" - وهذه معًا هي إجابة Harness الكاملة على "قد يعمل العميل إلى الأبد". ما تحله هذه الآليات ليس "القدرة النموذجية غير الكافية" ولكن "متانة النظام في ظل الظروف الحدودية": ستستمر النماذج في التزايد، لكن الشبكات ستنخفض، وستتوقف العمليات، وسيقوم المستخدمون بأشياء غير متوقعة. والأهم من ذلك، **لا يتم تحديد موثوقية الوكيل من خلال ما إذا كان يرتكب أخطاء، ولكن من خلال ما إذا كان لكل فئة من الأخطاء مسار اكتشاف واسترداد وإنهاء مناسب**.

### نصائح التنفيذ لوكلاء الترميز
...
يجب على وكلاء الترميز الحديثين الاستفادة بشكل كامل من استجابات التدفق: قدم الفصل الثاني هذه الآلية عند مناقشة ترتيب مخرجات النموذج - بمجرد إنشاء معلمات استدعاء الأداة الأول بالكامل واجتياز التحقق من الصحة، يمكن أن يبدأ التنفيذ على الفور، دون انتظار النموذج لإنشاء استدعاءات الأداة اللاحقة. على سبيل المثال، إذا كان النموذج يحتاج إلى إخراج ثلاثة استدعاءات للأدوات في استدلال واحد - كود البحث، والتحقق من ملفات التكوين، وقراءة السجلات - فيمكن أن يبدأ الاستدعاء الأول في التنفيذ بمجرد اكتمال معلماته والتحقق من صحتها، مع التداخل مع إنشاء الاستدعاءين الآخرين. يمكن أيضًا تنفيذ المكالمات المستقلة بالتوازي بدلاً من وضعها في قائمة الانتظار. يؤدي هذا التنفيذ المتداخل إلى تقليل زمن الوصول الشامل بشكل كبير، مما يجعل استجابات الوكيل أكثر مرونة.

الجانب الآخر من التنفيذ المتوازي هو معالجة الأخطاء. يجب أن يعلن كل تعريف أداة ما إذا كان يدعم التنفيذ المتزامن (الافتراضي هو لا، آمن من الفشل). عند فشل مكالمة، تقوم آلية الإجهاض المتتالية بإنهاء المكالمات الأخرى التي بدأت في نفس الدفعة والتي تعتمد على نتيجتها، ولكنها لا تؤثر على المكالمات المستقلة أو العملية الأصلية - وهذا تطبيق ملموس لمبدأ "التحكم في حدود الخطأ" من قسم هندسة الحزام.

**إدارة السياق الدقيقة.**

التحدي الأساسي الذي يواجه وكلاء الترميز هو أن قواعد التعليمات البرمجية عادة ما تكون كبيرة، ولكن نافذة سياق النموذج محدودة. حتى لو ادعت النماذج المتقدمة أنها تدعم ملايين الرموز، فإن حشو قاعدة التعليمات البرمجية بأكملها في السياق ليس اقتصاديًا ولا ضروريًا. تحتاج إدارة السياق الذكية إلى العمل على مستويات متعددة.

على مستوى قراءة الملف، لا ينبغي للوكيل دائمًا قراءة الملف بأكمله. بالنسبة للملفات الكبيرة، يجب أن تدعم الأداة قراءة نطاقات أسطر معينة - على سبيل المثال، قراءة الأسطر من 100 إلى 150 فقط، بدلاً من تحميل ملف يحتوي على آلاف الأسطر. والأهم من ذلك، عند إرجاع المحتوى، يجب إرفاق أرقام الأسطر - كل سطر من التعليمات البرمجية يكون مسبوقًا برقم السطر الفعلي الخاص به. يجلب هذا التصميم الذي يبدو بسيطًا قيمة كبيرة: يمكن للنموذج أن يشير بدقة إلى "السطر 42 من `src/main.py`"، مما يقلل من الغموض ويجعل عمليات التحرير اللاحقة أكثر موثوقية.
...
يعد هذا مظهرًا مركزًا لتقنية شريط حالة الوكيل من الفصل الثاني في وكلاء الترميز. على عكس الوكلاء العامين، يعتمد وكلاء الترميز بشكل كبير على حالة بيئة التنفيذ. قبل كل استدلال، يجب إدخال معلومات البيئة الرئيسية التالية في نهاية السياق في شكل شريط حالة الوكيل:
...
لا ينبغي ترميز هذه المعلومات ضمن مطالبات النظام الثابتة - والتي قد تؤدي إلى تدمير كفاءة KV Cache - ولكن يجب أن يتم إنشاؤها وإدخالها ديناميكيًا كشريط حالة الوكيل الملحق. بهذه الطريقة، يكتسب الوكيل "الوعي البيئي"، حيث يعتمد كل قرار على فهم دقيق للحالة الحالية، بدلاً من الافتراضات التي عفا عليها الزمن.
...
يوضح هذا مرة أخرى قيمة تقنية شريط حالة الوكيل. بعد قيام الوكيل بتعديل التعليمات البرمجية، يجب ألا ينتظر المستخدم ليطلب الاختبار بشكل صريح قبل التحقق من بناء الجملة. تتمثل الطريقة الأكثر فعالية في أن تقوم طبقة الأداة بتشغيل مدقق linter أو بناء الجملة المقابل تلقائيًا بمجرد اكتمال عملية كتابة الملف وتقديم النتائج كجزء من القيمة المرتجعة للأداة إلى الوكيل. إذا تم اكتشاف خطأ في بناء الجملة، يرى الوكيل معلومات الخطأ التفصيلية على الفور في جولة الاستدلال التالية - تمامًا كما يقوم IDE بوضع علامة على قوس غير متطابق على الفور. تعمل آلية ردود الفعل الفورية هذه على تقليل تكلفة إصلاح الأخطاء بشكل كبير، لأن الوكيل يمكنه تصحيح الخطأ في لحظة تقديمه، دون الانتظار حتى إجراء الاختبارات لاكتشاف المشكلة.

تشكل تقنيات التنفيذ الخمسة هذه - التوازي والتدفق، وإدارة السياق، والوعي البيئي، واستمرارية الحالة، والتغذية الراجعة الفورية - معًا الأساس الفني لوكيل الترميز الفعال. إنها ليست نقاط تحسين معزولة، ولكنها قرارات تصميمية يعزز بعضها بعضًا، وتشير جميعها نحو هدف واحد: تمكين الوكيل من العمل بسلاسة مثل المطور ذي الخبرة.

### أدوات البحث في وكلاء الترميز

يعد تحديد موقع التعليمات البرمجية ذات الصلة في قاعدة تعليمات برمجية كبيرة بمثابة نقطة البداية لعمل وكيل الترميز. ويقارن الشكل 5-3 بين العديد من أدوات البحث التكميلية، موضحًا كيف ينبغي لوكيل الترميز الناضج أن يختار طرق الاسترجاع بناءً على طبيعة المهمة.

![الشكل 5-3: مقارنة أدوات البحث عن وكيل الترميز](images/fig5-3.svg)
...
### أدوات تحرير الملفات في وكلاء الترميز
...
**نصيحة عملية.** ينقسم وكلاء الترميز السائدون إلى معسكرين، كل منهما له الرائد الخاص به: رمز Claude يأخذ "السلسلة القديمة إلى السلسلة الجديدة" - الموثوقية أولاً، وسهولة التنفيذ، ولا حاجة إلى نموذج إضافي؛ لقد دفع المؤشر مسار تطبيق النموذج إلى أقصى حدوده — حيث دفع تكاليف التدريب والاستدلال على نموذج مخصص سريع التطبيق مقابل إنتاجية تحرير أعلى. إذا كنت تقوم ببناء وكيل خاص بك، فإن "السلسلة القديمة إلى السلسلة الجديدة" هي نقطة البداية الأكثر أمانًا؛ بالنسبة للتعديلات واسعة النطاق، فإن "بداية السلسلة + مطابقة النهاية" هي الحل الوسط الأكثر اقتصادا؛ لا يمكن الاعتماد على نهج رقم السطر إلا من خلال تكامل IDE العميق (حيث يحتفظ المحرر بتعيين مباشر لرقم السطر ويعيد توفير النموذج بعد كل تحرير) - وإلا فإن انحراف رقم السطر سوف يغرقه.

## الكود: القدرة الفوقية للوكيل العام

أظهر القسم السابق كيفية إنشاء وكيل ترميز موثوق به — بدءًا من الهندسة المعمارية وحتى تنفيذ الأدوات وحتى هندسة التسخير. لكن قيمة توليد التعليمات البرمجية تمتد إلى ما هو أبعد من كتابة البرامج.
...
دع LLM يكون مسؤولاً عن فهم المشكلة وكتابة الكود، ودع مترجم الكود يكون مسؤولاً عن الحساب الدقيق - يتيح تقسيم العمل هذا لكل فرد الاستفادة من نقاط قوته.

قدم ستيفن ولفرام، مبتكر Mathematica، نظرة عميقة حول هذا الأمر. قبل وجود نماذج LLM، كانت هناك بالفعل أنظمة قادرة على إجراء حسابات رياضية دقيقة، وكانت تعمل باستخدام **الحساب الرمزي**، أي معالجة التعبيرات باستخدام الرموز الرياضية بدلاً من القيم الرقمية التقريبية. على سبيل المثال، الآلة الحاسبة التقليدية ستقرب $\sqrt{2}$ إلى 1.414، في حين أن نظام الحساب الرمزي سيحتفظ بالشكل الدقيق $\sqrt{2}$، ولا يتحول إلا إلى رقم عشري عند الضرورة. Wolfram Alpha، الذي أنشأه Wolfram، هو مثل هذا النظام: يقوم المستخدمون بإدخال مسألة رياضية، وتقوم بإرجاع إجابة دقيقة. ومع ذلك، فإن فهم اللغة الطبيعية هش للغاية وتغطيته ضيقة - فهو يعتمد على محلل نحوي مدمج يمكنه التعرف فقط على مجموعة محدودة من العبارات؛ قد يؤدي التغيير الطفيف في الصياغة إلى فشل التحليل، ومن المؤكد أنه لا يمكنه التعامل مع المنطق متعدد الخطوات للمجال المفتوح. نماذج LLM يملأون هذه الفجوة بشكل مثالي، فهم يتفوقون في فهم تعبيرات اللغة الطبيعية المختلفة ولكنهم لا يجيدون الحساب الدقيق. النموذج التعاوني الجديد هو: دع LLM يكون مسؤولاً عن فهم سؤال اللغة الطبيعية للمستخدم، وتحديد البنية الرياضية أو المنطقية داخله، وترجمته إلى لغة رسمية (مثل لغة Mathematica أو مكتبة SymPy الخاصة بـ Python)؛ ثم قم بتسليمها إلى محرك حساب رمزي مخصص أو أداة حل القيود للتنفيذ للحصول على نتائج دقيقة.

> **التجربة 5-1 ★★: استخدام أدوات إنشاء التعليمات البرمجية لتحسين القدرة على حل المشكلات الرياضية**
>
> **هدف التجربة**: التحقق من تحسين دقة التفكير الرياضي للوكيل عندما يحصل على مساعدة من مترجم الكود.
>
> **المنهج الفني**: تجهيز الوكيل بصندوق حماية Python يحتوي على مكتبات رياضية مثل Sympy وnumpy وscipy. عندما يواجه الوكيل مشكلة رياضية، فإنه يقوم بإضفاء الطابع الرسمي عليها في كود Python:sypy للحساب الرمزي (حساب التفاضل والتكامل، حل المعادلات)، scipy للتحسين العددي، numpy لعمليات المصفوفة. يتم تنفيذ التعليمات البرمجية التي تم إنشاؤها في وضع الحماية لإرجاع نتائج دقيقة.
>
...
>
> **المنهج الفني**: قم بتجهيز الوكيل بمترجم التعليمات البرمجية الذي يحتوي على مكتبة القيود python. يترجم الوكيل الألغاز المنطقية، مثل مسائل الفرسان والأشرار، إلى نماذج قيود رسمية: فهو يحدد المتغيرات (هوية كل فرد من سكان الجزيرة)، وترميز قواعد مثل "الفرسان يقولون الحقيقة" كقيود، ويستدعي الحل للعثور على مهمة مُرضية.
>
...
تكشف هذه التجربة أيضًا عن نمط أكثر عمومية: النموذج والحزام يتنافسان ضد بعضهما البعض. عندما يكون النموذج قويًا بدرجة كافية، يمكن أن يصبح الحزام أرق، حيث يفكر النموذج بشكل صحيح من تلقاء نفسه، وتتقلص المكاسب من حل التعليمات البرمجية. عندما يكون النموذج أضعف، يجب على الحزام بذل المزيد من الجهد - تفريغ المنطق المنطقي الرئيسي إلى أدوات حل التعليمات البرمجية والقيود لضمان الصحة. ولهذا السبب تستخدم هذه التجربة نموذجًا أضعف عمدًا لتضخيم التباين: في النموذج الضعيف، يخطئ التفكير الخالص في التقدير باستمرار وتساعد المساعدة البرمجية على رفع الدقة بشكل كبير؛ في نموذج تفكير قوي بما فيه الكفاية، غالبًا ما يحل التفكير الخالص كل الألغاز، ويتقارب المكاسب من المساعدة البرمجية إلى ما يقرب من الصفر. إذن، يعتمد مدى سماكة الحزام على المكان الذي تقع فيه حدود قدرة النموذج الخاص بك - وهي فرضية يتم التغاضي عنها بسهولة عند تقييم أي تقنية وكيل: نفس الحزام، مقترنًا بنماذج ذات قوة مختلفة، يمكن أن يدعم استنتاجات معاكسة.

### التعليمات البرمجية كقيد لقواعد العمل

يعد هذا القسم ردًا مباشرًا على قسم هندسة الحزام الموجود سابقًا في هذا الفصل. أحد المبادئ الأساسية للأداة هو "القيود: مشفرة، غير موثقة" - تحويل القواعد من وثائق اللغة الطبيعية إلى تعليمات برمجية قابلة للتنفيذ، مما يجعلها قيودًا إلزامية على سلوك النظام بدلاً من المبادئ التوجيهية الاستشارية. يتيح إنشاء التعليمات البرمجية للوكيل إكمال عملية التحويل هذه بشكل مستقل.
...
تتيح كتابة القواعد في موجه النظام للنموذج **شرح السياسات** للمستخدمين، **تحديد البدائل المتوافقة مع السياسة** (على سبيل المثال، "إعادة الحجز بدلاً من الإلغاء")، وإصدار حكم جدوى أولي قبل استدعاء الأداة.
...
ومن الناحية العملية، ينبغي استخدامهما معًا: يحتوي موجه النظام على قواعد اللغة الطبيعية للفهم والتواصل، في حين يتم تجهيز نقاط القرار الرئيسية بأدوات التحقق المقننة التي تعمل بمثابة "حراس البوابة" لضمان الامتثال.
...
**الجمع بين التحقق من الصحة والتنفيذ: الاستدلال بدليل قوائم المراجعة؛ التحقق من الحقيقة الأرضية يحرس البوابة**
...
**المستوى الثاني: التحقق من الحقيقة الأرضية من جانب الخادم باعتباره حارس البوابة.** لاحظ التصميم الرئيسي في الكود: فئة المقصورة، وحالة التأمين، ووقت الحجز، واستخدام القطاع، وحالة الرحلة كلها يتم الاستعلام عنها من قاعدة البيانات بواسطة الخادم؛ الوقت الحالي يأتي من ساعة الخادم. **لا توجد حقيقة سياسية تأتي من المعلمات التي أبلغ عنها النموذج ذاتيًا.** وهذا ليس تكرارًا لا داعي له: فقد يهذي النموذج أو يتم التلاعب به عن طريق الحقن الفوري، و- كما أظهر تحليل Lethal Triad السابق - لا يمكن للعميل الذي يعمل ضمن سياق واحد التحقق من صحة سلوكه بشكل موثوق. إذا تم تصميم `cabin_class`، و`has_insurance`، وحتى `current_time` كمعلمات تم ملؤها بواسطة النموذج، فيمكن لقيمة خاطئة واحدة - سواء كانت عرضية أو مستحثة - تجاوز حارس البوابة. ولابد أن يبنى خط الدفاع الأخير على بيانات لا يستطيع النموذج تزويرها ــ وهذا يتفق مع الموقف السابق المتمثل في أن "العمليات الحاسمة تتطلب التحقق المستقل": فالاستقلال لا يشير إلى نموذج مستقل فحسب، بل يشير أيضاً إلى مصدر بيانات مستقل.

وبذلك تكون الحماية المكونة من ثلاثة مستويات كاملة: (1) قواعد اللغة الطبيعية في النظام تساعد على الفهم والتفسير؛ (2) تعمل أوصاف الأداة وتصميم المعلمات كقائمة مرجعية لتوجيه النموذج للتحقق بشكل صريح من الشروط قبل الاتصال؛ (3) التحقق من صحة التعليمات البرمجية من جانب الخادم باستخدام الحقيقة الأساسية لقاعدة البيانات بمثابة حارس البوابة النهائي. يقلل المستويان الأولان من حدوث الأخطاء، ويضمن المستوى الثالث عدم تحول الأخطاء إلى خسائر لا رجعة فيها.
...
>
> **النهج الفني**: صمم تجربة مضبوطة بناءً على سيناريو خدمة عملاء شركات الطيران. **المجموعة الضابطة**: قواعد لغة طبيعية خالصة، تعتمد على المنطق الخاص بالنموذج. **المجموعة التجريبية**: حماية من ثلاثة مستويات — يحتفظ موجه النظام بقواعد اللغة الطبيعية؛ يسرد وصف الأداة السياسة الكاملة ويستخدم معلمات `expected_*` الاختيارية لتوجيه النموذج للتحقق من كل شرط واحدًا تلو الآخر قبل الاتصال (قائمة المراجعة)؛ تقوم الأداة داخليًا بإجراء التحقق من الصحة القائم على التعليمات البرمجية بناءً على محاكاة الحقيقة الأساسية لقاعدة البيانات (يتم الحصول على جميع حقائق السياسة من قاعدة البيانات، ويتم أخذ الوقت من ساعة الخادم، ولا يتم الوثوق بالمعلمات المبلغ عنها ذاتيًا للنموذج). مقاييس التقييم: معدل نجاح المهمة، عدد انتهاكات السياسة، عدد استدعاءات الأداة غير الصالحة، تجربة المستخدم.
>
...
إن إنشاء العديد من المستندات المعقدة هو في الأساس تنظيم وعرض البيانات المنظمة. سواء كان ذلك عرضًا تقديميًا أو تقريرًا فنيًا أو تطبيقًا تفاعليًا، يتم تعريف البنية الأساسية بواسطة رمز - يصف HTML البنية، ويتحكم CSS في النمط، وينفذ JavaScript التفاعلية. يعتمد إنشاء المستندات التقليدية على محررات WYSIWYG المستندة إلى واجهة المستخدم الرسومية، والتي لا تناسب الوكلاء لأنها تتطلب تفسيرًا مرئيًا ووضع مؤشر دقيق. من خلال إنشاء التعليمات البرمجية، يتغلب الوكلاء على تحدي تحديد المواقع المرئية ويحصلون على تحكم دقيق في المستندات - يتم تحديد موضع كل عنصر ونمطه ومحتواه بوضوح ويمكن تعديله وتحسينه برمجيًا.
...
>
> يؤدي تغليف تحليل الفيديو كعامل فرعي إلى منع عدد كبير من لقطات الشاشة من احتلال سياق الوكيل الرئيسي. بعد الترجمة، يقوم مقدم العرض بإنشاء البرنامج النصي Blender API. يقوم وكيل المراجع بإجراء معاينة سريعة، والتحقق من الإطارات الرئيسية، وتقديم تعليقات للتعديل، والتكرار حتى يتم استيفاء المعيار قبل العرض الكامل.
>
...
يعد الاستعلام عن قاعدة البيانات سيناريو يمكن أن يؤدي فيه إنشاء التعليمات البرمجية إلى تحسين تجربة التفاعل بشكل كبير. يعتمد الوصول التقليدي إلى قاعدة البيانات على أدوات واجهة المستخدم الرسومية أو SQL المكتوبة بخط اليد؛ الأول مرهق في التشغيل، والأخير يتطلب من المستخدم معرفة متخصصة. يمكن للوكيل ترجمة اللغة الطبيعية إلى SQL، ولكن هناك خيار تصميم رئيسي: هل يجب على الوكيل تنفيذ الاستعلام ووصف النتائج باللغة الطبيعية، أم يجب عليه إنشاء SQL كقطعة أثرية ليقوم النظام بتنفيذها والواجهة الأمامية لعرضها؟

يبدو الأسلوب الأول أكثر "ذكاءً" ولكنه غير فعال إلى حد كبير - فقد يؤدي الاستعلام مقابل جدول كبير إلى إرجاع آلاف الصفوف. إن قراءة LLM لكل ذلك ووصفه في النثر يحرق الرموز والوقت، والأسوأ من ذلك، أن نماذج LLM معرض للخطأ عند "نسخ" البيانات. النهج الأفضل هو **نمط القطعة الأثرية**. يوضح الشكل 5-9 سير عمل وكيل استعلام SQL: بدلاً من قراءة البيانات نفسها، يقوم الوكيل بإنشاء استعلام SQL وتمريره إلى النظام باعتباره **عنصرًا قابلاً للتنفيذ** مستقلًا. يقوم النظام بتنفيذ الاستعلام على قاعدة البيانات ويعرض النتائج في جدول للمستخدم. وبالتالي تتدفق البيانات مباشرة من قاعدة البيانات إلى الواجهة دون المرور عبر LLM؛ يكتب LLM الاستعلام ولكن لا يتعين عليه أبدًا قراءة وإعادة صياغة آلاف الصفوف. هذا النهج أسرع وأكثر دقة.
...
>
> يعد برنامج ERP (تخطيط موارد المؤسسات) نظامًا مهمًا للشركات، وعادةً ما يستخدم واجهة المستخدم الرسومية (GUI) حيث تتطلب العمليات المعقدة نقرات متعددة بالماوس. يمكن لعامل الذكاء الاصطناعي ترجمة طلبات المستخدمين باللغة الطبيعية إلى استعلامات SQL، مما يتيح الوصول الآلي إلى قاعدة البيانات.
>
...
ومع ذلك، فإن التوليد الديناميكي بالكامل مكلف وبطيء، وهو أكثر ملاءمة لتوضيح ما هو ممكن أكثر من استخدام الإنتاج. النهج الأكثر واقعية هو **تخصيص إطار العمل الحالي**. يحافظ هذا النموذج "شبه المخصص" على استقرار البرنامج الأساسي مع تعريض الجوانب المحددة لتحكم المستخدم. يمكن للمستخدم أن يقول "اجعل الزر أزرقًا" أو "أضف قائمة مختصرة إلى الشريط الجانبي" أو "قم بالتبديل إلى خط أكثر قابلية للقراءة"؛ يقوم الوكيل بتحديث رمز الواجهة الأمامية، ويقوم HMR (استبدال الوحدة الساخنة - الذي يقوم بتحديث الوحدات المتأثرة دون إعادة تحميل صفحة كاملة ويحافظ عادةً على حالة التطبيق) بتطبيق التغييرات على الفور. يصبح المنتج ذو الحجم الواحد الذي يناسب الجميع تجربة مصممة خصيصًا لكل مستخدم.
...
>
> **النهج الفني**: قم ببناء تطبيق chatbot أساسي (واجهة React الأمامية وواجهة FastAPI الخلفية)، وقم بتشغيل كلا المكونين في وضع التطوير مع تمكين إعادة التحميل السريع (إعادة تحميل React HMR وFastAPI). يقترح المستخدمون متطلبات تخصيص واجهة المستخدم (الألوان والخطوط والتخطيط ومواضع المكونات وما إلى ذلك) أثناء المحادثة. يقوم الوكيل بتعديل الكود بشكل مستقل. تكتشف آلية التحديث السريع تغييرات الملف تلقائيًا، وتتم إعادة ترجمة الواجهة الأمامية وتحديثها، ويرى المستخدم تغييرات الواجهة في الوقت الفعلي. يدعم النظام جولات متعددة من التخصيص التكراري.
>

### كود إنشاء الكود: Agent Bootstrapping
...
أولاً، يجب توضيح تقسيم العمل في هذا القسم مع الفصل الثامن. يناقش هذا القسم كيفية استخدام وكيل الترميز للتعليمات البرمجية **لإصلاح وإنشاء وكلاء من نوعه** — الإصلاح الذاتي، والنسخ المتماثل الذاتي، وإنشاء وكلاء جدد عند الطلب. ينصب تركيزها على إنشاء التعليمات البرمجية والقدرة على بناء النظام، لذلك تسمى هذه العملية **bootstrapping**. لا يشرح الفصل 8 مرة أخرى كيفية كتابة هذا الرمز؛ وبدلاً من ذلك، فهو يركز على كيفية تحفيز تجربة الإنتاج المقيَّمة للتعديل الذاتي: تحديد المعرفة أو التعليمات أو البرامج أو المعلمات كهدف للتحديث؛ توليد نسخة مرشحة من نسخة مستقرة؛ والتحكم في المخاطر من خلال اختبار الانحدار، وإصدارات الكناري، والتراجع. يتقاطع الفصلان عند "تعديل الكود"، لكنهما يجيبان على أسئلة مختلفة.
...
أحد المتطلبات الأساسية الحاسمة لتمهيد العميل هو القدرة على الإصلاح الذاتي. يجسد أمر `doctor` في OpenClaw هذه الإمكانية، حيث يمكنه اكتشاف ثلاثة أنواع من المشكلات تلقائيًا:
...
يعد إنشاء وكيل عالي الجودة أصعب بكثير من إنشاء كود تطبيق عادي، لأنه يتطلب فهمًا عميقًا لأنماط بنية الوكيل وأفضل الممارسات والمزالق الشائعة. وبدون هذه الخبرة في هذا المجال، حتى أقوى نماذج توليد التعليمات البرمجية تنتج عملاء يعانون من عيوب معمارية خطيرة. تشمل العيوب الشائعة ما يلي:
...
لا يتمثل المسار الأكثر فعالية لحل هذه المشكلات في إدراج جميع القواعد في الموجه بشكل شامل، ولكن **توفير تطبيقات وكيل عالية الجودة كأمثلة مرجعية**، وتوجيه وكيل إنشاء التعليمات البرمجية لتعديلها بدلاً من البدء من الصفر.

إن ميزة الإنشاء المبني على الأمثلة واضحة: رمز المثال نفسه يحمل أفضل الممارسات. الوكيل الذي يتكيف مع التنفيذ الذي تم التحقق من صحته يحصل على الأمور في نصابها الصحيح في كثير من الأحيان أكثر من الوكيل الذي يبدأ من الصفر، لأن التنفيذ يحافظ على الاختيارات المعمارية السليمة دون الحاجة إلى توضيح كل قاعدة في الموجه.

عندما يتلقى وكيل مهمة لتطوير وكيل جديد، يجب عليه أولاً نسخ الكود الخاص به (أو تطبيقات أخرى عالية الجودة تم التحقق منها) ثم إجراء تعديلات مستهدفة: ضبط موجه النظام لمطابقة الدور الجديد، واستبدال أو إضافة أدوات لتناسب الوظائف الجديدة، وتعديل منطق الأعمال مع الحفاظ على الإطار المعماري. يضمن نمط "التكاثر الذاتي مع التعديل التكيفي" أن يرث العميل الجديد المزايا التقنية الأساسية مع السماح بالتمايز في أبعاد محددة - يشبه إلى حد كبير تكرار الجينات مع الطفرة في علم الأحياء.
...
>
> **النهج الفني**: تزويد وكيل الترميز بتطبيقات الوكيل عالية الجودة كأمثلة مرجعية (يمكن استخدام مشروع ch5/coding-agent نفسه). عند تكليفه بإنشاء وكيل جديد، يقوم الوكيل أولاً بنسخ رمز المثال هذا ثم يقوم بإجراء التعديلات المستهدفة بناءً على احتياجات المستخدم المحددة.
>
...
توصل قسم هندسة البرمجيات إلى نتيجة مركزية واحدة: وكلاء التشفير ناضجون ليس لأن نماذج توليد التعليمات البرمجية قوية بشكل استثنائي، ولكن لأن عقودًا من البنية التحتية المتراكمة لهندسة البرمجيات - مجموعات الاختبار وأنظمة الكتابة والتحكم في الإصدار - تشكل بشكل طبيعي أداة قوية. يستحق هذا الاستنتاج السفر إلى سيناريوهات العملاء الأخرى. يقدم القسم الخاص بالفشل واسترداد الأخطاء الجانب الآخر من نفس الموضوع: لا يتم تحديد موثوقية الوكيل من خلال ما إذا كان النموذج يرتكب أخطاء، ولكن من خلال ما إذا كان لكل فئة من حالات الفشل مسارًا مطابقًا للكشف والاسترداد والإنهاء.
...
2. ★★★ يتيح تشغيل العميل - وهو وكيل يمكنه إنشاء عملاء - إمكانية "إعادة إنتاج الذكاء ذاتيًا". لكن كل تكرار للتمهيد قد يؤدي إلى تحيزات أو أخطاء جديدة. فهل تتراكم هذه الأخطاء عبر الأجيال؟ كيف يمكن منع التدهور في عملية تشغيل العامل؟
...
5. ★★ يوضح هذا الفصل عدة طرق لوكيل الترميز لدمج الخبرة المكتسبة من خلال التنفيذ وتصحيح الأخطاء مرة أخرى في قاعدة التعليمات البرمجية - كتابة ملفات قاعدة المعرفة، وتحديث وثائق الهندسة المعمارية، والحفاظ على ملفات تعليمات المشروع، وترميز التسلسلات التشغيلية كرمز. إذا تم تحويل هذه التجربة إلى قواعد في موجه النظام، فستستمر مجموعة القواعد في التوسع بمرور الوقت. كيف يمكن تنفيذ "جمع البيانات المهملة" على القواعد المتراكمة لتحديد وإزالة الإدخالات الزائدة أو القديمة؟ لماذا لم يعد تعديل الكود الناجح الوحيد تطورًا مستمرًا بالمعنى الوارد في الفصل الثامن؟
