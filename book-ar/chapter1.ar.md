...
عند تصميم أدوات للوكيل، اجعلها عامة قدر الإمكان كي تمنح النموذج اللغوي مساحة أوسع للعمل. فبدل إنشاء حاسبة متخصصة، يمكن توفير مفسّر Python داخل بيئة تنفيذ معزولة وآمنة. وبدل أداة مخصصة لتسجيل يوميات العمل، يمكن توفير أدوات لقراءة الملفات وكتابتها داخل نظام ملفات افتراضي. تتيح الأدوات العامة للوكيل أن يركّب القدرات الأساسية بطرائق مبتكرة لحل المشكلات.
...
أظهرت المناقشة السابقة كيف يمكن للتعلم المعزز أن يستوعب سياسات استخدام الأدوات كقدرات نموذجية أصلية. لكن التغييرات في سلوك الوكيل لا تحدث أثناء التدريب فقط. استنادًا إلى مكان حدوث التحديث ومدة استمراره، يمكن فهم هذه التغييرات على أنها ثلاثة مسارات تكميلية (الشكل 1-1): التكيف السياقي داخل المهمة، والتحديثات عبر المهام للعناصر الخارجية، وتحديثات المعلمات أثناء دورات التدريب.
...
يعد GPT-5.6 مثالًا ناضجًا على "النموذج كوكيل" - بحث الويب ومفسّر الشفرة والأدوات المضمنة الأخرى للاستجابات API التي يتم تنفيذها في حلقة مغلقة على الخادم؛ تنتقل حلقة التزامن من الوكيل إلى خادم API، مما يبسط عملية تنفيذ الوكيل. لا يزال النموذج يصدر استدعاءات الأدوات القياسية؛ لم يعد الوكيل ببساطة مضطرًا إلى إنشاء إطار التنسيق "البحث والقراءة والتحليل" نفسه. الجانب الأكثر جدارة بالملاحظة هو آلية توضيح النية: فبدلاً من تنفيذ مهمة على الفور، يؤكد النموذج أولاً ما يحتاجه المستخدم حقًا، ثم يصوغ استراتيجية بحث. تتم معالجة الفجوة بين "ما قاله المستخدم" و"ما يريده المستخدم بالفعل" قبل بدء التنفيذ.
...
بتعبير أدق، كل البنية التحتية خارج النموذج تنتمي إلى منظومة التشغيل. جوهر الأداة هو السياق والأدوات، حيث يتم بناء ثلاثة أنواع من الضمانات الهندسية:
...
تشكل الوظائف الخمس حلقة مغلقة: يدعم السياق والأدوات اتخاذ القرار، ويمنع القيد الأخطاء، ويكشف التحقق عن الانحرافات، ويغلق التصحيح الدورة. إذا كان هناك أي رابط مفقود، فإن النظام يطور فجوة موثوقية. قبل فحص أنماط التنسيق المحددة وتصميمات ضوابط الأمان، قمنا أولاً بوضع المبادئ الأساسية لبناء وكلاء فعالين ولاختيار النموذج، وهو الأساس لكل قرار تصميم يتبع ذلك.
