...
يمكن فهم نظام الذاكرة المستمرة هذا على مقياسين. **ذاكرة المستخدم** هي ذاكرة مخصصة لمستخدم فردي — يتعلم الوكيل تدريجيًا تفضيلات كل مستخدم وعاداته واحتياجاته من خلال التفاعلات، وبناء نموذج معرفي فريد لذلك المستخدم. **قاعدة المعرفة** هي المعرفة الجماعية المشتركة بين جميع المستخدمين، مثل الإطار التنظيمي للصناعة، أو إجراءات التشغيل الداخلية للشركة، أو الوثائق الفنية المتخصصة في أحد المجالات. الأول يجعل الوكيل "مساعدًا شخصيًا يعرفك"، بينما الثاني يجعل الوكيل "خبيرًا في المجال".

وهما في الواقع نفس المشكلة على مستويات مختلفة - أحدهما يتركز على الفرد، والآخر على المجموعة. ولهذا السبب يتشاركون في الكثير من التكنولوجيا الأساسية (استرجاع المتجهات، وضغط المعرفة) ويواجهون نفس أنماط الفشل: المعلومات المتضاربة، والمعرفة التي لا معنى لها، والاسترجاع غير الدقيق.

استمرارًا لنهج هندسة السياق من الفصل الثاني، يوسع هذا الفصل إدارة السياق من محادثات الجلسة الواحدة إلى نظام المعرفة المستمرة عبر الجلسات. نستكشف أولاً كيفية بناء نظام ذاكرة المستخدم، ثم نتعمق في الجيل الاسترجاعي المعزز (RAG) لقواعد المعرفة وكيف يعزز ذاكرة المستخدم.
...
وبناءً على ذلك، قمنا بتصميم إطار تقييم من ثلاثة مستويات أكثر ملاءمة لسيناريوهات العميل، مما أدى إلى تحليل قدرات الذاكرة إلى مستويات تقدمية. يتكرر هذا الإطار خلال هذا الفصل - ستستخدمه التجارب 3-10 و3-12 لاحقًا لقياس كيفية تحسين تقنيات الاسترجاع لقدرات الذاكرة.
...
**تمثل بطاقات JSON المتقدمة** نقلة نوعية في تصميم نظام الذاكرة — من تخزين المعلومات إلى إدارة المعرفة. لا تسجل كل بطاقة الحقائق فحسب، بل تسجل أيضًا السياق السردي (الخلفية الدرامية) لمصدر المعلومات، وهوية الموضوع (الشخص)، والعلاقة مع المستخدم (العلاقة)، ​​والطابع الزمني. الفكرة الأساسية هي أن نفس المعلومة يمكن أن يكون لها معاني مختلفة تمامًا في سياقات مختلفة - "الدكتور تشانغ" يمكن أن يكون طبيب أسنان المستخدم أو طبيب القلب الخاص بوالد المستخدم؛ إذا تم تجريد المعلومات من سياقها، فلا يمكن فهمها بشكل صحيح.

هذا التصميم يحل مشكلة توضيح الأنظمة التقليدية. في سيناريوهات العالم الحقيقي، قد يكون لدى المستخدم عدة أطباء (لنفسه، ولوالديه، ولأطفاله)، ولا يمكن للتخزين البسيط للقيمة الأساسية التمييز بينهم بدقة. توفر بطاقات JSON المتقدمة السياق الذي تم فيه الحصول على المعلومات ("لماذا" لتخزين هذه المعلومات) من خلال `backstory`، وإنشاء نموذج كيان واضح ("لمن" يتم تخزين المعلومات) من خلال حقلي `person` و`relationship`. عندما يقول المستخدم "ساعدني في ترتيب الفحوصات السنوية لعائلتي"، يستطيع النظام التعرف على جميع أفراد العائلة من خلال `relationship` وفهم التاريخ الصحي من خلال `backstory`. التكلفة هي ارتفاع تكاليف الإنتاج والصيانة.

تكشف مقارنة هذه الأوضاع الأربعة عن توتر أساسي في تصميم نظام الذاكرة: المفاضلة بين البساطة والتعبير. تختار Simple Notes البساطة الشديدة على حساب الاكتمال الدلالي؛ تختار الملاحظات المحسنة اكتمال السرد على حساب البنية وقابلية التحديث؛ تختار بطاقات JSON البنية على حساب المرونة؛ تختار بطاقات JSON المتقدمة الشمولية على حساب البساطة. ليس لهذه المقايضة فائز مطلق، فهي تعتمد كليًا على حالة الاستخدام المحددة. قد يحتاج نظام AI Agent الناضج إلى استخدام مزيج من الأوضاع: ملاحظات بسيطة لتسجيل المعلومات العابرة بسرعة، وبطاقات JSON المتقدمة للتعامل مع المعلومات الهامة التي تتطلب توضيحًا دقيقًا وصيانة طويلة الأمد.

معيار الاختيار العملي هو: استخدام بطاقات JSON المتقدمة للبيانات **المهمة، ذات الحجم المنخفض** (على سبيل المثال، تفضيلات المستخدم، والعلاقات الشخصية الرئيسية) لضمان إمكانية الاسترجاع؛ استخدم ملاحظات بسيطة **لكميات كبيرة من الحقائق التحادثية غير الهامة** لتقليل التكلفة. تتبنى معظم أنظمة الإنتاج نهجًا هجينًا - حيث تتبع أنواع مختلفة من المعلومات داخل نفس الوكيل مسارات مختلفة.
...
التنسيقات الأربعة التي تمت مناقشتها أعلاه، سواء كانت بسيطة أو معقدة، هي في الأساس **نص** — مما يعني أن "التخزين" و"استخدام" الذاكرة يظلان خطوتين منفصلتين: قم أولاً باسترداد النص ذي الصلة، ثم قم بإرساله إلى LLM المعرض للأخطاء لقراءته وحسابه. تتفوق الذاكرة النصية في تذكر الحقائق الفردية ولكنها تواجه صعوبة في تجميع الإحصائيات عبر العديد من السجلات، أو اكتشاف الحقائق المتناقضة، أو فرض القواعد المنطقية، لأن كل هذه العمليات تعتمد على "الحساب الذهني" الخاص بـ LLM. يقترح المستخدم كرمز[^uac] حلاً: تحويل وسيط التمثيل من النص إلى **الكود القابل للتنفيذ**. إنه يتعامل مع نموذج الوكيل للمستخدم باعتباره **مشروع هندسة برمجيات حي** - باستخدام كائنات Python المكتوبة لتخزين حالة المستخدم ووظائف Python العادية لتشفير قواعد القيد، بحيث يحدث "تمثيل المستخدم" و"الاستدلال حول المستخدم" في نفس الوسيط الذي يمكن تنفيذه بواسطة مترجم.

فهو يقسم تحديثات الذاكرة إلى مرحلتين[^uac]: **مرحلة الذاكرة** (بعد كل جلسة، يستخرج LLM الحقائق من المحادثة واحدة تلو الأخرى كسلاسل، ويلحقها بسجل حقائق للإلحاق فقط) و**مرحلة الهيكلة** (بشكل دوري، يقوم LLM بإعادة إنشاء تمثيل Python المكتوب بالكامل من سجل الحقائق الكامل — تنظيم الحقائق في فئات البيانات، باستخدام `date()` للتواريخ، والقوائم المكتوبة للمجموعات، و`notes: list[str]` للعناصر المتنوعة التي يصعب كتابتها). هذا هو التصميم الكلاسيكي "سجل الكتابة المسبق + نقطة التفتيش الدورية" من قواعد البيانات، والذي تم تطبيقه على ذاكرة LLM لأول مرة: يضمن سجل الإلحاق فقط عدم فقدان أي حقائق، وتقوم نقطة التفتيش الدورية بضغطها في بنية نظيفة وقابلة للاستعلام. (تتوافق عملية إعادة البناء الدورية هذه مع "آلية ضغط وتنظيم الذاكرة" التي تمت مناقشتها لاحقًا في هذا الفصل، باستثناء أن الإخراج عبارة عن رمز وليس نصًا.)

وفيما يلي مثال مبسط. تقوم مرحلة الهيكلة بتخزين جواز سفر المستخدم ورحلاته حسب الحالة المكتوبة:
...
مع الحالة المكتوبة، أصبحت الآن ثلاث مهام كانت تتطلب سابقًا من LLM "قراءة النص وإجراء الحساب الذهني" رمزًا حتميًا:
...
من منظور العلوم المعرفية، يوفر تعقيد نظام الذاكرة البشرية رؤى مهمة لتصميم ذاكرة الذكاء الاصطناعي. يقسم العلم المعرفي الذاكرة إلى **الذاكرة العاملة** والذاكرة طويلة المدى. تتوافق الذاكرة العاملة مع نافذة سياق الوكيل — وهي مساحة معلومات مؤقتة للتعامل مع المهمة الحالية (المسار هو المحتوى الأساسي للذاكرة العاملة، ولكن قد تتضمن الذاكرة العاملة أيضًا معلومات تم تنشيطها وتحميلها من الذاكرة طويلة المدى). تنقسم الذاكرة طويلة المدى أيضًا إلى ثلاثة أنواع، لكل منها نظير مباشر في ذاكرة العميل:
...
على عكس التضمينات الكثيفة، التي تلتقط التشابه الدلالي، فإن التضمينات المتفرقة متجذرة في استرجاع المعلومات التقليدية: في جوهرها توجد المطابقة الدقيقة للكلمات الرئيسية. يمثل التضمين المتناثر وثيقة كمتجه عالي الأبعاد للغاية حيث تكون معظم الأبعاد صفرًا - فقط الأبعاد المقابلة للكلمات التي تظهر في الوثيقة هي غير صفرية. الأساس النظري هو نموذج حقيبة الكلمات الكلاسيكي (BoW)، الذي يتعامل مع جزء من النص على أنه "حقيبة كلمات"، مع الاهتمام فقط بالكلمات التي تظهر وعدد مرات ظهورها، متجاهلاً ترتيب الكلمات تمامًا: "قطة تطارد كلبًا" و"قطة تطارد كلبًا" متطابقتان في BoW. تطورت خوارزميات التصنيف الاحتمالية الأكثر تعقيدًا من هذا الأساس.

![الشكل 3-8: آلية تسجيل BM25](images/fig3-8.svg)

#### من TF-IDF إلى BM25

دعونا نبني الحدس بمثال ملموس. افترض أن قاعدة المعرفة تحتوي على 100 مقالة فنية، وأن المستخدم يبحث عن "نموذج التقطير". تظهر كلمة "نموذج" في 60 مقالة (شائعة جدًا، ذات قوة تمييزية منخفضة)، بينما تظهر كلمة "التقطير" في 3 مقالات فقط (نادر جدًا، ذات قوة تمييزية عالية). يجب أن تعطي خوارزمية الاسترجاع الجيدة وزنًا أكبر لكلمة "التقطير" - فالمقالات التي تحتوي على "التقطير" من المرجح أن تكون ما يبحث عنه المستخدم بالفعل. هذه هي الفكرة الأساسية وراء TF-IDF وBM25.

يعتمد TF-IDF على حدس بسيط: كلما زاد تكرار ظهور الكلمة في مستند (TF، تردد المصطلح)، وكلما قل عدد المستندات الموجودة في المجموعة التي تحتوي عليها (انخفاض تردد المستند، أو DF، وبالتالي ارتفاع تردد المستند العكسي، أو IDF)، زادت أهمية الكلمة. في المثال أعلاه، يحتوي "النموذج" على `df/N = 60%`، لذا فإن قيمة IDF الخاصة به منخفضة؛ يحتوي "التقطير" على `df/N = 3%`، لذا فإن قيمته في IDF عالية - وبالتالي، يساهم "التقطير" في التصنيف أكثر بكثير من "النموذج". ومع ذلك، فإن TF-IDF لا يأخذ في الاعتبار طول الوثيقة (المستندات الأطول لها بطبيعة الحال ترددات مصطلحات أعلى)، ونمو تردد المصطلح خطي: ​​هل الكلمة التي تظهر 10 مرات هي في الحقيقة ضعف أهمية الكلمة التي تظهر خمس مرات؟ يقدم BM25 معلمتين رئيسيتين لتصحيح هذه المشكلات. يتحكم `k1` في "تشبع" تردد المصطلح: بشكل بديهي، المقالة التي تذكر "التقطير" 20 مرة ليست في الحقيقة ذات صلة بضعف تلك التي تذكرها 10 مرات. يؤدي `k1` إلى استقرار مساهمة تكرار المصطلح تدريجيًا مع زيادته، مما يمنع المستندات الطويلة من السيطرة بشكل غير عادل بسبب تراكم تكرار المصطلح. يتحكم `b` في تسوية طول المستند، مما يسمح للخوارزمية بالتعامل مع المستندات ذات الأطوال المختلفة بشكل أكثر عدلاً. وهذا يجعل BM25 وظيفة تصنيف أكثر قوة وفعالية، ويظل مكونًا أساسيًا لا غنى عنه في محركات البحث الرئيسية اليوم.
...
>
> ```
> رموز الاستعلام: ["النموذج"، "التقطير"]
>
> كلمة "نموذج" → يصل الفهرس المقلوب إلى 3 مستندات (df=3, IDF=ln((10−3+0.5)/(3+0.5))=0.76):
>   doc_1: TF=5، طول المستند=200 كلمة، مساهمة BM25=1.52
>   doc_3: TF=2، طول المستند=500 كلمة، مساهمة BM25=0.82
>   doc_7: TF=8، طول المستند=150 كلمة، مساهمة BM25=1.68
>
> كلمة "التقطير" → الفهرس المقلوب يصل إلى وثيقتين (df=2, IDF=ln((10−2+0.5)/(2+0.5))=1.22، أندر من "model"):
>   doc_1: TF=3، طول المستند=200 كلمة، مساهمة BM25=2.15 ← "التقطير" نادر، كل تكرار يساهم أكثر
>   doc_5: TF=1، طول المستند=250 كلمة، مساهمة BM25=1.22
>
> الترتيب النهائي: doc_1 (3.67) > doc_7 (1.68) > doc_5 (1.22) > doc_3 (0.82)
> ```
>
> لاحظ أنه في doc_1، "التقطير" له تردد مصطلح أقل (TF=3) من "model" (TF=5)، ولكن نظرًا لأن IDF الخاص به أعلى (وهو نادر في المجموعة)، فإنه يساهم بشكل أكبر في درجة doc_1 (2.15 مقابل 1.52) - هذا هو المنطق الأساسي لـ BM25. نظرًا لأن doc_1 يطابق كلا مصطلحي الاستعلام، فإنه يتقدم بفارق كبير عند 3.67، مما يؤكد مدى تراكب عدد مرات ظهور المصطلح في الترتيب.
>
...
تشبيه: يقوم مسؤول التوظيف بتصفح السير الذاتية للمقطع الأول وهو المشفر الثنائي؛ الشخص الذي يجري المقابلة في محادثة عميقة مع كل مرشح هو المشفر المتقاطع. الشاشات السابقة على نطاق واسع على الميزات المستخرجة مسبقًا؛ يتيح الأخير للاستعلام وكل مستند مرشح الالتقاء "وجهًا لوجه" ويتم تقييمهما كلمة بكلمة. تستخدم أداة إعادة الترتيب بنية "Cross-Encoder"، في تناقض صارخ مع "Bi-Encoder" المستخدم في مرحلة الاسترجاع. يقوم **Bi-Encoder** بإنشاء متجهات مستقلة للاستعلام والمستند ويحسب التشابه من خلال عمليات المتجهات - سريع جدًا، ولكنه غير قادر على التقاط علاقات مطابقة عميقة، ومناسب للفحص الأولي من البيانات الضخمة. **يقوم **Cross-Encoder** **بربط الاستعلام والمستند المرشح في جزء واحد من النص** ويغذيهما بالنموذج، مما يسمح للنموذج بمقارنة كلمة بكلمة وإخراج درجة صلة شاملة[^ch3-cross-encoder] — أبطأ بكثير، ولكنه أكثر دقة في أحكام الصلة. نماذج إعادة الترتيب شائعة الاستخدام مثل [BAAI/bge-reranker-v2-m3](https://huggingface.co/BAAI/bge-reranker-v2-m3) تعتمد هذه البنية.
...
| متري | شرح بديهي |
|-------------------------------|----------------------------------------------------------------|
| استدعاء @ ك [^ ch3-استدعاء] | نسبة الاستعلامات التي يظهر لها مستند يحتوي على الإجابة الصحيحة في نتائج استرجاع k العليا - الإجابة "هل تم العثور على المستندات الصحيحة؟" إنه المقياس الأكثر توافقًا مع المتطلبات الأساسية لـ RAG: طالما أن المستند ذي الصلة يدخل السياق، فإن LLM لديه فرصة لاستخدامه. |
| MRR (متوسط ​​الرتبة المتبادلة) | بالنسبة لكل استعلام، خذ معكوس رتبة المستند الأول ذي الصلة، ثم متوسط جميع الاستعلامات - وأجب عن "ما مدى ارتفاع النتيجة الأولى؟" المرتبة 1 تعطي درجة 1، المرتبة 10 تعطي 0.1 فقط. |
| nDCG (الربح التراكمي المخصوم الطبيعي) | يأخذ في الاعتبار مرتبة وأهمية جميع الوثائق ذات الصلة؛ يزداد خصم النتيجة للمستندات ذات الصلة كلما تراجع الترتيب الذي تظهر فيه - الإجابة على "ما هي الجودة الإجمالية للقائمة التي تم فرزها؟" |

[^ch3-recall]: بالمعنى الدقيق للكلمة، فإن "recall@k" المحدد في هذا الكتاب هو في الواقع **معدل الدخول** (يُسمى أيضًا Success@k) - وهو يحسب النتيجة طالما ظهر مستند واحد ذي صلة على الأقل في أعلى نتائج k. يشير الاستدعاء الأكاديمي القياسي @k إلى **نسبة المستندات ذات الصلة التي تم استرجاعها** (عدد المستندات ذات الصلة في أعلى نتائج k ÷ إجمالي عدد المستندات ذات الصلة لهذا الاستعلام)؛ عندما يحتوي الاستعلام على عدة مستندات ذات صلة، فإن الاثنين غير متساويين. يتبنى هذا الكتاب هذا التعريف المبسط للتوافق مع اتفاقيات إعداد التقارير الخاصة بتقرير "الاسترجاع السياقي" الخاص بـ Anthropic والذي تم الاستشهاد به لاحقًا. يجب أن يكون القراء على دراية بالتعريفات الدقيقة عند المقارنة بين المصادر.
...
>
> توضح النتائج التجريبية بوضوح المفاضلات بين الثلاثة: **يحقق **وضع الوسائط المتعددة الأصلي** أفضل أداء في مهام مثل تحليل المخططات وفهم تخطيطات المستندات، وذلك بفضل فهمه العميق للمعلومات المرئية والمكانية. **الاستخراج إلى وضع النص** هو الأكثر فعالية من حيث التكلفة للمستندات التي يهيمن عليها النص العادي ولكنه يفشل تمامًا في الاستعلامات التي تتطلب معلومات مرئية. يُظهر **الوضع القائم على الأدوات** المرونة في السيناريوهات التفاعلية، والتعامل مع معظم الاستعلامات الأولية بتكلفة منخفضة وإجراء تحليل عميق عالي التكلفة عبر استدعاءات الأداة عند الحاجة، ولكنه لا يؤدي أداءً جيدًا مثل الوضع الأصلي في السيناريوهات التي تتطلب فهمًا عميقًا لمرة واحدة وشاملاً.
>
...
### قاعدة المعرفة التوقيت المناسب والحوكمة
...
### الوكيل RAG: تحول نموذجي نحو استرجاع المعرفة القائمة على الأدوات
...
في هذا النموذج الجديد، لم يعد استرجاع قاعدة المعرفة خطوة أولية آلية. وبدلاً من ذلك، يتم تغليفها كأداة ** يمكن للوكيل الاتصال بها في أي وقت. يتبنى الوكيل نمط ReAct (انظر التعريف في الفصل 1)، ويقود العملية من خلال حلقة "فكر → فعل → مراقبة".
...
![الشكل 3-12: مقارنة RAG الوكيل وRAG غير العامل](images/fig3-12.svg)
...
>
> تكلفة تحسين الأداء هذا هي مكالمات LLM الإضافية أثناء مرحلة الفهرسة. ومع ذلك، يمكن التحكم في هذا بشكل كامل من خلال التخزين المؤقت الفوري (آلية التخزين المؤقت للطلبات المتبادلة التي تم تقديمها في الفصل 2، حيث تكلف الاستدعاءات المتكررة لنفس بادئة المطالبة حوالي 1/10 من الأصل)، وبذلك تصل التكلفة إلى حوالي 1 دولار لكل مليون رمز مميز للوثيقة. وفقا لبحث Anthropic، فإن الجمع بين هذه التقنية مع BM25 يمكن أن يقلل من معدل فشل الاسترجاع (أي أعلى 20 معدل فشل مذكور في "كيفية قياس جودة الاسترجاع"، 1 - الاستدعاء@20) بنسبة 49%، وبنسبة 67% عند دمجه مع أداة إعادة الترتيب. تقدم التجربة حجة قوية: عند بناء RAG من فئة الإنتاج، فإن الاستثمار في المعالجة المسبقة للمعرفة الأكثر ذكاءً ووعيًا بالسياق هو قرار هندسي ذو عائد كبير.
...
>
> لتحقيق أعلى مستوى، **المستوى 3 (الخدمة الاستباقية)**، **بطاقات JSON المتقدمة** التي تم تقديمها مسبقًا** (تنظيم الحقائق الأساسية، الموجودة في سياق الوكيل، على سبيل المثال، "تنتهي صلاحية جواز سفر المستخدم جيسيكا في 18 فبراير 2025")، يجب دمجها مع الاسترداد السياقي لهذا الفصل (الوصول الدقيق عند الطلب إلى تفاصيل المحادثة الأصلية) في ذاكرة ذات مستويين هيكل. في `layer3/01_travel_coordination.yaml`:
>
