...
| معيار الاختيار | السياق المشترك (Shared Context) | السياق غير المشترك (Unshared Context) |
|---------------|-----------------------------------|--------------------------------------------|
| **عدد المهام الفرعية** | قليل (2-3 أدوار متسلسلة) | متعدد (يتطلب معالجة متوازية) |
| **سعة نافذة السياق** | تتسع لاستيعاب معلومات كافة الأدوار | نافذة واحدة غير كافية |
| **التوازي والتزامن** | تسلسلي بالأساس (تتناوب الأدوار) | قابل للتوسع المتوازي بحرية (سياقات مستقلة) |
| **عزل المعلومات والأمان** | غير مطلوب (تتشارك كافة الأدوار المعلومات) | مطلوب (مثلاً: عدم كشف تفاصيل الأمان الداخلية) |
| **ميزانية التكلفة (Tokens)** | مسار واحد تراكمي عبر المراحل | وكلاء مستقلون، وإجمالي الرموز يعتمد على التنسيق |
...
> **التجربة 10-1 ★★: التبديل متعدد الأدوار**
>
...
جدول 10-3 المقابلة المفاهيمية بين أنظمة التشغيل والأنظمة متعددة الوكلاء
...
**ما الذي يتم تمريره أثناء عملية التسليم دون سياق مشترك؟** يقارن الشكل 10-10 بين نوعين من عمليات التسليم. في التجربة 10-1، يستخدم `transfer_to_agent` سياقًا مشتركًا، وبالتالي يرث الدور الجديد السجل الكامل تلقائيًا. في نمط سلسلة التسليم، لا تتم مشاركة السياق، لذلك يجب على الوكيل المرسل تجميع المعلومات التي يحتاجها الوكيل المتلقي بشكل صريح.
...
تتمثل الرؤية الأساسية لـ **MetaGPT** في أن **إجراءات التشغيل القياسية (SOPs)** المستلهمة من فرق البرمجيات يمكن ترميزها كبروتوكولات تواصل صريحة للأنظمة متعددة الوكلاء، مما يتيح لكل دور (مثل خطوط التجميع الهندسية) إنتاج مخرجات موحدة ومحددة سلفًا.

تتوزع الأدوار في MetaGPT وفق خط تسلسلي منتظم (`مدير المنتج` ← `المهندس المعماري` ← `مدير المشروع` ← `المهندس` ← `مهندس الجودة`)، وتُنتج التسليمات الهيكلية التالية:
- **وكيل مدير المنتج**: يتلقى المتطلبات ويصوغ **وثيقة متطلبات المنتج (PRD)** المحتوية على الميزات وسيناريوهات الاستخدام والأولويات.
- **الوكيل المعماري**: يقرأ وثيقة PRD، ويتخذ القرارات المعمارية (حزمة التقنيات، تقسيم المكونات، الواجهات، وتصميم البيانات) ليخرج وثيقة التصميم المعماري.
- **وكيل مدير المشروع**: يحول التصميم المعماري إلى مهام برمجة محددة ويعينها للمهندسين بحسب التبعيات.
- **الوكلاء الهندسيون**: ينفذون المكونات البرمجية المحددة في بيئات التضمين، مع إمكانية عمل عدة وكلاء بالتوازي.
- **وكيل مهندس ضمان الجودة**: يقرأ الشفرات ووثيقة المتطلبات ويكتب حالات الاختبار ويتحقق من خلو النظام من الأخطاء.

تكمن المساهمة المعمارية الحقيقية لـ MetaGPT في آلية التواصل غير المتمركزة عبر **مجمع الرسائل المشترك مع نمط النشر والاشتراك (Shared Message Pool & Pub/Sub)**: حيث ينشر كل وكيل مخرجاته الهيكلية في مجمع رسائل عام، بينما تشترك الأدوار الأخرى في الرسائل ذات الصلة باختصاصها فقط دون حاجة لتصل صريح نقطة بنقطة (Point-to-Point).
