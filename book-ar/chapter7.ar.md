...
تغطي هذه القائمة حالات الفشل التي تُعلن عن نفسها فقط. ويجب أن تُضاف إليها الفئات التي لا تخلّف أي خطأ: فهم المتطلبات ومعالجة الالتباس، حين يبني الوكيل ما أعاد صياغته هو لا ما طُلب منه، أو يختار بصمت قراءةً واحدة لطلب ملتبس؛ اختراق بيئة التحقق، حين يعدّل تأكيدًا أو يضيف `skip` أو يستبدل المنطق المختبَر بـ mock أو يعلن نجاح اختبار لم يُشغَّل أصلًا؛ التعديل غير المكتمل، حين تُحدَّث ثلاثة مواضع استدعاء ويُغفَل الرابع — استدعاء ديناميكي أو ربط بلغة أخرى أو schema — ومع ذلك تنجح الترجمة؛ إبلاغ المستخدم بمعلومة خاطئة، حين تكون كل استدعاءات الأدوات والحالة النهائية صحيحة بينما المبلغ أو الحالة أو التاريخ في الرد خاطئ؛ والانحدارات غير الوظيفية، حين تتغير واجهة عامة أو schema دون ترحيل، أو يُحذف تحقق لكي يمر فحص. في كل هذه الحالات لا يكون الخطأ الأول قيمة تعيدها أداة، بل **رسالة assistant**: حكمٌ أو افتراضٌ أو سؤالٌ كان ينبغي طرحه ولم يُطرح.
...
>
> الخطوة 1: أخذ عينة طبقية. اختر عشرة مسارات فاشلة من `t3a_failed.md`، على أن يكون ثلاثة منها على الأقل حالات فشل صامت بلا أي خطأ في الأدوات؛ والمعيار هو ألّا يرجع أي استدعاء أداة بخطأ، وأن يعلن الوكيل الإنجاز بنفسه أو تنفد خطواته، وألّا يشير إلى الفشل سوى حكم المدقّق الختامي.
>
