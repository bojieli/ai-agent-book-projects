...
Policy RL tidak meniru langsung token dari jawaban referensi, melainkan memakai reward untuk menilai jawaban yang **dihasilkan sendiri** oleh model; perhitungan reward-nya tetap boleh memanfaatkan jawaban referensi atau data preferensi. Untuk bisa belajar dari sinyal semacam ini, setidaknya dua prasyarat harus terpenuhi: output dapat diverifikasi, dan policy saat ini sesekali mampu menjelajahi perilaku yang bernilai.

Prasyarat pertama adalah **dukungan format**. Jika tugas menuntut JSON atau tool call sementara model memuntahkan teks yang tak bisa di-parse, fungsi reward bahkan tidak sanggup menilai "berhasil atau gagal". Di sini SFT dapat lebih dulu berperan "membuat model bicara dengan rapi": sedikit demonstrasi menstabilkan format dan alur dasar sehingga reward menjadi terhitung, barulah RL mengoptimalkan policy. Inilah pola yang lazim disebut "SFT dulu, RL kemudian".

Prasyarat kedua lebih mendasar, yaitu **dukungan kapabilitas**. Lakukan sampling pada tugas hold-out dengan temperature yang dekat dengan setelan pelatihan, lalu ukur `pass@1` dan `pass@k`. Bila peluang berhasil sekali percobaan adalah $p$, maka dengan sampling yang kira-kira independen peluang berhasil setidaknya sekali dalam $k$ percobaan adalah

$$
\operatorname{pass@}k = 1-(1-p)^k.
$$

Bila `pass@1` rendah tetapi `pass@k` naik jelas seiring $k$, berarti strategi yang benar sudah berada di dalam distribusi model, hanya massa probabilitasnya terlalu kecil; RL, rejection sampling, maupun distilasi sama-sama punya sesuatu untuk diperkuat. Sebaliknya, bila pada $k$, temperature sampling, dan cakupan tugas yang wajar `pass@k` terukur masih mendekati 0, model dasar praktis tidak mampu menghasilkan trajektori yang berhasil. Ketika reward hanya diberikan 0/1 di ujung, satu grup rollout GRPO sangat mungkin bernilai nol semua sehingga advantage dalam grup langsung lenyap; PPO pun tak melihat contoh positif yang menunjukkan "harus bergerak ke mana". Terus memperbesar jumlah sampel hanya berarti menunggu keberhasilan kebetulan dengan laju sekitar $1/p$, dan efisiensinya cepat kehilangan makna praktis.

Pada titik ini tanyakan dulu apa yang kurang. Bila yang kurang adalah bahasa domain, fakta, pola kode, atau kapabilitas dasar konteks panjang, utamakan Mid-training untuk membenahi fondasi. Bila kapabilitasnya ada tetapi tidak bisa diungkapkan sesuai antarmuka, mulai dengan SFT. Bila model bisa maju sebagian tetapi tak sampai ke ujung, tambahkan partial reward yang dapat diverifikasi atau curriculum learning. RL unggul dalam mengangkat perilaku sukses yang **sudah ada tetapi berprobabilitas rendah**, dan lemah dalam menciptakan dari nol—di tengah reward yang seluruhnya nol—pengetahuan serta kapabilitas yang belum pernah dipelajari model.

Ada satu batas penting: "SFT harus lebih dulu" hanya berlaku selama format output atau perilaku dasar belum terbentuk. Eksperimen 8-11 memperlihatkan bahwa Llama-3.2-Vision-11B gagal bila langsung di-RL tanpa SFT dalam setelan output terstruktur yang ketat; sebaliknya model dasar yang cukup kuat dan sudah punya tingkat keberhasilan bukan nol boleh melewati SFT—DeepSeek-R1-Zero adalah contohnya. Cold-start SFT yang ditambahkan belakangan terutama memperbaiki keterbacaan dan konsistensi bahasa, bukan menyuntikkan pengetahuan tugas untuk RL. Alur pemilihan Mid-training/SFT/RL yang lebih lengkap dibahas pada subbab keputusan tersendiri di belakang.
...
Mid-training terutama menutup dua jenis celah:

- **Celah pengetahuan**: pra-pelatihan umum tidak cukup mencakup bahasa target, domain keuangan/medis/hukum, dokumen internal perusahaan, atau suatu jenis codebase, sehingga model bahkan tidak memahami konsep dan istilahnya.
- **Celah kapabilitas dasar**: tugas target menuntut konteks panjang, pola kode, penurunan matematis, atau representasi lintas modal yang belum terbentuk pada base model. Persoalannya bukan sekadar format jawaban yang salah, melainkan model nyaris tidak pernah sampai ke solusi yang benar meski disampel berkali-kali.
...
LLM RL berskala besar biasanya memakai inference engine seperti vLLM/SGLang untuk menghasilkan rollout, lalu menghitung ulang log probability dan gradien dengan training engine seperti FSDP/Megatron. Sekalipun kedua sisi memuat bobot yang sama, perbedaan presisi floating point, urutan reduksi, cara tensor parallelism, ukuran batch, KV cache, dan fused kernel bisa membuat log probability token yang sama sedikit berbeda. Akibatnya $\rho_t$ yang seharusnya bernilai 1 sebelum update sudah menyimpang dari 1: secara nominal sistem sudah menyinkronkan bobot, tetapi secara numerik ia mengubah pelatihan on-policy menjadi off-policy. Eksperimen terkendali menunjukkan bahwa selisih training–inference sekecil apa pun di tingkat token, berdiri sendiri saja, sudah dapat meruntuhkan pelatihan[^ch8-33].

Sensitivitas ini berasal dari satu rantai penguat: **galat kecil pada log probability → simpangan rasio probabilitas yang dieksponensialkan → akumulasi pada prefix panjang → berubahnya clipping/pembobotan advantage → berubahnya arah gradien dan effective sample size**. Misalnya, bila log ratio pada 4.000 token semuanya menyimpang $10^{-3}$ ke arah yang sama, rasio pada tingkat trajektori terakumulasi menjadi $e^4\approx54.6$; galat nyata belum tentu setanda, tetapi contoh ini menjelaskan mengapa deret panjang memperbesar galat yang "tampak sangat kecil per token". Selisih probabilitas mungil pada token-token awal bahkan dapat mengubah token yang benar-benar tersampel, sehingga seluruh trajektori state sesudahnya bercabang. Gejalanya bukan sekadar "prompt yang sama sesekali menghasilkan jawaban berbeda", melainkan bisa berupa lonjakan importance ratio, banyaknya token yang terpangkas, lompatan gradien atau panjang respons, lalu reward dan entropi runtuh bersamaan. Perubahan ukuran batch yang mengubah cara reduksi dan merusak batch invariance numerik juga telah teramati langsung mengubah RL yang semestinya on-policy menjadi off-policy implisit; menyamakan numerik sampling–training atau memakai koreksi off-policy eksplisit sama-sama memperbaiki kestabilan[^ch8-34].

Secara rekayasa hal ini harus diperlakukan sebagai masalah inti, bukan derau floating point biasa:

- **Sebelum pembaruan parameter apa pun**, bandingkan token log probability sampler dan trainer pada batch trajektori yang sama, lalu pantau mean, kuantil, maksimum $\rho_t$, approximate KL, dan proporsi token yang terpangkas; inilah unit test on-policy yang paling langsung.
- Yang disinkronkan bukan hanya bobot, melainkan juga LoRA adapter, tokenizer, chat template, revisi model, dan konfigurasi positional encoding; rollout harus menyimpan behavior log probability saat pembangkitan, bukan menyulihnya belakangan dengan model saat ini.
- Sedapat mungkin selaraskan presisi, tata letak paralelisme, dan kernel komputasi utama antara sampling dan training; bila tidak memungkinkan, perlakukan selisihnya secara eksplisit sebagai off-policy, gunakan koreksi importance, dan pantau effective sample size, alih-alih berasumsi PPO clipping akan menambal semuanya.
- Jaga rollout tetap segar, batasi jumlah putaran update pada tiap batch data dan staleness asinkron. Memakai ulang data lama memang menukar bias dengan throughput, tetapi itu harus menjadi trade-off yang terukur, bukan percepatan gratis.
