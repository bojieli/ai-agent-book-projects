...
> >
> > Sebuah perwujudan tampilan grafis peta sebaran persebaran panas dari nilai intensitas sebaran dari nilai-perhatian / **attention heatmap** menata susun menguraikan menata menjajarkan tampilan wujud bobot-perhatian/nilai attention (attention weights) perhatian letak titik hubung/relasi rentang perhatian-relasi penjalinan-perhatian nilai-bobot antar rentang antar sisi antar tiap bagian dari (between each word and all preceding words) masing masing wujud satu bagian dari bentuk tiap kata dan kesemuanya dari kelompok kumpulan gabungan (all preceding words) atas segala segenap kesemua rupa rupa (all preceding words) tiap seluruh jajaran kumpulan kata kata dari (preceding words) pendahulunya ke arah dalam tata tatanan barisan bentuk rupa kerangka rupa struktur wujud berupa sebuah kerangka barisan/sebuah matrik/larik matriks (matrix). Susunan penampang bagian letaknya sebelah belahan tata rupa posisi sisi tatanan rupa susunan batas belahan sisi pada pada rupa sisi dari letak bawah (lower part) dari Gambar 2-6 mendemonstrasikan menjabarkan wujud ulasan menunjukan (shows) bentuk peta map/heatmap grafis sebar nilai sebaran dari-intensitas per-titik-relasi-fokus map rupa pola susun (heatmap) yang versi seutuhnya / (the complete heatmap): rupa di mana disana terlihat bahwa rupa dari penampang letak susun per barisannya wujud (each row) baris (row) barisan garis datarnya merupakan wujud dari rupa (is a Query) adalah bagian kueri/wujud penanya/sebuah si Query/kueri porsi nilai sebagai nilai si kueri/Query (yaitu bentuk rupa kata wujud rupa yang pada masa rentang pada wujud saat kini-tengah sesaat berlangsungnya/saat posisi tsb sedang lagi ditangani/diproses si mesin / (the word currently being processed)), lalu masing masing porsi bagian bagian garis rupa wujud lajur (each column) pada ruang (column) porsi lajur lurus tegak/vertikal lajur kolomnya bertindak sebagai bertindak-sebagai berlaku ibarat berperan ibarat dari/suatu nilai laksana se-sebuah nilai si-Kunci/Key (is a Key) ber-wujud porsi nilai rupa dari bentuk posisi parameter si Key/kunci-nya (yaitu letak rupa si-kata yang di jadikan ditunjuk selaku target rupa tertuju sasar posisi arah tuju si kata objek si posisi si penerima tunjukan sebagai si ter-atensi diperhatikan / dituju difokus (the word being attended to)), dan pola susunan warna pada balok sel dari tabel-matrik (cells) balok yang lebih (darker) memgelap yang makin/lebih-gelap menandai wujud menunjukkan memberikan isyarat indikasi (indicate) mendeskripsikan menggambarkan bahwasanya (higher) sebaran persentase bobot perhatian besaran bobot nilainya porsi bagian porsi-perhatian tersebut terhitung menunjuk nilai sebaran yg sebaran rentang lebih besar ukuran-lebih dominannya sebaran perhatian yg terbilang bobot terukur makin rupa-nilainya makin wujud lebih besar tinggi. Wujud rupa-tata peta heatmap rupa intensitas ini sendiri menyusun berpola rupa pola bentuk berupa sebentuk (is triangular) menyorot rupa menampang dengan posisi tata susunan (triangular) pola segi-tiga wujud bentuk bangun-segitiga ini dikarenakan/beralaskan sebab akibat dari kenyataan rupa bahwa sifat kerjanya (the model generates text) model rupa bahasa pada (generates text) proses dalam menghasilkan kata memproduksi (generates text) merangkaikan rupa mencetak memproduksi mencetak kalimat membuahkan membentuk menyusun luaran keluarannya tulisan kata pada metode memproses (generates text) wujud pemrosesan lajur/alur pola tata tulisan kerjanya dari-kiri (from left) bergerak dari sisi arah kiri merangkak mengarah merangkai berarah ke menyisir merangkak berurut menuju bagian (to right) sebelah urutan sisi rupa jalur sisi ke ke arah kanan: setiap tiap masing tiap katanya sekedar cuma sanggup memberi daya mengarahkan memberi menyalurkan atensi-fokus nilai titik perhati mengarahkan nilai menyalurkan mengalirkan (can attend) menyiramkan merelasi-fokus hanya pada ditujukan bisa mengena/merelasi memokus menaruh porsi titik-menuju/relasi memberi relasi perhatian semata kepada/kepada hal rupa dirinya pribadinya wujud wujud per-kata dirinya (to itself) ke hal pada wujud wujud dari/atas benda wujud entitas bentuk objek ke/pada pada sebatas dirinya bentuk (to itself) dirinya pribadi tunggalnya (itself) sendiri serta wujud bentuk untuk kepada wujud ke jajaran himpunan deretan himpunan kepada barisan dan pada (and the words) perkataan yang muncul hadir sebelumnya/letaknya/posisinya sebelum mendahuluinya muncul darinya posisinya letak letak posisinya tersebut darinya/dia si dirinya si sebuah posisi dari posisi kata posisinya posisi kata pos (before it), namun tiada sedikitpun bakalan tidak akan tak (not) mustahil menembus sampai bisa tertuju teralih akan dialirkan kepada tumpuan posisi hal isi kepada perihal dituju untuk (to content that) pada menyorot merelasi atas urusan bentuk muatan kepada konten bagian-isi belum hal posisi obyek bentuk materi materi konten belum porsi yang nyatanya sifat nyatanya (has yet to be generated) belum sedikitpun termanifestasi terbentuk tercetak tercipta masih belum pada pada posisi masa saat itu (has yet to be generated) blum ada tewujud tertulis terproses blum saat-waktu rupa belum-jua diproduksi dilahirkan termuncul termunculkan ada ada/terbuat dilahirkan dicetak-dibuat terbentuk (generated).
> >
> > **Mengapa rupa wujud besaran besaran parameter ukuran bentuk perhitungan nilai Key maupun besaran wujud hitung dari nilainya Value teramat butuh diperlukan sangat mendesak perlu butuh diperlukan penting buat butuh tersimpan simpan dalam porsi tercache dipertahankan/disimpan dalam ranah wujud tampung posisi memori pada-cache (cached)?** Dari mengamati/telaah mencermati rupa perwujudan wujud tampilan porsi penampang tampilan-tata petanya panas (Observing the heatmap) itu maka mengemuka mengungkapkan terang terungkap-buka mengungkap membongkar membeber kebenaran mendedah menelanjangi memperlihatkan mengungkap (reveals) bahwasannya setiap sebuah kata bentuk rupa yang baru saja terproses terwujud tercipta tercetak dihasilkan/dituntaskan terbentuk dihasil (is generated), parameter wujud hitungan (its Query) kuerinya kepunyaannya miliknya si-kumpulan kepemilikannya sendiri tadi hal hitung ukurnya si peranti ukurnya besaran rupa-Query Query-nya pribadi (its Query) wajib mutlak-harus diperlakukan pada tahapan dicocok-paskan dibanding disepadankan divalidasi silang/dikomparasi dicocok dicocokkan (matched against) menyilangkan uji sandingkan merelasikannya dipertemukan diselidik kesesuaiannya membanding merelasikan nilainya dipadukan dicocokan/dipadan divalidasikan bersilang dengan wujud perhitungan-kumpulan (the Keys) nilai hasil-Keys dari segala kesemua segenap pada kumpulan dari (of all preceding words) ke-total-seluruh pada total rupa/wujud bagian kepada rupa dari (all) atas dari baris rupa kumpulan keseluruhan utuh semua tiap kesemua keseluruhan (all preceding words) untai rentetan himpunan dari pada kata sebelum perkata-perkata sebelumnya, baru sesudah barulah setelah dilampauinya bagian tsb beres selesai-usainya lantas barulah berlanjut ke bagian tahap hitung komputasinya jumlah pembobotan keseluruhan (a weighted sum of all Values) agregat hitungan keseluruhan ukuran/nilai (Values)-nya dikomputasi (is computed). Jika seluruh segenap total hitungan semua wujud kumpulan besaran ukur dari nilai (K and V) rupa nilai Keys dan Values rupa-ukur rupa besaran K serta besaran K dan V-nya tadi seluruhnya kesemuanya segenap (recalculated) terpaksa senantiasa dihitung diolah dikalkulasi ulang kembali dihitung balik dari nol awal ulang mulai baru sedari nol-ulang wujud perhitungan komputasinya sedari ulang awal-dasar kalkulasinya-awal perhitungan nilai kalkulasi secara dari nol dasar murninya (from scratch) tiap waktu momen setiap kejadian masing masing proses berulang masa kejadian saatnya bergulirnya waktunya-kapanpun/setiap per-putaran tiap saat terjadinya wujud hal berulangnya saat siklusnya kala berlakunya saat kala per putaran pada tiap dari tiap waktu langkah masing kalinya/berlakunya rupa momennya, hal tersebut bakal membuat komputasi pertumbuhannya-tumbuh (grow) laju beban perhitungan angka bertumbuh/menumpuk nilainya akan melaju menggelembung nilainya ukur besar per-kembangannya akan bereskalasi tumbuh makin mengembang bengkak melonjak tumbuh selaras berbanding merambak tumbuh bertumbuh setara selaras searah paralel membesar merangkak proporsional melaju bertambah ukur searah laju-banding menyaingi berbandulan membesar selaras searah linier (with the context length) berbanding lurus seiring berbareng dengan memanjangnya rentangan jarak laju kepanjangan makin (the context length) seukuran bertambah besaran membengkak dengan ukuran nilai wujud luasan porsi dari konteks nilainya luasan context length panjang nilai jumlah panjangannya-isi batasan isi rentang (length)-ukur dari batas konteks kepanjangan batas (context length) batasan ukuran ukuran batasan limit ruang area rupa-rentang context. Rupa tampungan alat tampung sarana media KV Cache akan bekerja menyimpan (stores) angka parameter parameter ukur besaran perwujudan wujud-nilai rupa wujud wujud peranti ukuran angka hitungan wujud parameter parameter porsi ukuran dari besaran (already computed K and V values) dari rentang rupa nilai ukur perhitungan parameter (values) bagian perhitungan (K and V)-nya si parameter/parameter bentuk ukur Key and Value K serta-serta nilai wujud ukur nilai parameter bagian porsi nilai komputasi besaran angka dr V besaran (K and V) yang telah selesai tuntas di-hitung beres di-hitung komputasi sebelum terkomputasi kelar dihitungnya terdahulu diproses sebelumnya lebih duluan sudah/sudah pernah awal diproses tuntas telah hitung yg pernah beres komputasinya wujudnya rupa sudah (already) rupa angka hasil komputasinya sdh lebih kalkulasinya, sedari rupa bentuk tatanannya/pembawaannya memperkenankan membiarkan lantas membukakan (allowing) bagi munculan kata barunya sanggup (new words) merenggut bisa dengan mempermulus untuk sanggup dengan bisa serta merta bisa pada per/wujud/sarana (new words) sekumpulan wujud untuk rupa rupa (new words) hadirnya rentetan kata data bentuk masukan bagi rentetan kata-kata kata kata kalimat (new words) untuk segera dapat seketika seketika spontan bisa secara berlanjut dengan langsung (directly reuse them) menggunakan memanfaatkan kembali nilai memakainya lagi pakai menggunakan angka hal halnya hal tsb rupa rupa mereka ulang rupa ulang hal benda tadi hal-besaran benda nilainya berulangnya pemakaian kembali secara lagi (reuse) bisa secaraman secara instan wujud/wujud langsung memanfaatkan memanfaatkan memakainya secara-langsung pakai peroleh pakai memakai mengambil (directly reuse them) — hal ini merupakan dasar letak pada perwujudan esensi wujud titik pada-hakikat (is the core optimization) rupa bentuk intisari (is the core optimization) sari pati (core optimization) rupa-rupa optimisasi saripati dari titik titik/ruang-sisi dari letak pada poin puncak porsi letak poros pemusatan bentuk optimisasi pusat (is the core optimization) pada poin intinya pengoptimalan pokok sisi core letak letak pusaran pusat/optimisasinya optimalisasi puncak (is the core optimization) pengoptimasian-hal porsi rupa ruang pokok di hal wujud inti (core optimization)-nya atas optimalisasi utama di dari (discussed next) wujud ruang objek titik inti bahas obrol yang akan kita pada ulas akan bahas bahas terangkan rupa urusan perihal akan diperbincangkan urusan diobrol disinggung diurai ulas urainya-terangkan ulas diurai pada wujud ulasan ulasan bahasan diurai bincangkan didiskusikan (discussed next) dijabarkan kemudian bahas setelah sesudah berikutnya.
> >
> > Memiliki bekal landasan pijak landas modal pemahaman (With a basic understanding of the attention mechanism) suatu bekal instuisi wujud pengertian pengetahuan secukup porsi pemahaman di porsi ukuran secara perolehan tingkat rupa tingkatan (basic) landasan mendasarnya rupa modal porsi batas wawasan ukuran mendasar pengetahuan pondasi (basic understanding) perihal pemahaman pokok esensi (basic understanding) batas porsi pengenalan taraf halnya dari per-dasarnya awalan pada terhadap seluk-seluk sisi kerangka pada peranti dari tentang bekal dasar (the attention mechanism) pemahamannya peranan dari sifat dasar sistem hal di rupa mekanisme attention pada/dari atas attention-mekanisme pada mekanisme mechanism-nya rupa rupa tersebut rupa wujud dari perhatian attention, kini akhirnya kini saatnya (we can now) mari seketika mulai tiba kini kita pun kita lalu sanggup mampu bisa rupa wujud kini akan bisa kita kita menempat menaruh beralih posisi kita bisa (observe) melakukan untuk menginspeksi menatap memeriksa rupa bentuk menelaah melihat secara melihat (observe) langsung menyaksikan (observe) observasi menilik wujud distribusi pada sebaran (the attention distribution) porsi tataan luasan besaran rupa penampang rupa rupa tebaran wujud rentangan hampar luasan wilayah penampang wujud wujud penyebaran-tatanan pembagian sebaran rupa rentang alur penampang penyebaran besaran di posisi di hamparan penyebaran/pembagian ukuran nilai nilai wujud sebar tebaran (distribution) rentang pembagian tebar hampar dari nilai atas porsi titik rupa-rupa sebaran letak nilai nilai attention pada/bagi kepemilikan sebuah wujud/sarana rupa kerangka bentuk jenis rupa (of a real model) sosok perangkat wujud sebuah nyata sarana/produk rupa model riil sesungguhnya rupa model model rupa-nyata rupa asli ril sebenarnya sesungguhnya melalui perantaraan peranti (through) dari penjalaran uji tes perangkat atas dari rupa rupa (through) melalui uji atas jalan wadah wahana rupa dari/via sebuah proyek pada ranah pengerjaan wahana percobaan (the `attention_visualization` experiment) eksperimen perihal dari "visualisasi attention-atas" "attention_visualization" sarana praktikum alat (experiment).
...
Chat Template (Templat Obrolan) merupakan **konsep fundamental di seluruh buku ini**. Konsep ini tak hanya berdampak pada perilaku KV Cache, melainkan juga pada mekanisme seperti pemanggilan tool multi-putaran, persistensi chain-of-thought, maupun penyuntikkan status bar. Karenanya, hal tersebut layak mendapat penjelasan tersendiri. Urutan token dalam percobaan visualisasi attention (mis., token khusus seperti `<|im_start|>`, `<|im_end|>`) terlihat amat sangat berbeda dari format pesan API berbentuk JSON seperti yang ditunjukkan sebelumnya. Alasannya adalah bahwa pesan-pesan API terstruktur harus dikonversikan menjadi urutan token (token stream) linear yang dapat diproses oleh model. Komponen yang bertanggung jawab atas konversi ini adalah **Chat Template**.
...
**Pertama, ini menjelaskan mengapa format API standar harus digunakan.** Jika developer melewatkan API lalu menggabungkan pesan secara manual (misalnya, melewatkan hasil tool sebagai user message biasa alih-alih sebagai tool message), Chat Template tersebut berpotensi merepresentasikan percakapan tersebut secara keliru. Dengan Chat Template milik Qwen3, misalnya, tool call multi-putaran mampu menahan konten penalaran internal sebelumnya di dalam tag `<think>`, yang akan mempertahankan kesinambungan (continuity) melewati batas panggilan tool. Saat template mendeteksi giliran user yang baru, ia membersihkan context penalaran tersebut dan memulai yang baru. Apabila tool result ditandai secara keliru sebagai user message, hal ini pun bisa memicu peng-reset-an (reset) tersebut di waktu yang salah, yang mana akan melemahkan koherensi pada penalaran multi-langkah. Perhatikan bahwa rumpun model berbeda-beda cukup tajam dalam urusan menangani chain-of-thought (CoT) historis, dan ragam strategi itu pun dengan cepat mengalami evolusi. Arahan resmi (official guidance) pada era DeepSeek R1 dulunya adalah **menanggalkan semua penalaran historis (strip all historical reasoning)**: dalam percakapan multi-putaran, hanya `content`-nya sajalah yang diteruskan kembali (passed back), bukan `reasoning_content`—dikarenakan CoT historis tak pernah muncul dalam input pelatihan (training input) R1, mengumpankannya kembali (feeding it back) merupakan input out-of-distribution (di luar distribusi) yang malah berpotensi merusak (interfere) output-nya, dan ini juga menghemat banyak sekali jumlah token. Tetapi, strategi tersebut memiliki kelemahan di skenario Agent: penalaran intermediat mengemban state (keadaan) kritis seperti "mengapa tool ini dipanggil dan hipotesis apa saja yang dikesampingkan"; begitu ia ditanggalkan, model akan bernalar dari nol pada setiap putarannya, membuatnya rentan mengulang kesalahan dan melupakan rencana jangka-panjangnya. DeepSeek pada akhirnya **memutar balik (reversed) sepenuhnya** kebijakan tersebut di V4, memandatkan bahwa `reasoning_content` dari setiap pesan assistant (termasuk yang memuat `tool_calls`) harus diteruskan kembali (passed back) apa adanya (verbatim), jika tidak, API akan mereturn sebuah pesan error—Kimi K2, GLM-5, dan yang lainnya turut mengadopsi protokol yang sama. Sementara itu, Claude mewajibkan pihak klien untuk meneruskan kembali blok pemikiran (thinking block) tersebut (bersama verifikasi tanda-tangan (signature verification)) secara persis utuh kepada API selama di dalam perulangan tool call, sementara pihak server akan mengabaikan pemikiran historis setelah masuk ke giliran interaksi dengan user yang baru. Pergeseran di seluruh industri dari langkah "menanggalkan (stripping)" menuju "wajib diteruskan kembali (mandatory pass-back)" ini dengan sendirinya menjadi bukti kuat: **untuk skenario Agent, pemikiran (thinking) bukanlah beban mubazir melainkan wujud dari state (keadaan)**. Konsultasikanlah dokumentasi template terbaru milik sang model sebelum menggunakannya.
...
Untuk memahami nilai dari KV Cache, pertama-tama pertimbangkanlah apa yang terjadi tanpa kehadirannya. Umpamakan suatu Agent telah mencapai giliran interaksi yang keenam dan mengumpulkan 2.000 token context. Tanpa adanya cache, tiap tambahan token-baru mengharuskan si model untuk mengkalkulasi ulang vektor K dan V untuk keseluruhan prefix. Walaupun lima giliran sebelumnya tidak berubah, giliran keenam masih harus mengomputasi ulang keseluruhannya, dan prefix yang bertambah panjang bikin putaran ini lebih mahal ketimbang di putaran pertama. Tanpa caching, komputasi attention di tahap prefill (fase di mana model memproses seluruh token masukan (input) sebelum menghasilkan respons) akan membengkak naik membubung menjadi kuadratik selaras seiring panjangnya context, menyebabkan peningkatan pesat atas waktu laten (latency) dan beban tagihan (cost) tatkala percakapan tersebut makin dalam/panjang. Ini menjadi masalah besar terkhusus untuk tugas-tugas Agent yang membutuhkan pemanggilan tool bertubi-tubi.

![Gambar 2-10: Mekanisme Penggunaan Kembali Prefix dari KV Cache](images/fig2-10.svg)

**Memahami KV Cache melalui contoh sederhana.** Umpamakan context tersebut mempunyai 4 token [A, B, C, D], dan model tersebut akan menghasilkan token kelima, E. Operasi inti dari attention membandingkan vektor Query milik E berlawan tanding (against) atas vektor Key yang ada/dimiliki token-token saat ini guna mengkalkulasikan bobot (match scores) angka nilai kemiripan-cocok/relasinya (terkait jabaran pemahaman atas wujud dasar produk perkalian silang-dot product secara langsung, silakan merujuk Eksperimen 2-2). Model itu lalu memfungsikan deretan besaran angka bobot/skor tsb guna menjumlahkan hasil perkalian jumlah-tertimbang (weighted sum) pada posisi parameter dari vektor-vektor Value, sehingga lahirlah wujud nilai dari vektor representasi cetak milik si-luaran token E (E's output representation).

Tanpa kehadiran KV Cache, setiap kalinya sebuah token baru dilahirkan, maka vektor-vektor K beserta vektor V milik seluruh token-token sebelum token-tersebut (all preceding tokens) bakalan terpapar keharusan-komputasi-ulang harus senantiasa (must be) wajib mutlak dikalkulasikan direka diperhitungkan berhitung komputasikan diracik diproses ulang kalkulasi-ulang diperhitungkan diramu (recalculated) balik mulai awalan nol asal mula di dari kondisi awalan dasar mula (from scratch): memproduksi token E membutuhkan hitungan proses komputasi 5 rangkaian set parameter atas K beserta V-nya, melahirkan mencetak kemunculan pembentukan perwujudan token (token) yang per ke-enam bakal memerlukan hitungan-ulang rupa tuntutan komputasi/hitungan ukuran kebutuhan butuh-komputasi bagi 6 deret set-parameter K dan parameter V... dan sampainya ketika mencetak wujud pencetakan hingga titik porsi luaran untuk sampai berlarut menyentuh di bagian di wujud token-yang-ke N (the Nth token), butuh dikomputasikannya ke N set jumlah total parameter deret yang butuhnya wajib terkomputasi hitungannya (computed), sehingga keseluruhan jumlah besaran angka wujud dari ukuran proses ukuran komputasinya ini pun akan lantas bakal berbanding berjalan (proportional) selaras proporsional bersama-dengan hitungan (N²).

Dengan KV Cache, vektor-vektor K maupun parameter vektor V kepunyaan dari wujud rentang pada titik token si-A, B, C, maupun D, disisihkan ditaruh didalam ruang ter-cache (cached) disimpan disimpan di/masuk di-cache kelar/setelah ia rampung selesai kelar dirampungkan pengkomputasiannya seberes-sebatas diproses sesudah ditangani tuntas dihitung proses (computed) kelar dihitung sebanyak beres diproses beres dikerjakannya 1 putaran-kali usai dikalkulasikannya sekali ukur-kali (once). Tatkala model hendak saatnya (When generating) tatkala kala di/kala tatkala rupa wujud giliran saat waktunya mau merangkaikan wujud bentuk (generating E) wujud/hendak kemunculan akan membikin membentuk wujud akan memproduksi cetakan luaran atas keluaran rupa si memproduksi token E (generating E), cukup (only E's own K and V) murni cuma vektor nilai vektor (K and V) K dan vektor V kepunyaan murni miliknya pribadi si nilai (own) E kepunyaan E sendirinya E murni si token E kepunyaannya-sajalah perwujudan rupa rupa murni yg (need to be computed) butuh yang haruslah perlu perlu butuh dicarikan dikalkulasi hitung nilai rupa dihitung di-komputasikannya (computed), lalu barulah selepas hitungan itu tahap tahap komputasi kalkulasi penghitungan nilai hitung operasi komputasi atas (the attention calculation) attention lantas mulai dilaksanakan (is performed) dilakukan dengan memanfaatkan memadukan rupa nilai vektor vektor vektor vektor ini berpadu mendampingi digabung bersama berdampingi dengan menyanding 4 himpun set data set-data dari rupa parameter-tampung hasil/set dari rupa (cached sets) set cached himpun-tersimpan 4 set rupa set ter-cache yg di cache-nya (the 4 cached sets). Catat cermat-cermat waspada harap bahwasanya bahwasanya harap wujud KV Cache mereduksi/mampu membebaskan/hemat biaya nilai-beban wujud beban muatan harga tagihan proses/biaya harga rupa rupa (saves) bantu menghemat perihal urusan operasi rupa perihal beban rupa hitung komputasi balik operasi komputasi proses komputasi-ulang merangkai ulangnya per/urusan hitung pengkomputasian (recomputation) merangkai-ulang (of the K and V projections) dari proyeksi proyeksi atas-K (of the K) beserta V (projections) V untuk dari wujud/bagi ranah para-deretan/token deret bentuk dari bagian/bentuk token kelompok token (historical tokens) historis, alhasil-hingga oleh alhasil sehingga sehingga karenanya rupa dari itu alhasil pada posisi di tahapan masing wujud tahap tahap putaran wujud per porsi rupa wujud sisi proses tahapan langkah decoding tidaklah perlunya/tak (does not need) rupa rupa bakal butuh perlu usah dituntut haruslah mengharuskan tiada musti mengharuskannya tiada tak tak tidak/tiada butuh untuk wajib/harus mengharuskan perlu merangkaikan ulang hitung hitung ulang-mengalkulasi mengomputasikan kalkulasi balik menyusun ulang (recompute) seluruh segenap total/seluruh keseluruhan dari batas panjang area luasan wujud area/kumpulan rupa awalan utuh rupa keseluruhan wujud rupa seluruh prefix (the entire prefix); walau walau begitu, walaupun, rupa/proses kalkulasi (the attention calculation) attention untuk/bagi bagi/untuk porsi tiap bagian luaran luaran unit satu untuk masing (each new token) baru setiap masing untuk untuk token rupa setiap porsi nilai untuk wujud (each new token) per/setiap se-wujud baru baru bagi sebuah satuan per (token) untuk (each new token) masing tiap luaran untuk per token barunya senantiasa mutlak bakalan mesti (still needs) butuh senantiasanya tetap masih (still needs) untuk di diharuskannya senantiasanya tetap menyisir menelusur-lewat menyisir mengeksplor melintasi menguliti mengeksekusikan (traverse) melintasi atas menyisir menyinggahi segala wujud besaran besaran angka semua semua peroleh rupa dari dari rupa atas segenap rupa kumpulan (all cached K and V values) himpun tumpuk semua angka K serta maupun nilainya nilai (values) rupa dari seluruh (all) K wujud dan V data nilai dan V ter-cached yg (values), yang dibarengi menyatu berbareng/menyatu rupa dibarengi berbareng (with) dengan wujud/beban komputasi dari proses yang mana pertumbuhannya-tumbuh membesar mengembang melaju berjalan me-linier garis linier (linearly) searah bersama panjang lurus dengan bersama-dengan dengan wujud ukuran batasan luasan rentang panjang-rentangnya context (context length) — alasan karena-itu-inilah rupa hal yang yang alasan letak ini makanya mengapa inilah/inilah alasannya memproduksi dari (decoding) men-decoding luasan panjang (long-context decoding) pada konteks dari rupa context-panjang di men-decoding di-dari konteks rentang kepanjangan decoding wujud rentang panjang porsi context lama-kelamaan bakal menjadi menjadi tambah bertambah (becomes increasingly) memuncak sangat melambat-lamban dan semakin pelan/lambat/makin/tambah berujung makin pelan, kemudian ranah beban urusan dari porsi muatan beban bandwidth ukuran (bandwidth) kapasitas batas transfer-kirim-besaran daya-bandwidth dan daya kapasitas muatan memori ranahnya memory memori batas kapasitas-memori ukur muatan-sisi (memory) memory KV Cache akhirnya berujung bakal akan mewujud berubah beralih tumbuh lantas (become) menjadi/bikin merubah jadi berposisi (the inference bottleneck) hambatan penyempit leher botol (bottleneck)-nya hambatan rintangan sumbatan si sumber sandungan letak rintangan sumbatan simpul si-penyumbatnya sumbatan rintangan (bottleneck) kelajuannya dari pada proses laju bagian-kerjanya inferensinya inferensi-modelnya kerjanya (inference bottleneck).
...
Perkiraan tingkat keberhasilan dan kalkulasi jumlah juga perlu dispesifikasikan secara tepat agar bisa dieksekusi. Tingkat keberhasilan mesti dievaluasi secara bertahap menurut suatu proses yang tetap, dan probabilitas yang diperkirakan itu harus terpetakan secara langsung ke model penagihannya. Sebagai contoh, tugas-tugas dengan taksiran probabilitas keberhasilan di atas 60% mungkin menggunakan model yang dapat dikembalikan biayanya (refundable model), sementara tugas di bawah 30% mungkin harus ditolak. Kalkulasi jumlah mutlak mendefinisikan perincian batasan penagihannya—umpamanya, panggilan telepon ditarifkan seharga $0.05 per menitnya, dan jumlah keseluruhannya dibulatkan menuju nominal satuan dolar terdekat—serta menyatakan secara gamblang bahwa "savings" (penghematan) dikalkulasikan semata dari nilai tagihan yang berjalan. Apabila tidak, model itu berpotensi mengambil penalaran, "Bila nilainya kelak melonjak hingga $180 tahun depannya tanpa negosiasi, lantas aku membantu mempertahankannya di harga $150, hal tersebut berarti berhemat sebesar $30," secara keliru menyimpulkan perihal menghindari kenaikan harga di masa depan sebagai sebuah penghematan.
...
Filosofi desain inti di sini adalah bahwasanya large language models (LLM) itu kuat dalam hal mengikuti instruksi yang kompleks dan mengekstraksi informasi dari context yang panjang, tetapi mereka tidak seharusnya diberikan keleluasaan (discretion) berlebih dalam merumuskan aturan bisnis. Dengan menyediakan kerangka kerja operasional (operational framework) yang jelas, sumber daya kognitif model akan dibebaskan sehingga dapat fokus pada bagian-bagian yang benar-benar memerlukan penalaran. Pelatihan yang efektif tidaklah membiarkan orang untuk menerka (infer) sendiri prosesnya; pelatihan semacam itu memberikan prosedur operasi standar (standard operating procedures) yang terperinci yang memungkinkan orang beroperasi dalam suatu kerangka kerja yang jelas.

### Contoh Beberapa-Bidikan (Few-Shot Examples): Kapan Harus Menunjukkan Model Contoh-Contoh

Selain aturan dan proses, contoh (few-shot examples) adalah jenis lain dari konten system prompt yang penting. Manakala output yang diinginkan itu susah untuk diutarakan secara persis melalui aturan—seperti copywriting dalam gaya khusus, format laporan terstruktur, ataupun nuansa nada dari balasan customer service—maka sering kali jauh lebih baik untuk menghadirkan dua atau tiga contoh berpasangan input-output bermutu tinggi (high-quality) ketimbang menuliskan deskripsi abstrak yang panjang. Model dapat menyesuaikan dirinya dengan pola-pola ini di dalam context saat ini, sering kali lebih efektif ketimbang saat ia mengikuti instruksi abstrak dengan jumlah yang sama (mekanisme internal di baliknya akan diuraikan pada bagian Kompresi Context di bab ini). Sebaliknya, untuk tugas yang model sudah tangani dengan baik dan aturannya mudah dinyatakan, contoh hanya membuang-buang token.
...
**Pendekatan Satu: Suntikkan ke System Prompt (system message)**. Tambahkan konten Skill secara langsung ke system prompt. Kemampuan model dalam mengikuti instruksi adalah paling unggul untuk konten di bagian letak sistem (sebab masa pelatihannya sangat sarat menggunakan instruksi pada letak bagian ini), menjadikan proses eksekusi Skill berjalan maksimal. Permasalahannya: saban kali satu Skill baru dimasukkan, isi dari pesan sistem (system message) pun berganti, membuat awalan KV Cache batal. Jika Agent berulang kali berganti-ganti Skills (misal, tugas menuntut pencarian dengan Skill tertentu, lalu beralih ke Skill dokumen), cache akan dibatalkan berkali-kali, meningkatkan latensi dan biaya secara signifikan.

**Pendekatan Dua: Baca layaknya file biasa, dengan konten muncul di tengah-tengah context**. Agent melahap dokumen Skill lewat tool pemindai berkas biasa, lantas isi dari berkas tersebut tersaji dalam rupa hasil tool di bagian rentetan percakapan—berada di pertengahan batas context. Cara semacam ini tidak akan menjamah letak KV Cache (system prompt-nya tak berubah), namun bakal mematok tuntutan yang lebih keras bagi si model di area kemampuannya **dalam mematuhi instruksi (instruction-following)**: si model harus sanggup menjaring lalu mematuhi aturan instruksional di isi Skill yang nyelip di antara rentetan batas context yang panjang, ketimbang sekadar menyikapinya sebatas rupa output tool biasa yang bisa sekadar ditengok saja. Dalam kenyataannya, masing-masing model membawa tingkat keberhasilan yang beda-beda di bawah bentuk tipe mode ini—Claude menampilkan performa yang cukup solid dikarenakan ia sarat dengan materi pematuhan instruksi di lokasi tengah saat berlatih; untuk ragam model lainnya kerap kali kewalahan saat mematuhi instruksi yang dilesakkan ke titik pertengahan dari context.

**Pendekatan Tiga (Implementasi Produksi): Metadata diinjeksikan pada akhir dari context, konten penuhnya barulah diakses saat ada keharusan dengan memakai tool terdedikasi**. Inilah hal langkah yang diusung oleh Claude Code. Ianya membedah fungsi "routing" dan "eksekusi" jadi 2 fase, menghindarkan model dari beban utama dua gaya sebelumnya:

- **Daftar metadata**—nilai `name` + `description` miliknya segenap Skills yang dipasang (sebatas angka nilai hitungan beratus token saja)—lalu diinjeksi selaku wujud pesan **user-role meta message** (pesan tingkat meta peran user) di ujung batas dari context, berbalut kaitan `<system-reminder>`. Pesan inipun tiada memodifikasi bagian wujud dari awalan system prompt, menyebabkan letak KV Cache pada prefix-nya terawat konstan utuh gigih. Langkah ini pun menampik keharusan penempatan data meta-nya itu di area titik paruh dari wujud luasan letak context, agar tak berkurang atensi fokus (recency salience) atas nilai informasi yang gress (kebaruannya). Claude Code meramu taktik gaya sorong tambahan progresif (incremental sending strategy): satu porsi per-tiap Skill cumalah sekadar dikirim kalau barulah pertama muncul melulu, sedang rupa Skills apa-apa sudah pernah disorong/dimuat ya tidak bakalan dilempar dobel kembali ulang. Gaya metode ini mematok nilai beban biaya data meta di laju posisi antengnya itu ke titik nilai nol, lantas menyimpan rapi keutuhan bagian cachenya. Cuma saja hal/keunikan yang menonjol dari letak di batas penghujung cuman ada pada saat pas bagian menyisipkan/injeksi pertama melulu (insertion turn). Sewaktu trajectory itu bertumbuh tambah merentang makin memanjang, sisi data meta lantas terlempar pelan-pelan mingser maju menjurus mengarah di posisi tengah di luasan dari rupa perihal context lalu hal posisinya tergerus kehilangan posisinya yang menonjol (salience). Urusan ini memantik persoalan/bikin trade-off penyeimbangan atas bentuk "sorong sekadar/sekali aja, rawat cachenya" berhadapan dengan wujud "selalunya pampang di letak ujung di setiap putarnya, lestarikan posisi perhatian (attention)," rupa urusan perihal ini kelak akan bermunculan juga lagi di perbincangan dari sub rubrik berikutnya bab ini di urusan topik tentang kelakuan update tambal-sambung wujud gaya append (persistent append-style updates) (update tipe penambahan permanen).
- **Konten penuh (Full content)** lalu diangkut ditarik rupa ketika memang per-butuh (on demand) melalui sarana tool wujud milik Skill itu sendiri. Dikala si model sudah menyisir lalu mengetahui menelusuri mengetahui via baca atas runut rupa meta daftar datanya itu (metadata list) bahwasanya rupa sebuah nilai ada satu macam jenis Skill itu pas pas benar ampuh buat operasi per tugas ketika itu (current task), model lalu lekas lantas memfungsikan salah satu rupa hal wujud dari porsi dari jenis tool semisal hal porsi `Skill(skill: "pdf")`. Tool itu pada balutan rupa-operasinya itu dari internal merapal/baca membuka `SKILL.md` lalu lekas menghidangkan rupa luaran/melontarkannya berbalik (returns it), kemudian hasilannya lantas bakal memampang timbul mengemuka di area porsi bagian luasan riwayat obrol (conversation history) seumpama selaku hasil per/sebagai balasan dari tool. Gaya rupa ini menyingkir menepis rintangan instruksi/risiko pematuhan instruksi porsi dari (instruction-following risk) milik dari tipe langkah Pendekatan yang Dua—si sang model kelak memliki peluang/probabilitas condong wujud besar-kuat buat mengaplikasikan mempergunakan bentuk luaran rupa nilai hal dari alat yang secara rupa barusan-baru saja porsi ia (it just actively called) memintanya panggil secara tindakan mandiri-aktif untuk memfungsikannya ketimbang meladeni berlakunya isi file awam biasa yg diletak ditaruh tertanam pada di sela (middle) tengah batasan dari rupa context.

Perlu dicatat bahwa "pesan meta peran-pengguna (user-role meta message) di akhir context" bukanlah jalur yang unik untuk Skills, melainkan pola injeksi informasi meta umum—bagian berikutnya tentang **Agent Status Bar** akan secara sistematis memperluas mekanisme ini, dan daftar metadata Skill dapat dilihat sebagai contoh spesifik dari hal itu.
...
Kumpulan teknik ini memiliki keuntungan praktis: semua informasi meta muncul dalam context di wujud yang dapat dibaca manusia (human-readable), memungkinkan para developer untuk menginspeksi informasi apa yang Agent terima dan keputusan apa yang dibuatnya. Lebih penting lagi, pendekatan ini tidak menuntut perubahan pada model. Tidak diperlukan fine-tuning; teknik ini dapat bekerja pada segala macam model bahasa dan dapat diuji secara terpisah ataupun digabungkan sesuai kebutuhan.

## Strategi Kompresi Context

Bagian-bagian sebelumnya membahas apa saja yang perlu disertakan dalam context: prompt engineering menentukan apa yang mesti ditulis, Skills menetapkan apa yang mesti dimuat on demand (sesuai kebutuhan), dan Agent Status Bar menunjuk pada informasi meta apa yang perlu diinjeksi. Akan tetapi, di saat interaksi multi-putaran semakin mendalam, context pun terus mengembang. Bagian ini akan beralih ke persoalan yang berkebalikan: **bagaimana menyusutkan/mereduksi konten dalam context**—kapan mengompresi, bagaimana mengompresi, serta mengapa kompresi sanggup teramat bermanfaat bahkan sebelum context window (jendela konteks) mencapai kepenuhan.
...
**Pertama, menangani kendala panjang dan biaya.** Ini adalah alasan paling intuitif: context window (jendela konteks) ada batasnya (misalnya, 128K token), sedangkan hasil panggilan tool sering kali secara wajar bisa memanjang hingga puluhan ribu karakter, sehingga hanya dengan beberapa putaran interaksi saja bisa memenuhi jendelanya dan langsung memangkas-berhenti (cut short) langkah tugas tersebut. Bertambahnya jumlah token juga ekuivalen pada tagihan ongkos API yang melesat meninggi serta menambah latensi (latency) inferensi dengan terjal dan kentara.

**Kedua, meningkatkan kualitas penalaran—pengetahuan yang disarikan (summarized knowledge) lebih bernilai bagi model ketimbang rentetan informasi mentah.** Motivasi ini lebih mendalam dan kerap luput diabaikan. Sekalipun context window tersebut memadai-besar kelonggarannya, memboyong semua informasi mentah ke dalam context belumlah tentu sebuah opsi penentu yang teramat optimal-bagus.

Pertimbangkan satu contoh konkret: sewaktu menggarap satu tugas kompleks, sebuah Agent menimbun data informasi sebuah topik setelah menjalani 10 ragam jejak putaran-pencarian lewat laman website (web searches). Jejeran temuan ragam-data pencarian web itu tersebut tertebar wujudnya berceceran wujudnya apa adanya secara mentah acak-adul semrawut di segala pojokan luasan context—ragam hasil putaran jejak-ke 2 tumpah letaknya tak jauh di depan di area gerbang pangkal awal, kemudian rentetan hasil putaran sabetan sabetan per putaran ke-9 rupa hasilannya diletak ditumpuk terletak merapat dekat arah berlabuh buritan ujung batas akhir penghujung akhir (near the end). Ketika Agent diharuskan menoreh sebuah kata putus pengambilan kebijakan (decision) pamungkas ujungnya dari seluruh timbunan data-informasi itu, ia dihadapkan pada keharusan melacak mengutip mengambil-tarik keping temuan cuil remah hal informasi (fragments) bernas relevannya (relevant fragments) yg bercecer kesana kemari membentang membelah rupa wujud berselerak melintasi rupa rentangan luasan merentang jauh membentang sepanjang rupa puluhan berpuluh ribu token-token. Alhasil daya atensi terpusatnya pun menjadi memudar luntur mengambang pecah buyar-buram menjalar (diffuse), lantas modelnya dengan gampang lalai membiarkan lepas (miss) rentetan jejer data/informasi keterangan kuncinya.

Sesudah ragam penelusuran pencarian rupa yang putaran (search) pencarian ke-10, akan tetapi, sekali panggilan LLM porsi sebuah panggilan tunggal (a single LLM call) sanggup me-telurkan mencetak menelurkan rupa merilis membuat mendedahkan luaran bentuk cetak ringkasan ikhtisar (structured summary) simpulan wujud terstruktur dari timbunan muatan himpunan-muatan yg akumulasi informasinya-informasi yg telah diakumulasikan itu (accumulated information): "Berdasar atas kondisi temuan ragam data pengetahuan saat nian (Currently known): A bentuk rupanya adalah..., B wujud aslinya adalah..., seputar rupa keping keterangan pada (C) ihwal isinya nyatanya masih nihil masih buram tiada absen-kosong (still missing)." Lantas sang si model sanggup men-eksploitasi merapal/menggunakan model representasi sajian format rupa bentukan kemasan penyulingan (refined) hasil saringan kemasan/format dari (refined knowledge representation) pola pengetahuan yang telah dibikin disuling matang/digodok murni nian itu ke dalam rentetan untaian rantai benang-benang proses penalarannya rupa (subsequent reasoning) penalarannya pada waktu rupa setelahnya/sesudahnya-kelak di kemudian, lepas nir dengan (without) dari beban paksa beban mesti tanpa perlu (without) perlu keharusan guna harus meng-ekstraknya menggali cabut (re-extracting it) mengekstrak hal data itu balik-ulang menyaring-mengeruk halnya ulang lagi balik (re-extracting it) bersumber membedah mendaras bedah menelusuri dari jejalan kubangan data mentahnya.
...
3.  **Ini adalah trade-off yang disadari**: tanpa kompresi, context membesar melebihi batas window dan tugasnya langsung gagal secara absolut; dengan menggunakannya, sebagian cache memang hilang, namun panjang context tetap terkendali dan kepadatan informasi pun meningkat. Maka dari itu, frekuensi kompresi butuh ditimbang/disesuaikan (weighed)—kompresi yang terlampau sering (frequent) akan terlalu sering merusak cache (frequently break the cache). Jalan yang paling pas adalah melangsungkan sistem gaya kompresi rombongan-borong sekaligus secara paket tunggal (batch compression) manakala angka jumlah ukuran context mulai menghampiri titik limit ambang batasnya (threshold), melainkan memaksakan kompresi per tiap satu putaran jalan babak ronda putarannya.
