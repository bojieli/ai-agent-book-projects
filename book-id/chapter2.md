...
>
> **Proses Penalaran Internal Model**: Model yang mendukung chain-of-thought (mis., Qwen3) pertama-tama akan bernalar di dalam tag `<think>` sebelum menghasilkan panggilan tool—menganalisis niat pengguna, mengevaluasi tool mana yang cocok, dan merencanakan urutan pemanggilan. Proses penalaran ini sangat berharga untuk men-debug perilaku Agent. Perlu dicatat bahwa `<think>` adalah tag pada aliran keluaran mentah model dan tidak selalu muncul apa adanya di respons API: eksperimen ini mengirimkan `think=True` ke Ollama, sehingga Ollama mengurai bagian penalaran itu sebelum mengembalikannya dan menempatkannya di field `thinking` tersendiri (saat streaming berupa potongan `thinking` inkremental), sehingga tag `<think>` tidak muncul di `content`. Keduanya hanyalah cara penyajian yang berbeda untuk isi penalaran yang sama, dan kode eksperimen menangani kedua kasus tersebut.
>
