...
Desain dua lapis—katalog kecil yang selalu tersedia dan isi lengkap yang dimuat sesuai kebutuhan—inilah yang membuat Skills tetap mudah ditemukan tanpa membebani context.
...
#### Catatan implementasi: standar versus format wire

Standar Agent Skills menentukan urutan progressive disclosure, bukan role pesan API secara harfiah: katalog harus terlihat sebelum instruksi lengkap, dan `SKILL.md` lengkap baru dimuat setelah Skill dipilih. Dokumentasi Anthropic menyebut katalog sebagai konteks tingkat sistem yang dimuat saat startup, sedangkan dokumentasi Claude Code menyatakan bahwa instruksi Skill yang dipanggil dimasukkan sebagai pesan user pada titik pemanggilan. Karena itu, `<system-reminder>` ber-role user yang terlihat pada trace Claude Code tertentu merupakan representasi wire yang bergantung pada versi dan Harness, bukan persyaratan protokol Agent Skills.

Codex CLI memakai Harness yang berbeda. [Dokumentasi Skills OpenAI](https://developers.openai.com/codex/skills/) menyatakan bahwa katalog awalnya berisi nama, deskripsi, dan path setiap Skill. Source code publik saat ini merender katalog sebagai fragmen konteks `developer`, sedangkan isi Skill terpilih menjadi fragmen user yang dibungkus `<skill>` ([`fragments.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/fragments.rs), [`extension.rs`](https://github.com/openai/codex/blob/main/codex-rs/ext/skills/src/extension.rs)). Gateway tertentu dapat menampilkan konten `developer` pada area yang tampak seperti system. Dengan demikian, tabel yang dikaitkan dengan DeepSeek hanya merupakan snapshot versi klien tertentu; role, wrapper, dan rekonstruksi per giliran adalah detail Harness. Biaya cache juga harus dievaluasi per runtime, bukan menyalin model Claude Code ke semua klien.
